...
homogeneous polynomials of degree $k$ in $\displaystyle \RR^n$ presented in
...
$\displaystyle \phi_k(\VEC{x}) = \ps{f}{p_{\VEC{x}}^{[k]}}$ for all
...
enough such that $\displaystyle (n+p-2) < 2^p$, we have
\[
...
\end{equation}
as $\displaystyle \epsilon \rightarrow 0^-$,  where
\[
...
\end{split}
\end{equation}
as $\displaystyle \epsilon \rightarrow 0^-$ because
$\displaystyle \graD u(\VEC{x} + \epsilon \nu(\VEC{x})) \cdot \nu(\VEC{x})
\to \pdydx{u}{\nu^-}(\VEC{x})$ uniformly on $\partial \Omega$ as
$\displaystyle \epsilon \to 0^-$ since $u \in C_{\nu}(\Omega)$, and
$\displaystyle u\big(\VEC{x} + \epsilon \nu(\VEC{x})\big) \to
u(\VEC{x})$ uniformly on $\partial \Omega$ as $\displaystyle \epsilon \to 0^-$
...
be the set bounded by $\displaystyle \partial \Omega^{\prime}_\epsilon
= \{ \VEC{x} + \epsilon \nu(\VEC{x}) : \VEC{x}
...
\end{equation}
as $\displaystyle \epsilon \rightarrow 0^+$.
...
$\displaystyle \graD u(\VEC{x} + \epsilon \nu(\VEC{x})) \cdot \nu(\VEC{x})
\to \pdydx{u}{\nu^+}(\VEC{x})$ uniformly on $\partial \Omega$ as
$\displaystyle \epsilon \to 0^+$ since
$\displaystyle u \in C_{\nu}(\Omega^{\prime})$, and
$\displaystyle u\big(\VEC{x} + \epsilon \nu(\VEC{x})\big) \to
u(\VEC{x})$ uniformly on $\partial \Omega$ as $\displaystyle \epsilon \to 0^-$
...
\int_{\partial B_r(\VEC{0})} \pdydx{u}{\nu} \dss{S}{x}
\end{equation}
because $\displaystyle \Delta u = 0$ on $\displaystyle \Omega_0^{\prime}$.
We have
...
$\displaystyle \partial \Omega^{\prime}_0$ as
$\displaystyle \epsilon \to 0^+$ since
$\displaystyle u \in C_{\nu}(\Omega^{\prime})$.
...
$\displaystyle T_{K_\epsilon^{[o]}}:L^2(S) \rightarrow L^2(S)$ is compact.
...
$\displaystyle (-\lambda T_{K_1})^j g\in C(S)$ for all $j$.  Moreover,
the series $\displaystyle \sum_{j=0}^\infty \|\lambda T_{K_1}\|_\infty^j$
...
is the closure of a bounded and open subset of $\displaystyle \RR^{n-1}$.
...
we have that $\displaystyle \|\VEC{x} - \VEC{y}\|^{1-n}
...
$\displaystyle \psi_{\VEC{x}_j}^{-1}$ is of class $\displaystyle C^2$
on the compact set $\displaystyle \overline{W_j}$, we
...
$\displaystyle B:\RR^n \times \RR^n \to \RR^m$,
...
$\displaystyle M_\epsilon \leq C \epsilon^2$ for some constant $C$.  Thus,
$\displaystyle A_\epsilon \leq
...
(\ref{pot_lem_dbl3E2}), the set $\displaystyle \Omega^{\prime}$ is
...
\right| \dss{S}{y} \leq B
\]
for all $\displaystyle \VEC{x} \in \RR^n\setminus S$.
\end{lemma}
...
and $\displaystyle \VEC{z} \in \partial B_1(\VEC{0}) \subset \RR^{n-1}$,
...
Then $\displaystyle u_t \rightarrow u^{[i]}$ uniformly on $S$ as
$\displaystyle t\rightarrow 0^-$ and
$\displaystyle u_t \rightarrow u^{[o]}$ uniformly on $S$ as
...
\stage{iii}
We prove that the convergence $\displaystyle u_t \rightarrow u^{[i]}$ (resp.
$\displaystyle u_t \rightarrow u^{[o]}$) is uniform on $S$ as
$\displaystyle t\rightarrow 0^-$ (resp. $\displaystyle t\rightarrow 0^+$).
...
\end{align*}
for $\displaystyle \phi,\psi \in L^2(S)$.
...
\footnote{If $\displaystyle \VEC{z} \in S \subset \RR^3$, then we have that
$\displaystyle \nu(\VEC{z})
...
$O_\mu$ is an open subset of $\displaystyle \RR^n$ because $S$ is a manifold of
...
uniformly on $S$ as $\displaystyle t\rightarrow 0^+$ is similar.  We
then also get that (\ref{pot_ddn_unif}) is satisfied and thus complete
the proof that $\displaystyle u \in C_{\nu}(\Omega^{\prime})$.
\end{proof}
...
we have that $u=0$ on $\displaystyle \Omega^{\prime}$ because
...
We also have that $\displaystyle \Lambda\big|_{W_-}$ is onto because
$\displaystyle \dim \Lambda(W_-) = m^{\prime}$.  Hence, 
...
$\displaystyle \Omega^{\prime} \cap B_r(\VEC{0})\phi \in V_+^\perp$,
we have
\[
...
$\displaystyle \Omega^{\prime} \cap B_r(\VEC{0})|u(\VEC{x})|
...
because $\displaystyle \psi\in W_+^0$, we also have from
Lemma~\ref{pot_1to1n2} that $w$ is harmonic at infinity.  Since
$\displaystyle w \in C(\RR^n)$ by 
