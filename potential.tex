...
In this chapter, $\Omega$ is a bounded open subset of $\DS \RR^n$.  We
assume that $\DS \Omega = \bigcup_{i=1}^{m} \Omega_i$, where the
$\Omega_i$ are the connected components of $\Omega$.  In particular,
$\Omega_i \cap \Omega_j = \emptyset$ for $i\neq j$.  Moreover, we
assume that $\DS \Omega^\prime = \RR^n \setminus \overline{\Omega}
= \bigcup_{i=0}^{m^{\prime}} \Omega_i^{\prime}$,
where the $\DS \Omega_i^{\prime}$ are the connected components of
$\DS \Omega^{\prime}$ with
$\DS \Omega_0^{\prime}$ being the unbounded component.
We assume that $\partial \Omega$ is $\DS C^2$ and observe that
$\DS \partial \Omega = \partial \Omega^{\prime}$.
...
$\DS u \in C^1(\Omega) \cap C(\overline{\Omega})$ at
...
$\DS u \in C^1(\Omega^{\prime}) \cap C(\overline{\Omega^{\prime}})$ at
$\DS \VEC{x} \in \partial \Omega^{\prime}$ is
\[
\pdydx{u}{\nu^+}(\VEC{x})
= \lim_{t\rightarrow 0^+} \frac{1}{t}
\left( u(\VEC{x} + t \nu(\VEC{x})) - u(\VEC{x}) \right)
...
$\DS u \in C^1(\Omega) \cap C(\overline{\Omega})$ such that the
...
of functions
$\DS u \in C^1(\Omega^{\prime}) \cap C(\overline{\Omega^{\prime}})$
such that the exterior normal derivative exists at all points
$\DS \VEC{x} \in \partial \Omega^{\prime}$ and
...
and $\DS u\big|_{\partial \Omega} = f$.
\item[D2 - Exterior Dirichlet Problem:] Given
$\DS f\in C(\partial \Omega^{\prime})$, find
$\DS u \in C(\overline{\Omega^{\prime}})$
such that $\Delta u = 0$ in
$\DS \Omega^{\prime} \cup \{\infty\}$ (in the sense of
distributions) and $\DS u\big|_{\partial \Omega} = f$.
...
Suppose that $g: V\rightarrow W$ is a $\DS C^\infty$ diffeomorphism
between two open subsets $V$ and $W$ of $\DS \RR^n$.
Let $\DS G(\VEC{x}) = \big(\diff g(\VEC{x})\big)
...
\]
Let $\DS h = g^{-1}$ and
$\DS H(\VEC{y}) = \big(\diff h(\VEC{y})\big)^\top
...
\]
We have that
$\DS G(h(\VEC{y})) = (H(\VEC{y}))^{-1}$ for all $\VEC{y} \in W$
because
\[
...
$\DS \diff g(h(\VEC{y})) = (\diff h(\VEC{y}))^{-1}$
...
Suppose that $\DS u\in C^2(V)$ and let
$\breve{u}(\VEC{y}) = u(h(\VEC{y}))$ for all $\VEC{y} \in W$.
We have the following result.

\begin{prop}
Suppose that $u$, $g$, $\DS h = g^{-1}$ and $G$ are as
defined above.  Then
...
\]
We also have that
$\DS G_{i,j}(h(\VEC{y})) = \delta_{i,j} \|\VEC{y}\|_2^4$
because $\DS G(h(\VEC{y})) = (H(\VEC{y}))^{-1}$ for all
$\DS \VEC{y} \in T(\Omega^{\prime})$.

If follows from (\ref{pot_chain_rule}) with $h=T$ and
$\DS u\in C^2(\Omega^{\prime})$ that
\begin{align*}
\Delta u (\VEC{x})
...
\end{align*}
for $\VEC{y} = T(\VEC{x})$, where we have used
$\DS \Delta \|\VEC{y}\|_2^{2-n} = 0$ for
$\VEC{y} \neq \VEC{0}$ for the second to last equality.
...
\]
for $\DS \VEC{x} \in \Omega^{\prime}$.
The following result follows from this relation.

\begin{prop} \label{pot_u_ut_harm}
If $\DS u\in C^2(\Omega^{\prime})$, then $u$ is harmonic on
$\DS \Omega^{\prime}$
if and only if $\tilde{u}$ is harmonic on $\DS T(\Omega^{\prime})$.
\end{prop}

\begin{defn}
We say that $\DS u\in C(\Omega^{\prime})$ is harmonic at
...
Suppose that $\DS U \subset \RR^n$ is an open set and that
$\DS \VEC{p} \in \RR^n$.
If $u:U \to \RR$ is an harmonic function on $U \setminus \{\VEC{p}\}$
such that
$\DS \left|u\left(\VEC{x}\right)\right|
...
Let $\DS V = R^{-1} \left( U - \{\VEC{p}\} \right)$.
The lemma will be proved if we prove that
$\DS v: V  \to \RR$ defined by
$\DS v(\VEC{x}) = u\left(R(\VEC{x} + \VEC{p})\right)$
has a removable singularity at $\VEC{0}$.  We may therefore assume
that $\DS \overline{B_1(\VEC{0})} \subset U$.
...
$\DS \overline{B_1(\VEC{0})} \setminus B_\delta(\VEC{0})$
and harmonic on
$\DS B_1(\VEC{0}) \setminus \overline{B_\delta(\VEC{0})}$
because $v$, $w$, $\DS \|\VEC{x}\|^{2-n} - 1$ when $n>2$,
and $\DS \ln\left(\|\VEC{x}\|)^{-1}\right)$ when $n=2$ are
continuous on
$\DS \overline{B_1(\VEC{0})} \setminus B_\delta(\VEC{0})$
and harmonic on
$\DS B_1(\VEC{0}) \setminus \overline{B_\delta(\VEC{0})}$.
...
Let $\DS m = \min_{\VEC{x} \in \overline{B_1(\VEC{0})}} |w(\VEC{x})|$.
...
$\DS \VEC{x} \in \overline{B_1(\VEC{0})} \setminus B_\delta(\VEC{0})$.
Since this is true for all $\delta$ small enough, we get that
$f(\VEC{x}) \leq 0$ for
$\DS \VEC{x} \in \overline{B_1(\VEC{0})} \setminus \{\VEC{0}\}$.
Letting $\epsilon$ converges to $0$, we get that
$\DS v(\VEC{x}) - w(\VEC{x}) \leq 0$ for
$\DS \VEC{x} \in \overline{B_1(\VEC{0})} \setminus \{\VEC{0}\}$.

Proceeding similarly with $-f$ instead of $f$, we get that
$\DS w(\VEC{x}) - v(\VEC{x}) \geq 0$ for
$\DS \VEC{x} \in \overline{B_1(\VEC{0})} \setminus \{\VEC{0}\}$.
Completing the proof that
$\DS w(\VEC{x}) - v(\VEC{x}) = 0$ for
$\DS \VEC{x} \in \overline{B_1(\VEC{0})} \setminus \{\VEC{0}\}$.
\end{proof}

\begin{prop} \label{pot_infty_u}
Suppose that $\DS u\in C^2(\Omega^{\prime})$ is harmonic on
$\DS \Omega^{\prime}$.
The following statement are equivalent.
\begin{enumerate}
\item $u$ is harmonic at infinity.
\item $\DS u(\VEC{x}) = O(\|\VEC{x}\|^{2-n})$ as
...
$\DS B_{\delta}(\VEC{0}) \subset T\left(\Omega^{\prime}\right)$
...
Let
$\DS M = \max_{\VEC{x} \in \overline{B_{\delta/2}(\VEC{0})}}
|w(\VEC{x})|$.
...
homogeneous polynomials of degree $k$ in $\DS \RR^n$ presented in
Section~\ref{SectHramPoly}.

\begin{prop} \label{pot_infty_ddu}
Suppose that $\DS u\in C^2(\Omega^{\prime})$ is harmonic on
$\DS \Omega^{\prime} \cup \{\infty\}$.  Let
$\nu(\VEC{x})$ be the outward
unit normal to $\DS \partial B_r(\VEC{0}) \subset \Omega^{\prime}$ at
...
$\DS \left\{ \VEC{x} : \|\VEC{x}\| > R \right\} \subset
\Omega^{\prime}$.
If it is not so, choose $q > 0$ such that 
$\DS \left\{ \VEC{x} : \|\VEC{x}\| > q/2 \right\} \subset
\Omega^{\prime}$ and set $\DS W = q^{-1} \Omega^{\prime}$.
The function $w: W \to \RR$ defined by
$w(\VEC{x}) = u(q \VEC{x})$ for $\VEC{x} \in W$
is of class $\DS C^2$ and is harmonic on $W \cup \{\infty\}$.
Since $\DS \left\{ \VEC{x} : \|\VEC{x}\| > 1/2 \right\} \subset W$,
we may choose $0<R<1$ such that
$\DS \left\{ \VEC{x} : \|\VEC{x}\| > R \right\} \subset W$.
...
$\DS \overline{B_1(\VEC{0})} \subset T\left(B_R(\VEC{0})\right)$.
Let $S = \partial B_1(\VEC{0})$ and consider the functions $f: S \to \RR$
and $\DS \phi_k \in H_k\big|_S$ defined by
$f(\VEC{x}) = \tilde{u}(\VEC{x})$ and
$\DS \phi_k(\VEC{x}) = \ps{f}{p_{\VEC{x}}^{[k]}}$ for all
$\VEC{x} \in S$ respectively.  Recall that
...
\]
for $\|\VEC{x}\| > 1$.  If
$\VEC{x} = r \VEC{y}$ with $r > 1$ and $\|\VEC{y}\|=1$, then we have
\[
...
enough such that $\DS (n+p-2) < 2^p$, we have
\[
...
\]
For $r>3$ and $p$ large enough such that $\DS p < 2^{p+1}$,
we have
\[
...
that $\DS u \in C(\overline{\Omega^{\prime}})$ is a solution
of (D2) with $u = f = 0$ on $\DS \partial \Omega^{\prime}$.
According to Proposition~\ref{pot_u_ut_harm}, The Kelvin transform
$\tilde{u}$ of $u$ is harmonic in $\DS T(\Omega^{\prime})$
and satisfies $\tilde{u} = \tilde{f} = 0$ on
$\DS T(\partial \Omega^{\prime}) = \partial\,T(\Omega^{\prime})$.
Because $\DS T(\Omega^{\prime})$
is a bounded open subset of $\DS \RR^n$, we have that
$\tilde{u} = 0$ on $\DS T(\Omega^{\prime})$
by uniqueness of solution for the problem (D1). 
This implies that $u =0$ on $\DS \Omega^{\prime}$.
...
Suppose that $\DS S \subset \RR^n$ is a $(n-1)$-dimensional
compact oriented manifold of class $\DS C^k$ with $k>1$.
...
\end{align*}
is a diffeomorphism of class $\DS C^{k-1}$ onto $O_\mu$.
\end{theorem}
...
\end{equation}
as $\DS \epsilon \rightarrow 0^-$,  where
\[
...
\end{split}
\end{equation}
as $\DS \epsilon \rightarrow 0^-$ because
$\DS \graD u(\VEC{x} + \epsilon \nu(\VEC{x})) \cdot \nu(\VEC{x})
\to \pdydx{u}{\nu^-}(\VEC{x})$ uniformly on $\partial \Omega$ as
$\DS \epsilon \to 0^-$ since $u \in C_{\nu}(\Omega)$, and
$\DS u\big(\VEC{x} + \epsilon \nu(\VEC{x})\big) \to
u(\VEC{x})$ uniformly on $\partial \Omega$ as $\DS \epsilon \to 0^-$
...
that $\DS \int_\Omega \| \graD u \|^2 \dx{\VEC{x}} = 0$.
Since $\DS u \in C^1(\Omega)$, this implies that $\graD u = \VEC{0}$
...
Suppose that $\DS u \in C_{\nu}(\Omega^{\prime})$ is a
solution of (N2) with $\DS \pdydx{u}{\nu^+} = f = 0$ on
$\partial \Omega$.

Let $\DS \Omega^{\prime}_\epsilon \subset \Omega^{\prime}$
be the set bounded by $\DS \partial \Omega^{\prime}_\epsilon
= \{ \VEC{x} + \epsilon \nu(\VEC{x}) : \VEC{x}
...
O_{\epsilon} = \left\{ \VEC{x} + t\, \nu(\VEC{x}) : \VEC{x} \in
  \partial \Omega \ , \ |t|<\epsilon \right\}$ is a tubular
neighbourhood of $\DS \partial \Omega = \partial \Omega^{\prime}$.

Choose $r>0$ such that
$\DS \overline{\RR^n \setminus \Omega^{\prime}_\epsilon}
\subset B_r(\VEC{0})$.
From the Green's identity (\ref{laplace_green1}) with $v=u$, we have
\begin{align}
...
$\DS \graD u(\VEC{x} + \epsilon \nu(\VEC{x})) \cdot \nu(\VEC{x})
\to \pdydx{u}{\nu^+}(\VEC{x})$ uniformly on $\partial \Omega$ as
$\DS \epsilon \to 0^+$ since
$\DS u \in C_{\nu}(\Omega^{\prime})$, and
$\DS u\big(\VEC{x} + \epsilon \nu(\VEC{x})\big) \to
u(\VEC{x})$ uniformly on $\partial \Omega$ as $\DS \epsilon \to 0^-$
since $u$ is uniformly continuous on the compact set
$\DS \overline{B_r(\VEC{0})\cap\Omega^{\prime}}$.
...
\]
as $r \rightarrow \infty$.
Since $\DS u \in C^1(\Omega^{\prime})$, we find that
$\graD u = \VEC{0}$ everywhere on $\DS \Omega^{\prime}$.
Thus $u$ is constant on each connected component of
$\DS \Omega^{\prime}$.

Moreover, $u=0$ on $\DS \Omega_0^{\prime}$, the unbounded component
of $\DS \Omega^{\prime}$, because $u(\VEC{x}) \rightarrow 0$ as
$\|\VEC{x}\|\rightarrow \infty$ according to
Proposition~\ref{pot_infty_u}.  This is possible for a constant
function $u$ on $\DS \Omega_0^{\prime}$ only if $u=0$
on $\DS \Omega_0^{\prime}$.
...
$\DS \partial \Omega^{\prime}$, the following statements are true.
\begin{enumerate}
\item For $n=2$, $u$ is constant on each connected component
$\DS \Omega^{\prime}_j$,
$\DS 0\leq j \leq m^{\prime}$, of $\DS \Omega^{\prime}$.
\item For $n>2$, $u$ is constant on each bounded connected component
$\DS \Omega^{\prime}_j$,
$\DS 1\leq j \leq m^{\prime}$, of $\DS \Omega^{\prime}$
...
$\DS \int_{\partial \Omega_i} f =0$ for $1 \leq i \leq m$.
\item If $u$ is a solution of (N2), then
$\DS \int_{\partial \Omega^{\prime}_i} f =0$ for
$\DS 1 \leq i \leq m^{\prime}$.
Moreover, $\DS \int_{\partial \Omega^{\prime}_0} f =0$ for $n=2$.
\end{enumerate}
\end{prop}
...
\end{equation}
because $\Delta u = 0$ on $\Omega_i$.  However, we have
\[
\int_{\partial \Omega_{i,\epsilon}} \pdydx{u}{\nu} \dss{S}{x}
...
$\DS \int_{\partial \Omega_i} f \dss{S}{x} = 0$.

\stage{2} The prove that
$\DS \int_{\partial \Omega^{\prime}_i} f \dss{S}{x} = 0$ for
$\DS 1 \leq i \leq m^{\prime}$ is identical to the proof given in
(i) if $\Omega_i$ is replaced by $\DS \Omega_i^{\prime}$ and
$\epsilon >0$ is used.

If $n=2$, let
$\DS \Omega^{\prime}_{0,\epsilon} \subset \Omega^{\prime}_0$ be
the set bounded by
$\DS \partial \Omega^{\prime}_{0,\epsilon}
= \{ \VEC{x} + \epsilon \nu(\VEC{x}) :
...
neighbourhood of $\DS \partial \Omega = \partial \Omega^{\prime}_0$.

Choose $r>0$ large enough such that
$\DS \partial \Omega^{\prime}_{0,\epsilon} \subset B_r(\VEC{0})$.
...
\int_{\partial B_r(\VEC{0})} \pdydx{u}{\nu} \dss{S}{x}
\end{equation}
because $\DS \Delta u = 0$ on $\DS \Omega_0^{\prime}$.
We have
...
$\DS \int_{\partial \Omega^{\prime}_0} f \dss{S}{x} = 0$.
\end{proof}
...
Let $\Omega$ be an open subset of $\DS \RR^n$ and $S$ be a subset of
$\Omega$.  Let $\DS \{V_i\}_{i=1}^\infty$ be a collection of
open subsets of $\Omega$ such that
$\DS S \subset \bigcup_{i=1}^\infty V_i$.
A {\bfseries partition of unity}\index{Partition of Unity}
for $S$ subordinated to $\DS \{V_i\}_{i=1}^\infty$ is a set
of functions $\DS \{\phi_i\}_{i=1}^\infty$ such that:
\begin{enumerate}
\item The $\phi_i$ are $\DS C^\infty$ functions on $\Omega$.
...
\item $\DS \sum_{i=1}^\infty \phi_i(\VEC{x}) = 1$ for all
$\VEC{x}\in S$.
\end{enumerate}
The definition of partition of unity subordinated to a finite
collection of open subsets $\DS \{V_i\}_{i=1}^N$ is identical.
...
\begin{prop}
Let $\Omega$ be an open subset of $\DS \RR^n$ and $S$ be a subset of
$\Omega$.  If $\DS \{ V_i\}_{i=1}^\infty$ is a collection
of open sets in $\Omega$ such that
$\DS S \subset \bigcup_{i=1}^\infty V_i$.
Then, there exists a partition of unity for $S$
subordinate to $\DS \{ V_i\}_{i=1}^\infty$.
\end{prop}
...
$\DS \{V_i\}_{i=1}^\infty$
be a collection of open subsets of $\DS \RR^n$ such that
$\DS \Omega = \bigcup_{i=1}^\infty V_i$.
A {\bfseries partition of unity}\index{Partition of Unity}
for $\Omega$ subordinated to $\DS \{V_i\}_{i=1}^\infty$ is a
set of functions $\DS \{\phi_i\}_{i=1}^\infty$ such that:
\begin{enumerate}
\item The $\phi_i$ are $\DS C^\infty$ functions on $\Omega$.
\item $0 \leq \phi_i(\VEC{x}) \leq 1$ for all $i$ and all $\VEC{x} \in \Omega$.
\item $\supp \phi_i \subset V_i$ for all $i$.
...
\begin{prop}
Let $\Omega$ be an open subset of $\DS \RR^n$ and
$\DS \{ V_i\}_{i=1}^\infty$ is be collection
of open sets in $\Omega$ such that
$\DS S \subset \bigcup_{i=1}^\infty V_i$.
...
$\DS \BB = \{ (U_j, \psi_j) \}_{j\in J}$ be an atlas for
$S$, where $J \subset \NN$ is an index set.
Moreover, let $\DS \{ \phi_j \}_{j\in J}$ be a partition of unity
for $S$ subordinated to $\BB$; namely,
$\DS \{ \phi_j \}_{j\in J}$ is a partition of unity for $S$
subordinated to $\DS \{ V_j \}_{j\in J}$,
where $U_j = V_j \cap S$ and $V_j$ is an open subset of
$\DS \RR^n$.  Note 
that we assume that the topology on $S$ is the induced topology from
$\DS \RR^n$.

The integral $\DS \int_S f(\VEC{x}) \dx{S}$ is defined by
\begin{align*}
...
where $\DS \Omega \subset \RR^n$ is a bounded open set.  We also
assume $S$ is of class at least $\DS C^2$.  The orientation on $S$ is
...
chart $\DS \left(U_{\VEC{x}},\psi_{\VEC{x}}\right)$ such
that:
\begin{itemize}
\item $\DS U_{\VEC{x}}$ is a connected set containing
$\VEC{x}$ and $\DS U_{\VEC{x}} = S \cap V_{\VEC{x}}$ for
some bounded open set $\DS V_{\VEC{x}} \subset \RR^n$.  This is
possible because $S$ is the boundary of an open set.
\item $\DS \psi_{\VEC{x}}$ is the projection of
$\DS U_{\VEC{x}}$ onto
$\DS W_{\VEC{x}} \subset T_{\VEC{x}}$ along the vector
$\nu(\VEC{x})$, where $\DS T_{\VEC{x}}$ is the
tangent space to $\DS U_{\VEC{x}}$ at the point
$\VEC{x}$.  We have that
$\DS \psi_{\VEC{x}}^{-1}: W_{\VEC{x}} \to \RR^n$ is of
class $\DS C^2$.  Using translation and rotation, we may consider
that $\DS U_{\VEC{x}}$ is the graph
of a function of class $\DS C^2$ from $\DS \RR^{n-1}$
to $\RR$
(Figure~\ref{MAN_PARAM_V2}).
\item By shrinking $\DS V_{\VEC{x}}$ and $\DS U_{\VEC{x}}$
if necessary, we may assume that $\DS \psi_{\VEC{x}}^{-1}$ is of class
$\DS C^2$ on the compact set $\DS \overline{W_{\VEC{x}}}$.
...
{Chart $\DS (U_{\VEC{x}},\psi_{\VEC{x}})$ of the manifold $S$ in
Section~\ref{pot_int_man}.}{MAN_PARAM_V2}

Since $S$ is compact, there exists a finite cover
$\DS \left\{ V_{\VEC{x}_j} \right\}_{j\in J}$ of $S$
and so a finite atlas
$\DS \BB
= \left\{ \left(U_{\VEC{x}_j}, \psi_{\VEC{x}_j}\right) \right\}_{j\in J}$
of $S$.

Let $\DS Q_{\VEC{x}_j} = \max_{\VEC{w} \in \overline{W_{\VEC{x}_j}}}
= \left|\det \diff \psi_{\VEC{x}_j}^{-1}(\VEC{w})\right|$.  We have that
$\DS Q_{\VEC{x}_j} < \infty$ because we assume that
$\DS \phi_{\VEC{x}_j}^{-1}$ is of class $\DS C^2$
on the compact set $\DS \overline{W_{\VEC{x}_j}}$.
If $\DS Q = \max_{j \in J} Q_{\VEC{x}_j}$, then
\begin{equation} \label{defnQdetpsim1}
\left|\det \diff \psi_{\VEC{x}_j}^{-1}(\VEC{w})\right| < Q
\end{equation}
for all $\DS \VEC{w} \in W_{\VEC{x}_j}$ and all $j \in J$.

Given $\VEC{x} \in S$ and $\eta < 1$,
let $\DS S_{\VEC{x},\eta} = S \cap B_{\eta}(\VEC{x})$.
...
open covering $\DS \{ V_{\VEC{x}_j}\}_{j\in J}$ of
$S$~\footnote{\cite{Mu} is a good reference on the subject of the Lebesgue
number.}, then
$\DS \overline{B_\eta(\VEC{x})} \subset V_{\VEC{x}_j}$
for some $j \in J$.  Thus
$\DS \overline{S_{\VEC{x},\eta}} \subset U_{\VEC{x}_j}$
(Figure~\ref{MAN_PARAM_V3}).

\pdfF{potential/man_param_v3}{Small region on a manifold}
{Region $\DS S_{\VEC{x},\eta}$ of the manifold $S$ in
Section~\ref{pot_int_man}.}{MAN_PARAM_V3}

\stage{$\mathbf{n>2}$}
Since $\DS \|\VEC{x}-\VEC{y}\| 
\geq \|\psi_{\VEC{x}_j}(\VEC{x}) - \psi_{\VEC{x}_j}(\VEC{y})\|$ for all
$\DS \VEC{x},\VEC{y} \in U_{\VEC{x}_j}$ and
$\DS \psi_{\VEC{x}_j}(S_{\VEC{x},\eta}) \subset
B_{\eta}(\psi_{\VEC{x}_j}(\VEC{x}))$, we get
\begin{align*}
&\int_{S_{\VEC{x},\eta}} \|\VEC{x}-\VEC{y}\|^{-\alpha} \dss{S}{y}
...
$0 \leq r < \eta$ and
$\DS \VEC{z} \in \partial B_1(\VEC{0}) \subset \RR^{n-1}$.
...
\left| \det \diff \psi_{\VEC{x}_j}^{-1}(\VEC{w})\right| \dx{\VEC{w}} \\
&\qquad \leq \int_{\psi_{\VEC{x}_j}(S_{\VEC{x},\eta})}
\left| \ln\left( \left\|\psi_{\VEC{x}_j}(\VEC{x})-\VEC{w}\right\|\right)\right|
\, \left| \det \diff \psi_{\VEC{x}_j}^{-1}(\VEC{w})\right| \dx{\VEC{w}} \\
&\qquad \leq Q \int_{B_{\eta}(\psi_{\VEC{x}_j}(\VEC{x}))}
\left|\ln\left( \left\|\psi_{\VEC{x}_j}(\VEC{x})-\VEC{w}\right\|\right)\right|
\dx{\VEC{w}}
= Q \int_{\partial B_1(\VEC{0})} \int_0^\eta |\ln(r)|
  r^{n-2} \dx{r} \dx{S} \\
&\qquad \leq Q \omega_{n-1} \left( \frac{|\ln(\eta)|\, \eta^{n-1}}{n-1}  +
...
$\DS C^2$-manifold of dimension $n-1$ in $\DS \RR^n$.
...
\end{equation}
for $\DS f \in L^p(S)$, where $A$ and $B$ are given in
Definition~\ref{pot_kerOk}.
\end{prop}

\begin{proof}
That $\DS T_K : L^p(S) \rightarrow L^p(S)$ defines a bounded linear
...
let $\DS \{ \phi_{\VEC{x}_j} \}_{j\in J}$ be a partition of unity
...
$\DS \|\VEC{x}-\VEC{y}\|
\geq \|\psi_{\VEC{x}_j}(\VEC{x}) - \psi_{\VEC{x}_j}(\VEC{y})\|$ for all
$\DS \VEC{x},\VEC{y} \in U_{\VEC{x}_j}$.
The last inequality is true because
$\DS \psi_{\VEC{x}_j}^{-1}(U_{\VEC{x}_j}\cap B_\epsilon(\VEC{x}))
...
$\DS C = \frac{Q\, \omega_{n-1} |J|}{n-\alpha-1}$
...
\int_{U_{\VEC{x}_j}\cap B_\epsilon(\VEC{x})} \phi_{\VEC{x}_j}(\VEC{y})
\dss{S}{\VEC{y}} \\
&\quad \leq \|A\|_\infty\sum_{j\in J}
\int_{U_{\VEC{x}_j}\cap B_\epsilon(\VEC{x})}
\left| \ln\left( \left\| \VEC{x} - \VEC{y} \right\|\right) \right|
\dss{S}{\VEC{y}} + \|B\|_\infty \sum_{j\in J}
...
\dx{\VEC{w}} \\
&\quad \qquad + \|B\|_\infty \sum_{j\in J}
\int_{\psi_{\VEC{x}_j}^{-1}(U_{\VEC{x}_j}\cap B_\epsilon(\VEC{x}))}
\left|\det \diff \psi_{\VEC{x}_j}^{-1}(\VEC{w}) \right| \dx{\VEC{w}} \\
&\quad \leq \|A\|_\infty Q \sum_{j\in J}
\int_{\psi_{\VEC{x}_j}^{-1}(U_{\VEC{x}_j}\cap B_\epsilon(\VEC{x}))}
\left| \ln\left( \left\| \VEC{x} - \psi_{\VEC{x}_j}^{-1}(\VEC{w}) 
...
$\DS \lim_{r\rightarrow 0} r^\alpha \ln(r)=0$ whatever $\alpha>0$.
...
$0\leq \alpha < n -1$, then $\DS T_K: L^2(S) \rightarrow L^2(S)$ is
compact.
\end{prop}

\begin{proof}
Given $0 < \epsilon <1$, defined
$\DS K_\epsilon^{[o]}:S\times S \rightarrow \RR$
by
\[
...
\end{cases}
\]
and $\DS K_\epsilon^{[i]}:S\times S \rightarrow \RR$ by
$\DS K_\epsilon^{[i]} = K - K_\epsilon^{[o]}$.  Choose $r>0$
such that $S \subset \overline{B_r(\VEC{0})}$.

\stage{i} $\DS K_\epsilon^{[o]} \in L^2(S \times S)$ because
$\DS K_\epsilon^{[o]}$ is bounded on the compact set $S \times S$.
...
$\DS \left|K_\epsilon^{[o]}(\VEC{x},\VEC{y})\right| = 0$
...
Thus $\DS K_\epsilon^{[o]}$ is an Hilbert-Schmidt kernel according to
Theorem~\ref{fu_an_HSKern}.  This implies in particular that
$\DS T_{K_\epsilon^{[o]}}:L^2(S) \rightarrow L^2(S)$ is compact.

\stage{ii} Since $\DS K_\epsilon^{[i]}$ is a kernel of order $\alpha$,
...
Hence $\DS T_{K_\epsilon^{[o]}} \rightarrow T$ as
$\epsilon \rightarrow 0$.
Since the operators $\DS T_{K_\epsilon^{[o]}}$ are compact,
...
= S \cap B_r(\VEC{w}) \ .
\]
Given $\delta > 0$ and $\VEC{y} \in S_{\VEC{x},\delta}$, we have
\begin{align*}
...
\stage{iii} Hence,
$\DS \left| (T_K f)(\VEC{x}) - (T_K f)(\VEC{y}) \right| < \epsilon$
...
$\DS f \in L^2(S)$ and $f + \lambda T_K f \in C(S)$,
 then $f \in C(S)$.
\end{prop}
...
the diagonal $\{(\VEC{z},\VEC{z}) : \VEC{z} \in S\}$, we have that
$K_2$ is continuous on $S\times S$.

Let
$\DS h_{\VEC{y}}(\VEC{z}) = \left|K_2(\VEC{x},\VEC{z}) -
K_2(\VEC{y},\VEC{z})\right|^2$ for $\VEC{z} \in S$, and define
$h:S \to \RR$ by $\DS h(\VEC{z}) = 4\|K_2\|_\infty^2$
 for all $\VEC{z} \in S$.
...
then have that $\DS \| \lambda T_{K_1} \|_p < 1$.
It follows from
the Banach Lemma that
$\DS \Id + \lambda T_{K_1}:L^2(S) \rightarrow L^2(S)$ is
invertible.  More precisely,
$\DS (\Id + \lambda T_{K_1})^{-1} = \sum_{j=0}^\infty
(-\lambda T_{K_1})^j$.
Hence $\DS f = \sum_{j=0}^\infty (-\lambda T_{K_1})^j g$, where the
convergence is in $\DS L^2(S)$.

Since $g$ is continuous on the compact set $S$, $g$ is bounded.  It
follows from Proposition~\ref{pot_Kc_fb} that
$\DS (-\lambda T_{K_1})^j g\in C(S)$ for all $j$.  Moreover,
the series $\DS \sum_{j=0}^\infty \|\lambda T_{K_1}\|_\infty^j$
converges because $\|\lambda T_{K_1}\|_\infty <1$.  Hence,
given $\delta>0$, we can choose $N$ large enough such that
\[
...
is the closure of a bounded and open subset of $\DS \RR^{n-1}$.
The proofs are almost identical.  They are even simpler because
the interior of the sets of the form
$\DS S_{\VEC{x},\epsilon} = \left\{\VEC{y} \in S :
\|\VEC{y}-\VEC{x}\| < \epsilon \right\}$ are open subsets of
$\DS \RR^{n-1}$.
...
to being a compact $\DS C^2$-manifold of dimension $n-1$ in
$\DS \RR^n$ with the topology on $S$ induced from the
topology on $\DS \RR^n$, $S$ is a measurable space.
...
\]
for $\VEC{x} \neq \VEC{y}$ because
$\DS N:\RR^n \setminus \{ \VEC{0} \} \to \RR$ is harmonic.
Thus
$\DS \VEC{x} \mapsto \pdydx{N}{\nu_{\VEC{y}}}(\VEC{x},\VEC{y})$
is harmonic on $\DS \RR^n\setminus \{\VEC{y}\}$ for all
$\DS \VEC{y} \in \RR^n$.  Moreover, since 
$\DS (\VEC{x},\VEC{y}) \mapsto
\diff^\alpha_{\VEC{x}}
\left(\pdydx{N}{\nu_{\VEC{y}}}(\VEC{x},\VEC{y})\right)$
is continuous on $\DS (\RR^n\setminus S)\times S$ for all
...
some results about $K$ and $\DS \pdydx{N}{\nu_{\VEC{y}}}$.

\begin{lemma} \label{pot_lem_dbl1}
There exists $C>0$ such that
$\DS \left| (\VEC{x}-\VEC{y}) \cdot \nu(\VEC{x}) \right| \leq C
\|\VEC{x}-\VEC{y}\|^2$ for all $\VEC{x},\VEC{y} \in S$.
\end{lemma}

\begin{proof}
\stage{i}
Let $\DS \BB = \left\{ \left(U_{\VEC{x}_j},
...
to the open cover $\DS \{ V_{\VEC{x}_j}\}_{j=1}^J$ of $S$.  Thus,
...
$\DS S_{\VEC{x},\epsilon} \subset U_{\VEC{x}_j}$ for some $j \in J$.
Since we assume that
$\DS \psi_{\VEC{x}_j}^{-1}$ is of class $\DS C^2$
on the compact set $\DS \overline{W_j}$, we
have that $\DS C_j = \max_{\VEC{w} \in \overline{W_j}}
\left\| \diff^2 \psi_j^{-1}(\VEC{w}) \right\| < \infty$
\footnote{For a bounded bilinear mapping
$\DS B:\RR^n \times \RR^n \to \RR^m$,
$\DS \|B\| = \max_{\|\VEC{x}\|= \|\VEC{y}\|=1}
\|B(\VEC{x},\VEC{y})\|$.
Hence, $\|B(\VEC{x},\VEC{y})\| \leq \|B\| \, \|\VEC{x}\|\, \|\VEC{y}\|$
for all $\DS \VEC{x}, \VEC{y} \in \RR^n$.}.

Let $\DS \breve{\VEC{x}} = \psi_{\VEC{x}_j}(\VEC{x})$ and, given
$\DS \VEC{y} \in S_{\VEC{x},\epsilon}$,
let $\DS \breve{\VEC{y}} = \psi_{\VEC{x}_j}(\VEC{y})$.
...
\right) \cdot \nu(\VEC{x})\right|
\]
for some $\DS \VEC{w} \in W_{\VEC{x}_j}$.  However,
\begin{align*}
...
\end{align*}
because $\DS \pdydx{\psi^{-1}_{\VEC{x}_j}}{w_k}(\breve{\VEC{x}})$
...
$\DS \|\breve{\VEC{y}}- \breve{\VEC{x}}\|
=\| \psi_{\VEC{x}_j}(\VEC{y})- \psi_{\VEC{x}_j}(\VEC{x})\|
\leq \| \VEC{y}- \VEC{x}\|$ since
$\DS \psi_{\VEC{x}_j}$ is the projection of
$\DS U_{\VEC{x}_j}$ along the vector
$\DS \nu(\VEC{x}_j)$ onto
$\DS W_{\VEC{x}_j}$.

\stage{iii} We get the conclusion of the lemma with
$\DS C = \max_{0\leq j \leq J} C_j$.
\end{proof}
...
\begin{proof}
We have that $\DS K(\VEC{x},\VEC{y}) = A(\VEC{x},\VEC{y})
...
$\DS \left| A(\VEC{x},\VEC{y})\right| \leq C/\omega_n$ for
...
Given $\DS \VEC{x} \in \Omega^{\prime}$, we have that
$w(\VEC{y}) = N(\VEC{x},\VEC{y})$ for $\VEC{y} \in \overline{\Omega}$
is harmonic on $\Omega$ and
$\DS w \in C^2(\Omega) \cap C^1(\overline{\Omega})$
...
Let $\DS \Omega_\epsilon = \Omega \setminus
...
$\DS w \in C^2\left(\Omega_\epsilon\right) \cap
...
$\DS D_\epsilon = \left\{\VEC{y} \in \partial B_\epsilon(\VEC{x}) :
  (\VEC{y}- \VEC{x})\cdot \nu(\VEC{x}) < 0 \right\}$
...
$\DS w \in C^2(B_\epsilon)\cap C^2\left(\overline{B_\epsilon}\right)$,
...
$\DS 4 \arctan(M_\epsilon/\sqrt{\epsilon^2 - M_\epsilon^2})$
of the sphere $\partial B_\epsilon(\VEC{x})$ covered by $Q_\epsilon$. 
However, we have from Lemma~\ref{pot_lem_dbl1} that
$\DS M_\epsilon \leq C \epsilon^2$ for some constant $C$.  Thus,
$\DS A_\epsilon \leq
4 \arctan(M_\epsilon/\sqrt{\epsilon^2 - M_\epsilon^2}) \to 0$ 
...
(\ref{pot_lem_dbl3E2}), the set $\DS \Omega^{\prime}$ is
then replaced by the complement of
$\overline{\Omega_j}$.     \label{pot_lem_dbl3_rmk} 
\end{rmk}
...
\right| \dss{S}{y} \leq B
\]
for all $\DS \VEC{x} \in \RR^n\setminus S$.
\end{lemma}

\begin{proof}
Given $\DS \VEC{x} \in \RR^n\setminus S$, let
$\DS d_{\VEC{x}} = \min_{\VEC{y} \in S}\|\VEC{y} - \VEC{x}\|$.
Since $S$ is compact, $\DS d_{\VEC{x}} >0$.
...
\stage{i} If $\DS d_{\VEC{x}} \geq \delta$, then
\[
\left| \pdydx{N}{\nu_{\VEC{y}}}(\VEC{x},\VEC{y})\right| 
...
For $\VEC{y} \in S \setminus S_{\VEC{x}_o,2\delta}$, we have
\[
\|\VEC{x} - \VEC{y} \|
...
$\DS \|\VEC{x}-\VEC{y}\|^n$ with $\VEC{y} \in S_{\VEC{x}_o,2\delta}$.
We have
\[
...
$\|\VEC{x}_o - \VEC{y}\| < 2\delta < 1/(2C)$, we get
\[
2\left| (\VEC{x} - \VEC{x}_o) \cdot (\VEC{x}_o - \VEC{y}) \right|
...
\]
If we substitute this expression in (\ref{Bounddny3}), then we get
\begin{align*}
\left| \pdydx{N}{\nu_{\VEC{y}}}(\VEC{x},\VEC{y})\right| 
...
\end{align*}
where we have used (\ref{defnQdetpsim1}),
$\DS \psi_{\VEC{x}_j}(S_{\VEC{x}_o,2\delta}) \subset
B_{2\delta}(\psi_{\VEC{x}_j}(\VEC{x}_o))$, and
$\DS \|\VEC{x}-\VEC{y}\| 
\geq \|\psi_{\VEC{x}_j}(\VEC{x}) - \psi_{\VEC{x}_j}(\VEC{y})\|$ for all
$\DS \VEC{x},\VEC{y} \in U_{\VEC{x}_j}$ to obtain the
inequality.  Using the change of variables
$\VEC{w} = \psi_{\VEC{x}_j}(\VEC{x}_o) + r \VEC{z}$ with $0 \leq r < 2\delta$
and $\DS \VEC{z} \in \partial B_1(\VEC{0}) \subset \RR^{n-1}$,
and posing $a = \|\VEC{x}-\VEC{x}_o\|$, we get
\begin{align}
&\int_{S_{\VEC{x}_o,2\delta}}
\frac{\left\|\VEC{x}-\VEC{x}_o\right\|}
{\left(\|\VEC{x}-\VEC{x}_o\|^2 + \|\VEC{x}_o-\VEC{y}\|^2\right)^{n/2}}
...
Then $\DS u\big|_\Omega$ has a continuous extension to
$\overline{\Omega}$ and $\DS u\big|_{\Omega^{\prime}}$
has a continuous extension to $\DS \overline{\Omega^{\prime}}$.

To be more precise, let
$\DS u_t(\VEC{x}) = u(\VEC{x} + t \nu(\VEC{x}))$
...
\end{equation}
for $\VEC{x} \in S$.
In other words, $\DS u^{[i]} = \frac{1}{2} \phi + T_K \phi$ and
$\DS u^{[o]} = -\frac{1}{2} \phi + T_K \phi$ respectively on $S$.
Then $\DS u_t \rightarrow u^{[i]}$ uniformly on $S$ as
$\DS t\rightarrow 0^-$ and
$\DS u_t \rightarrow u^{[o]}$ uniformly on $S$ as
$\DS t\rightarrow 0^+$.
\end{theorem}
...
\dss{S}{y} \ .
\]
From Lemma~\ref{pot_lem_dbl3}, we get
\begin{align*}
...
\stage{iii}
We prove that the convergence $\DS u_t \rightarrow u^{[i]}$ (resp.
$\DS u_t \rightarrow u^{[o]}$) is uniform on $S$ as
$\DS t\rightarrow 0^-$ (resp. $\DS t\rightarrow 0^+$).

Let $B$ be the constant from Lemma~\ref{pot_lem_dbl4} and let $A$
be a constant such that \\
$\DS \int_S \left| K(\VEC{x},\VEC{y}) \right|
...
Hence, for all $\VEC{x} \in S$ and $|t|<\delta$, we have
\begin{align*}
...
$\DS \RR^n \setminus \{ \VEC{x} \}$.
\item For $n>2$,
$\DS N(\VEC{x},\VEC{y}) = O(\|\VEC{y}\|^{2-n})$ as
...
have used (\ref{defnQdetpsim1}), $\DS \|\VEC{x}-\VEC{y}\| 
\geq \|\psi_{\VEC{x}_j}(\VEC{x}) - \psi_{\VEC{x}_j}(\VEC{y})\|$ for all
$\DS \VEC{x},\VEC{y} \in U_{\VEC{x}_j}$ and
$\DS W_{\VEC{x}_j} = \psi_{\VEC{x}_j}(U_{\VEC{x}_j}) \subset
...
we have used (\ref{defnQdetpsim1}), $\DS \|\VEC{x}-\VEC{y}\| 
\geq \|\psi_{\VEC{x}_j}(\VEC{x}) - \psi_{\VEC{x}_j}(\VEC{y})\|$ for all
$\DS \VEC{x},\VEC{y} \in U_{\VEC{x}_j}$ and
$\DS W_{\VEC{x}_j} = \psi_{\VEC{x}_j}(U_{\VEC{x}_j}) \subset
...
$\DS \RR^n \setminus S$.  Given $\mu >0$, let
$\DS S_{\VEC{z},\mu} = \{ \VEC{y} \in S : \|\VEC{z}-\VEC{y}\|<\mu\}
...
$\DS \VEC{x} \in B_{\mu/2}(\VEC{z}) \subset \RR^n$ as we
explain below.
...
get that
$\DS S_{\VEC{x},3\mu/2} =  B_{3\mu/2}(\VEC{x}) \cap S
...
\left| \det \diff \psi_{\VEC{x}_j}^{-1}(\VEC{w}) \right| \dx{\VEC{w}} \\
& \leq \frac{\|\phi\|_\infty Q}{(n-2)\omega_n}
\int_{\psi_{\VEC{x}_j}(S_{\VEC{x},3\mu/2})}
...
and $\DS \|\VEC{x}-\VEC{y}\|
\geq \|\psi_{\VEC{x}_j}(\VEC{x}) - \psi_{\VEC{x}_j}(\VEC{y})\|$ for all
$\DS \VEC{x},\VEC{y} \in V_{\VEC{x}_j}$, where we consider
$\psi_{\VEC{x}_j}$ to be the projection of $\DS \RR^n$ onto
...
$\DS \VEC{x} \in B_{\mu/2}(\VEC{z}) \subset \RR^n$.
...
\]
for all $\DS \VEC{x} \in B_\delta(\VEC{z}) \subset \RR^n$.
Hence
\[
...
$\DS \left| u(\VEC{x}) - u(\VEC{x}_0) \right| \leq \epsilon$ for all
...
Let $\DS K^\ast(\VEC{x},\VEC{y}) = K(\VEC{y},\VEC{x})$
for $\VEC{x},\VEC{y} \in S$,  where $K$ is defined in
(\ref{pot_K_defn}).  We have
\[
...
$\DS T_K:L^2(S)\rightarrow L^2(S)$ and
$\DS T_{K^\ast}:L^2(S)\rightarrow L^2(S)$ are
compact operators.  Moreover, since $K$ and $\DS K^\ast$ are
...
\dss{S}{x} \\
&=\int_S \left( \int_S K(\VEC{x},\VEC{y}) \psi(\VEC{x}) \dss{S}{x}
...
\end{align*}
for $\DS \phi,\psi \in L^2(S)$.
Thus $\DS T_{K^\ast} = T_K^\ast$, the adjoint operator to $T_K$ on
$\DS L^2(S)$.
...
\begin{proof}
Let $\DS \BB = \{ (U_{\VEC{x}_j}, \psi_{\VEC{x}_j}) \}_{j\in J}$
...
have used $\DS \|\VEC{x}-\VEC{y}\| 
\geq \|\psi_{\VEC{x}_j}(\VEC{x}) - \psi_{\VEC{x}_j}(\VEC{y})\|$ for all
$\DS \VEC{x},\VEC{y} \in U_{\VEC{x}_j}$ to get the last
inequality.

We get the first conclusion of the lemma with
$\DS C_1 = \max_{j \in J} M_j$ and $\eta \leq \epsilon /2$.
...
If $\phi \in C(S)$, then
$\DS u\big|_{\overline{\Omega}} \in C_{\nu(\Omega)}$ and
$\DS u\big|_{\overline{\Omega^{\prime}}} \in
C_{\nu(\Omega^{\prime})}$.
Moreover,
\[
...
then $\DS w_t \rightarrow \pdydx{u}{\nu^-}$ uniformly
on $S$ as $t\rightarrow 0^-$ and
$\DS w_t \rightarrow \pdydx{u}{\nu^+}$ uniformly on
$S$ as $t\rightarrow 0^+$.
\end{theorem}
...
$\DS u \in C(\overline{\Omega^{\prime}})$.  By definition of
the single layer potential, we also have that
$\DS u \in C^1(\Omega)$ and
$\DS u \in C^1(\Omega^{\prime})$.
...
\pdydx{N}{\nu_{\VEC{y}}}(\VEC{x},\VEC{y}) \dss{S}{y}
\]
for $\DS \VEC{x} \in \RR^n \setminus S$, and the function
\[
f(\VEC{x}) =
\begin{cases}
\DS v(\VEC{x}) + \pdydxS{u}{\nu_{\VEC{x}}}(\VEC{x}) & \quad
...
$O_\mu$ is an open subset of $\DS \RR^n$ because $S$ is a manifold of
...
$\DS f\big|_{O_\mu \setminus S}$ can be
continuously expanded to a continuous function on $O_\mu$ by
$\DS f(\VEC{x}) = (T_K\phi)(\VEC{x}) + (T^\ast_K\phi)(\VEC{x})$
for $\VEC{x} \in S$.
...
$S_{\VEC{x},\eta} \subset U_{\VEC{x}_j}$.  we have
\begin{align*}
...
\pdydxS{N}{\nu_{\VEC{x}}}(\VEC{z}_t,\VEC{y})\right| \dss{S}{y} \  .
\]
Using Lemma~\ref{pot_inequ}, we have
\begin{align*}
&\left| \pdydx{N}{\nu_{\VEC{y}}}(\VEC{z}_t,\VEC{y}) +
...
\dss{S}{y} \right)^{-1}
\]
for all $\DS \VEC{y} \in S \setminus S_{\VEC{x},\eta}$,
$|t| < \delta_1$ and $\VEC{x} \in S$.
Hence,
$\DS I_1(\VEC{x},t,\eta) \leq \epsilon/4$ for all $\VEC{x} \in S$
and $|t| < \delta_1$.

\stage{i.e} Since $\DS (t,\VEC{x},\VEC{y}) \mapsto
\nabla N(\VEC{x} + t \nu(\VEC{x}) - \VEC{y}) \cdot \nu(\VEC{x})$
is continuous on the compact set
...
Hence,
$\DS I_2(\VEC{x},t,\eta) \leq \epsilon/4$ for all $\VEC{x} \in S$
and $|t| < \delta_2$.

\stage{i.f} Hence, for $\delta < \min\{\delta_1,\delta_2\}$, we have
that 
$\DS \left| f(\VEC{z}_t) - f(\VEC{x}) \right|< \epsilon$ for
...
\stage{iii}
From Theorem~\ref{pot_double_layer}, we have
\[
\sup_{\VEC{x}\in S}
...
\]
From the claim in (i) above, we have
\begin{align*}
&\sup_{\VEC{x}\in S}
...
The proof that $\DS w_t \rightarrow \pdydx{u}{\nu^+}$
uniformly on $S$ as $\DS t\rightarrow 0^+$ is similar.  We
then also get that (\ref{pot_ddn_unif}) is satisfied and thus complete
the proof that $\DS u \in C_{\nu}(\Omega^{\prime})$.
\end{proof}
...
If $\phi \in C(S)$ and $\DS f = \frac{1}{2} \phi + T^\ast_K \phi$ on
$S$, where $K$ is defined in (\ref{pot_K_defn}), then
$\DS \int_{S} f \dx{S} = \int_{S} \phi \dx{S}$.
\end{lemma}
...
$\DS M = \max_{\VEC{y} \in S} \|\VEC{y}\| < \infty$.
...
$\DS \int_S \phi(\VEC{y}) \dss{S}{y} = 0$.
Moreover, if $\DS \int_S \phi(\VEC{y}) \dss{S}{y} = 0$, then
\begin{align*}
u(\VEC{x})
&= \frac{1}{\omega_2}
\int_S \left( \ln(\|\VEC{x}-\VEC{y}\|) - \ln(\|\VEC{x}\|)
...
The following subspaces of $\DS L^2(\partial \Omega)$ will play a
...
$\DS T_K^\ast$ are compact
linear operators from $\DS L^2(\partial \Omega)$ to itself.
...
components of $\Omega$.
\item $\DS \dim V_- = \dim W_- = m^{\prime}$, where $m'$ is
the number of bounded and connected components of
$\DS \Omega^{\prime}$.
\item If $\DS \VEC{a} \in \CC^{m^{\prime}}$, then there
exists a unique $\phi_{\VEC{a}} \in W_-$ such that the single layer
potential $w$ with moment $\phi_{\VEC{a}}$ satisfies
$\DS w\big|_{\Omega_0^{\prime}}=0$ and
$\DS w\big|_{\Omega_j^{\prime}} = a_j$ for
$1 \leq j \leq m^{\prime}$.
\item If $n>2$ and $\DS \VEC{a} \in \CC^m$, then there
exists a unique $\phi_{\VEC{a}} \in W_+$ such that the single layer
potential $u$ with moment $\phi_{\VEC{a}}$ satisfies
$\DS u\big|_{\Omega_j} = a_j$
for $1 \leq j \leq m$.
\item If $n=2$, there exists an $(m-1)$-dimensional subspace $X$ of
$\DS \CC^m$ with the following properties.
\begin{enumerate}
\item $\DS \CC^m = X \oplus \CC\left\{(1,1,1,\ldots,1)\right\}$.
...
satisfies $\DS u\big|_{\Omega_j} = a_j$
for $1 \leq j \leq m$.
\end{enumerate}
\end{enumerate}
\end{prop}
...
$\DS \phi_j^{\prime} : \partial \Omega \rightarrow \RR$ for
$\DS 1\leq j \leq m^{\prime}$, defined by
...
$\DS \partial \Omega_j^{\prime} = \bigcup_{i\in I} \partial \Omega_i$.
The minus sign in front of the sum comes from the change of orientation
between the integral over $\DS \partial \Omega_j^{\prime}$
and the integrals over the $\partial \Omega_i$.
If $\DS \VEC{x} \in \partial \Omega_j^{\prime}$, then
...
\]
for all $\DS \VEC{x} \in \partial \Omega^{\prime}$.
Hence, $\DS \phi_j^{\prime} \in V_-$ for
$\DS 1 \leq j \leq m^{\prime}$.
Since the $\DS \phi_j^{\prime}$ are linearly independent,
$\DS \dim V_- \geq m^{\prime}$.

\stage{iii}
Since $\DS -\frac{1}{2} \Id + T_K$ and
$\DS \frac{1}{2} \Id + T_K$ are compact operators, it
follows from Theorem~\ref{fu_an_comp_oper} that
$\DS \dim\ \KE \left(-\frac{1}{2} \Id + T_K\right) =
\dim\ \KE \left(-\frac{1}{2} \Id + T_K^\ast \right)$ and \\
$\DS \dim\ \KE \left(\frac{1}{2} \Id + T_K\right) =
\dim\ \KE \left(\frac{1}{2} \Id + T_K^\ast \right)$.

Since $\DS V_+ = \KE \left(-\frac{1}{2} \Id + T_K\right)$
and $\DS W_+ = \KE \left(-\frac{1}{2} \Id + T_K^\ast \right)$, 
we get that $\dim V_+ = \dim W_+$.  Since
$\DS V_- = \KE \left(\frac{1}{2} \Id + T_K\right)$
and $\DS W_- = \KE \left(\frac{1}{2} \Id + T_K^\ast \right)$, 
we get that $\dim V_- = \dim W_-$.

\stage{iv} If $\phi \in W_+$ and $u$ is the single layer potential with
moment $\phi$, then $\DS \pdydx{u}{\nu^-}(\VEC{x}) = 0$
...
$\Delta v = 0$ on $\DS \Omega^{\prime} \cup \{\infty\}$ 
with $\DS v\big|_{\partial \Omega^{\prime}}=0$,
we have that $u=0$ on $\DS \Omega^{\prime}$ because
$v \equiv 0$ is a solution of this problem.  Hence, $u=0$ on
$\DS \RR^n$.
...
We also have that $\DS \Lambda\big|_{W_+}$ is onto because
$\dim \Lambda(W_+) = m$.  Hence, {\em item (4) of the proposition} is
true because $\DS \Lambda\big|_{W_+}$ is one-to-one and onto
$\DS \CC^m$.
...
$\DS \Lambda\big|_{W_+^0} : W_+^0 \rightarrow \CC^m$ is
one-to-one.  In particular, this proves that $\DS \dim W_+^0 \leq m$.

Moreover, item (3) of Lemma~\ref{pot_1to1n2} shows that the space
$\DS Y = \left\{ \alpha (1,1,\ldots,1) : \alpha \in \CC \right\}$
is not in the image $X$ of $\Lambda\big|_{W_+^0}$.
Thus $\DS \dim W_+^0 \leq m-1$ because
$\Lambda\big|_{W_+^0}$ is one-to-one.
Also $\DS \dim W_+^0 \geq \dim W_+ - 1$ because
$\DS W_+^0$ is the kernel of the
linear functional
$\DS \phi \mapsto \int_{\partial \Omega} \phi(\VEC{x}) \dss{S}{x}$
from $W_+$ to $\CC$.
Therefore, $\dim W_+ \leq m$.  Combined with (i) and (iii), this shows
that {\em item (1) of the proposition is true for $n=2$}.

Since $\dim W_+ = m$, it follows that $\DS \dim W_+^0 = m - 1$.
Hence, $\DS X = \Lambda(W_+^0)$ is of dimension $m-1$ because
$\DS \Lambda\big|_{W_+^0}$ is one-to-one.
We then get that
$\DS \CC^m = X \oplus \CC\left\{(1,1,1,\ldots,1)\right\}$.
This completes the proof of {\em item (5) of the proposition} because
$\DS \Lambda\big|_{W_+^0}$ is one-to-one and onto $X$.

\stage{v}
We first claim that if $\phi \in W_-$ and $u$ is the single layer
potential with moment $\phi$, then $u$ is constant on each bounded
component $\DS \Omega_j^{\prime}$ of
$\DS \Omega^{\prime}$ and $u=0$ on the unbounded
component $\DS \Omega_0^{\prime}$.  We have from
Theorem~\ref{pot_slp_exist}
that $\DS \pdydx{u}{\nu^+}(\VEC{x}) = 0$
for $\DS \VEC{x} \in \partial \Omega^{\prime}$.  For $n>2$,
...
on each bounded component of $\DS \Omega^{\prime}$ and also
on $\DS \Omega_0^{\prime}$.
From Lemma~\ref{pot_sol_zeroB}, we have that
$\DS \int_{\partial \Omega} \phi(\VEC{x}) \dss{S}{x} = 0$.
Hence, Lemma~\ref{pot_1to1n2} implies that $u$ vanishes at infinity.  Since
$u$ is constant on $\DS \Omega_0^{\prime}$, $u=0$ on
$\DS \Omega_0^{\prime}$.
...
$\DS \Omega^{\prime}$.  We also have that $u = 0$ on the unbounded
components since this is true for $u_1$ and $u_2$.   Thus,
$u=0$ on $\DS \overline{\Omega^{\prime}}$.
By uniqueness of the interior Dirichlet problem (D1) given by
$\Delta v =0$ on $\Omega$ with $\DS v\big|_{\partial \Omega}=0$,
we have that $u=0$ on $\Omega$ because $v\equiv 0$ is a solution of this
problem.   Hence, $u = 0$ on $\DS \RR^n$.  It follows from
Corollary~\ref{pot_cor_pdpdm} that $\phi =0$ on
$\DS \partial \Omega^{\prime}$.
Thus $\phi_1=\phi_2$ on $\DS \partial \Omega^{\prime}$.

This shows that
$\DS \dim W_- \leq \dim \Lambda(W_-) \leq m^{\prime}$.  Combined
with (ii), this shows that $\DS \dim W_- = m^{\prime}$ and proves
{\em item (2) of the proposition}.

We also have that $\DS \Lambda\big|_{W_-}$ is onto because
$\DS \dim \Lambda(W_-) = m^{\prime}$.  Hence, 
{\em item (3) of the proposition} is true because
$\DS \Lambda\big|_{W_-}$ is one-to-one and onto
$\DS \CC^{m^{\prime}}$.
\end{proof}
...
product on $\DS L^2(\partial \Omega)$ defined by
\[
\ps{f}{g} = \int_{\partial \Omega} f(\VEC{x})\,\overline{g(\VEC{x})} \dx{S}
\]
for $\DS f,g \in L^2(\partial \Omega)$.

\begin{proof}
\stage{i} We first prove that
$\DS L^2(\partial \Omega) = V_+^\perp \oplus W_+$.  It
follows from Proposition~\ref{pot_split_prop} that
$\DS V_+^\perp$ is a
closed subspace of codimension $m$ in
$\DS L^2(\partial \Omega)$ and $W_+$
is a subspace of dimension $m$ in
$\DS L^2(\partial \Omega)$.  Thus, we
only have to prove that $\DS V_+^\perp \cap W_+ = \{0\}$.

Suppose that $\DS \phi \in V_+^\perp \cap W_+$.
Since $\phi \in W_+$, we have that
$\DS \phi \in \KE \left(T^\ast_K - \frac{1}{2} \Id\right)$.
Since $\DS \phi \in V_+^\perp
= \left(\KE \left(T_k -\frac{1}{2} \Id\right)\right)^\perp$,
it follows from Theorem~\ref{fu_an_RIOrth} that
$\DS \phi \in \IMG \left( T^\ast_K - \frac{1}{2} \Id\right)$
because $\DS T_k^\ast$ is a compact operator
and so $\DS \IMG \left( T^\ast_K - \frac{1}{2} \Id\right)$
is a close subspace according to Theorem~\ref{fu_an_comp_oper}.  Thus, there
exists $\DS \psi \in L^2(\partial \Omega)$ such that
$\DS \phi = T^\ast_K \psi - \frac{1}{2} \psi$.  Moreover,
since $\DS T^\ast_K\phi - \frac{1}{2} \phi = 0$, it follows
...
$\DS T^\ast_K \psi - \frac{1}{2} \psi = \phi$.

Let $u$ and $v$ be the simple layer potentials with moments $\phi$ and
$\psi$ respectively.  We have that $\DS u,v \in C(\RR^n)$
...
$\DS \partial B_r(\VEC{0}) \subset \Omega_0^{\prime}$,
the unbounded component of $\DS \Omega^{\prime}$.
...
\end{align}
because $u$ is harmonic in $\DS \Omega^{\prime}$.  The
normal to $\DS \partial (\Omega^{\prime} \cap B_r(\VEC{0}))$
points inside $\DS \Omega^{\prime} \cap B_r(\VEC{0})$.
...
$\DS \Omega^{\prime} \cap B_r(\VEC{0})\phi \in V_+^\perp$,
we have
\[
...
$\DS \sum_{i=0}^m \phi_i(\VEC{x}) = 1$ for all
$\VEC{x} \in \partial \Omega$.  It follows from
Lemma~\ref{pot_1to1n2} that $u$ is harmonica at infinity.

If $n>2$, then we have from Proposition~\ref{pot_infty_u} and
Proposition~\ref{pot_infty_ddu} that
$\DS \Omega^{\prime} \cap B_r(\VEC{0})|u(\VEC{x})|
\leq C_1\|\VEC{x}\|^{2-n}$ and
$\DS \left| \pdydx{u}{\nu}(\VEC{x})\right| \leq
...
$\DS \int_{\Omega^{\prime}} \|\graD u\|_2^2 \dx{\VEC{x}} = 0$.
Thus $\graD u = 0$ on $\DS \Omega^{\prime}$.  This shows that $u$ is
constant on each component of $\DS \Omega^{\prime}$.  From
this, we get that $\DS \phi = \pdydx{u}{\nu^+} = 0$.

\stage{ii} The proof that
$\DS L^2(\partial \Omega) = V_-^\perp \oplus W_-$ is very
similar to the proof given in (i). It
follows from Proposition~\ref{pot_split_prop} that
$\DS V_-^\perp$ is a
closed subspace of codimension $\DS m^{\prime}$ in
$\DS L^2(\partial \Omega)$ and $W_-$
is a subspace of dimension $\DS m^{\prime}$ in
$\DS L^2(\partial \Omega)$.  Thus, we
only have to prove that $\DS V_-^\perp \cap W_- = \{0\}$.
...
= \left(\KE \left(T_k + \frac{1}{2} \Id\right)\right)^\perp$,
it follows from Theorem~\ref{fu_an_RIOrth} that
$\DS \phi \in \IMG \left( T^\ast_K + \frac{1}{2} \Id\right)$ because
$\DS T_k^\ast$ is a compact operator
and so $\DS \IMG \left( T^\ast_K + \frac{1}{2} \Id\right)$
is a close subspace according to Theorem~\ref{fu_an_comp_oper}.  Thus, there
exists $\DS \psi \in L^2(\partial \Omega)$ such that
$\DS \phi = T^\ast_K \psi + \frac{1}{2} \psi$.  Moreover,
...
$\DS \Omega^{\prime}$, so inside $\Omega$.

From the Green's identity (\ref{laplace_green1}), we get
...
\end{align}
because $u$ and $v$ are harmonic in $\DS \Omega^{\prime}$.
The normal to $\DS \partial (\Omega^{\prime} \cap B_r(\VEC{0}))$
points outside $\DS \Omega^{\prime} \cap B_r(\VEC{0})$.
...
$\DS \int_{\partial \Omega} \phi \dx{S} = 0$.
Moreover, since $\DS T^\ast_K \psi + \frac{1}{2} \psi = \phi$,
we can use Lemma~\ref{pot_sol_zeroB} again to conclude that
$\DS \int_{\partial \Omega} \psi \dx{S} = 0$.
...
$\DS \int_{\partial B_r(\VEC{0})} u \pdydx{v}{\nu} \dx{S}$
and
$\DS \int_{\partial B_r(\VEC{0})} v \pdydx{u}{\nu}  \dx{S}$
converge to $0$ as $r \to \infty$.

Therefore, if we let $r \to \infty$ in (\ref{pot_L2_split1_E6}), then we
get from (\ref{pot_L2_split1_E4}) and (\ref{pot_L2_split1_E5}) that
$\DS \int_{\Omega} \|\graD u\|_2^2 \dx{\VEC{x}} = 0$.
...
\begin{proof}
Since $\DS T_K$ is a compact operator, it
follows from Theorems~\ref{fu_an_comp_oper} and \ref{fu_an_RIOrth} that\\
$\DS  \IMG \left(-\frac{1}{2} \Id + T_K\right)
...
$\DS W_+^\perp \cap V_+ = W_-^\perp \cap V_- = \{0\}$
because $\DS W_+^\perp$ has codimension $m$ and $V_+$ has
dimension $m$, and $\DS W_-^\perp$ has codimension
$\DS m^{\prime}$ and $V_-$ has dimension $\DS m^{\prime}$.

Suppose that $\DS \psi \in W^\perp_+ \cap V_+$.  It follows from
Proposition~\ref{pot_L2_split1} that $\psi = \psi_1+\psi_2$, where
$\DS \psi_1 \in V^\perp_+$ and $\psi_2 \in W_+$.  Thus
\[
...
A similar proof yields $\DS W_-^\perp \cap V_- = \{0\}$.
\end{proof}
...
$\DS f \in  C(\partial \Omega^{\prime})$.
\item ({\bfseries N1}) has a solution for $f \in  C(\partial \Omega)$
if and only if
$\DS \int_{\partial \Omega_i} f(\VEC{x})\dss{S}{x}=0$
for $1 \leq i \leq m$.  The solution is unique modulo a
function which is constant on each connected component of $\Omega$.
\item ({\bfseries N2}) has a solution for
$\DS f \in  C(\partial \Omega^{\prime})$ if and only if
$\DS \int_{\partial \Omega_j^{\prime}} f(\VEC{x})\dss{S}{x}=0$
for $\DS 1\leq j \leq m^{\prime}$ when $n>2$, and
$\DS 0\leq j \leq m^{\prime}$ when $n=2$.  The solution is
unique modulo a function $h$ such that $h(\VEC{x}) = c$, a fixed
constant, on each connected component
$\DS \Omega_j^{\prime}$ for $\DS 1\leq j \leq m^{\prime}$.
\end{enumerate}
\end{theorem}
...
$\DS \{\phi_i\}_{i=1}^m$ is a basis of $V_+$.  Hence,
\[
...
we have that $\DS g \in \IMG \left(-\frac{1}{2} \Id +T^\ast_K\right)$
if and only if\\
$\DS g \in \left( \KE \left( -\frac{1}{2} \Id +
T_K\right)\right)^\perp = V_+^\perp$.

By hypothesis $\DS f \in V_+^\perp$.  Therefore, there exists
$\DS \phi \in L^2(\partial \Omega)$ such that
\[
\boxed{
...
we have that $\DS g \in \IMG \left(\frac{1}{2} \Id +T^\ast_K\right)$
if and only if\\
$\DS g \in \left(\KE \left(\frac{1}{2} \Id +T_K\right)\right)^\perp
= V_-^\perp$.
...
\]
Since $\DS f\in C(\partial \Omega^{\prime})$, we have that
$\DS \phi \in C(\partial \Omega^{\prime})$ by
...
$\DS \int_{\partial \Omega_0^{\prime}} f(\VEC{x})\dss{S}{x} = 0$ 
...
\]
for some $\DS \phi \in L^2(\partial \Omega)$ and $a_j \in \CC$.
Since $f\in C(\partial \Omega)$ and
$\DS \phi_j^{\prime} \in C(\partial \Omega)$
...
$\DS \frac{1}{2} \phi + T_K \phi$ instead of $f$ as boundary
condition.
...
\]
satisfies $\DS w\big|_{\Omega_j^{\prime}} = a_j$ for
$\DS 1\leq j \leq m^{\prime}$, and
$\DS w\big|_{\Omega_0^{\prime}} = 0$.
Since $\DS w \in C(\RR^n)$ by
...
\]
for some $\DS \phi \in L^2(\partial \Omega)$ and $a_i \in \CC$.
...
\]
satisfies $\DS w\big|_{\Omega_i} = a_i$ for
$1\leq i \leq m$.
Since $\DS w \in C(\RR^n)$ by Proposition~\ref{pot_slp_cont},
$\DS 
...
\stage{$\mathbf{n=2}$}
Since
$\DS \sum_{i=1}^m \phi_i(\VEC{x}) = 1$ for
...
exists $\DS \psi \in W_+^0$ such that the single layer
...
\]
satisfies $\DS w\big|_{\Omega_i} = b_i$ for
$1\leq i \leq m$.  Since
$\DS \int_{\partial \Omega} \psi(\VEC{y}) \dss{S}{y} = 0$
because $\DS \psi\in W_+^0$, we also have from
Lemma~\ref{pot_1to1n2} that $w$ is harmonic at infinity.  Since
$\DS w \in C(\RR^n)$ by 
Proposition~\ref{pot_slp_cont},
$\DS w\big|_{\partial \Omega^{\prime}} = \sum_{i=1}^m b_i \phi_i$.
Therefore,
\[
\boxed{
