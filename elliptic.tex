...
open subset of $\DS \RR^n$ and $u$ be a distribution on $\Omega$.  If
$\VEC{\alpha} = \VEC{e}_j$, then we denote the derivative
$\DS \diff^{\VEC{\alpha}} u$ in the sense of distribution by
$\diff_{x_j} u$.  If $\VEC{\alpha} = \VEC{e}_{j_1} + 3 \VEC{e}_{j_2}$,
then we denote the derivative $\DS \diff^{\VEC{\alpha}} u$ in the
...
integrable derivatives, then $\DS \diff_{x_j} u = \pdydx{u}{x_j}$
and $\DS \diff_{x_{j_1}x_{j_2}x_{j_2}x_{j_2}} u
...
$\DS C^k$ functions that contains the weak solution $u$.
...
\end{split}
\end{equation}
where $\DS f \in L^2(]a,b[)$.  We assume that all functions are real
valued functions.                   \label{ell_example1}

If $f\in C([a,b])$, then the classical solution of
(\ref{ell_example1_eq1}) is a function $\DS u\in C^2([a,b])$ that
satisfies (\ref{ell_example1_eq1}).  Recall that $\DS C^k([a,b])$,
$k\in \NN$, is the set of all
$\DS u \in C([a,b]) \cap C^k(]a,b[)$ such
that $\DS \dydxn{u}{x}{j}$ can be continuously extended
...
\]
for all $\DS v \in C^1_c(]a,b[)$.
Since $\DS C^1_c(]a,b[)\supset \DD(]a,b[)$ is dense in
$\DS H^1_0(]a,b[)$, we get (\ref{ell_example1_eq4}).  To be
more precise, if
$\DS \left\{v_i\right\}_{i=1}^\infty \subset C^1_c([a,b])$
converges to $\DS v \in H^1_0(]a,b[)$ with respect to the norm
$\|\cdot\|_{1,2,]a,b[}$, then
$\DS \|v_i - v\|_{2,]a,b[} \to 0$ and
$\DS \|\diff_x v_i - \diff_x v\|_{2,]a,b[} \to 0$ as $i \to 0$.
...
\end{align*}
because $\DS u' \in L^2(]a,b[)$ since $u\in C^2([a,b])$.  Similarly,
\[
...
$\DS u \in H^1_0(]a,b[)$ because \\
$\DS u \in C^2([a,b]) \subset H^1(]a,b[) \cap C([a,b])$ and
$u(a)=u(b)=0$.
...
\]
for all $\DS u,v \in H^1_0(]a,b[)$.
It is clear that $B$ is bilinear.  To prove that $B$ is
bounded, we use Schwarz inequality in $\DS \RR^2$ and
$\DS L^2(]a.b[)$ to get
\begin{align*}
...
\end{align*}
for all $\DS u,v \in H^1_0(]a,b[)$.
Since $\DS B(u,u) = \|u\|_{1,2}^2$ for
$\DS u\in H^1_0(]a,b[)$, it is obvious
that $B$ is coercive.  It is also obvious that $B$ is
symmetric; namely, $B(u,v)=B(v,u)$ for all $\DS u,v \in H^1_0(]a,b[)$.
Moreover, the function $\DS f \in L^2(]a,b[)$ yields a
bounded functional on $\DS H^1_0(]a,b[)$ defined by
\[
\tilde{f}(u) = \ps{u}{f}_2 = \int_a^b f(x)u(x)\dx{x}
\]
for all $\DS u \in H^1_0(]a,b[)$.
...
that $\DS u\in H^2(]a.b[)$.

\subI{Classical Solution}
We prove that if $f \in C([a,b])$, then the weak solution $u$
belongs to $\DS C^2([a,b])$.  According to
Corollary~\ref{sob_cor_sob_lem},
we get that $\DS u \in C^1([a,b])$ because
$\DS u\in H^2(]a,b[)$.  We also
have from $B(v,u)=\ps{v}{f}_2$ with
$\DS v\in \DD(]a,b[) \subset H^1_0(]a,b[)$
...
$\DS \diff_x u \in C^1([a,b])$.
Thus $\DS u\in C^2([a,b])$.

We now prove that a weak solution
$\DS u \in C^2([a,b]) \cap H^1_0(]a,b[)$
is a classical solution.  If
$\DS u \in C^2([a,b]) \cap H^1_0(]a,b[)$ be
...
for all $\DS v \in C^1_c(]a,b[)$.
Since $\DS C^1_c(]a,b[) \supset \DD(]a,b[)$ is dense in
$\DS L^2(]a,b[)$, we obtain that
$-u'' + u -f = 0$ almost everywhere in $]a,b[$.  Since
$\DS u\in C^2([a,b])$, we have that 
...
$\DS u \in H^{1,2}_0(]a,b[) \cap C([a,b])$.  Thus $u$ is the classical
...
\end{split}
\end{equation}
where $\DS f\in L^2(]0,1[)$, $q\in C([0,1])$ and
$\DS p\in C^1([0.1])$.
...
\subI{Variational Problem}
The weak solution is the solution $\DS u\in H^{1,2}_0(]0,1[)$ of the
equation
\begin{equation} \label{ell_exampleSL_eq2}
\int_0^1 \left( p\, \diff_x u \, \diff_x v + q\, u\, v
- f\, v \right) \dx{x} = 0
\end{equation}
for all $\DS v \in H^{1,2}_0(]0,1[)$.

We prove that a classical solution is a weak solution.  
If $\DS u\in C^2([0,1])$ is a classical solution of
...
\]
for all $\DS v \in C^1_c(]0,1[)$.   Since
$\DS C^1_c(]0,1[) \supset \DD(]0,1[)$ is dense in
$\DS H^{1,2}_0(]0,1[)$, we have that
(\ref{ell_exampleSL_eq2}) is true.  Moreover, it follows from 
Proposition~\ref{sob_w0_trad2} that
$\DS u\in H^{1,2}_0(]0,1[)$ because
...
\]
Then, using Schwarz inequality in $\DS \RR^2$ and
$\DS L^2([0,1])$, we get
\begin{align*}
...
= M\| u\|_{1,2} \, \|v\|_{1,2}
\end{align*}
for all $\DS u,v \in H^{1,2}_0(]0,1[)$.
...
\int_0^1 \left( p\,u'\, v' + q\,u\,v - f\, v\right) \dx{x} = 0
\]
for all $\DS v \in C^1_c([0,1[)$.
Since $\DS C^1_c(]0,1[)\supset \DD(]0,1[)$ is dense in
$\DS L^2(]0,1[)$, we
get $\DS -(p\,u')' + q\,u - f = 0$ almost
everywhere.  Since $f$, $p'$ and $q$ are all continuous functions,
$\DS -(pu')' + q u -f = 0$ everywhere on $]0,1[$ because
$\DS u\in C^2(]0,1[)$.  It also follows from
Proposition~\ref{sob_w0_trad2} that $u(0)=u(1)=0$ because
$\DS u \in H^1_0(]0,1[)\cap C^2([0,1])$.  Thus $u$ is the classical
...
\end{equation}
where $\DS b_{\VEC{\alpha},\VEC{\beta}} \in C^\infty(\Omega)$ for all
...
on an open set $\DS \Omega \subset \RR^n$ if there exists a
constant $C>0$ such that
$\DS \RE Q_{df}(\VEC{x}, \VEC{\xi}) \geq C \|\VEC{\xi}\|^m$
for all $\VEC{x} \in \Omega$ and $\DS \VEC{\xi} \in \RR^n$.
Note that there is no factor $\DS (-1)^{m/2}$ on the left
side because we have a factor of $\DS (-1)^{m/2}$ in the
definition of $Q(\VEC{x},\VEC{\xi})$.  So, the factor on the left hand side is
$\DS (-1)^m =1$ since $m$ is even.
...
$\VEC{x} \in \Omega$ and $\DS \VEC{\xi} \in \RR^n$ with
$\VEC{\xi} \neq \VEC{0}$.
...
subset of $\DS \RR^n$ and that $\partial \Omega$ is a
$\DS C^\infty$-manifold, or at least that $\Omega$ satisfies
...
$\DS u \in C^{2k}_b(\Omega) \cap C^{k-1}_b(\overline{\Omega})$
such that
\[
...
\begin{defn}
If $\DS f\in L_{loc}^2(\Omega)$, a {\bfseries strong solution}
\index{Dirichlet Problem!Strong Solution}
for (\ref{ell_PDE1}) is a distribution
$\DS u \in H^{2k,2}(\Omega) \cap H^{k,2}_0(\Omega) \subset
\DD'(\Omega)$ such that
\[
...
$\DS u\in H^{k,2}_0(\Omega)$ such that
\begin{equation} \label{ell_PDE2}
B(v,u) = \ps{v}{f}_2
\end{equation}
for all $\DS \DS v \in H^{k,2}_0(\Omega)$, where
...
for $\DS u,v \in L^2(\Omega)$ is the scalar product in
$\DS L^2(\Omega)$.  Moreover, we assume that the
...
\end{align*}
for all $\DS (v,u) \in H^{k,2}(\Omega) \times H^{k,2}(\Omega)$,
where we have used Schwarz inequality for the second inequality and
set $\DS C = \sum_{|\VEC{\alpha}|,|\VEC{\beta}|\leq k} M $.
...
$\DS v\in H^{k,2}(\Omega)$, and
\item there exists a constant $C$ such that
$\DS \left| B(v,u) \right| \leq C \| u\|_{k,2} \, \| v\|_{k,2}$
for $\DS (v,u) \in H^{k,2}(\Omega) \times H^{k,2}(\Omega)$,
\end{enumerate}
...
$\DS \Delta$ in $\DS \RR^2$.  Let $\eta=x-y$ and
$\xi = x+y$.
...
Suppose that $\DS b_{\VEC{\alpha},\VEC{\beta}}\in
...
$\DS u \in C^{2k}_b(\Omega)\cap C^{k-1}_b(\overline{\Omega})$
satisfy (\ref{ell_PDE2}) for all $\DS v \in C^{k}_c(\Omega)$.  Using
...
\dx{\VEC{x}}
\]
for all $\DS v \in C^{k}_c(\Omega)$.
Since $\DS C^{k}_c(\Omega) \supset \DD(\Omega)$ is
dense in $\DS L^2(\Omega)$, and $L(\VEC{x},\diff)u$ and $f$
...
\]
Moreover, since $\DS u \in H^{k,2}_0(\Omega)$ because we assume that
...
that $\DS b_{i,j} \in C^\infty(\overline{\Omega})$ for all
$i$ and $j$, and $\DS b_0 \in C^\infty(\overline{\Omega})$.
...
\quad , \quad \VEC{x} \in \Omega \ ,
\]
where $\DS b_i = \frac{1}{h}\pdydx{h}{x_i}$ for $1 \leq i \leq n$,
...
$\DS u \in C^2_b(\Omega) \cap C^1_b(\overline{\Omega})$ such that
\[
L(\VEC{x}, \diff)u(\VEC{x}) = -\sum_{i,j=1}^n
\diff_{x_i} \left( b_{i,j} \diff_{x_j} u \right)(\VEC{x})
...
$\DS u\in H^{1,2}(\Omega)$ such that
\begin{equation} \label{ell_PDE5}
B(v,u) = \ps{v}{f}_2
\end{equation}
for all $\DS v \in H^{1,2}(\Omega)$, where
...
that $B$ is a bounded sequilinear form on $\DS H^{1,2}(\Omega)$.

\begin{defn}
If $\DS b_{i,j} \in C^\infty_b(\overline{\Omega})$,
$\DS b_i \in C^\infty_b(\Omega)$ and
$\DS f \in L^2(\Omega)$,
a {\bfseries weak solution}\index{Neumann Problem!Weak Solution}
for (\ref{ell_PDE5}) is a function
$\DS u \in H^{1,2}(\Omega)$ such that
$\DS B(v,u) = \ps{v}{f}_2$ for all
$\DS v \in H^{1,2}(\Omega)$.
\end{defn}

Consider the variational problem (\ref{ell_PDE5}), where
$\DS b_{i,j}\in C^{1}_b(\overline{\Omega})$, $b_0 \in C_b(\Omega)$ and
$f\in C_b(\Omega)$.  Suppose that
$\DS u \in C^2_b(\Omega)\cap C^1_b(\overline{\Omega})$ is a
solution of \ref{ell_PDE5}.

For all $\DS v \in C^1_b(\overline{\Omega})$, the divergence
theorem yields
\begin{align*}
\int_{\partial \Omega}
...
\pdydx{\overline{u}}{x_j}\right)}{x_i} \,v \dx{\VEC{x}} 
\]
for all $\DS v \in C^1_b(\overline{\Omega})$.  It then follows from
$B(v,u) = \ps{v}{f}$ that
...
\,v \, \nu_i \right) \dx{\VEC{x}} 
\end{equation}
for $\DS v \in C^{1}_b(\overline{\Omega})$.
From (\ref{ell_Neum_CWC}) with $\DS v \in C^1_b(\Omega)$, we get
\[
\ps{v}{f}_2 = \int_\Omega \overline{\left(-
\sum_{i,j=1}^n \pdfdx{\left( b_{i,j} \pdydx{u}{x_j} \right)}{x_i}
+ b_0 u \right)} \, v \dx{\VEC{x}}
\]
for $\DS v \in C^{1}_b(\Omega)$.
Since $\DS C^{1}_b(\Omega) \supset \DD(\Omega)$ is dense in
$\DS L^2(\Omega)$, and $L(\VEC{x},\diff)u$ and $f$ are
...
$\DS \ps{v}{f} = \int_\Omega v\,f \dx{\VEC{x}}$, do not
depend on the value of $\DS v \in C^1_b(\overline{\Omega})$ on the
boundary $\partial \Omega$ (a set of measure zero).  For
(\ref{ell_Neum_CWC}) to be true for all
$\DS v \in C^1_b(\overline{\Omega})$,
we must have that
\[
\int_{\partial \Omega}
\sum_{i,j=1}^n \left( \overline{b_{i,j}}\,\pdydx{\overline{u}}{x_j}
...
\]
for all $\DS v \in C^{1}_b(\overline{\Omega})$.
Finally, since $\DS C_b^1(\overline{\Omega})\big|_{\partial \Omega}$
is dense in $\DS L^2(\partial \Omega)$, and the $b_{i,j}$ and
$\DS \pdydx{u}{x_j}$ are continuous on $\partial \Omega$, we
...
subset of $\DS \RR^n$ satisfying at least a strong
$m$-extension property with $m\geq k$, and $X$ is a subspace of
$\DS H^{k,2}(\Omega)$ containing
$\DS H^{k,2}_0(\Omega)$.  We consider the elliptic partial 
...
all of class $\DS C^\infty_b(\Omega)$ and $f\in L^2(\Omega)$.
...
Recall that we have defined $\DS \pdydxS{u}{\nu}$
...
$\DS \{ P_j(\VEC{x},\diff)\}_{j=0}^{k-1}$,
there exists a set of differential operators
$\DS \{ \tilde{P}_j(\VEC{x},\diff) \}_{j=k}^{2k-1}$ such that
\[
...
\]
for all $\DS u,v \in C^{2k}(\overline{\Omega})$.
Moreover, if $B$ is coercive, the set of differential operators
$\DS \{ \tilde{P}_j(\VEC{x},\diff) \}_{j=k}^{2k-1}$ is normal.
\end{theorem}

A long proof of this theorem is given in \cite{FoPDE}.

\begin{theorem} \label{ell_normal_BCs}
Suppose that $\Omega$ is a bounded open subset of $\DS \RR^n$
satisfying at a strong $k$-extension property.
Let $\DS \{ P_j(\VEC{x},\diff) \}_{j=0}^{k-1}$, be a normal set of
differential operators associated to $L(\VEC{x},\diff)u=f$.  Given
$\DS g_j\in C^\infty(\partial \Omega)$ with $0\leq j<k$, there
exists $\DS g \in C^\infty_c(\RR^n)$ such that
$P_j(\VEC{x},\diff)g=g_j$ on $\partial \Omega$ for $0\leq j<k$.
\end{theorem}

\begin{proof}
\stage{i} Suppose that
$\DS P_j(\VEC{x},\diff) = \pdydxnS{}{\nu}{j}$ on a
tubular neighbourhood $O_\epsilon$ of $\partial \Omega$ as given
by Theorem~\ref{pot_TBN}.  Choose
$\DS \phi\in C^\infty_c(\RR)$ such that
$\DS \supp \phi \subset ]-\epsilon/2, \epsilon/2[$ and
$\phi(x) = 1$ for $x\in]-\epsilon/4, \epsilon/4[$.

Consider the function $\DS g:\RR^n \rightarrow \RR$ defined by
\[
g(\VEC{x}) =
\begin{cases}
\DS \phi(t) \sum_{j=0}^k \frac{t^j}{j!} g_j(\VEC{y}) &\quad
...
\end{cases}
\]
We have that $\DS g\in C^\infty_c(\RR^n)$,
$\DS \supp g \subset O_\epsilon$ and
\[
...
the $f_j$ are given recursively by $\DS f_0 = \frac{g_0}{q_0}$
and
$\DS f_j = \frac{1}{q_j} \left( g_j - \sum_{i=0}^{j-1}
...
differential operators $\DS \{ \tilde{P}_j\}_{j=k}^{2k-1}$
such that
...
\]
for all $\DS  v\in H^{k,2}(\Omega)$.
One can show that the solution $u$ is in fact in
$\DS H^{2k,2}(\Omega)$ if $f$ is nice\\
...
$\DS \left\{ P_j \right\}_{j=0}^{k-1}$ for
$L(\VEC{x},\diff)u = f$ in $\Omega$, where $\DS f \in L^2(\Omega)$.
...
\overline{\tilde{P}_{2k-1-j}(u)(\VEC{x})} \dx{S}
\]
for all $\DS v,u \in C^\infty(\overline{\Omega})$.
Hence
...
\overline{\tilde{P}_{2k-1-j}(u)(\VEC{x})} \dx{S}
\end{equation}
for all $\DS v,u \in X \cap C^\infty(\overline{\Omega})$.
Given $\DS u\in X \cap C^\infty(\overline{\Omega})$, we may use
Theorem~\ref{ell_normal_BCs} to find
$\DS v \in C^\infty(\overline{\Omega})$ such that
...
$\DS v\in X \cap C^\infty(\overline{\Omega})$.  Therefore,
we must require that $\DS \tilde{P}_{2k-1-j}(u) = 0$ on
...
$\DS \RE B(f,f) \geq C \|f||_{k,2,\Omega}^2 - \lambda
...
\begin{proof}
\stage{i} Choose $R>0$ such that
$\DS (1+r^2)^s < \epsilon (1+r^2)^t$ for $r> R$.
This is possible because
\[
\lim_{r\rightarrow \infty} \frac{(1-r^2)^s}{(1-r^2)^t} = 0
\]
for $t>s$.  Let $\DS C= (1+R^2)^{s-r}$.  Then
\[
...
+ C_1 \| f \|_{r,\rho}^2
\]
for $\DS f \in H^{t,2}(\RR^n)$.  Hence,
\[
\frac{1}{Q_1}\, \|f\|_{s,2,\RR^n}^2 \leq \frac{\epsilon}{Q_1}
\| f\|_{t,2,\RR^n}^2 + C_1 Q_3 \| f \|_{r,2,\RR^n}^2
\]
for $\DS f \in H^{t,2}(\RR^n)$.  We then get that
\[
\|f\|_{s,2,\RR^n}^2 \leq \epsilon \| f\|_{t,2,\RR^n}^2 + C \| f \|_{r,2,\RR^n}^2
\]
for $\DS f \in H^{t,2}(\RR^n)$, where $C= C_1 Q_1 Q_3$.
\end{proof}

\begin{lemma} \label{ell_garding2}
Let $\Omega$ be a bounded open subset of $\DS \RR^n$.  Suppose that
\[
...
\]
for $\DS u  \in H^{k,2}_0(\Omega)$.
\end{lemma}

\begin{proof}
For $\DS u \in H^{k,2}_0(\Omega)$, let
$\DS \underline{u}: \RR^n \rightarrow \CC$ be the function defined by
\[
\underline{u}(\VEC{x}) =
\begin{cases}
...
&\quad = \int_{\RR^n} \left( \sum_{|\VEC{\alpha}|,|\VEC{\beta}|= k}
...
\end{align*}
for $\DS u \in H^{k,2}_0(\Omega)$, where
...
where $\DS 1/C = \max_{\|\VEC{x}\|=1}
\sum_{|\VEC{\alpha}|=k} \VEC{x}^{2\alpha}$,
and taken $\theta_2 = C \theta_1$.

Since $\underline{u}(\VEC{x}) = 0$ for
$\DS \VEC{x} \in \RR^n \setminus \Omega$, we get
$\DS \RE B(u,u) \geq \theta_3 \|u\|_{k,2,\Omega}^2$ for all
$\DS u \in H^{k,2}_0(\Omega)$ with $\theta_3>0$.  We take
...
$\DS \RE B(u,u) \geq C_1 \|u\|_{k,2,\Omega}^2$ for all
$\DS u\in H^{k,2}_0(\Omega)$ such that $\supp u$ is small enough.
...
\end{equation}
for all $\DS \VEC{\xi} \in \RR^n$ and $\VEC{z}\in \overline{\Omega}$.
...
\end{equation}
for $\DS u \in H^{k,2}_0(\Omega)$ and $\VEC{z} \in \Omega$.
...
\end{equation}
for all $\DS u \in H^{k,2}_0(\Omega)$ such that there
...
$\DS \{\VEC{z}_j\}_{j=1}^J$ of $\Omega$ such that
$\DS \{B_\delta(\VEC{z}_j)\}_{j=1}^J$ is an open cover of
$\overline{\Omega}$.  Let $\DS \{\phi_j\}_{j=1}^J$ be a
partition of unity subordinate to this open cover.  Since
$\DS \sum_{j=1}^J \phi(\VEC{x}) = 1$ for all
$\VEC{x} \in \overline{\Omega}$,
we have that $\DS \sum_{j=1}^J |\phi(\VEC{x})|^2 > 0$ for all
$\VEC{x} \in \overline{\Omega}$.  Let
$\DS \psi_j(\VEC{x}) = \phi_j(\VEC{x})
\left( \sum_{j=1}^J |\phi(\VEC{x})|^2\right)^{-1}$
for $\VEC{x} \in \overline{\Omega}$.
We have that $\DS \psi_j \in C^\infty(\overline{\Omega})$
for all $j$ and $\DS \sum_{j=1}^J |\psi_j(\VEC{x})|^2 = 1$ for all
...
\end{align*}
Since $\DS \psi_j\,u \in H^{k.2}_0(\Omega)$ has compact support in
...
\end{align*}
for $\DS u \in H^{k,2}_0(\Omega)$ and
...
polynomial expressions in $\DS \diff^{\VEC{\gamma}} \psi_j$ for
...
$\DS b = \frac{C_1C_3 + C_4}{\sqrt{C_1C_2}}\|u\|_{k-1,2,\Omega}$ in
$\DS ab \leq \frac{1}{2}(a^2+b^2)$, then we get
\[
...
If $\DS \underline{u}:\RR^n\rightarrow \RR$ is defined by
\[
\underline{u}(\VEC{x}) =
\begin{cases}
u(\VEC{x}) & \quad \text{if} \ \VEC{x} \in \Omega \\
...
and $\DS \epsilon = \frac{(C_1C_2)^2}{2(C_1C_3 + C_4)^2}$ that
...
Let $\Omega$ be a bounded open subset of $\DS \RR^n$ with a
boundary of class $\DS C^2$.  Suppose that
\[
L(\VEC{x},\diff)u = \sum_{|\VEC{\alpha}|,|\VEC{\beta}|\leq k}
(-1)^{\VEC{\alpha}} \diff^{\VEC{\alpha}}
\left( b_{\VEC{\alpha},\VEC{\beta}}(\VEC{x}) \diff^{\VEC{\beta}} u\right) \  ,
...
$\DS b_{\VEC{\alpha},\VEC{\beta}} \in C(\overline{\Omega})$ for
$|\VEC{\alpha}|=|\VEC{\beta}|=k$ and
$\DS b_{\VEC{\alpha},\VEC{\beta}} \in L^\infty(\Omega)$ for
$|\VEC{\alpha}|+|\VEC{\beta}|<2k$ with $|\VEC{\alpha}|,|\VEC{\beta}|\leq k$.
Then
$\DS B : H^{k,2}_0(\Omega) \times H^{k,2}_0(\Omega)\rightarrow \CC$
...
\]
for $\DS u, v \in H^{k,2}_0(\Omega)$, where
\[
...
$\DS \| b_{\VEC{\alpha},\VEC{\beta}} \|_{\infty}$ is finite
...
Lemma~\ref{ell_garding4} with $r=0$, $s=k-1$, $t=k$
and $\DS \epsilon = \frac{C_1^2}{2C_2^2}$ that
...
\|u\|_{2,\Omega}^2
\]
for $\DS u \in H^{k,2}_0(\Omega)$.  We then get from
...
\begin{align*}
\RE B(u,u) &= \RE B_1(u,u) + \RE B_2(u,u) \\
&\geq C_1\|u\|_{k,2,\Omega}^2 - \lambda_1 \|u\|_{2,\Omega}^2 - 
...
$\DS \lambda = \lambda_1 + (C_2^2 C_3)/(2C_1C_2)$.
\end{proof}
...
\begin{theorem}
Let $\Omega$ be a bounded open subset of $\DS \RR^n$.  consider
\[
L(\VEC{x},\diff)u = \sum_{|\VEC{\alpha}|,|\VEC{\beta}|\leq k}
(-1)^{\VEC{\alpha}} \diff^{\VEC{\alpha}}
\left( b_{\VEC{\alpha},\VEC{\beta}}(\VEC{x}) \diff^{\VEC{\beta}} u\right) \ ,
\]
where $\DS b_{\VEC{\alpha},\VEC{\beta}} \in C(\overline{\Omega})$ for
$|\VEC{\alpha}|=|\VEC{\beta}|=k$ and
$\DS b_{\VEC{\alpha},\VEC{\beta}} \in L^\infty(\Omega)$ for
$|\VEC{\alpha}|+|\VEC{\beta}|<2k$ with $|\VEC{\alpha}|,|\VEC{\beta}|\leq k$.
If $\DS B : H^{k,2}_0(\Omega) \times H^{k,2}_0(\Omega)
...
\end{split}
\end{equation}
where $\DS b_{i,j} \in C^{0,\alpha}(\overline{\Omega})$ for
all $1\leq i, j \leq n$ and
$\DS b_i \in C^{0,\alpha}(\overline{\Omega})$ for all
...
$\DS f \in C^{0,\alpha}(\overline{\Omega})$,
then there exists a unique solution
$\DS u \in C^{2,\alpha}(\overline{\Omega})$
...
Let $\Omega$ be an open subset of $\DS \RR^n$ and let $X$ be a closed
subspace of $\DS H^{k,2}(\Omega)$ such that
$\DS H^{k,2}_0(\Omega) \subset X$.
Given $\DS f \in L^2(\Omega)$, let
...
\left( b_{\VEC{\alpha},\VEC{\beta}}(\VEC{x}) \diff^{\VEC{\beta}} u\right) = f \  ,
\end{equation}
where $\DS b_{\VEC{\alpha},\VEC{\beta}} \in L^\infty(\Omega)$ for all
$\VEC{\alpha}$ and $\VEC{\beta}$.
If $B$ is coercive on $X$, then there exists a one-to-one, bounded
linear mapping $\DS F:L^2(\Omega) \rightarrow X$ such that
$F(f)$ is a weak solution of (\ref{ell_exist_1}); namely,
$\DS B(v,F(f)) = \ps{v}{f}_2$ for all $\DS v \in X$.
\end{theorem}

\begin{proof}
Given $\DS g \in L^2(\Omega)$, we have that
...
$\DS |\tilde{g}(v)| =|\ps{v}{g}_2|
\leq \|g\|_2 \, \|v\|_2 \leq \|g\|_2 \|v\|_{k,2}$
for all $\DS v \in H^{k,2}(\Omega)$.  Thus 
$\|\tilde{g}|| \leq \|g\|_2$.  In particular, this shows that
$\DS T_1:L^2(\Omega) \to X^\ast$ defined by
...
$\DS T_1\left(\big(\|f\|_2 / \ps{v}{f}_2\big) v\right) = \|f\|_2$.}.
...
one-to-one, bounded linear mapping
$\DS T_2:X \rightarrow X^\ast$ such that
$\DS B(v,T_2(\tilde{f})) = \tilde{f}(v)$ for all $v \in X$.

We have that $F = T_2 \circ T_1$.
\end{proof}

Let $X$ be a closed subspace of $\DS H^{k,2}(\Omega)$ containing
$\DS H^{k,2}_0(\Omega)$ and let $f$ be an element of
$\DS L^2(\Omega)$ as in
...
variational formulation of $\DS L^\ast(\VEC{x},\diff)v = f$
is to find $v\in X$ such that
...
\end{equation}
for all $u,v \in X$,
$\DS B^\ast$ is a bounded sequilinear form. The variational problem
...
$\DS B^\ast=B$.  This implies that
$\DS L(\VEC{x},\diff)=L^\ast(\VEC{x},\diff)$.  To
prove this, consider $\DS B^\ast(v,u) = B(v,u)$ for
$u,v \in \DD(\Omega) \subset X$ and use integration by parts.

Moreover, from (\ref{ell_Bs_B}), we get that $B$ is coercive
if and only if $\DS B^\ast$ is coercive.  We can even take the same
constant $C$ in the definition of a coercive sequilinear form applied
to $B$ and $\DS B^\ast$.  Similarly, $B$ is weakly coercive
if and only if $\DS B^\ast$ is weakly coercive.  Again, we
can take the same constants $C$ and $\lambda$ in the definition of a
weakly coercive sequilinear form applied to $B$ and $\DS B^\ast$.

If $B$ is coercive, it follows from Theorem~\ref{ell_exist_th1}
that there exists a bounded linear mapping
$\DS F_B:L^2(\Omega)\to X$ such that
$\DS  B(v,F_B(f)) = \ps{v}{f}_2$ for all $v \in X$.
Similarly, since $\DS B^\ast$ is also coercive, there exists a bounded
linear mapping $\DS F_{B^\ast}:L^2(\Omega)\to X$ such that
$\DS  B^\ast(u,F_{B^\ast}(f)) = \ps{u}{f}_2$ for all
$u \in X$.  Let $\DS i:X\to L^2(\Omega) = H^{0,2}(\Omega)$ be
...
$\DS K_1=i \circ F_B:L^2(\Omega) \to L^2(\Omega)$
and $\DS K_2=i \circ F_{B^\ast}:L^2(\Omega) \to L^2(\Omega)$ are
compact operators.  The following sequence of equalities proves that
$\DS K_1^\ast = (i\circ F_B)^\ast = i \circ F_{B^\ast} = K_2$.
\begin{align*}
...
\end{align*}
for all $\DS f,g \in L^2(\Omega)$.

\begin{theorem} \label{ell_exist_th2}
Let $\Omega$ be an open subset of $\DS \RR^n$ and let $X$ be a closed
subspace of $\DS H^{k,2}(\Omega)$ containing
$\DS H^{k,2}_0(\Omega)$.
...
that $\DS \RE B(u,u) \geq C \|u\|_{k,2,\Omega}^2
...
$\DS \RE \tilde{B}(u,u) \geq C \|u\|_{k,2,\Omega}^2$
for all $u \in X$.  According to the discussion before the statement
of the theorem with $B$ replaced by $\tilde{B}$, there exists a
compact operator
$\DS K_1:L^2(\Omega)\rightarrow L^2(\Omega)$ such that
$\IMG(K_1) \subset X$ and
\[
\tilde{B}(v,K_1(f)) = \ps{v}{f}_2
\]
for all $v\in X$ and $\DS f\in L^2(\Omega)$.

Given $\DS f\in L^2(\Omega)$, we have that $u\in X$ satisfies
\begin{align}
...
With $f=0$, we get $u\in V$ if and only if $\DS u\in L^2(\Omega)$
satisfies $\DS \lambda^{-1} u = K_1(u)$ because
...
= \KE(K_1-\lambda^{-1}\Id) \  .
\]
A similar reasoning with $\DS B^\ast$ instead of $B$ shows that there
exists a compact operator
$\DS K_2:L^2(\Omega) \rightarrow L^2(\Omega)$
...
$\DS g_j \in C^\infty(\partial \Omega)$ for all $j$.

First, find $\DS g\in H^{2k,2}(\RR^n)$ such that
$\DS \pdydxn{g}{\nu}{j} = g_j$ on $\partial \Omega$ for
$0 \leq j < k$.  According to Theorem~\ref{ell_normal_BCs}, such a
function $g$ can be found if
$\DS g_j \in C^\infty(\partial \Omega)$ for
...
\begin{egg}
Let $\Omega$ be a bounded subset of $\DS \RR^n$.  Find the
...
\end{split}
\end{equation}
where $\DS f \in L^2(\Omega)$.       \label{ell_example2}

\subI{Variational Problem}
The weak solution is the solution $\DS u\in H^{1,2}_0(\Omega)$ of the
equation
\begin{equation} \label{ell_example2_eq2}
\int_\Omega \left( \graD u \cdot \graD v) + u\, v - f\, v \right)
\dx{\VEC{x}} = 0
\end{equation}
for all $\DS v \in H^{1,2}_0(\Omega)$.

We prove that a classical solution is a weak solution.  If
$\DS u\in C^2(\overline{\Omega})$ is a classical solution of
(\ref{ell_example2_eq1}), integrating (\ref{ell_example2_eq1})
multiplied by $\DS v \in C^1_c(\Omega)$ over $\Omega$ gives
\[
\int_\Omega \left( \graD u \cdot \graD v
+ u\, v - f\, v \right) \dx{\VEC{x}} = 0
\]
for all $\DS v \in C^1_c(\Omega)$.
The integral was computed using Green's identity (\ref{laplace_green1}).
Since $\DS C^1_c(\Omega)$ is dense in
$\DS H^{1,2}_0(\Omega)$, we have that
...
$\DS u\in H^{1,2}_0(\Omega)$ because
$\DS u\in C^2(\overline{\Omega})$ and $u(\VEC{x})=0$ for all
$\VEC{x} \in \partial \Omega$.
...
Schwarz inequality, one in $\DS \RR^n$ and the other in
$\DS L^2(\Omega)$;
namely,
\begin{align*}
...
= \| u\|_{1,2} \, \|v\|_{1,2}
\end{align*}
for all $\DS u,v \in H^{1,2}_0(\Omega)$.
Since $\DS B(u,u) = \|u\|_{1,2}^2$ for
$\DS u\in H^{1,2}_0(\Omega)$, it is
obvious that $B$ is coercive.  It is also obvious that $B$ is
symmetric.  The function $\DS f \in L^2(\Omega)$ yields a bounded
functional on $\DS H^{1,2}_0(\Omega)$ defined by
\[
\tilde{f}(u) = \ps{u}{f}_2 = \int_\Omega f\,u \dx{\VEC{x}}
\]
for all $\DS u \in H^{1,2}_0(\Omega)$.
We may thus apply Lax-Milgram Theorem to (\ref{ell_example2_eq2}) to
conclude that there exists a unique weak solution 
$\DS u \in H^{1,2}_0(\Omega)$ given by
\[
...
\subI{Regularity of the solution}
As a consequence of Theorem~\ref{ell_regular}, $\DS u\in H^2(\Omega)$.

\subI{Classical solution}
If the weak solution $u$ is in $\DS C^2(\overline{\Omega})$
and assuming that $\Omega$ has a boundary of class
$\DS C^1$, it follows from
Proposition~\ref{sob_w0_trad2} that $u(\VEC{x})=0$ for
all $\VEC{x}\in \partial \Omega$ because
$\DS u \in H^{1,2}_0(\Omega)\cap C^2(\overline{\Omega})$.
...
\]
for all $\DS v \in C^1_c(\Omega)$.
Since $\DS C^1_c(\Omega)$ is dense in $C_c(\Omega)$, we
get that $\DS \Delta u + u - f = 0$ almost
everywhere.  If $f \in C(\Omega)$, then we have
$\DS -\Delta u + u -f = 0$ everywhere on $\Omega$ since
$\DS u\in C^2(\Omega)$.  Thus $u$ is the classical solution of
(\ref{ell_example2_eq1}).
\end{egg}

\begin{egg}
Let $\Omega$ be a bounded subset of $\DS \RR^n$ with a
boundary of class $\DS C^2$.
...
\end{split}
\end{equation}
where $\DS f \in L^2(\Omega)$.

\subI{Variational Problem}
The weak solution is the solution $\DS u\in H^{1,2}(\Omega)$ of
the equation
\begin{equation} \label{ell_example4_eq2}
\int_\Omega \left( \graD u \cdot \graD v
+ u\, v - f\, v \right) \dx{\VEC{x}} = 0
\end{equation}
for all $\DS v \in H^{1,2}(\Omega)$.
...
v \dx{\VEC{x}} = 0
\end{equation}
for all $\DS v \in C^1(\overline{\Omega})$.
...
\]
for all $\DS v \in C^1(\overline{\Omega})$.
Since $\DS C^1(\overline{\Omega}) \supset \DD(\Omega)$ is
dense in $\DS H^{1,2}(\Omega)$, we
...
$\DS B:H^{1,2}_0(\Omega)\times H^{1,2}_0(\Omega) \rightarrow \RR$
is coercive, symmetric and bounded.  Moreover, we also have that the
functional on $\DS H^{1,2}(\Omega)$ defined by 
\[
\tilde{f}(u) = \ps{u}{f}_2 = \int_\Omega f\, u\dx{\VEC{x}}
\]
for all $\DS u \in H^{1,2}(\Omega)$ is a bounded mapping.
We may use Lax-Milgram Theorem to
(\ref{ell_example4_eq2}) to conclude that there exists a unique weak
solution $\DS u \in H^{1,2}(\Omega)$ given by
\[
\frac{1}{2} \int_\Omega \left( \graD u \cdot \graD u 
+u\, u \right) \dx{\VEC{x}} -\int_\Omega f\, u \dx{\VEC{x}}
...
\subI{Classical solution}
If the weak solution $u$ is in $\DS C^2(\overline{\Omega})$,
then we get from (\ref{ell_example4_eq2}) and Green's identity
(\ref{laplace_green1}) that
...
Since $\DS C^1_c(\Omega) \subset C^1(\overline{\Omega})$, we get from
...
$\overline{\Omega}$).  Since $\DS C^1_c(\Omega)
\supset \DD(\Omega)$ is dense in
$\DS L^2(\Omega)$, we get that
$\DS -\Delta u + u - f = 0$ almost
everywhere.  If $f \in C(\Omega)$, then we have
$\DS -\Delta u + u -f = 0$ everywhere on $\Omega$ since
$\DS u\in C^2(\Omega)$.  Back to (\ref{ell_GW_classic}),
this implies that
\[
\int_{\partial \Omega} v\,\pdydx{u}{\nu} \dss{S}{x} = 0
\]
for all $\DS v \in C^1(\overline{\Omega})$.
Since $\DS C^1(\overline{\Omega})\big|_{\partial \Omega}
\supset \DD(\partial \Omega)$ is dense in $\DS L^2(\partial \Omega)$,
we get $\DS \pdydx{u}{\nu} =0$ almost everywhere on
$\partial \Omega$ (using the measure along $\partial \Omega$).  Since
$\DS u\in C^2(\overline{\Omega})$, this implies that
$\DS \pdydx{u}{\nu}(\VEC{x}) =0$ for all
$\VEC{x} \in \partial \Omega$.
\end{egg}
...
\end{split}
\end{equation}
where $\DS f \in L^2(\Omega)$ and $g\in C^\infty(\partial \Omega)$.  

\subI{Variational Problem}
Choose
$\DS \tilde{g} \in H^{1,2}(\Omega) \cap C(\overline{\Omega})$
such that
...
\right\} \ .
\]
$K$ is a closed and convex subset of $\DS H^{1,2}(\Omega)$.
...
$\DS u\in K \subset H^{1,2}(\Omega)$ of the equation
...
if $\DS u\in C^2(\overline{\Omega})$ is a classical solution of
...
$\DS C^1_c(\Omega) \supset \DD(\Omega)$ is dense in
$\DS H^{1,2}_0(\Omega)$, we have that
(\ref{ell_example3_eq2}) is true.  Moreover, it follows from 
Proposition~\ref{sob_w0_trad2} that
$\DS u- \tilde{g} \in H^{1,2}_0(\Omega)$ because
$\DS u - \tilde{g} \in H^{1,2}(\Omega)\cap C(\overline{\Omega})$ and
...
\end{equation}
for all $v \in K$.
If $u$ is a weak solution, then we get from (\ref{ell_example3_eq2})
that (\ref{ell_example3_eq3}) is true because
$\DS u-v\in H^1_0(\Omega)$ for all $v\in K$.  In fact we have equality
in (\ref{ell_example3_eq3}).  Conversely, if $u\in K$ satisfies
(\ref{ell_example3_eq3}), then we substitute $v=u\pm w$ with
$\DS w\in H^1_0(\Omega)$ in (\ref{ell_example3_eq3}) to get
respectively
\begin{align*}
...
\end{align*}
for all $\DS w \in H^1_0(\Omega)$.  Thus
(\ref{ell_example3_eq2}) is true.

We may now apply Stampacchia Theorem with $\DS H=H^{1,2}(\Omega)$ to
...
\subI{Regularity of the solution}
As a consequence of Theorem~\ref{ell_regular},
$\DS u\in H^2(\Omega)$.

\subI{Classical solution}
If $f\in C(\Omega)$ and the weak solution $u$ is in
$\DS C^2(\overline{\Omega})$, we can proved as we did many
times before that $\DS -\Delta u + u -f = 0$ everywhere on $\Omega$.
Moreover, it follows from Proposition~\ref{sob_w0_trad2} that
$u - \tilde{g}=0$ on $\partial \Omega$
because $\DS u \in K \cap C^2(\overline{\Omega})$ implies
that
$\DS u- \tilde{g} \in H^{1,2}_0(\Omega)\cap C(\overline{\Omega})$.
\end{egg}

If the sequilinear form in (\ref{ell_exist_1}) is weakly coercive and
symmetric, then we get the following additional properties.
...
\begin{enumerate}
\item There exists an orthonormal basis $\DS \{u_j\}_{j=1}^\infty$ of
$\DS L^2(\Omega)$ consisting of eigenvectors of the sequilinear form
$B$ defined in (\ref{ell_exist_1}).  We define
$\DS u_j \in L^2(\Omega)$ as
...
\item The eigenvalues $\DS \{\lambda_j\}_{j=1}^\infty$ can
be arranged in an increasing sequence converging to $+\infty$.
\item If the coefficients $b_{\VEC{\alpha},\VEC{\beta}}$ of
$L(\VEC{x},\diff)$ are all of class $\DS C^\infty(\Omega)$,
then the eigenvectors $u_j$ are all in $\DS C^\infty(\Omega)$.
\end{enumerate}
\end{theorem}
...
here because we do not have to play with $\DS \lambda^{-1}$.
...
$B$ replaced by $\tilde{B}$ and $\DS B^\ast$ replaced by
$\DS \tilde{B}^\ast$ respectively.
...
where $\DS F_{\tilde{B}}:L^2(\Omega)\rightarrow X$ given by
Theorem~\ref{ell_exist_th1} is one-to-one and
$\DS i:X\rightarrow L^2(\Omega)$ is the inclusion.
...
eigenvectors $\DS \{u_j\}_{j=1}^\infty \subset L^2(\Omega)$
of $K_1$ such that the eigenvalue $\beta_j$ associated to $u_j$ is real and the
sequence $\DS \{|\beta_j|\}_{j=1}^\infty$ is a decreasing sequence
...
\]
for all $v \in X$, we get that
$\DS \tilde{B}(v, u_j) = \ps{v}{\beta_j^{-1}u_j}_2$ for all
$v \in X$.

Let $\DS \lambda_j = \beta_j^{-1}-\lambda$ for $j>0$.  Then 
\begin{align*}
...
\]
for all
$\DS v \in \DD(\Omega) \subset H^{k,2}_0(\Omega) \subset$.  Since
$L(\VEC{x},\diff) - \lambda_j \Id$ is an elliptic operator with
coefficients of class $\DS C^\infty$ on $\Omega$, it follows from
Corollary~\ref{ell_Hak_Cinfty} that we will see in the next section
that $\DS u_j \in C^\infty(\Omega)$.
\end{proof}
...
Let $\Omega$ be a bounded open subset of $\DS \RR^n$ with a
boundary of class $\DS C^2$.  If the operator
$L(\VEC{x},\diff)$ in the Dirichlet problem
(\ref{ell_PDE1}) is strongly elliptic on $\overline{\Omega}$, and
$\DS L(\VEC{x},\diff)=L^\ast(\VEC{x},\diff)$, then we get the
following results.
\begin{enumerate}
\item There exists an orthonormal basis $\DS \{u_j\}_{j=1}^\infty$ of
$\DS L^2(\Omega)$ consisting of eigenvectors of $L(\VEC{x},\diff)$.
\item $\DS u_j \in C^\infty(\overline{\Omega})$ for all $j$.
...
also coercive, then we get the following result.
\begin{enumerate}
\setcounter{enumi}{4}
...
\begin{proof}
Let $\DS B(v,u) = \ps{v}{f}_2$ for all
$\DS u, v \in H^{k,2}_0(\Omega)$
be a variational formulation of the Dirichlet problem (\ref{ell_PDE1})
with $\DS f\in L^2(\Omega)$, where $B$ is defined in
(\ref{ell_exist_1}) with $\DS X=H^{k,2}_0(\Omega)$.  A
variational formulation of the adjoint equation
$\DS L^\ast(\VEC{x},\diff)v = f$ is
\begin{equation} \label{ell_th4_lbl1}
B^\ast(u,v) = \ps{u}{f}_2 \quad , \quad u,v \in H^{k,2}_0(\Omega) \ ,
\end{equation}
where $\DS B^\ast$ is defined in (\ref{ell_var_adj2}) with
$\DS X=H^{k,2}_0(\Omega)$. Since
$\DS L^\ast(\VEC{x},\diff)=L(\VEC{x},\diff)$,
...
$\DS u_j \in C^\infty(\Omega)$ for all $j$.
...
Since $\DS u_j \in H^{k,2}_0(\Omega) \cap C^\infty(\overline{\Omega})$,
Proposition~\ref{sob_w0_trad2} implies that
$\DS \diff^{\VEC{\alpha}} u_j(\VEC{x}) = 0$ for all
$\VEC{x} \in \partial \Omega$ and $|\VEC{\alpha}|<k$. 
\end{proof}

\begin{cor}
Let $\Omega$ be a bounded open subset of $\DS \RR^n$ with a
boundary of class $\DS C^2$.  There exists an
orthonormal basis $\DS \{u_j\}_{j=1}^\infty$ of
$\DS L^2(\Omega)$ consisting
of eigenvectors of $-\Delta$ such that
$\DS u_j \in C^\infty(\overline{\Omega})$ and
...
and $\DS L^\ast(\VEC{x},\diff) = L(\VEC{x},\diff)$.  We may
then apply the previous theorem.  Note that the variational formulation of
$\DS L(\VEC{x},\diff)u = - \Delta u = f \in L^2(\Omega)$ is
\[
...
Let $I = ]0,1[$.  Suppose that $\DS p \in C^1(\overline{I})$ and that
there exists $\alpha>0$ such that $p(x) \geq \alpha$ for all
...
$\DS \left\{\lambda_j\right\}_{j=1}^\infty$ of real
numbers and a sequence $\DS \left\{u_j\right\}_{j=1}^\infty$
of functions in $\DS L^2(I)$ such that
\begin{enumerate}
\item $u_j$ is a weak solution of (\ref{ell_eigA}) when
$\lambda = \lambda_j$.
\item $\DS \left\{u_j\right\}_{j=1}^\infty$ is a basis of
$\DS L^2(I)$.
\item $\lambda_j \leq \lambda_{j+1}$ for all $j$ and
$\lambda_j \to \infty$ as $j \to \infty$.
\item $\DS u_j \in C^2(\overline{I})$ for all $j$,
and the $u_j$'s are
...
$\DS f \in L^2(I)$ there
exists a unique solution $\DS u \in H^2(I) \cap H^1_0(I)$ of this
variational problem.  We define an operator $\DS T:L^2(I) \to L^2(I)$
by $T(f) = u$, where $\DS u \in H^2(I) \cap H^1_0(I)$ is the
...
prove that $\DS T:L^2(I) \to H_0^1(I)$ is continuous.  Suppose that
...
$\DS \|u\|_2^2 + \|u'\|_2^2 \leq C^2 \|u'\|_2^2$ yields
$\DS \|u \|_2 \leq \sqrt{C^2-1} \|u'\|_2$.
We then have from (\ref{ell_px_evpEq1}) that
$\DS \alpha \|u'\|_2^2 \leq \sqrt{C^2-1} \|u'\|_2 \|f\|_2$
and therefore
$\DS \|u'\|_2 \leq \frac{\sqrt{C^2-1}}{\alpha} \|f\|_2$.
We finally get from (\ref{ell_px_evp}) that
\[
\|T(f)\|_{1,2} = \|u\|_{1,2} \leq \frac{C\sqrt{C^2-1}}{\alpha}
\|f\|_2 \ .
\]
Since the injection of $\DS H^1(I)$ into
$\DS C(\overline{I}) \subset L^2(I)$
is compact according to Corollary~\ref{sob_cor_sob_lem} to the Sobolev Lemma,
we get that $\DS T:L^2(I) \to L^2(I)$ is compact because the
...
\end{equation}
for all $\DS f,g \in L^2(I)$.  Let $u=T(f)$ and $v=T(g)$.
We then have that
\[
\int_I f \, v \dx{x}
= \int_I \left( p\, u'\, v' + q\, u\, v \right) \dx{x}
...
\]
for all $\DS f \in L^2(I)$.  Thus
\[
\mu_j \int_I u_j^2 \dx{x}
= \int_I T(u_j)\, u_j \dx{x} \geq 0
\]
implies that $\mu_j\geq 0$ for all $j$.  Hence, we have that
...
functions.  Let
$\DS B:H^{1,2}(\Omega)\times H^{1,2}(\Omega) \rightarrow \RR$
...
then $B$ is weakly coercive over $\DS H^{1,2}(\Omega)$, and
over any closed subspaces of $\DS H^{1,2}(\Omega)$.
\end{lemma}

\begin{proof}
Let $\DS c_{i,j} = (b_{i,j}+ b_{j,i})/2$.  Then, for
each $\VEC{x} \in \Omega$,
\[
R(\VEC{x},\VEC{\xi}) = \sum_{i,j=1}^n c_{i,j}(\VEC{x}) \xi_i,\xi_j
\]
for all $\DS \VEC{\xi} \in \RR^n$
...
\,\diff_{x_j}\overline{u}(\VEC{x}) \dx{\VEC{x}}
&\geq C_1 \sum_{j=1}^n \int_\Omega
\left|\diff_{x_j} u(\VEC{x})\right|^2 \dx{\VEC{x}} \nonumber \\
...
\]
for all $\DS u \in H^{1,2}(\Omega)$.  From
$\DS ab \leq (a^2+b^2)/2$ with
$\DS a = \sqrt{C_1} \|u\|_{1,2,\Omega}$ and
$\DS b = \frac{(n+1)C_2}{\sqrt{C_1}} \|u\|_{2,\Omega}$, we get
\[
(n+1)C_2 \|u\|_{2,\Omega} \, \|u\|_{1,2,\Omega}
\leq \frac{1}{2} \left(
C_1 \|u\|_{1,2,\Omega}^2 + \frac{(n+1)^2C_2^2}{C_1} \, \|u\|_{2,\Omega}^2 \right)
\]
for all $\DS u \in H^{1,2}(\Omega)$.  Therefore,
\begin{align*}
\RE B(u,u) &\geq C_1 \left( \|u\|_{1,2,\Omega}^2 - \|u\|_{2,\Omega}^2 \right) -
\frac{1}{2} \left(
...
\end{align*}
for all $\DS u \in H^{1,2}(\Omega)$, where
$\DS C= \frac{C_1}{2}$ and
$\DS \lambda = C_1 + \frac{(n+1)^2C_2^2}{2C_1}$.
This proves that $B$ is weakly coercive on $\DS H^{1,2}(\Omega)$.
\end{proof}
...
functions of class $\DS C^\infty(\Omega)$.  If the
...
\]
then we get $a_{i,j} = b_{i,j}$, 
$\DS a_i = - \sum_{j=1}^n \pdydx{b_{j,i}}{x_j}$ and
$b_0 = a_0$.  The condition
$\DS a_i = - \sum_{j=1}^n \pdydx{a_{j,i}}{x_j}$ for
...
Let $\Omega$ be a bounded open subset of $\DS \RR^n$ with a boundary
of class $\DS C^2$.
Suppose that the operator $L(\VEC{x},\diff)$ in the Newmann problem
(\ref{ell_PDE4}) is elliptic on $\overline{\Omega}$, that
$\DS L(\VEC{x},\diff)=L^\ast(\VEC{x},\diff)$, that the
coefficients $b_{i,j}$ of $L(\VEC{x},\diff)$ are real-valued
functions of class $\DS C^\infty(\Omega)$, and that the \nn
matrix $B$ with entries
...
\|\VEC{\xi}\|^2 
\]
for all $\DS \VEC{\xi} \in \CC^n$ for some constant $C_1$.
...
$\DS \RR^n$ with a sufficiently smooth boundary.
We seek $\lambda$ such that there exists a non-trivial
function $\DS u \in H^1_0(\Omega)$ satisfying
...
$\DS u_1 \in L^2(\Omega)$ is an eigenfunction associate to 
$\lambda_1$ and $\DS u_2 \in L^2(\Omega)$ is an
...
$\DS \ps{f}{g} = \int_{\Omega} f\, g \dx{\VEC{x}}$
in $\DS L^2(\Omega)$.

Let $\DS \left\{ u_j \right\}_{j=1}^\infty$ be an orthogonal basis of
...
if $\DS f \in L^2(\Omega)$.  Suppose that
$\DS f = \sum_{j=1}^\infty a_j u_j$ is the Fourier series
expansion of $f$ with
$\DS a_j = \frac{\ps{f}{u_j}_2}{\ps{u_j}{u_j}_2}$.  If 
$\DS u = \sum_{j=1}^\infty b_j u_j$ is the solution
...
series above converges with respect to the $\DS L^2$-norm on
$\DS L^2(\Omega)$ and that $-\Delta u = f$ in the variational sense.
\end{egg}
...
standard $\DS L^2$-norm on $\DS L^2(\Omega)$ and
\[
a_j(\VEC{x})
...
$\DS L^2$-norm on $\DS L^2(\Omega\times\Omega)$.
\end{egg}

\section{Regularity of Weak Solutions} \label{ell_reg_of_sols}

Let $\Omega$ be an open subset of $\DS \RR^n$.  In this section, we
...
\end{equation}
with $\DS a_{\VEC{\alpha}} \in C^\infty(\Omega)$ for all
...
\end{equation}
for all $\DS \phi \in \SS(\RR^n)$ and
$\DS f \in H^{s-1,2}(\RR^n)$.
Choose $\DS v \in L^2(\RR^n)$ such that
$\DS \hat{v}(\VEC{y}) = (1+\|\VEC{y}\|_2^2)^{(s-1)/2} \hat{f}(\VEC{y})$
for $\DS \VEC{y} \in \RR^n$.  Such a $v$ exists
according to Plancherel theorem, Theorem~\ref{distr_plancherel},
because $\DS f \in H^{s-1,2}(\RR^n)$ implies that
$\DS (1 + \|\VEC{y}\|_2^2)^{(s-1)/2} \hat{f} \in L^2(\RR^n)$
...
Since
$\DS \pdfdx{(1+w^2)^{s/2}}{w} = sw(1+w^2)^{(s-2)/2}$, we get
...
\, \big| \|\VEC{x}\|_2 - \|\VEC{y}\|_2 \big| \\
&\qquad \leq |s| \sup_{w} \left|(1+w^2)^{(s-1)/2}\right|
\, \|\VEC{x} - \VEC{y}\|_2 \ ,
\end{align*}
where the supremum is taken over all $w$ between $\|\VEC{x}\|_2$ and
$\|\VEC{y}\|_2$.  Since $\DS w \to (1+w^2)^{(s-1)/2}$ is increasing on
...
For the second integral, let $\DS g \in L^2(\RR^n)$ be such that
$\DS \hat{g} = (1 + \|\VEC{y}\|_2^2)^{s/2} \hat{f}$.  Such a
$g$ exists according to Plancherel theorem, Theorem~\ref{distr_plancherel},
because $\DS f \in H^{s,2}(\RR^n)$ implies that
$\DS (1 + \|\VEC{y}\|_2^2)^{s/2} \hat{f} \in L^2(\RR^n)$ by
definition.  We have
\begin{align*}
&\left( \int_{\RR^n} \left(
\int_{\RR^n} (1 + \|\VEC{y}\|_2^2)^{s/2} \hat{\phi}(\VEC{x}-\VEC{y})
\hat{f}(\VEC{y}) \dx{\VEC{y}} \right)^2 \dx{\VEC{x}} \right)^{1/2} \\
&\quad = \left( \int_{\RR^n} \left(
\int_{\RR^n} \hat{\phi}(\VEC{x}-\VEC{y}) \hat{g}(\VEC{y})
\dx{\VEC{y}} \right)^2 \dx{\VEC{x}} \right)^{1/2} \\
&\quad = (2\pi)^{n/2} \left(\int_{\RR^n}
...
For $\DS f\in L^2(\RR^n)$ and $h \in \RR\setminus \{0\}$, let
\begin{equation}  \label{ell_diffQuot}
\left(\Delta_{h\VEC{e}_j} f\right)(\VEC{x}) =
\frac{1}{h} \left( f(\VEC{x} + h\VEC{e}_j) -f(\VEC{x}) \right)
\end{equation}
and, recursively,
$\DS \Delta_{h\VEC{e}_j}^q f =
\Delta_{h\VEC{e}_j} \left(\Delta_{h\VEC{e}_j}^{q-1} f \right)$
for $q>1$.
We also define $\DS \Delta^0_{h\VEC{e}_j}$ as
$\DS \Delta^0_{h\VEC{e}_j} f = f$ for $h \in \RR$.
For $\DS \VEC{h} \in \RR^n$ and
$\DS \VEC{\alpha} \in \NN^n$ with $h_j \neq 0$ for
$\alpha_j \neq 0$, let
\[
...
\]
$\DS \Delta_{\VEC{h}}^{\VEC{\alpha}}$ is independent of the
order of the difference quotients
$\DS \Delta_{h_i\VEC{e}_i}^{\alpha_i}$ chosen.
We also have that
$\DS \Delta_{\VEC{h}}^{\VEC{\alpha}}
\left( \Delta_{\VEC{h}}^{\VEC{\beta}} f\right)
= \Delta_{\VEC{h}}^{\VEC{\alpha}} \left( \Delta_{\VEC{h}}^{\VEC{\beta}} f\right)$
for all multi-indices $\VEC{\alpha}$ and $\VEC{\beta}$ in $\DS \NN^n$.
If $\DS f \in H^k(\RR^n)$, it is useful to note that
$\DS \diff^{\VEC{\alpha}}\left( \Delta_{\VEC{h}}^{\VEC{\beta}} f \right)
= \Delta_{\VEC{h}}^{\VEC{\beta}} \left(\diff^{\VEC{\alpha}} f \right)$ for all
multi-indices $\VEC{\alpha}$ and $\VEC{\beta}$ in $\DS \NN^n$ with
$|\VEC{\alpha}|\leq k$, where the
derivative is in the sense of distribution.  The proofs of these
statements is left to the reader.

The {\bfseries adjoint} of $\DS \Delta_{\VEC{h}}^{\VEC{\alpha}}$ is
$\DS (-1)^{|\VEC{\alpha}|}\Delta_{-\VEC{h}}^{\VEC{\alpha}}$.  The
...
Suppose that $k \in \NN$ and that $\DS f,g \in L^2(\Omega)$,
where $\Omega$ is an open subset of $\DS \RR^n$.  Moreover,
...
$\DS \VEC{h} \in \RR^n$ such that $\|\VEC{h}\|<\delta$.
\end{prop}

\begin{proof}
\stage{i} We first consider $\VEC{\alpha} = \alpha_j \VEC{e}_j$ with
$\alpha_j \leq k$ and $\|\VEC{y}\|< \delta$.  We have
\begin{align*}
...
\begin{cor}
Suppose that $\DS f,g \in L^2(\RR^n)$.  Then
\[
\int_{\RR^n} \left(\Delta^{\VEC{\alpha}}_{\VEC{h}} f\right) g \dx{\VEC{x}}
= (-1)^{|\VEC{\alpha}|}
\int_{\RR^n} f\,\left(\Delta^{\VEC{\alpha}}_{-\VEC{h}} g\right) \dx{\VEC{x}}
\]
for all multi-indices $\VEC{\alpha}$ and all $\DS \VEC{h} \in \RR^n$.
\end{cor}

\begin{proof}
The conditions $\DS \supp f + B_{k\delta}(\VEC{0}) \subset \RR^n$ and
$\DS \supp g + B_{k\delta}(\VEC{0}) \subset \RR^n$ of the previous
proposition are satisfied for all $\delta>0$ and $k \in \NN$.
\end{proof}

\begin{prop} \label{ell_reg_prop3}
Given $\DS f\in H^{s,2}(\RR^n)$ and
$\DS \VEC{\alpha} \in \NN^n$, we have
...
It follows that $\DS \diff^{\VEC{\alpha}} f \in H^{s,2}(\RR^n)$
if and only if $\DS \limsup_{\VEC{h}\rightarrow \VEC{0}}
\big\|\Delta^{\VEC{\alpha}}_{\VEC{h}} f\big\|_{s,\rho}$ is finite.

Moreover,
$\DS \left\| \Delta^{\VEC{\alpha}}_{\VEC{h}} f  \right\|_{s,\rho}
\leq \left\| \diff^{\VEC{\alpha}} f\right|_{s,\rho}$ for all
$\DS f\in H^{s,2}(\RR^n)$, $\VEC{h} \in \RR^n$ and
$\DS \VEC{\alpha} \in \NN^n$ with $h_j \neq 0$ for $\alpha_j \neq 0$.
\end{prop}
...
$\DS \VEC{x},\VEC{r} \in \RR^n$.
From Proposition~\ref{FTLofPs}, we have that
$\DS (f_{\VEC{r}})^\wedge(\VEC{y}) =
...
e^{i h_j y_j \alpha_j/2}
\sin^{\alpha_j}\left(\frac{h_j y_j}{2}\right) \right) \\
&\qquad\qquad \left( \frac{(2i)^{\alpha_{m+1}}}{h_{m+1}^{\alpha_{m+1}}}
...
\end{equation}
for all $\DS \VEC{y} \in \RR^n$.  Moreover, since
$\DS \lim_{h_j \to 0} \frac{2}{h_j} \sin\left(\frac{h_j y_j}{2}\right)
...
\end{equation}
for all $\DS \VEC{y} \in \RR^n$.

\stage{i} If $\DS \diff^{\VEC{\alpha}} f \in H^{s,2}(\RR^n)$, then
...
\end{split}
\end{equation}
for all $\DS \VEC{y} \in \RR^n$, and from (\ref{ellRegProp3Eq4}) that
\[
...
= (1+\|\VEC{y}\|_2^2)^s \left| (\diff^{\VEC{\alpha}} f)^\wedge(\VEC{y})\right|^2
\]
for all $\DS \VEC{y} \in \RR^n$.  Hence, we get from the Lebesgue
...
Since $\DS \lim_{p \to 0}\left|\frac{\sin(p)}{p}\right| = 1$, 
there exists $P>0$ such that
$\DS \left|\frac{\sin(p)}{p}\right| \geq \frac{1}{2}$
for $|p|<P$.  Therefore,
$\DS \left| \frac{2}{h_jy_j} \sin\left(\frac{h_j y_j}{2}\right)\right|
\geq \frac{1}{2}$ for $\DS \left|\frac{h_jy_j}{2}\right|<P$.
It follows that
$\DS \left| \frac{2}{h_jy_j} \sin\left(\frac{h_j y_j}{2}\right)\right|
\geq \frac{1}{2}$ for all $y_j$ such that $|y_j| \leq R$ if
$\DS |h_j| < 2P/R$.

Therefore, we have for $\DS \|h\|_2 < 2P/R$ that
\begin{align*}
...
and so $\DS \diff^{\VEC{\alpha}} f \in H^{s,2}(\RR^n)$ (see
...
$\DS \diff^{\VEC{\alpha}} f \in H^{s,2}(\RR^n)$, and that we
have in fact $\DS \lim_{\VEC{h}\rightarrow \VEC{0}}
\|\Delta^{\VEC{\alpha}}_{\VEC{h}} f\|_{s,\rho}
= \|\diff^{\VEC{\alpha}} f\|_{s,\rho}$.
\end{enumerate}
\end{rmkList}

Given $\DS \phi \in \SS(\RR^n)$, we define the linear operator
$\DS [\Delta^{\VEC{\alpha}}_{\VEC{h}}, \phi]$ on
$\DS H^{s,2}(\RR^n)$ as
\[
...
Suppose that $\DS \phi \in \SS(\RR^n)$,
$\DS \VEC{\alpha} \in \NN^n$, $\DS \VEC{h}\in \RR^n$
and $s \in \RR$.  Then,
\begin{align*}
...
\stage{i} We consider the case $|\VEC{\alpha}|=1$.  We have
\begin{align*}
...
\end{align*}
Moreover, $\DS f \to f_{-h_j\VEC{e}_j}$ is an isometry from
$\DS H^{s,2}(\RR^n)$ onto itself. We leave the proof of this
claim as an exercise for the reader.  Recall that
$\DS \left(f_{-h_j\VEC{e}_j}\right)^\wedge(\VEC{y}) = e^{i h_j\,y_j}
\hat{f}(\VEC{y})$.

Since $\DS \Delta_{h_j\VEC{e}_j} \phi \in \SS(\RR^n)$, we have
...
\end{align*}
for all $\DS \VEC{z} \in \RR^n$, $h_j \in \RR$ and $j$.
...
\end{align*}
for all $\DS \VEC{z}\in \RR^n$, $h_j \in \RR$ and $j$
because $\DS \hat{\phi} \in \SS(\RR^n)$ by
...
\end{align*}
for all $\DS \VEC{y}\in \RR^n$, $h_j \in \RR$ and $j$  because
$\DS \hat{\phi} \in \SS(\RR^n)$ by Proposition~\ref{distr_frr_tempT}.
...
We have that $\DS [\Delta^{\VEC{\alpha}}_{\VEC{h}}, \phi] f$
is a finite sum of terms of the form
$\DS \Delta^{\VEC{\mu}}_{\VEC{h}} \left[
...
\end{equation}
for all $t\in \RR$, $\DS g \in H^{t,2}(\RR^n)$,
$\DS \VEC{h} \in \RR^n$ and
$\DS \VEC{\beta} \in \NN^n$ with $h_j \neq 0$ for
$\beta_j \neq 0$.  Hence,
\begin{align*}
...
\]
is a bounded operator from $\DS H^{s,2}(\RR^n)$ into
$\DS H^{s-k-|\VEC{\beta}|+1,2}(\RR^n)$ with the bound
independent of $h$.
\end{cor}

\begin{proof}
Since $\DS \diff^{\VEC{\alpha}}$ commutes with
$\DS \Delta_{\VEC{h}}^{\VEC{\beta}}$, we get
\begin{align*}
...
\end{align*}
for all $\DS f \in H^{s,2}(\RR^n)$.  It follows from
...
$\DS \RR^n_+ = \{ (x_1,x_2,\ldots,x_n) \in \RR^n : x_n>0 \}$
and $\DS \RR^n_- = \{ (x_1,x_2,\ldots,x_n) \in \RR^n : x_n<0 \}$.

\begin{cor} \label{ell_reg_cor3}
Let $\DS \Omega_r = \RR^n_+ \cap B_r(\VEC{0})$ for some $r>0$.
If the support of $\DS f\in H^{k,2}(\RR^n)$ is a subset of
$\DS \Omega_{r,0} = (\RR^n \setminus \RR^n_-) \cap
B_r(\VEC{0})$ (Figure~\ref{ellOmegaR}) and
$\DS \VEC{\alpha} \in \NN^n$ is a multi-index with $\alpha_n=0$, then
$\DS \diff^{\VEC{\alpha}} f \in H^{k,2}(\Omega_r)$ if and only if
\[
A_{f,\VEC{\alpha}} \equiv \limsup_{\VEC{h}\rightarrow \VEC{0}}
\|\Delta^{\VEC{\alpha}}_{\VEC{h}} f\|_{k,\Omega_r} < \infty \ .
\]
In particular, if $\DS \diff^{\VEC{\alpha}} f \in H^{k,2}(\Omega_r)$,
then there exists a constant $C$, independent of $f$ and $r$, such that 
$\DS \left\| \diff^{\VEC{\alpha}} f \right\|_{k,\Omega_r} \leq
C A_{f,\VEC{\alpha}}$.
\end{cor}

\begin{proof}
According to Theorem~\ref{sob_Rp_ext}, there exists a strong
$k$-extension operator $E$ on $\DS \RR^n_+ $ and a strong
$(k-|\VEC{\alpha}|)$-extension operator $E_{\VEC{\alpha}}$ on
$\DS \RR^n_+$ such that
$\DS \diff^{\VEC{\alpha}} (E(g)) =
E_{\VEC{\alpha}}(\diff^{\VEC{\alpha}} g)$
for all $\DS g \in H^{k,2}(\RR^n_+)$.
Since $\alpha_n=0$, a quick look at the proof of
Theorem~\ref{sob_Rp_ext} shows that $E_{\VEC{\alpha}} = E$.
Moreover, we also see that
$\DS E\left( \Delta^{\VEC{\alpha}}_{\VEC{h}} g\right) =
\Delta^{\VEC{\alpha}}_{\VEC{h}} E(g)$
for all $\DS g \in H^{k,2}(\RR^n_+)$ because $\alpha_n=0$.

We have that $\DS E:H^{k,2}(\RR^n_+) \to H^{k,2}(\RR^n)$ is a
bounded operator and $E(g) = g$ on $\DS \RR^n_+$ for all
$\DS g \in H^{k,2}(\RR^n_+)$.  So, there exists a
positive constant $C_1$ such that
$\DS \left\| E(g) \right\|_{k,2,\RR^n} \leq C_1
\left\| g \right\|_{k,2,\RR^n_+}$ for all $\DS g \in H^{k,2}(\RR^n_+)$.

Since the norms $\DS \left\|\cdot\right\|_{k,2,\RR^n}$ and 
$\DS \left\|\cdot\right\|_{k,\rho}$ are equivalent, there
...
\]
for all $\DS g \in H^{k,2}(\RR^n)$.

\stage{i} Suppose that $\DS \diff^{\VEC{\alpha}} f
\in H^{k,2}(\Omega_r)$.
Since $\supp f \subset \Omega_{r,0}$, we have that\\
$\DS \left\|\diff^{\VEC{\alpha}} f\right\|_{k,2,\RR^n_+}
= \DS \left\|\diff^{\VEC{\alpha}} f \right\|_{k,2,\Omega_r} < \infty$.
Hence,
\[
...
$\DS \supp \Delta^{\VEC{\alpha}}_{\VEC{h}} f \subset \Omega_{r,0}$ for
...
$\DS \supp \Delta^{\VEC{\alpha}}_{\VEC{h}} f \subset \Omega_{r,0}$
for $\|\VEC{h}\| < \delta$ for the second to last equality in the
previous expression.
It follows from Proposition~\ref{ell_reg_prop3} that 
$\DS \diff^{\VEC{\alpha}} E(f) \in H^{k,2}(\RR^n)$.
Since $\DS \Omega_r \subset \RR^n$, we have that
$\DS \diff^{\VEC{\alpha}} E(f) \in H^{k,2}(\Omega_r)$.  Since
$E(f) = f$ on $\DS \RR^n_+ \supset \Omega_r$, we have that
$\DS \diff^{\VEC{\alpha}} f \in H^{k,2}(\Omega_r)$.

\stage{iii} It also follows from Proposition~\ref{ell_reg_prop3}
that $\DS \left\|\diff^{\VEC{\alpha}} E(f)\right\|_{k,\rho}
= \limsup_{\VEC{h}\rightarrow \VEC{0}}
\left\| \Delta^{\VEC{\alpha}}_{\VEC{h}} E(f)\right \|_{k,\rho}$.
...
$\DS \left\|\cdot\right\|_{k,2,\RR^n}$ and 
$\DS \left\|\cdot\right\|_{k,\rho}$.
\end{proof}
...
$\DS \breve{\VEC{x}} = (x_1,x_2,\ldots,x_{n-1})$.}{ellOmegaR}
...
depends on $s\in \RR$ and $\DS \phi \in \DD(\RR^n)$ only.  We get
...
$\DS [\Delta^{\VEC{\alpha}}_{\VEC{h}}, \phi] f$
is a finite sum of terms of the form
$\DS \Delta^{\VEC{\eta}}_{\VEC{h}} \left[
...
$\DS \left\| \Delta^{\VEC{\beta}}_{\VEC{h}} g \right\|_{t,\rho}
\leq \left\| \diff^{\VEC{\beta}} g \right\|_{t,\rho}$
for all $t\in \RR$, $\DS g \in H^{t,2}(\RR^n)$,
$\DS \VEC{h} \in \RR^n$ and $\DS \VEC{\beta} \in \NN^n$
with $h_j \neq 0$ for $\beta_j \neq 0$.
...
\diff^{\VEC{\xi}} g
\end{align*}
for all multi-indices $\VEC{\beta}$ and $\VEC{\eta}$ in $\DS \NN^n$,
and $\DS g \in H^{|\VEC{\eta}|}(\RR^n)$. The previous
...
assume that $\DS \diff^{\VEC{\beta}} f \in H^{s,2}(\RR^n)$ for all
$\VEC{\beta} < \VEC{\alpha}$.
...
\end{equation}
for $\DS f \in H^{k,2}(\Omega_r)$ with
$\DS \supp f \subset \Omega_{\delta r,0}
...
$\DS \left\|\cdot\right\|_{k,2,\RR^n}$ and 
$\DS \left\|\cdot\right\|_{k,\rho}$.  Suppose that
\[
C_3 \left\|g \right\|_{k,2,\RR^n} \leq \left\|g \right\|_{k,\rho}
\leq C_2 \left\|g \right\|_{k,2,\RR^n}
\]
for all $\DS g \in H^{k,2}(\RR^n)$ and some positive
...
$\DS \diff^{\VEC{\beta}} f \in H^{k,2}(\Omega_r)$ for all
$\VEC{\beta} < \VEC{\alpha}$.
...
$\DS E(\diff^{\VEC{\beta}} f) \in H^{k,2}(\RR^n)$
...
that $\alpha_n = 0$.  Hence,
$\DS \supp [\Delta^{\VEC{\alpha}}_{\VEC{h}}, \phi] f
\subset \Omega_{r,0}$.

Hence, for $\|\VEC{h}\| < (1-\delta)r$, we have
\begin{align*}
...
& \leq C_0 C_3^{-1} \sum_{\VEC{\beta}<\VEC{\alpha}}
...
\end{align*}
This gives (\ref{ellRegCor2Eq1}) with $\DS C = C_0 C_1 C_2 C_3^{-1}$.
\end{proof}
...
\]
for $\DS f \in H^{k+|\VEC{\alpha}|-1,2}(\Omega_r)$ with
$\DS \supp f \subset \Omega_{\delta r,0}
= (\RR^n \setminus \RR^n_-) \cap B_{\delta r}(\VEC{0})$ and
$\|\VEC{h}\|<(1-\delta)r$.
...
definition of $\DS \| \cdot \|_{k+|\VEC{\alpha}|-1,2,\Omega_r}$.

According to Theorem~\ref{sob_Rp_Totalext}, there exists a total
extension operator $E$ on $\DS \RR^n_+ $.  From a quick look
at the definition of $E$ given in the sketch of the proof of
Theorem~\ref{sob_Rp_Totalext}, we see
that $\DS E\left( \Delta^{\VEC{\alpha}}_{\VEC{h}} f\right) =
\Delta^{\VEC{\alpha}}_{\VEC{h}} E(f)$ because $\alpha_n=0$.

Since $E$ is a total extension operator $E$ on $\DS \RR^n_+ $, we
have for every $q \in \NN$ that
$\DS E:H^{q,2}(\RR^n_+) \to H^{q,2}(\RR^n)$ is a
bounded operator and $E(g) = g$ on $\DS \RR^n_+$ for all
$\DS g \in H^{q,2}(\RR^n_+)$.  It can be
shown that there exists a positive constant $C_1$ independent of $q$ such that
$\DS \left\| E(g) \right\|_{q,2,\RR^n} \leq C_1
\left\| g \right\|_{q,2,\RR^n_+}$ for all $\DS g \in H^{q,2}(\RR^n_+)$.

To prove our claim, we will need the equivalence of the norms
$\DS \left\|\cdot\right\|_{q,2,\RR^n}$ and 
$\DS \left\|\cdot\right\|_{q,\rho}$ for $q=k$ and
$q=k+|\VEC{\alpha}|-1$.
Namely,
\[
...
\]
for all $\DS g \in H^{q,2}(\RR^n)$ and some positive
...
\]
for all $\DS g \in H^{k+|\VEC{\alpha}|-1,2}(\RR^n)$.
...
\end{align*}
where $\DS C = C_0 C_1 C_{k+|\VEC{\alpha}|-1,,3}^{-1}$.
\end{rmk}
...
Suppose that $\DS \Omega \subset \RR^n$ is a bounded open
...
\end{equation}
for $\DS u \in H^{s,2}_0(\Omega)$, where as usual
\[
...
\end{equation}
for $\DS u \in H^{s,2}_0(\Omega)$.
\end{prop}

\begin{proof}
\stage{i} We first consider the elliptic operator
$\DS L_1(\VEC{x},\diff) = \sum_{|\VEC{\alpha}|= k}
...
\hat{u}(\VEC{y})
\]
for all $\DS u \in H^{s,2}(\RR^n)$.  Since
...
\end{align*}
where $\DS C_1^2 = 2^k \max\{1,1/C_0^2\}$.  We then get
...
where we assume that $\DS a_{\VEC{\alpha}} \in \DD(\RR^n)$.
We may make this assumption because we assume that the $a_{\VEC{\alpha}}$ are
of class $\DS C^\infty$ in an open neighbourhood $U$ of
$\overline{\Omega}$.
Therefore, we may choose $\DS \phi \in \DD(\RR^n)$ such that
...
\end{cases}
\]
to get functions $\DS \tilde{a}_{\VEC{\alpha}} \in \DD(\RR^n)$
...
Suppose that $\DS u \in H^{s,2}(\RR^n)$ is such that
$\DS \supp u \subset B_\delta(\VEC{x}_0)$.
Then it follows from Lemma~\ref{ell_reg_lemma1} that there exists a
...
&\qquad \leq C_2 \delta \, \big\|\diff^{\VEC{\alpha}} u\big\|_{s-k,\rho}
+ C_4 \, \big\|\diff^{\VEC{\alpha}} u \big\|_{s-k-1,\rho} 
\leq C_2 \delta \, \|u\|_{s,\rho}
+ C_4 \, \|u \|_{s-1,\rho}  \ ,
\end{align*}
where $t > n/2$ is arbitrary but fixed,
$\DS C_2 = 2 (2\pi)^{n/2}K$ and \\
$\DS C_4 \geq C_3 \max_{\VEC{x}_0 \in \overline{\Omega}}
...
$\DS H^{|s-k-1|+1+t}(\RR^n)$.  Recall that
...
+C_4 N \| u \|_{s-1,\rho} \ ,
\end{align*}
where $\DS N = \sum_{|\VEC{\alpha}|=k} 1$.  From
...
\end{align*}
If we choose $0<\delta < 1/(2C_1C_2 N)$, then we get
\[
\|u\|_{s,\rho}
...
$\DS \{x_i\}_{i=1}^I$ of $\overline{\Omega}$ such that
$\DS \left\{B_\delta(x_i)\right\}_{i=1}^I$ is an open cover
of $\overline{\Omega}$ with $\delta$ selected in (ii).  Let
$\DS \{\phi_i\}_{i=1}^I$ be a
partition of unity of $\overline{\Omega}$ subordinate to
$\DS \left\{B_\delta(x_i)\right\}_{i=1}^I$.

Given $\DS u \in H^{s,2}_0(\Omega)$, then
$\DS \underline{u} \in H^{s,2}(\RR^n)$ according to
...
\]
where $\DS \psi_{\VEC{\alpha}} \in \DD(\RR^n)$, we get from
...
If $\DS C_8 = \sum_{|\VEC{\alpha}|\leq k-1} R_{\VEC{\alpha},s-k}$,
then we get
...
$\DS \big\|L_2(\VEC{x},\diff)(\phi_i\,\underline{u})
...
\end{align*}
for $\DS u \in H^{s,2}_0(\Omega)$, where $C = C_9 \max\{1, 1+C_{11}\}$.
...
$\DS f \in H^k_0(\Omega)$ and $k \in \NN$.
\end{proof}
...
$\DS \|u \|_{s,2} \leq C_t \left( \| L(\VEC{x},\diff) u \|_{s-k,2}
+ \|u\|_{t,2} \right)$ for all $\DS u \in H^{s,2}_0(\Omega)$.
\end{cor}

\begin{proof}
From Lemma~\ref{ell_garding4} with
$\DS \epsilon = \left(\frac{1}{2C}\right)^2$, where $C$ is the
...
\end{equation}
for all $\DS u \in H^{s,2}_0(\Omega)$.
...
\]
for all $\DS u \in H^{q,2}_0(\Omega)$, where
\[
\underline{u}(\VEC{x}) = \begin{cases}
u(\VEC{x}) & \quad \text{if} \ \VEC{x} \in \Omega \\
  0 & \quad \text{if} \ \VEC{x} \in \RR^n \setminus \Omega
...
If we substitute in (\ref{ell_reg_in_eqtV2}), then we get
\begin{align*}
...
(\ref{ell_reg_ellOp}).  Suppose that $\DS \phi \in \DD(\RR^n)$
and $s\in \RR$.
...
\]
for $\DS v \in H^{s,2}(\RR^n)$ is a bounded mapping from
$\DS H^{s,2}(\RR^n)$ into $\DS H^{s-k+1,2}(\RR^n)$
\footnotemark.
\end{lemma}
...
\end{align*}
for $\DS v \in H^{s,2}(\RR^n)$,
where the coefficients $A_{\VEC{\eta}}$ are in $\DS \DD(\RR^n)$.
Note that
$\DS a_{\VEC{\nu}+\VEC{\eta}}(\VEC{x})\, b_{\VEC{\nu},\VEC{\eta}}
...
\end{equation}
for $\DS v \in H^{s+1,2}(\RR^n)$ and $|\VEC{\eta}|<k$.  Thus
...
$\DS \RR^n$ and $s\in \RR$.  If $\DS u\in H^{s,2}(\RR^n)$
and $\DS L(\VEC{x},\diff) u \in H^{s-k+1,2}_{loc}(\Omega)$, then
$\DS u \in H^{s+1,2}_{loc}(\Omega)$.
\end{lemma}

\begin{proof}
According to Proposition~\ref{sob_Hsloc_cond}, to prove that
$\DS u \in H^{s+1,2}_{loc}(\Omega)$, we only need to prove that
$\DS \phi u \in H^{s+1,2}(\RR^n)$ for all $\phi\in \DD(\Omega)$,
where as usual $\phi \in \DD(\Omega)$ is identified to
$\DS \phi \in \DD(\RR^n)$ with
support in $\Omega$.  Recall that $\DS \phi  u \in \DD'(\RR^n)$ is
defined by $(\phi u)(\psi) = u(\phi \psi)$ for all
$\DS \psi \in \DD(\RR^n)$.

Given $\phi \in \DD(\Omega)$, we have from
Proposition~\ref{sob_T_dd_wk2} that $\DS \phi u \in H^{s,2}(\RR^n)$
and from Proposition~\ref{sob_Hsloc_cond} that
$\DS \phi L(\VEC{x},\diff) u \in H^{s-k+1,2}(\RR^n)$.
Moreover, we have from Lemma~\ref{ell_reg_in_lem1}
that $\left[ L(\VEC{x},\diff), \phi\right]$ is a bounded mapping from
$\DS H^{s,2}(\RR^n)$ into $\DS H^{s-k+1,2}(\RR^n)$.
Thus,
$\DS \left[ L(\VEC{x},\diff), \phi\right]u \in H^{s-k+1,2}(\RR^n)$.
Therefore,
$\DS L(\VEC{x},\diff) \big(\phi u\big) = \phi L(\VEC{x}, \diff) u +
...
$\DS \Delta_{h_j\VEC{e}_j} (\phi u) \in H^{s,2}(\Omega)$
with $\supp \Delta_{h_j\VEC{e}_j} (\phi u) \subset Q
\subset \Omega$ for all $j$.  Hence
$\DS \Delta_{h_j\VEC{e}_j} (\phi u) \in H^{s,2}_0(\Omega)$
...
$\DS \limsup_{\VEC{h}\rightarrow \VEC{0}} \left\| \Delta_{h_j\VEC{e}_j}
(\phi u)\right\|_{s,\rho} < \infty$
for $1\leq j \leq n$.  It follows from Proposition~\ref{ell_reg_prop3} that
$\DS \diff_{x_j} (\phi u) \in H^{s,2}(\RR^n)$
for $1\leq j \leq n$.  Finally, Proposition~\ref{sob_DinDm} implies that
$\DS \phi u \in H^{s+1,2}(\RR^n)$.  Since $\phi \in \DD(\Omega)$
was arbitrary, we have that $\DS u \in H^{s+1,2}_{loc}(\Omega)$.
\end{proof}
...
$\DS f \in H^{s,2}_{loc}(\Omega)$ and $u \in \DD'(\Omega)$ satisfies
$L(\VEC{x},\diff) u=f$ in the sense of distribution,
then $\DS u\in H^{s+k,2}_{loc}(\Omega)$.
...
$\DS \psi_0 u \in \EE'(\RR^n)$; namely, $\psi_0 u $ is a distribution
...
$\DS \{ \psi_j, U_j\}_{j=0}^{m-1}$ such that
...
We prove by induction on $j$ that $\DS \psi_j u \in H^{t+j,2}(\RR^n)$
for $0 \leq j \leq m$.  The result is true for $j=0$ by construction.  
suppose that $\DS \psi_j u \in H^{t+j,2}(\RR^n)$ for $j <m$.
We have for all $\DS \mu \in \DD(\RR^n)$ that
\begin{align*}
...
\end{align*}
where $\DS \psi_{j+1} f \in H^{s,2}(\RR^n)$ because
$\DS f \in H^{s,2}_{loc}(\RR^n)$ and $\psi_{j+1} \in \DD(\Omega)$,
and $\DS \left[ L(\VEC{x},\diff),\psi_{j+1}\right](\psi_j u)
\in H^{t+j-k+1,2}(\RR^n)$ according to 
Lemma~\ref{ell_reg_in_lem1} because
$\DS \psi_j u \in H^{t+j,2}(\RR^n)$.
Therefore, \\
$\DS L(\VEC{x},\diff)(\psi_{j+1} u) \in H^{t+j-k+1,2}(\RR^n)$.
Moreover,
$\DS \psi_{j+1} u = \psi_{j+1}\psi_j u \in H^{t+j,2}(\RR^n)$
according to Proposition~\ref{sob_T_dd_wk2} because
$\DS \psi_j u \in H^{t+j,2}(\RR^n)$ by our hypothesis of induction.
It then follows from Lemma~\ref{ell_reg_in_lem2} that
$\DS \psi_{j+1} u \in H^{t+j+1,2}_{loc}(\Omega)$.

Given $\eta \in \DD(\Omega)$ with $\eta(\VEC{x}) = 1$ for all
$\VEC{x} \in V_j$, we have that
$\DS \eta \psi_{j+1} u \in H^{t+j+1,2}(\RR^n)$
according to Proposition~\ref{sob_Hsloc_cond}.  However,
$\eta \psi_{j+1} u = \psi_{j+1} u$ on $\DS \RR^n$ because
\[
(\eta \psi_{j+1} u)(\mu) = u\left(\eta \psi_{j+1} \mu\right) =
u\left(\psi_{j+1} \mu\right) = (\psi_{j+1} u)(\mu)
\]
for all $\DS \mu \in \DD(\RR^n)$ because
$\supp \psi_{j+1} \subset V_j$ and
so $\eta \psi_{j+1} \xi = \psi_{j+1} \xi$.
Therefore, $\DS \psi_{j+1} u \in H^{t+j+1,2}(\RR^n)$.
This complete the proof by induction.

For $j=m$, we get $\DS \phi u = \psi_m u \in H^{t+m,2}(\RR^n)
= H^{s+k,2}(\RR^n)$.  Since $\phi$ is arbitrary, we get that
$\DS u \in H^{s+k,2}_{loc}(\RR^n)$ according to
...
$\DS L(\VEC{x},\diff)u = f \in C^\infty(\Omega)$ in
the sense of distributions, then $\DS u \in C^\infty(\Omega)$.
Namely, $L(\VEC{x},\diff)$ is hypoelliptic.
\end{cor}

\begin{proof}
Since $\DS f \in C^\infty(\Omega)$, we have that
$\DS f \in H^{s,2}_{loc}(\Omega)$
for all $s$ according to Proposition~\ref{sob_Hsloc_cond}.
Therefore, $\DS u \in H^{s+2,2}_{loc}(\Omega)$ for all $s$
...
$\DS u \in C^\infty(\Omega)$.  To be more precise, let $V$ be an open
...
$\DS v \in H^{s,2}(\RR^n)$ such that $v=u$ on $V$ in the
sense of distributions.  By the Sobolev Lemma,
$\DS v \in C^k(\RR^n)$.  Thus
$\DS u \in C^k(V)$.  Since $V$ and $k$
are arbitrary, this implies that $\DS u\in C^k(\Omega)$ for all
...
Let $\Omega$ be a bounded open subset of $\DS \RR^n$
(or $\DS \RR^n_+$)
with a boundary $\partial \Omega$ is of class $\DS C^{q+2}$.  Let
$\DS X = H^{1,2}(\Omega)$ or
$\DS X = H^{1,2}_0(\Omega)$, and suppose that
...
$\DS f \in H^{q,2}(\Omega)$, then
$\DS u\in H^{q+2,2}(\Omega)$.  Moreover, there exists a constant $C$
...
Let $\Omega$ be an open subset of $\DS \RR^n$, and $k$ and $q$ be two
integers.  There exists a constant $C$ such that
$\DS \left\| \Delta^{\VEC{\gamma}}_{\VEC{h}} f  \right\|_{k,2,\Omega}
\leq C \left\| \diff^{\VEC{\gamma}} f\right|_{k,2,\Omega}$ for all
$\DS f\in H^{k,2}(\Omega)$ with compact support in $\Omega$,
multi-indices $\VEC{\gamma}$ such that $|\VEC{\gamma}|\leq q$, and
$\DS \VEC{h} \in \RR^n$ such that
$\DS \supp f + B_{q\|\VEC{h}\|}(\VEC{0}) \subset \Omega$.
\end{lemma}

\begin{proof}
We have from Lemma~\ref{sob_denselem1} that
$\DS \underline{f} \in H^{k,2}(\RR^n)$.  It follows from
...
Since $\DS \supp \diff^{\VEC{\alpha}} f \subset \diff f
\subset \Omega$ and
$\DS \supp \Delta^{\VEC{\gamma}}_{\VEC{h}} f \subset
\supp f + B_{q\|\VEC{h}\|}(\VEC{0}) \subset \Omega$, we have that
$\DS \diff^{\VEC{\alpha}} \underline{f}
= \underline{\diff^{\VEC{\alpha}} f}$
and $\DS \Delta^{\VEC{\gamma}}_{\VEC{h}} \underline{f}
...
\end{align*}
with $\DS C = C_1^{-1} C_2$.  Note that $C$ does not depend
of $f$ or $\VEC{h}$.
\end{proof}

We begin by assuming that $\DS \Omega = \RR^n_+$.

\begin{prop} \label{ell_regular_Rnp}
Let $\DS \Omega = \RR^n_+$ and
$\DS X = H^{1,2}(\Omega)$ or
$\DS X = H^{1,2}_0(\Omega)$.  For $r>0$, let
$\DS \Omega_r = \Omega \cap B_r(\VEC{0})$
and
$\DS X_r = \left\{ u \in X : \supp u \subset \Omega_\rho \
\text{for some} \ \rho <r \right\}$.
Suppose that $\DS f \in H^{q,2}(\Omega_r)$ for some $r>0$.
...
\end{equation}
for $\DS v \in X_r$, where $B$ is weakly coercive, then
$\DS u\in H^{q+2,2}(\Omega_\rho)$ for
...
$\DS \diff^{\VEC{\gamma}} u \in H^{1,2}(\Omega_{\rho})$ for all
...
$\DS r_1 = \frac{2\rho+r}{3}$ and
$\DS r_2 = \frac{\rho+2r}{3}$.  We have $\rho <r_1<r_2< r$.

The induction hypothesis applied to the domain $\Omega_{r_2}$ instead
of $\Omega_\rho$ says that
$\DS \diff^{\VEC{\gamma}} u \in H^{1,2}(\Omega_{r_2})$ for
...
If $\DS \|\VEC{h}\| < (r_2-r_1)/(j+1)$,
then $\DS \Delta_{\VEC{h}}^{\VEC{\gamma}} (\psi u) \in X_{r_2}$ because
$\supp \psi \subset B_{r_1}(\VEC{0})$, and
$\DS \Delta_{\VEC{h}}^{\VEC{\gamma}} v \in X_{r_2}$ for
$v \in X_{r_1}$.  In
the sequel, we always assume that $\|\VEC{h}\|$ satisfies
$\DS \|\VEC{h}\| < (r_2-r_1)/(j+1)$.
...
$\DS \overline{\psi} \Delta_{-\VEC{h}}^{\VEC{\gamma}} v
...
$\DS C_{E_i} \|v\|_{1,2,\Omega_{r_2}}\, \left( \|f\|_{q,2,\Omega_r}
+ \|u\|_{1,2,\Omega_r} \right)$ for each of the $E_i$, where the $C_{E_i}$
are constants.

We first consider $E_1$.  Since $\supp (\psi u) \subset \Omega_{r_1}$
and $\DS \supp \Delta_{\VEC{h}}^\gamma (\psi u) \subset \Omega_{r_2}$,
we may use Schwarz inequality in $\DS L^2(\Omega_r)$ and
...
\]
for all $v \in X_{r_2}$, where
$\DS C_{E_1} = Q_1 C_2 \sum_{|\VEC{\eta}|\leq j,\,\eta_n=0} 1$.
...
\end{cases}
\]
Hence, we get from Schwarz inequality in $\DS L^2(\Omega_r)$ that
\begin{align*}
\left| E_2(u,v) \right|
...
$\DS \sup_{\VEC{x} \in \overline{\Omega}} \left|\diff^{\VEC{\mu}}
b_{\VEC{\alpha},\VEC{\beta}}(\VEC{x})\diff^{\VEC{\beta}+\VEC{\nu}}
\psi(\VEC{x}) \right|$
for $\VEC{\mu}+\VEC{\nu} \leq \VEC{\gamma} - \VEC{\eta}$, and
$\DS Q_2 = \max_{\VEC{\eta} \leq \VEC{\gamma}}
\sum_{|\VEC{\beta}|=1,\,|\VEC{\alpha}|\leq 1}
...
It follows from Schwarz inequality in $\DS L^2(\Omega_r)$ that
\begin{align*}
...
summations of terms of the form
$\DS \sup_{\VEC{x} \in \overline{\Omega_r}} \left|\diff^{\VEC{\mu}}
b_{\VEC{\alpha},\VEC{\beta}}(\VEC{x})\diff^{\VEC{\alpha}+\VEC{\nu}}
\psi(\VEC{x}) \right|$
for $\VEC{\mu}+\VEC{\nu} \leq \tilde{\VEC{\gamma}} - \VEC{\eta}$, and
$\DS Q_3 = P_1^2 \max_{\VEC{\eta} \leq \tilde{\VEC{\gamma}}}
\sum_{|\VEC{\beta}|=1,\,|\VEC{\alpha}|\leq 1}
\tilde{Q}_{\VEC{\alpha},\VEC{\beta},\VEC{\eta}}$.
...
Using Schwarz inequality in $\DS L^2(\Omega_r)$, we get
\begin{align*}
...
\]
We therefore have that $\DS C_{E_4} = P_1^2 
...
$\DS v = \Delta^{\VEC{\alpha}}_{\VEC{h}} (\psi u)$ in
(\ref{ell_wcREG}) and use (\ref{ell_reg_B3}), then we get
\begin{align*}
...
$\DS \diff^{\VEC{\gamma}} (\psi u) \in H^{1,2}(\Omega_{r_2})$
because
$\DS \limsup_{\VEC{h}\rightarrow \VEC{0}}
...
$\gamma_n \leq j$.  Then,
$\DS \diff^{\VEC{\gamma}} u \in L^2(\Omega_\rho)$ and there
...
\diff^{\VEC{\alpha}} u \ ,
\end{align*}
where the $\DS g_{\VEC{\alpha}}, h_{\VEC{\alpha}}
...
\]
where
$\DS A = \sum_{|\VEC{\alpha}|\leq q}
\sup_{\VEC{x}\in \overline{\Omega}_r} \left| g_{\VEC{\alpha}}(\VEC{x}) \right|$
and
$\DS B = C_7(j-1,\rho) \sum_{|\VEC{\alpha}|\leq q+2,\, \alpha_n\leq j-1}
\sup_{\VEC{x}\in \overline{\Omega}_r} \left| h_{\VEC{\alpha}}(\VEC{x}) \right|$.
Hence,
\[
...
$\DS \tilde{C} = \max_{0\leq j \leq q+2} C_7(j.\rho)$, where the
$C_7(j,\rho)$ are given by claim B.  Thus
$\DS \diff^{\VEC{\alpha}} u \in L^2(\Omega_\rho)$ and
\[
...
that $\DS u \in H^{k+2,2}(\Omega_\rho)$ and
\[
...
need to provide a lemma like\\
Lemma~\ref{ell_reg_in_lem1} but for
$\Omega$ a bounded open subset of $\DS \RR^n$.

\begin{lemma} \label{ell_reg_out_lem1}
We consider the elliptic operator $L(\VEC{x},\diff)$ defined in
(\ref{ell_reg_ellOp}).  Suppose that $\Omega$ is a bounded open subset
of $\DS \RR^n$, $\phi \in \DD(\Omega)$ and $q\in \NN$.
The operator $\left[ L(\VEC{x},\diff), \phi\right]$ defined by
\[
\left[ L(\VEC{x},\diff), \phi\right](v)
= L(\VEC{x},\diff) (\phi\, v) - \phi\, L(\VEC{x},\diff) v
\]
for $\DS v \in \DS H^{q,2}(\Omega)$ is a bounded
mapping from $\DS H^{q,2}(\Omega)$ into
$\DS H^{q-k+1,2}(\Omega)$.
\end{lemma}

\begin{proof}
The proof is identical to the proof of Lemma~\ref{ell_reg_in_lem1}
with $\DS \RR^n$ replaced by $\Omega$ except that
...
hyperplane $\DS \RR^n_0$ near the origin.}{REG_BDR}
...
$B_{\eta_{\VEC{z}}}(\VEC{z})$ and a $\DS C^{q+2}$-diffeomorphisms
$\DS \psi_{\VEC{z}}:B_{\eta_{\VEC{z}}}(\VEC{z}) \rightarrow \RR^n$
such that
\begin{enumerate}
\item $\psi_\VEC{z}(\VEC{z})=\VEC{0}$,
\item $\DS \psi_\VEC{z}(B_{\eta_\VEC{z}}(\VEC{z})\cap \Omega)
\subset \RR^n_+$, and
\item $\DS \psi_\VEC{z}(B_{\eta_\VEC{z}}(\VEC{z})\cap
\partial \Omega) \subset \RR^n_0=\{\VEC{x} \in \RR^n : x_n=0 \}$.
\end{enumerate}
Choose $r_\VEC{z}$ small enough such that
$\DS \Omega_{r_\VEC{z}} = B_{r_{\VEC{z}}}(\VEC{0}) \cap \RR^n_+
...
$\DS \{ W_j\}_{j=0}^{q+1}$ of open subsets of $\Omega$ such that
\[
...
$\DS \psi_j u \in H^{q+2-j,2}_0(W_{j+1})$ according to
...
inequality.  If
$\DS K_{j-1} = C_{1,j-1} + C_{1,j-1} C_{2,j-1} K_j$,
then we get (\ref{ell_ind_RT2}) for $i$ replaced by $i-1$.  This complete
...
\]
where $\tilde{B}$ is still weakly coercive and\\
$\tilde{v} \in \DS X_r = \left\{ \tilde{v} \in X : \supp u \subset
B_\rho(0)\cap \RR^n_+ \ \text{for some} \ \rho < r_{\VEC{z}} \right\}$
with $\DS X = H^{q,2}(\RR^n_+)$ or
$\DS X = H^{q,2}_0(\RR^n_+)$.
We can then conclude from Proposition~\ref{ell_regular_Rnp} that
$\DS \tilde{u}\in H^{q+2,2}\big(\Omega_{\rho_\VEC{z}}\big)$ and there
...
\]
We then have that
$\DS u = T_{\VEC{z}}(\tilde{u}) \in H^{q+2,2}
\big(\psi_{\VEC{z}}^{-1}(\Omega_{\rho_\VEC{z}})\big)
= H^{q+2,2} \big(V_j \cap \Omega\big)$.

Since $T_{\VEC{z}}$ and $\DS T_{\VEC{z}}^{-1}$ are bounded
mappings, there exist constants $C_1$ and $C_2$ such that
$\DS
\|T_{\VEC{z}}(\tilde{v})\|_{q,2,\psi^{-1}_{\VEC{z}}(\Omega_{r_\VEC{z}})}
\leq C_1 \|\tilde{v}\|_{q,2,\Omega_{r_\VEC{z}}}$
for $\DS \tilde{v} \in H^{q,2}\big(\Omega_{r_{\VEC{z}}}\big)$
and
$\DS
...
\stage{iii} Let $\DS \{\phi_j\}_{j=1}^s$ be a partition of unity of
$\Omega$ subordinated to $\DS \{ V_j \}_{j=0}^s$.
We have that $\DS u = \sum_{j=0}^s \phi_j u$, where
$\supp (\phi_j u) \subset V_j \cap \Omega$ for all $j$.  Hence,
from (i) and (ii), we have that $\DS u\in H^{q+2,2}(\Omega)$ and
\begin{align}
...
\end{align}
where $\DS K = \sum_{j=0}^s C_j K_j$.  We have used
...
\]
If we combine this inequality with (\ref{ell_ind_RT5}), then we get
\begin{align*}
\|u\|_{q+2.2,\Omega} &\leq 
...
\end{align*}
where $\DS C = \max\left\{K\lambda,
K \left(1+ \frac{1}{C_0}\right)\right\}$.
This complete the proof of Theorem~\ref{ell_regular}.
\end{proof}

\begin{cor} \label{ell_fCi_uCi}
In Theorem~\ref{ell_regular}, if
$\DS f \in C^\infty(\overline{\Omega})$
and $\partial \Omega$ is a $\DS C^\infty$-manifold, then
$\DS u\in C^\infty(\overline{\Omega})$.
\end{cor}

\begin{proof}
Since $\DS f \in H^{k,2}(\Omega)$ for all $k$
because all the derivatives of $f$ are bounded on $\overline{\Omega}$, 
we get from
Theorem~\ref{ell_regular} that $\DS u \in H^{k,2}(\Omega)$
for all $k$.  It then follows from Corollary~\ref{sob_cor_sob_lem} that
$\DS u \in C^k(\overline{\Omega})$ for all $k$.
\end{proof}

It follows from this corollary that eigenvectors of the strongly
elliptic operator $L(\VEC{x},\diff)$ are of class $\DS C^\infty$ on
$\overline{\Omega}$.
...
Let $\Omega$ be a bounded open subset of $\DS \RR^n$ with a boundary
$\partial \Omega$ of class $\DS C^\infty$.  If
$\DS f \in C^\infty(\overline{\Omega})$,
the solution of the Dirichlet problem $\Delta u = 0$ on $\Omega$ with
$u=f$ on $\partial \Omega$ is in $\DS C^\infty(\overline{\Omega})$.
\end{prop}
...
Let $\DS w \in H^{1,2}_0(\Omega)$ be the weak solution of
...
$\DS w \in C^\infty(\overline{\Omega})$.  Moreover, $w=0$ on
...
$\DS u = w+f \in C^\infty(\overline{\Omega})$.
\end{proof}
...
$\DS \phi \in \DD(\RR^n)$ such that $\phi(\VEC{x})=0$ and
...
$\DS u_{\VEC{x}} \in C^\infty(\overline{\Omega})$ since
$\DS M \in C^\infty(\overline{\Omega})$.
...
$\DS G(\VEC{x},\VEC{y}) = N(\VEC{x}-\VEC{y})
...
$\DS \{ g_j\}_{j=1}^\infty$ of polynomials such that
$\DS \sup_{\VEC{x} \in \partial \Omega}
...
$\DS u_j \in C^\infty(\overline{\Omega})$.  Hence, as it was
...
\pdydx{G_{\VEC{x}}}{\nu}(\VEC{y})\dss{S}{y}
\]
because $\DS g_j(\VEC{y})\,\pdydx{G_{\VEC{x}}}{\nu}(\VEC{y})$
converges uniformly to $\DS g(\VEC{y})\,
...
admit solutions in $\DS C^2(\Omega) \cap C(\overline{\Omega})$.
...
namely, $A$ is symmetric and $\DS \VEC{x}^\top A\VEC{x} \geq 0$ for
all $\DS \VEC{x} \in \RR^n$.
...
$\DS Q = P^\top A P$ is a diagonal matrix.  Since $A$ is positive
semi-definite, the elements $q_{i,i}$ on the diagonal of $Q$ (which
are the eigenvalues of $A$) are greater or equal to zero.

Let $\DS C = P^\top B P$.  We have that $C$ is negative
semi-definite because\\
$\DS \VEC{x}^\top C \VEC{x} = (P\VEC{x})^\top B (P \VEC{x}) \leq 0$
for all $\DS \VEC{x} \in \RR^n$.  Therefore
$\DS c_{i,i} = \VEC{e}_i^\top C \VEC{e}_i \leq 0$
for $1\leq i \leq n$.
...
\]
The fact that $\DS \tr(AB) = \tr(P^\top AB P)$ comes from
...
Let $\Omega$ be a connected open subset of $\DS \RR^n$ with a boundary
$\partial \Omega$ of class $\DS C^2$.  Suppose that
\[
L(\VEC{x},\diff)u = - \sum_{i,j=1}^n
\diff_{x_i} \left( b_{i,j}(\VEC{x}) \diff_{x_j} u \right)
\]
is an elliptic operator, where
$\DS b_{i,j} \in C^1(\overline{\Omega})$
for $1 \leq i,j\leq n$.
Let $\DS u \in C^2(\Omega) \cap C(\overline{\Omega})$ be the
solution of $L(\VEC{x},\diff)u = f$, where $f \in C(\overline{\Omega})$.
\begin{enumerate}
\item If $f(\VEC{x}) \geq 0$ for all $\VEC{x} \in \overline{\Omega}$
and $\DS \min_{\VEC{x}\in \overline{\Omega}} u(\VEC{x}) = u(\VEC{p})$
for some $\VEC{p} \in \Omega$, or
\item if $f(\VEC{x}) \leq 0$ for all $\VEC{x} \in \overline{\Omega}$
and $\DS \max_{\VEC{x}\in \overline{\Omega}} u(\VEC{x}) = u(\VEC{p})$
...
\end{equation}
where $\DS b_j = \sum_{i=1}^n \diff_{x_i} b_{i,j}$ for
$1 \leq i \leq n$.
...
$\gamma(1) = \VEC{p}$. 
Since $\gamma([0,1])$ is a compact subset of $\Omega$, we have
\[
...
\end{equation}
where $\alpha>0$ will be selected later.  We have
...
have that
$\DS \sum_{i,j=1}^n b_{i,j}(\VEC{x}) (x_i - r_i)(x_j - r_j) > 0$
...
Let $\DS m = \max_{\VEC{x} \in S_1} h(\VEC{x})$.  Note that
...
$\DS \sum_{i,j=1}^n \diff_{x_i x_j} v(\VEC{q}_2) y_i y_j \leq 0$
for all $\DS \VEC{y} \in \RR^n$.  Moreover, since
$L(\VEC{x},\diff)$ is elliptic on $\Omega$, we also get that
$\DS \sum_{i,j=1}^n b_{i,j}(\VEC{q}_2) y_i y_j \geq 0$
for all $\DS \VEC{y} \in \RR^n$.

Let $A$ and $\DS B^\top$ be the matrices with entries
...
Let $\Omega$ be a connected open subset of $\DS \RR^n$ with
a boundary $\partial \Omega$ of class $\DS C^2$.  Suppose that
\[
L(\VEC{x},\diff)u = - \sum_{i,j=1}^n
...
$\DS \min_{\VEC{x}\in \overline{\Omega}} u(\VEC{x}) = u(\VEC{p}) < 0$
for some $\VEC{p} \in \Omega$, or
\item if $f(\VEC{x}) \leq 0$ for all $\VEC{x} \in \overline{\Omega}$ and
$\DS \max_{\VEC{x}\in \overline{\Omega}} u(\VEC{x}) = u(\VEC{p}) > 0$)
for some $\VEC{p} \in \Omega$,
\end{enumerate}
then $u(\VEC{x})= u(\VEC{p})$ for all $\VEC{x} \in \Omega$.  In
...
$\DS u_1 , u_2 \in C^2(\Omega) \cap C(\overline{\Omega})$
satisfy $L(\VEC{x},\diff) u = f$ on $\Omega$ and $u_1 = u_2$ on
$\partial \Omega$, then $u_1 = u_2$ on $\Omega$.
\end{cor}

\begin{proof}
Since $\DS u = u_1-u_2 \in C^2(\Omega) \cap C(\overline{\Omega})$
...
Suppose that $\DS u \in C^2(\Omega) \cap C(\overline{\Omega})$ is a
...
Since $\partial \Omega$ is of class $\DS C^2$, we can find
...
$\DS m_1 = \max_{\VEC{x} \in S_1} h(\VEC{x})$
and $\DS m_2 = \max_{\VEC{x} \in S_1} u(\VEC{x})$.
...
= -2 \alpha \|\VEC{p}-\VEC{r}\| e^{-\alpha\|\VEC{p}-\VEC{r}\|^2}
 < 0 \ .
\end{align*}
Therefore, we get $\DS 0 < \pdydx{u}{\mu}(\VEC{p})$ from
(\ref{dirDerMaxAEq2}).
...
Since $\partial \Omega$ is of class $\DS C^2$, we can select a ball
...
that $\DS \pdydx{u}{\mu}(\VEC{p}) > 0$.
\end{proof}

The fact that $\DS \pdydx{u}{\nu} u(\VEC{x}) \leq 0$ 
...
Let $\Omega$ be an open subset of $\DS \RR^n$,  Suppose that
$\DS f\in L^2(\Omega)$ and that
$\DS u\in H^{1,2}(\Omega)\cap C(\overline{\Omega})$ is a solution of
\begin{equation} \label{ell_max_eq1}
B(v,u) = \int_\Omega \left( \graD u \cdot \graD v + u\,v \right) \dx{\VEC{x}}
= \int_\Omega f\,v \dx{\VEC{x}}
\end{equation}
for all $\DS v \in H^{1,2}_0(\Omega)$.  Then
\[
...
We choose a function $\DS g\in C^1(\RR)$ such that
...
$\DS v \in L^2(\Omega) \cap L^1(\Omega)$ because
$v$ is continuous on the compact set $\overline{\Omega}$.  Moreover,
for any multi-index $\VEC{\alpha}$ with $|\VEC{\alpha}|=1$, we have that
$\DS \diff^{\VEC{\alpha}} v = g'(u-K) \diff^{\VEC{\alpha}} u$
in the sense of distributions with $|g'|\leq 1$ and
$\DS \diff^{\VEC{\alpha}} u\in L^2(\Omega)$
because $\DS u \in H^{1,2}(\Omega)$.  Thus,
$\DS \diff^{\VEC{\alpha}} v \in L^2(\Omega)$.
Therefore, $\DS v\in H^{1,2}(\Omega)$.

Since $\DS v\in H^{1,2}(\Omega) \cap C(\overline{\Omega})$ and
$v(\VEC{x}) = g(u(\VEC{x})-K) = 0$ for $\VEC{x}\in \partial \Omega$ by
construction, we get from Proposition~\ref{sob_w0_trad2} that
$\DS v\in H^{1,2}_0(\Omega)$.
...
\dx{\VEC{x}} \ .
\]
Since $\DS v \in L^1(\Omega)$, we may subtract
$\DS K \int_{\Omega} g(u-K) \dx{\VEC{x}}$ from each side of
...
\dx{\VEC{x}} \ .
\]
Since $\DS \|\graD u \|_2^2\, g'(u-K)\geq 0$ and
...
\stage{i.b}
If $\Omega$ is not bounded, then we must have $K\geq 0$ because
$f(\VEC{x}) \leq K<0$ for almost all $\VEC{x}$ and
$\Omega$ not bounded implies that $\DS f \not\in L^2(\Omega)$.
Choose $K_0 > K$ and let
\begin{align*}
v: \overline{\Omega} & \rightarrow \RR \\
\VEC{x} &\mapsto g(u(\VEC{x})-K_0)
\end{align*}
for $g$ defined above.  To prove that $\DS v\in H^{1,2}_0(\Omega)$, we
...
|g'(y)|\,|x| \leq |x| \ .
\]
Hence $|g(u-K_0)| \leq |u|$.  Since $\DS u \in L^2(\Omega)$,
we have that $\DS v = g(u-K_0) \in L^2(\Omega)$.  The proof that
$\DS \diff^{\VEC{\alpha}} v \in L^2(\Omega)$ for the multi-index
$\VEC{\alpha}$ such that $|\VEC{\alpha}|=1$ is identical to the proof
given in (i.a).
As in (i.a), $\DS v\in H^{1,2}(\Omega) \cap C(\overline{\Omega})$ and
$v(\VEC{x}) = g(u(\VEC{x})-K_0) = 0$ for $\VEC{x}\in \partial \Omega$
implies that $\DS v\in H^{1,2}_0(\Omega)$.
...
$\DS v \in L^1(\Omega)$ as we now prove.  Let
$\DS S = \left\{ \VEC{x} : u(\VEC{x}) > K_0 \right\}$.  We
have
\[
...
\]
because $\DS u \in H^{1,2}(\Omega)$ and
$\DS u^2(\VEC{x})\geq K_0 \left|u(\VEC{x})\right|$ for all
$\VEC{x} \in S$.
It is here that we need $K_0>K\geq 0$ to get $K_0 \neq 0$.

Since $\DS K_0 \int_\Omega g(u-K_0) \dx{\VEC{x}} \in \RR$, we may
...
\begin{theorem}
Let $\Omega$ be an open subset of $\DS \RR^n$ and
\[
L(\VEC{x},\diff)u = - \sum_{i,j=1}^n
\diff_{x_i} \left( b_{i,j}(\VEC{x}) \diff_{x_j} u \right)
+ b_0(\VEC{x}) u
...
$\DS b_{i,j} \in H^{1,\infty}(\Omega)$ for all $i$ and $j$,
that $\DS b_j \equiv \sum_{i=1}^n \diff_{x_i} b_{i,j} = 0$
almost everywhere on $\Omega$ for all $j$, and that
$\DS b_0 \in L^\infty(\Omega)$
satisfies $b_0 \geq 0$ almost everywhere on $\Omega$.  Suppose that
$\DS f\in L^2(\Omega)$ and that
$\DS u\in H^{1,2}(\Omega)\cap C(\overline{\Omega})$ is a
...
$\DS u(\VEC{x}) \geq \inf_{\VEC{y}\in \partial \Omega} u(\VEC{y})$
for all $\VEC{x} \in \Omega$, and
\item $f\leq 0$ almost everywhere on $\Omega$ implies that
$\DS u(\VEC{x}) \leq \sup_{\VEC{y}\in \partial \Omega} u(\VEC{y})$
for all $\VEC{x} \in \Omega$, and
\item $f= 0$ almost everywhere on $\Omega$ implies that
$\DS \inf_{\VEC{y}\in \partial \Omega} u(\VEC{y}) \leq u(\VEC{x}) \leq
...
$\DS b_j \equiv \sum_{i=1}^n \diff_{x_i} b_{i,j} = 0$
...
\stage{1}  Let $\DS g\in C^1(\RR)$ be the function defined
in the proof of Proposition~\ref{ell_max_DP}.  We define
$\DS h\in C^1(\RR)$ by $h(x) = -g(-x)$
for $x\in \RR$.  As in the proof of Proposition~\ref{ell_max_DP}, we have
that $\DS v=h(u) \in H^{1,2}_0(\Omega)$.  If we substitute
$v=h(u)$ in (\ref{ell_max_eq3}), then we get
\[
\sum_{i,j=1}^n
...
\]
We get $\DS \|\graD u\|_2^2\, h'(u) =0$ almost everywhere on $\Omega$.
...
\stage{2} Let $\DS K = \inf_{\VEC{x}\in \partial \Omega} u(\VEC{x})$ and
consider the function $u-K$.  Since $\Omega$ is bounded, we have that
$\DS u-K \in H^{1,2}(\Omega)$.  Moreover, since $b_0 = 0$ almost
...
$\DS K = \max_{\VEC{x}\in \partial \Omega} u(\VEC{x})$ and
consider the function $K-u$.  Since $\Omega$ is bounded, we have that
$\DS K-u \in H^{1,2}(\Omega)$.  Moreover, since $b_0 = 0$ almost
...
Let $\Omega$ be an open subset of $\DS \RR^n$,  Suppose that
$\DS f\in L^2(\Omega)$ and that $\DS u\in H^{1,2}(\Omega)$
is a solution of 
\[
B(v,u) =
\int_\Omega \left( \graD u \cdot \graD v +
...
\]
for all $\DS v \in H^{1,2}(\Omega)$.  Then
\[
...
variational method in \cite{Sal} is slightly different than our
