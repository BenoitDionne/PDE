...
Let $S$ be an hyper-surface in $\DS \RR^n$ and
...
$\DS u: \RR^n \rightarrow \RR$ such that
...
\]
for all $\DS \VEC{\alpha} \in \NN^n$ with $\alpha_n = 0$.
From $\DS \pdydx{u}{x_n}(x_1,x_2, \ldots, x_{n-1},0)
...
\]
for all $\DS \VEC{\alpha} \in \NN^n$ with $\alpha_n = 1$.
Proceeding like this for the derivative of $u$ up to order $m-1$, we
can compute
$\DS \diff^{\VEC{\alpha}} u(x_1, x_2, \ldots, x_{n-1}, 0)$ for
all $\DS \VEC{\alpha} \in \NN^n$ with $\alpha_n < m$.

The problem is to compute
$\DS \pdydxn{u}{x_n}{j}(x_1,x_2,\ldots,x_{n-1},0)$ for $j\geq m$.
For this, we assume that (\ref{cauchy_GPDE}) can be solved for
$\DS \pdydxn{u}{x_n}{m}$ for $\VEC{x}$ near the hyper-surface $S$.
Namely, we assume that we can write
\begin{equation} \label{cauchy_CK_cond}
\DS \pdydxn{u}{x_n}{m} = H\left(\VEC{x},
\left\{ \diff^{\VEC{\alpha}} u\left(\VEC{x}\right) :
\VEC{\alpha} \in \NN^n \ , \ 
|\VEC{\alpha}| \leq m \ , \, \VEC{\alpha} \neq m \VEC{e}_n \right\} \right)
\end{equation}
for $\DS \VEC{x} \in \RR^n$ near $S$, where, as usual,
$\DS \left\{ \VEC{e}_1, \VEC{e}_2, \ldots, \VEC{e}_n\right\}$ 
is the canonical basis of $\DS \RR^n$.  Not all
...
Let $\DS S \subset \RR^n$ be a analytic hyper-surface.  Assume that
...
Consider the $\DS m^{th}$ order linear partial differential equation
\[
L(\VEC{x}, \diff)u = \sum_{|\VEC{\alpha}|\leq m}
a_{\VEC{\alpha}}(\VEC{x}) \diff^{\VEC{\alpha}} u = 0
\]
on $\DS \RR^n$, where the functions
$\DS a_{\VEC{\alpha}}:\RR^n \rightarrow \RR$
are real analytic. Suppose that $S$ is an analytic non-characteristic
hyper-surface.  Then, the unique solution of class $\DS C^m(\RR^n)$
which satisfy the Cauchy conditions
$\DS \diff^{\VEC{\alpha}} u = 0$
...
\begin{defn}
Let $\DS \Omega \subset \RR^n$ be an open set.  Suppose that
\begin{enumerate}
...
$B\times ]a,b[$, and
\item $\DS \Omega = \bigcup_{\lambda \in ]a,b[} S_\lambda$, 
where $\DS S_\lambda = \left\{ \VEC{x} : \VEC{x} = F(\VEC{y},\lambda)
\ \text{for} \ \VEC{y} \in B \right\}$.
\end{enumerate}
Then the family of hyper-surfaces
$\DS \left\{ S_\lambda \right\}_{\lambda \in ]a,b[}$ is an
...
a_{\VEC{\alpha}}(\VEC{x}) \diff^{\VEC{\alpha}} u = 0
\]
on an open set $\Omega$ of $\DS \RR^n$, where the functions
...
and
$\DS \Omega_\mu = \bigcup_{a<\lambda \leq \mu} S_\lambda$.
...
unique solution of class $\DS C^m(\overline{R})$ which
satisfy the Cauchy conditions $\DS \diff^{\VEC{\alpha}} u = 0$ on
$Z$ for all multi-indices $\VEC{\alpha}$ with $|\VEC{\alpha}|<m$
is $u=0$ on $R$.
\end{theorem}

If $\DS R\subset \RR^n$, recall that
$\DS f \in C^m(\overline{R})$ means that
$\DS f\in C(\overline{R}) \cap C^m(R)$ and
$\DS \diff^{\VEC{\alpha}} f$ can be extended
...
$\DS u:\RR^3\to \CC$ is a
solution of $L(x,y,\diff)u = f$ of class $\DS C^1$ in the
...
$\DS \int_0^{2\pi} \pdydx{v}{\theta}(r,\theta,t) \dx{\theta}
= v(r,\theta,t) \Big|_0^{2\pi} = 0 $ because $v$ is $2\pi$-periodic.

Let
$\DS F(t) = \int_0^t f(s)\dx{s}$ and $W(t,r) = V(t,r) + \pi\, i F(t)$
...
It follows that $\DS f = \dydx{F}{t}$ is real
analytic at $t=0$.
\end{proof}

\begin{rmk}
Given $\DS (x_0,y_0,t_0) \in \RR^3$, we define a translation
in $\DS \RR^3$ by
\begin{align*}
T:\RR^3 &\to \RR^3 \\
 (x,y,t) &\mapsto (x+x_0,y+y_0, t+t_0 +2y_0x - 2x_0 y)
\end{align*}
If $L(x,y,\diff)$ is the Lewy operator defined in Theorem~\ref{Lewy},
then we have
$\DS L(x,y,\diff)(u \circ T) = \left(L(x,y,\diff)u\right) \circ T$
for any function $\DS u \in C^1(\RR^3)$.
...
class $\DS C^1$ of $L(x,y,\diff)u = f(t)$ near
$(x_0,y_0,t_0)$ unless $f$ is
analytic at $t=-t_0-2y_0x + 2x_0y$.

H. Lewy has also shown that there are functions
$\DS f \in C^{\infty}(\RR^3)$
such that $L(x,y,\diff)u = f$ has no solution $u$ of class
$\DS C^{1,\mu}(\RR^n)$ for $\mu > 0$ in a neighborhood of
the origin and by extension anywhere in $\DS \RR^3$.  The spaces
$\DS C^{k,\mu}(\RR^n)$ of Hölder continuous functions are
