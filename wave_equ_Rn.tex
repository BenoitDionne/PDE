\chapter{Wave Equation in $\DS \mathbf{\RR^n}$} \label{ChapWaveEqunD}
...
\ ,
\]
where the matrix has all eigenvalues but one equal to
$\DS -c^2 <0$.  The other eigenvalue is $1>0$.
\begin{defn}
An hyper-surface $\DS S \subset \RR^n \times \RR$ is called
{\bfseries space-like}\index{Wave Equation!Space-Like} if its normal vector
\[
\nu(\VEC{x},t) = (v(\VEC{x},t),w(\VEC{x},t) ) \ ,
\]
where $\DS v:S \to \RR^n$ and $w:S \to \RR$,
satisfies $|w(\VEC{x},t)| > \|v(\VEC{x},t) \|$ for all
$(\VEC{x},t) \in S$.
\end{defn}

As in Figure~\ref{wave_FIG6}, if the surface is
$\DS S = \left\{ (\VEC{x},t) : t=\phi(\VEC{x}) \ \text{and}
\ \VEC{x} \in \RR^n \right\}$
for some smooth function $\DS \phi:\RR^n \to \RR$,
...
$\DS u \in C^2(R)$ satisfies\\
$L(\VEC{x},t,\diff)u = 0$ on $R$,  $u=0$ on $B$ and 
$\DS \pdydx{u}{t} = 0$ on $B$.  Then $u=0$ on
\[
...
$\DS \pdydxn{u}{s}{2} - \Delta_{\VEC{x}} u = 0$.  Thus, we
may assume that
$\DS L(\VEC{x},t,\diff) =
...
$\DS F(\VEC{x},t) = \left(\pdydx{u}{t}\right)^2 +
...
\dx{\VEC{x}} \\
&\qquad \qquad = \int_{B_t} \left( \sum_{j=1}^n
\pdfdx{ \left( \pdydx{u}{x_j} \pdydx{u}{t} \right) }{x_j}
...
\]
because
$\DS L(\VEC{x},t,\diff) u = \pdydxn{u}{t}{2} -
\Delta_{\VEC{x}} u = 0$.
Thus
\[
\int_{B_t} \left( \pdydxn{u}{t}{2} \pdydx{u}{t} +
...
\]
where we have use the inequality $\DS 2ab \leq a^2 + b^2$ for the last
inequality.  Therefore,
$\DS \dydx{E}{t} \leq 0$ for $0 <t < t_0$.

Since $u(\VEC{x},0)=0$ for all $\VEC{x} \in B$ by hypothesis, we get
$\DS \pdydx{u}{x_j}(\VEC{x},0) = 0$ for all $\VEC{x} \in B$.
Moreover, since $\DS \pdydx{u}{t}(\VEC{x},0)=0$ for all
$\VEC{x} \in B$ by hypothesis, we get $E(0)=0$.

Finally, since $E(t) \geq 0$ for $0 \leq t \leq t_0$ by definition,
$E(0) = 0$, and $\DS \dydx{E}{t} \leq 0$ for $0 <t < t_0$,
...
\begin{rmk}
Suppose that $\DS F:\RR^n\times \RR\rightarrow \RR$ is a
function of class $\DS C^1$ and
\[
...
\begin{cor}
If $u$ and $v$ in $\DS C^2(\RR^n\times \RR)$ are two solutions of
...
$\DS \big(L(\VEC{x},\diff)u\big)(\VEC{x}+\VEC{y}) =
L(\VEC{x},\diff)\big( u(\VEC{x}+\VEC{y}) \big)$ for all
$\DS u\in C^k(\RR^n)$, and for
...
\diff^{\VEC{\alpha}} u(\VEC{x}+\VEC{y})
\end{align}
for all $\DS u\in C^k(\RR^n)$, and for all $\VEC{x}$ and
$\VEC{y}$ in $\DS \RR^n$.  If
$\DS u(\VEC{x}) = \VEC{x}^{\VEC{\alpha}}$ for some multi-indices
$\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq k$, then
$\DS \diff^{\VEC{\beta}} u(\VEC{x})
= \frac{\VEC{\alpha}!}{(\VEC{\alpha}-\VEC{\beta})!}
\VEC{x}^{\VEC{\alpha}-\VEC{\beta}}$
for $\VEC{\beta} \leq \VEC{\alpha}$ and $0$ otherwise.  In particular,
$\DS \diff^{\VEC{\alpha}} u(\VEC{x}) = \VEC{\alpha}!$.  With
$\VEC{y} = - \VEC{x}$, (\ref{laplace_op1}) becomes
\[
...
We have for all $\DS u\in C^k(\RR^n)$ that
\[
...
If $\DS p(\VEC{y}) = \sum_{|\VEC{\alpha}|\leq k}
...
$j$ in $\DS \|\VEC{y}\|_2$;
namely, $\DS p_j(\VEC{y}) = b_j (i\|\VEC{y}\|_2)^j$ for some
constant $b_j$.  However, since $p$ is a polynomial, $b_j=0$ for $j$ odd.
...
$\DS \pdydx{r}{x_j} = \frac{x_j}{r}$ for $1 \leq j \leq n$, we get
\begin{align*}
...
$\Delta f = 0$ on $\DS \RR^n \setminus \{\VEC{0}\}$.  Then
\[
\phi(r) =
\begin{cases}
...
$\DS \phi''(r) + \frac{n-1}{r} \phi'(r) = 0$.
With $w(r) = \phi'(r)$, this last equation becomes the separable first
order ordinary differential equation
$\DS w'(r) + \frac{n-1}{r} w(r) = 0$
whose solution is $\DS w(r) = C r^{1-n}$ if $n\neq 1$, where $C$
is an arbitrary constant.

For $n\neq 2$, we get $\DS \phi(r) = b r^{2-n} + a$,
...
\]
if $\DS f \in C^2(\RR)$ and $\DS g \in C^1(\RR)$.
...
$\DS h:\RR^n\rightarrow \RR$ is defined by the surface integral
\begin{equation} \label{wave_nD_idx1}
M_h(\VEC{x},r) = \frac{1}{\omega_n} \int_{\|\VEC{y}\|=1} h(\VEC{x} + r \VEC{y}) 
\dss{S}{y} \ ,
\end{equation}
where $\omega_n$ is the area of a sphere of radius $1$ in $\DS \RR^n$.
...
$\DS \VEC{x} \in \RR^n$.
\item $\DS M_h \in C^k(\RR^n\times \RR)$ if
$\DS h \in C^k(\RR^n)$.
\item $M_h(\VEC{x},0) = h(\VEC{x})$ for $\DS \VEC{x} \in \RR^n$ if
$\DS h \in C(\RR^n)$.
\end{enumerate}
...
In $\DS \RR^2$, the surface integrals above are computed using polar
coordinates.  For (\ref{wave_nD_idx1}), we use
$\DS \VEC{y} = \left( \cos(\theta), \sin(\theta) \right)$
with $0\leq \theta < 2\pi$.  Thus
$\DS \dss{S}{y} = \dx{\theta}$.  For
(\ref{wave_nD_idx1_r}), we use 
$\DS \VEC{z} = \VEC{x} + r\left(\cos(\theta),\sin(\theta) \right)$
with $0\leq \theta < 2\pi$.  Thus
$\DS \dx{S} = r \dx{\theta}$.

In $\DS \RR^3$, the surface integrals above are computed using
spherical coordinates.  For (\ref{wave_nD_idx1}), we use
$\DS \VEC{y} = \left( \cos(\theta)\sin(\phi) ,
\sin(\theta)\sin(\phi) , \cos(\phi) \right)$
with $0\leq \theta < 2\pi$ and $0\leq \phi < \pi$.  Thus
$\DS \dss{S}{y} = \sin(\phi) \dx{\phi}\dx{\theta}$.  For
(\ref{wave_nD_idx1_r}), we use
$\DS \VEC{z} = \VEC{x} + r\left(\cos(\theta)\sin(\phi) ,
\sin(\theta)\sin(\phi) , \cos(\phi) \right)$
with $0\leq \theta < 2\pi$ and $0\leq \phi < \pi$.  Thus
$\DS \dx{S} = r^2 \sin(\phi) \dx{\phi}\dx{\theta}$.

Integrating $\DS e^{-\pi \|\VEC{x}\|_2^2}$ over
$\DS \RR^n$ using spherical coordinates \footnote{We call
spherical coordinates in $\DS \RR^n$ any coordinates of the
form $\VEC{x} = r \VEC{y}$ with $\|\VEC{y}\|=1$.}, the reader can verify that
$\DS \omega_n = 2 \pi^{n/2}/\Gamma(n/2)$ for
...
integration.
For any $\DS \VEC{y} \in \RR^n$ such that
$\|\VEC{y}\| = r> 0$, we have
\[
M_h(\VEC{x},r) =
...
transformations on $\DS \RR^n$.

Hence, if $\DS h \in C^2(\RR^n)$,
\begin{align}
...
Suppose that $\DS f \in C^2(\RR^n)$ satisfies $f(\VEC{x}) = g(r)$ with
$r=\|\VEC{x}\|$ for all $\DS \VEC{x} \in \RR^n$.
...
then we get from (\ref{HaarDeltaInv}) that
\[
...
\end{align*}
because
$\DS \diV_{\VEC{y}} \left( \graD h(\VEC{x} + r \VEC{y}) \right)
...
\]
for $\DS (\VEC{x},t) \in \RR^n \times \RR$.  This is the
spherical mean of the function $h(\VEC{x}) = u(\VEC{x},t)$ for
$t\in \RR$ fixed.  Then 
$u$ satisfies $\DS L(\VEC{x},t,\diff)u=0$ on
$\DS \RR^n$ if and only if
...
M_u((\VEC{x},t),r) = 0
\end{equation}
for $\DS (\VEC{x},t) \in \RR^n \times \RR$ and $r>0$.
\end{cor}
...
\end{align*}
and the fact that, for a continuous function
$\DS h:\RR^n\rightarrow \RR$,
$\DS M_h\left(\VEC{x}, r\right) = 0$ for all
$\DS \VEC{x}\in\RR^n$ and $r \in \RR$ if and only if
$h(\VEC{x}) = 0$ for all $\DS \VEC{x}\in\RR^n$.
\end{proof}
...
$\DS \pdydx{M_u}{t}((\VEC{x},0), r) = M_g(\VEC{x},r)$.
Then
$\DS u(\VEC{x},t) = \lim_{r\rightarrow 0} M_u( (\VEC{x},t), r)$.
...
\begin{lemma}
If $\DS h \in C^{k+2}(\RR)$ with $k>0$, then
...
Given $\DS h \in C^{k+2}(\RR)$ with $k>0$, let
...
\]
for $r > 0$ and $n>1$ odd.
In other words, $\DS \pdydxn{}{r}{2}$ is ``conjugate'' to
$\DS \left(\pdydxn{}{r}{2}
...
$\DS c_0 = 1 \cdot 3 \cdot 5 \cdot \ldots \cdot (2k-1)$.
\end{lemma}
...
$\DS c_0 r = \left(\frac{1}{r}\pdydx{}{r}\right)^{k-1}
...
(\ref{wave_nD_cond}), where $\DS f\in C^{(n+3)/2}(\RR^n)$ and
$\DS g\in C^{(n+1)/2}(\RR^n)$, is given by
...
\end{equation}
for $\DS (\VEC{x},s) \in \RR^n \times \RR$,
...
(\ref{wave_nD_cond_pr}).  For $\DS h\in C^{(n+1)/2}(\RR^n)$, let
\[
...
$\DS \pdydx{v_f}{s}$ is a solution (\ref{wave_nD_pr})
because $v_f$ is a solution and $\DS \pdydx{}{s}$ commutes
with $\DS \pdydxn{}{s}{2} - \Delta_{\VEC{x}}$.

Thus
\begin{equation} \label{wave_sol_nD_pr}
u(\VEC{x},s) = \frac{1}{1\cdot 3 \cdot 5 \cdot \ldots \cdot (n-2)}
\left( \DS \pdydx{v_f}{s} + v_{\tilde{g}} \right)(\VEC{x},s)
\end{equation}
...
\]
because $\DS \pdydx{M_f}{s}(\VEC{x},0)$ is even with respect
to $s$, and so $\DS \pdydx{M_f}{s}(\VEC{x},0) = 0$.
...
M_u((\VEC{x},t),r) = 0
\]
for $\DS (\VEC{x},t) \in \RR^3 \times \RR$ and $r > 0$.
...
equation $\DS \pdydxn{v}{t}{2} - c^2\, \pdydxn{v}{r}{2} = 0$ with
the initial conditions
$\DS v(r,0) = rM_u((\VEC{x},0),r) = rM_f(\VEC{x},r)$
and $\DS
\pdydx{v}{t}(r,0) = rM_{\pdydx{u}{t}}((\VEC{x},0),r) = rM_g(\VEC{x},r)$,
and the boundary condition
$\DS v(0,t) = \lim_{r\rightarrow 0} rM_u((\VEC{x},t),r) = 0$
because $\DS \lim_{r\rightarrow 0} M_u((\VEC{x},t),r) =
...
and $\DS \int_{r-ct}^0 \pdydx{v}{t}(s,0)\dx{s} =
\int_{ct-r}^{0} \pdydx{v}{t}(s,0)\dx{s}$ because
$v(r,0) = rM_f(\VEC{x},r)$ and
$\DS \pdydx{v}{t}(r,0) = rM_g(\VEC{x},r)$ are odd functions
...
\end{align*}
for $\DS \VEC{x} \in \RR^3$ and $t\geq 0$.   However,
\begin{align*}
...
\]
for $\DS \VEC{x}\in \RR^3$ and $t \geq 0$, which is
(\ref{wave_sol_3d}).
\end{rmk}
...
$\DS \RR^{n+1}\times \RR$ which is independent of
$x_{n+1}$, then $u$ is a solution on $\DS \RR^n\times \RR$.
...
$\DS f\in C^{(n+4)/2}(\RR^n)$ and
$\DS g\in C^{(n+2)/2}(\RR^n)$, is given by
\begin{align}
...
\begin{proof}
If we consider $f$ and $g$ as functions in $\DS \RR^{n+1}$ which are
...
\end{split}
\end{equation}
where $\VEC{x}$ and $\VEC{y}$ are in $\DS \RR^{n+1}$.
...
in $\RR^{n+1}$ with $\DS y_{n+1} = \phi_{\pm}(\tilde{\VEC{y}}) =
...
$f$ and $g$ are not null on $\DS \RR^2$, then $u$ will
...
not null on $\DS \RR^3$, then $u$ will generally be non-null at
$(\VEC{x},t)$ such that
...
Suppose that $\DS w\in C^{\intpt{n/2}+1}(\RR^n\times \RR)$.
...
have $u(\VEC{x},0) = 0$ for all $\DS \VEC{x} \in \RR^n$.  Moreover,
\[
...
must satisfy.  Hence $\DS \pdydx{u}{t}(\VEC{x},0) = 0$
...
$x_j$ , $1\leq j \leq n$, because $v$ is at least of class $\DS C^2$.

The condition $\DS w\in C^{\intpt{n/2}+1}(\RR^n\times \RR)$
...
and the initial conditions
$\DS u(x,y,0) = (4x-x^2)(2y-y^2)$ and
$\DS \pdydx{u}{t}(x,y,0) = 5\sin\left(\frac{3\pi x}{4}\right)
...
$\DS E\left(A \cos (ckt) + B \sin (ckt)\right)\left(C \cos (\mu x)
...
From $\DS u(x,y,0) = (4x-x^2)(2y-y^2)$, we get
\[
\sum_{n,m=1}^\infty A_{n,m}\sin\left(\frac{m\pi x}{4}\right)
\sin\left(\frac{n\pi y}{2}\right) = (4x-x^2)(2y-y^2) \ .
\]
This is the Fourier sine series in $\DS L^2(D)$ of
$\DS (4x-x^2)(2y-y^2)$
\footnote{We leave it to the reader to generalize the concept of
classical Fourier series in $\DS L^2(I)$, where $I$ is an interval in
$\RR$, to $\DS L^2(D)$, where $D$ is a rectangle in
$\DS \RR^2$, using Theorem~\ref{fu_an_complset}.}.  Thus
\begin{align*}
...
$\DS u_{n,m}(x,y)$ is the $n,m$-harmonic.  The
first-harmonic is given by $n=m=1$.  The frequency of the
first-harmonic is $3\sqrt{3}/4$.  However, the frequencies of the
other harmonics are $\DS 3\sqrt{m^2/4 + n^2/2}\,/2$ with $n+m >2$ and
...
The mappings above are not bijections from $\DS \RR^2$ (or
$\DS \RR^3$) to itself.  They are periodic of period $2\pi$
in $\theta$, $(0,\theta)$ is mapped to $(0,0)$ whatever the value of $\theta$,
$(r,\theta)$ and $(-r, \theta+\pi)$ are mapped to the same point
of $\DS \RR^2$, and so on.
...
from $\DS \RR^3$ to itself.  It is periodic of period $2\pi$
...
$\DS x^2 + y^2 = R^2$ and $t>0$, and the initial conditions
$\DS u(x,y, 0) = f\left(\sqrt{x^2+y^2}\right)$ and
$\DS \pdydx{u}{t}(x,y,0) = g\left(\sqrt{x^2+y^2}\right)$ for
$\DS x^2 + y^2 \leq R^2$.

Since the boundary and initial conditions are functions of
the distance $\DS r = \sqrt{x^2+y^2}$ between $(x,y)$ and
the origin, we may use polar coordinates and assume that $u$ is a
function of $r$ and $\theta$.  As is traditional, we keep the same
notation for $u$ as a function of $(x,y)$ and $u$ as a function of
$(r,\theta)$.
...
initial conditions are $\DS u(r,\theta, 0) = f(r)$ and
$\DS \pdydx{u}{t}(r,\theta,0) = g(r)$ for
...
If we substitute $u(r,t) = F(r)G(t)$ in (\ref{wave_spv_we_pc}), then we get
\[
F(r)\dydxn{G}{t}{2}(t) = c^2 \left( \dydxn{F}{r}{2}(r)G(t) +
  \frac{1}{r} \dydx{F}{r}(r)G(t) \right) \ .
\]
If we divide both side by $\DS c^2F(r)G(t)$, then we get
\[
...
must have $q\leq 0$.  Let $\DS q= -k^2$.

We consider (\ref{wave_spv_we_pcODE2}) with $\DS q= -k^2$
and the condition $F(R)=0$.  Let $s = kr$.  Since
$\DS \dydx{F}{r} = \dydx{F}{s} \dydx{s}{r} = k\dydx{F}{s}$
and
$\DS \dydxn{F}{r}{2} = \dfdx{\left(\dydx{F}{r}\right)}{r}
...
equation by $\DS s^2/k^2$, then we get
\[
...
The bounded solutions of (\ref{wave_spv_we_pcODE2}) with
$\DS q= -k^2$ are
...
$k = \DS k_n \equiv \alpha_{n,0}/R$ for $n>0$,
...
$\DS q= -k^2$ and satisfying $F(R)=0$ exist only for
$k=k_n = \alpha_{n,o}/R$ and are multiples of
$\DS F_n(r) \equiv J_0\left(\alpha_{n,0}r/R\right)$
for $n>0$.

For $n>0$ fix, the general solution of (\ref{wave_spv_we_pcODE1})
with $\DS q=-k_n^2 = -(\alpha_{n,0}/R)^2$ is
\[
...
\]
for $n>0$.  From $\DS \pdydx{u}{t}(r,0) = g(r)$, we get
\[
...
{\bfseries $\DS \mathbf{n^{th}}$-normal
mode}\index{Wave Equation!$n^{th}$-Normal
Mode} or {\bfseries $\DS \mathbf{n^{th}}$-harmonic}\index{Wave
...
Its frequency is $\DS c\alpha_{n,0}/(2\pi R)$.
...
$\DS r_{n,i} = \alpha_{i,0}R/\alpha_{n,0}$
for $0<i<n$.  The modal lines are not evenly distributed.

If we imagine that the domain of $u$ defined by
$\DS x^2 + y^2 \leq R^2$ is the membrane of a circular drum,
then the modal lines $\DS x^2 + y^2 \leq r_{n,i}^2$ are
regions of the membrane that not
vibrate under the frequency $\DS c\alpha_{n,0}/(2 \pi R)$.
