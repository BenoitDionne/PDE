\chapter{Wave Equation in $\displaystyle \mathbf{\RR^n}$} \label{ChapWaveEqunD}
...
$\displaystyle -c^2 <0$.  The other eigenvalue is $1>0$.
\begin{defn}
An hyper-surface $\displaystyle S \subset \RR^n \times \RR$ is called
{\bfseries space-like}\index{Wave Equation!Space-Like} if its normal vector
\[
\nu(\VEC{x},t) = (v(\VEC{x},t),w(\VEC{x},t) ) \ ,
\]
where $\displaystyle v:S \to \RR^n$ and $w:S \to \RR$,
...
$\displaystyle u \in C^2(R)$ satisfies\\
...
\]
where we have use the inequality $\displaystyle 2ab \leq a^2 + b^2$ for the last
...
\begin{rmk}
Suppose that $\displaystyle F:\RR^n\times \RR\rightarrow \RR$ is a
function of class $\displaystyle C^1$ and
\[
...
\begin{cor}
If $u$ and $v$ in $\displaystyle C^2(\RR^n\times \RR)$ are two solutions of
...
$\displaystyle \diff^{\VEC{\alpha}} u(\VEC{x}) = \VEC{\alpha}!$.  With
...
We have for all $\displaystyle u\in C^k(\RR^n)$ that
\[
...
namely, $\displaystyle p_j(\VEC{y}) = b_j (i\|\VEC{y}\|_2)^j$ for some
...
whose solution is $\displaystyle w(r) = C r^{1-n}$ if $n\neq 1$, where $C$
is an arbitrary constant.

For $n\neq 2$, we get $\displaystyle \phi(r) = b r^{2-n} + a$,
...
$\displaystyle \VEC{x} \in \RR^n$.
\item $\displaystyle M_h \in C^k(\RR^n\times \RR)$ if
$\displaystyle h \in C^k(\RR^n)$.
\item $M_h(\VEC{x},0) = h(\VEC{x})$ for $\displaystyle \VEC{x} \in \RR^n$ if
$\displaystyle h \in C(\RR^n)$.
\end{enumerate}
...
$\displaystyle h:\RR^n\rightarrow \RR$,
$\displaystyle M_h\left(\VEC{x}, r\right) = 0$ for all
$\displaystyle \VEC{x}\in\RR^n$ and $r \in \RR$ if and only if
...
(\ref{wave_nD_cond_pr}).  For $\displaystyle h\in C^{(n+1)/2}(\RR^n)$, let
\[
...
\end{align*}
for $\displaystyle \VEC{x} \in \RR^3$ and $t\geq 0$.   However,
\begin{align*}
...
\begin{proof}
If we consider $f$ and $g$ as functions in $\displaystyle \RR^{n+1}$ which are
...
\end{split}
\end{equation}
where $\VEC{x}$ and $\VEC{y}$ are in $\displaystyle \RR^{n+1}$.
...
in $\RR^{n+1}$ with $\displaystyle y_{n+1} = \phi_{\pm}(\tilde{\VEC{y}}) =
...
and the initial conditions
$\displaystyle u(x,y,0) = (4x-x^2)(2y-y^2)$ and
...
From $\displaystyle u(x,y,0) = (4x-x^2)(2y-y^2)$, we get
\[
\sum_{n,m=1}^\infty A_{n,m}\sin\left(\frac{m\pi x}{4}\right)
\sin\left(\frac{n\pi y}{2}\right) = (4x-x^2)(2y-y^2) \ .
\]
This is the Fourier sine series in $\displaystyle L^2(D)$ of
$\displaystyle (4x-x^2)(2y-y^2)$
\footnote{We leave it to the reader to generalize the concept of
classical Fourier series in $\displaystyle L^2(I)$, where $I$ is an interval in
$\RR$, to $\displaystyle L^2(D)$, where $D$ is a rectangle in
$\displaystyle \RR^2$, using Theorem~\ref{fu_an_complset}.}.  Thus
\begin{align*}
...
other harmonics are $\displaystyle 3\sqrt{m^2/4 + n^2/2}\,/2$ with $n+m >2$ and
...
the distance $\displaystyle r = \sqrt{x^2+y^2}$ between $(x,y)$ and
...
\]
If we divide both side by $\displaystyle c^2F(r)G(t)$, then we get
\[
...
must have $q\leq 0$.  Let $\displaystyle q= -k^2$.

We consider (\ref{wave_spv_we_pcODE2}) with $\displaystyle q= -k^2$
...
equation by $\displaystyle s^2/k^2$, then we get
\[
...
$\displaystyle q= -k^2$ and satisfying $F(R)=0$ exist only for
...
{\bfseries $\displaystyle \mathbf{n^{th}}$-normal
mode}\index{Wave Equation!$n^{th}$-Normal
Mode} or {\bfseries $\displaystyle \mathbf{n^{th}}$-harmonic}\index{Wave
...
$\displaystyle x^2 + y^2 \leq R^2$ is the membrane of a circular drum,
then the modal lines $\displaystyle x^2 + y^2 \leq r_{n,i}^2$ are
