...
is of class $\displaystyle C^1$ almost everywhere.
...
\]
where $\displaystyle v = u^2/2$.  This is simply the Burgers' equation
...
\]
where $\displaystyle v^+ = (u^+)/2$ and
$\displaystyle v^- = (u^-)/2$.  But this is clearly not
...
of $\Gamma$, then we get $\displaystyle \gamma'(y) = 1/2$ and the solution
...
is constant are given by $\displaystyle x= G'(u^-) y + s$ for
$s\neq 0$ where $u = u^-$,
$\displaystyle x= G'(u^+) y + s$ for $s\neq 0$ where $\displaystyle u = u^+$,
...
$\displaystyle t u_2^{[j]}(x,y) + (1-t) u_1^{[j]}(x,y) \in D$
...
Since $\displaystyle x^{[j]}(y,x,y) = x$ for all
...
functions $\displaystyle f \in C^1(\RR)$.
...
$\displaystyle \left\{ \left(x^{[j]}(s,x,y),s\right) : 0 < s \leq 1/N \right\}$
of the orbit $\displaystyle \Gamma_{(x,y)}^{[j]}$ of
...
$P$, then we have that $\displaystyle \int_0^P u(x,y) \dx{x}$ is constant
...
$x$, then we get
$\displaystyle \pdydxnm{u_2}{x}{y}{2}{}{} = \pdydxn{u_1}{y}{2}$
and
...
\end{equation}
where $\displaystyle u:\RR\times [0,\infty[ \to \RR^n$ and
$\displaystyle G:\RR^n \to \RR^n$
...
namely, $\displaystyle \diff G(\VEC{u}) :\RR^n \to \RR^n$ has $n$
distinct eigenvalues
$\lambda_1(\VEC{u}) < \lambda_2(\VEC{u}) < \ldots < \lambda_n(\VEC{u})$
for all $\displaystyle \VEC{u} \in \RR^n$.
...
where $\displaystyle p'(v) = -k\beta v^{-\beta-1} <0$ and
$\displaystyle p''(v) = k\beta(\beta+1) v^{-\beta-2} >0$ for all $v \in \RR$
...
\]
for $\displaystyle u_1 < u^-_1$.  We have
\[
...
= \sqrt{-p'(w_1)} \ .
\]
If we integrate from $\displaystyle u_1^-$ to $u_1$, then we get
...
$\lambda_1(\VEC{u}) < \lambda_2(\VEC{u}) < \ldots < \lambda_n(\VEC{u})$
are the eigenvalues of $\diff G(\VEC{u})$ for $\displaystyle \VEC{u} \in \RR^n$,
and $\displaystyle q_i(\VEC{u}) \in \RR^n$ is a (right) eigenvector of
$\diff G(\VEC{u})$
...
$\displaystyle p_i(\VEC{u}) \in \RR^n$
...
Laws!$i$-Riemann Invariant} on an open set $\displaystyle U \subset \RR^n$ is
...
conservation laws in $\displaystyle \RR^2$.  Moreover, suppose that
$\displaystyle U \subset \RR^2$ is an open and connected set.
...
$\displaystyle \VEC{u} \in \RR^2$, the integral curves associated to
...
\end{align*}
defines an isomorphism and $\displaystyle \Phi(U) \subset \RR^2$ is open and
connected.
...
constant on $\displaystyle \{ w(z) : z \in u(W) = [z^-,z^+]\}$ where
...
a basis of $\displaystyle \RR^n$ for all $y\in I$.  This is possible only if
...
$\displaystyle U \subset \RR^n$.  For each $1 \leq i \leq n$ and
...
\]
Suppose that $\displaystyle U \subset \RR^n$ is a {\bfseries sufficiently small}
...
$\displaystyle \VEC{u}_{j,k} \in \RR^n$ such that
...
that we have $\displaystyle \VEC{u}_{j,k} \in \RR^n$
...
$\displaystyle L^1_{loc}(\RR\times [0,\infty[)$ to a solution of
...
\begin{defn}
Suppose that $\displaystyle G:\RR^n \to \RR^n$ in
(\ref{systConLawEq1}) is of class $\displaystyle C^1$, and that
$E,F:U\to \RR$ are two continuously differentiable
functions defined on an open set $\displaystyle U \subset \RR^n$.  If
...
If $G$ is the gradient of a function $\displaystyle g:\RR^n \to \RR$,
...
\end{equation}
We claim that $\displaystyle E(\VEC{u}) = \|\VEC{u}\|^2/2$ and
$\displaystyle F(\VEC{u}) = \VEC{u}^\top (\diff g)(\VEC{u}) - g(\VEC{u})$
define an entropy pair for the (\ref{eggGradSyst}) on
$\displaystyle \RR^n$.  This follows
from
\[
...
A strong solution
$\displaystyle u:\RR\times ]0,\infty[\to \RR^n$ of (\ref{systConLawEq1})
...
\begin{lemma}
Suppose that $\displaystyle u:\RR\times [0,\infty[ \to \RR^n$ is a
strong solution of
...
\footnotetext{As usual, we assume that $u$ is of class $\displaystyle C^1$
...
$V_l$ because $u$ is of class $\displaystyle C^1$ on the left side of
$\Gamma$, we get that
...
$\displaystyle \diff^2 f(\VEC{u})$ (the Hessian matrix of $f$) is definite
...
$(E,F)$ is an entropy pair on $\displaystyle \RR^n$ with $E$ strictly convex on
$\displaystyle \RR^n$.  In particular, we assume that $G$, $E$ and $F$ are
...
$\displaystyle \VEC{u}^- = u^-(\VEC{p})$ is connected to
$\displaystyle \VEC{u}^+ = u^+(\VEC{p})$ 
...
required to conclude that it is true if $\displaystyle \VEC{u}^+$ is
close enough to $\displaystyle \VEC{u}^-$.  This is certainly what is
...
$\displaystyle (\diff E)(u^{[\epsilon]})$, then we get
\begin{align*}
...
$\displaystyle C^2(\RR^n)$.  Moreover, suppose that $E$ is such that
$\displaystyle \int_{\RR} \int_{[0,\infty[}
\left( (\diff E)(u^{[\epsilon]}) \,
A \pdydx{u^{[\epsilon]}}{x} \right) \pdydx{\psi}{x} \dx{y}\dx{x}$ is
bounded independently of $\epsilon$.
If $\displaystyle u^{[\epsilon]} \to u$ in the sense of distributions as
...
\]
for all $u$ between $\displaystyle u^-(\VEC{p})$ and
$\displaystyle u^+(\VEC{p})$, where
