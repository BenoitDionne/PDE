...
\end{equation}
where $\DS u:\RR\times [0,\infty[ \to \RR^n$
\footnote{Systems of conservation laws with
$\DS u:\RR^k\times[0,\infty[ \to \RR^n$ and $k>1$ could also
...
Since $\DS \dydx{x}{y} = g(u(x,y))$ and $y$ represents the time,
$g$ is called the
...
$\DS \Sigma_{s_i} = \left\{ (g(h(s_i))\,y + s_i,y) : y \geq 0
...
$\DS \Gamma = \left\{ (\gamma(y),y) : y \geq 0 \right\}$ of
class $\DS C^1$
...
$\DS C^\infty$ or analytic.  The nonlinearity of the partial
...
$\DS u \in C^1(\RR \times ]0,\infty[) \cap C(\RR \times [0,\infty[)$.
Since
\[
...
\dx{x}\dx{y} \\
&\quad = \int_B \left( u \pdydx{\psi}{y} + G(u)\,\pdydx{\psi}{x}
\right) \dx{x}\dx{y}
...
is of class $\DS C^1$ almost everywhere.
...
Since $\DS \pdydx{u}{y} + \pdfdx{G(u)}{x} = 0$ in
...
and that $\DS u \in C^1(\RR \times ]0,\infty[)
...
$\DS \pdydx{u}{y} + \pdfdx{G(u)}{x}$ is
continuous in $B$, we get that
$\DS \pdydx{u}{y} + \pdfdx{G(u)}{x} = 0$ in
...
$\DS \pdydx{u}{y} + \pdfdx{G(u)}{x} = 0$ in $]-a,a[\times ]0,b[$.
Since $\DS \DD{B}\big|_{\{(x,0):-a < x <a\}} \equiv \DD(]-a,a[)$
...
$\DS \Gamma = \left\{ (\gamma(y),y) : y \geq y_0 \right\}$ of
class $\DS C^1$ for some $y_0 \geq 0$.
...
assume that the function $u$ is of class $\DS C^1$, can be
...
domain of integration $V_l$ where $\DS u \in C^1(V_1)$, the
...
$\DS \dydx{\gamma}{y}(y)$ is the speed of the shock wave.
...
$\DS g(u) = u$.  We may expect a shock wave since $g\circ h$
...
$u=1-s$.  Thus $\DS u=\frac{x-1}{y-1}$ along the lines
$x = (1-s)y +s$.
...
defined by $\DS G(u) = \frac{u^2}{2}$ as the primitive of $g$.

The curve $\Gamma$ is defined only for $y\geq y_0 = 1$.  Whatever the choice
of $\Gamma$, we have that $\DS u^+(\VEC{p}) = 1$ and
$\DS u^-(\VEC{p}) = 0$ for all
$\VEC{p} \in \Gamma$.  Hence,
$\DS \dydx{\gamma}{y}(y) = \frac{1}{2}$ for all $y>1$.
Thus $\DS \gamma(y) = \frac{1}{2}\, y + \frac{1}{2}$
...
$\DS \pdydx{u}{y} + \pdfdx{\left(\frac{u^2}{2}\right)}{x} = 0$
...
\]
where $\DS v = u^2/2$.  This is simply the Burgers' equation
multiplied by $u$.  Then, the solution $v$ must now satisfy the
Rankine-Hugoniot formula
\[
\dydx{\gamma}{y}(y)
...
\]
where $\DS v^+ = (u^+)/2$ and
$\DS v^- = (u^-)/2$.  But this is clearly not
...
$\DS u(x,0) = h(x) = u^-$ for $x<0$ and
$\DS u(x,0) = h(x) = u^+$ for $x>0$.
...
We assume that $\DS \dydxn{G}{u}{2}(u) > 0$ for all $u\in
...
$\DS w \in C^1(\RR)$.  If we set $z = x/y$ and
substitute $u(x,y) = w(z)$ in (\ref{shock_burgers_g}), then we get
\begin{align*}
...
subset $V$ of $\RR$, then
$\DS \dydx{G}{w}(w(z)) = z$ for all $z\in V$.  In fact, the
converse is also true.  If we derive $\DS \dydx{G}{w}(w(z)) = z$
with respect to $z$, then we get
$\DS \dydxn{G}{w}{2}(w(z))\dydx{w}{z}(z) = 1$ for $z\in V$.  Since
$\DS \dydxn{G}{w}{2}(w) > 0$, we get that 
$\DS \dydx{w}{z}(z) \neq 0$ for $z \in V$.

Since $\DS \dydxn{G}{w}{2}(w) > 0$, we have that
$\DS \dydx{G}{w}(w) > 0$ is an increasing function.
In particular, $\DS \dydx{G}{w}$ is invertible.  The
unique solution for $w$ in $V$ is therefore given by solving
$\DS \dydx{G}{w}(w(z)) = z$ for $z \in V$.
Therefore, $\DS u(x,y) = w(z) = \left(\dydx{G}{w}\right)^{-1}(z)$
for $z \in V$ and $z = x/y$.

\begin{defn} \label{ConnectRaref}
We say that $\DS u^+$ is {\bfseries connected} to
$\DS u^-$ by a rarefaction
wave if $\DS \dydx{G}{u}(u^+) > \dydx{G}{u}(u^-)$, and
$\DS \dydx{G}{w}(w(z)) = z$ for
$\DS \dydx{G}{u}(u^+) > z > \dydx{G}{u}(u^-)$.
\end{defn}
...
We assume that $\DS u(x,0) = h(x) = u^-$ for $x<0$ and
$\DS u(x,0) = h(x) = u^+$ for $x>0$, and
$\DS G'(u^-)>0$.  The lines where $u$
is constant are given by $\DS x= G'(u^-) y + s$ for
$s\neq 0$ where $u = u^-$,
$\DS x= G'(u^+) y + s$ for $s\neq 0$ where $\DS u = u^+$,
and $x = G'(\tilde{u}) y$ where $u = \tilde{u}$ is between
$\DS u^+$ and $\DS u^-$.}{shock_fig13}
...
$\DS C^1$ for some $y_0 \geq 0$.  The solution $u$
...
$\DS G'(u^-(\VEC{p})) > G'(u^+(\VEC{p}))$ for all $\VEC{p} \in \Gamma$
because $\DS u^-(\VEC{p}) > u^+(\VEC{p})$.
...
shock wave is not time reversible, we cannot determine the state
...
Suppose that $\DS h \in L^\infty(\RR)$ and
$\DS G \in C^2(\RR)$.
Let $\DS H = \|h\|_\infty$,  $D = \{ u \in \RR : |u| \leq H\}$, and
$\DS M = \max_{u \in D} |G'(u)|$.  Assume that
$\DS m = \min_{u \in D} G''(u) > 0$.  Then, there exists a
...
satisfying (1) and (2), and $\DS u_{i,y_1} \to u_{i,y_2}$ in
$\DS L^1(K)$ as $y_1 \to y_2$ for all compact sets
...
from (\ref{shock_uniqueEq1}) with $i=2$, then we get
...
{\cal G}(p,q) =
\begin{cases}
\DS \frac{G(q)-G(p)}{q-p} & \quad \text{if} \ p \neq q \\
G'(p) & \quad \text{if} \ p = q
\end{cases}
\]
We have that $\DS {\cal G} \in C^1(\RR^2)$ with
$\DS \diff {\cal G}(p,p) =
...
$\DS u_i^{[j]}\in C(\RR\times [0,\infty[)$ for
...
$\DS {\cal G}^{[j]}(x,y) \equiv
{\cal G}(u^{[j]}_1(x,y),u^{[j]}_2(x,y))$
for $(x,y) \in \RR \times [0,\infty[$.

Given $\phi \in \DD(\RR\times ]0,\infty[)$ and $\DS \psi^{[j]}$
satisfying (\ref{shock_uniqueEq3}), we have
\begin{align}
&\int_{[0,\infty[} \int_{\RR}
...
\stage{i}  Choose $\DS \eta \in \DD(\RR)$ such that
$\eta(w) = 0$ for $|w|>1$, $\eta(w) \geq 0$ for all $w\in \RR$, and
$\DS \int_{\RR} \eta(w)\dx{w} = 1$.
Let $\DS \eta_j(w) = j \eta(j w)$ for
$w \in \RR$ and $j \in \NN$.  The functions $\DS u_i^{[j]}$
are defined by $\DS u_i^{[j]}(x,y) \equiv (u_{i,y} \ast \eta_j)(x)$
for all $(x,y) \in \RR\times [0,\infty[$, $j\in \NN$ and $i \in \{1,2\}$.

We have from Proposition~\ref{distr_smooth_convol} that
$\DS u_{i,y}^{[j]} \in C^\infty(\RR)$ for all $y \geq 0$,
$j \in \NN$ and $i\in \{1,2\}$.  In particular, this implies that
$\DS u_{i}^{[j]}$ is continuous with respect to its first
variable for all $j \in \NN$ and $i\in \{1,2\}$.

We now show that, for any $y_1,y_2 \in [0,\infty[$,
we have that $\DS u_{i,y_1}^{[j]} \to u_{i,y_2}^{[j]}$
...
\frac{\epsilon}{\DS \max_{x\in\RR} \eta(x)} 
\]
...
$\DS u_i^{[j]}$ is continuous with respect to its second
variable for all $j \in \NN$ and $i\in \{1,2\}$.  But this extra
uniform convergence on compact sets also ensures that
$\DS u_i^{[j]} \in C(\RR \times [0,\infty[)$.

\stage{ii} We prove that
$\DS \left\{ u_i^{[j]} \right\}_{j=0}^\infty$
converges to $u_i$ in $\DS L^1(K)$ for all compact sets
$\DS K \subset \RR\times [0,\infty[$ and
$i\in \{1,2\}$.

We have from Proposition~\ref{distr_limit_convol} that
$\DS \{ u_{i,y}^{[j]} \}_{j=0}^\infty$ converges uniformly
on compact sets to $u_{i,y}$ for all $y \geq 0$ and $i \in \{1,2\}$
\footnote{To be precise, we have to slightly adapt the case for
$\DS L^\infty(\RR^n)$ in Proposition~\ref{distr_limit_convol}.
We do not need $\DS f \in L^\infty(\RR^n)$ as long as
...
(1) that
$\DS \|u_{i,y}^{[j]} \|_\infty \leq \|u_{i,y}\|_\infty \, \|\eta\|_1
...
\end{align*}
because
$\DS \left| u_{i,y}^{[j]}(x) - u_{i,y}(x) \right| \to 0$
for all $(x,y) \in K \subset \RR\times [0,\infty[$ and
$\DS \left| u_{i,y}^{[j]}(x) - u_{i,y}(x) \right| \leq 
g(x,y) \equiv 2 H$ for all $(x,y) \in K$ with
$\DS g \in L^1(K)$ since $K$ is bounded.
...
\end{equation}
where $\DS x^{[j]}$ is the solution of the ordinary
...
$\DS u_i^{[j]} \in C(\RR \times [0,\infty[)$
and $\DS u_{i,y}^{[j]} \in C^1(\RR)$ for all $y \geq 0$
imply that $\DS {\cal G}^{[j]} \in C(\RR \times [0,\infty[)$
and $\DS {\cal G}^{[j]}_y \in C^1(\RR)$ for all $y \geq 0$
and $j \in \NN$.  Moreover, this
implies that $\DS (s,x,y) \mapsto x^{[j]}(s,x,y)$
is differentiable on $\DS ]0,\infty[\times \RR \times ]0,\infty[$
(Theorems~3.1 and 3.3 in Chapter 1 of \cite{Ha}).

To prove that $\DS \psi^{[j]}$ is a solution of
...
$\DS \supp \psi^{[j]} \subset R$ for all $j \in \NN$.

Since $\DS \|u_{i,y}^{[j]} \|_\infty \leq H$ for all
...
y/M + a < x < -y/M + b \right\} \ .
\]
To prove that $\DS \supp \psi^{[j]} \subset R$, we prove
that $\DS \psi^{[j]}(x,y) = 0$ for all $(x,y) \not\in R$.
If $y \geq Y$, then we have from (\ref{shock_uniqueEq5}) that
$\DS \psi^{[j]}(x,y) = 0$ for all $x \in \RR$
...
\end{align}
for $s \geq 0$.  Thus $\DS \psi^{[j]}$ is constant along the
orbits $\DS \Gamma_{(x,y)}^{[j]}$ with $(x,y) \not\in R$.  Since\\
$\DS \psi^{[j]}\left(x^{[j]}(Y,x,y),Y\right) = 0$,
we get that $\DS \psi^{[j]} = 0$ along the orbits
$\DS \Gamma_{(x,y)}^{[j]}$ with $(x,y) \not\in R$.  Thus,
$\DS \psi^{[j]}\left(x^{[j]}(y,x,y),y\right) =
\psi^{[j]}\left(x,y\right) = 0$ for $(x,y) \not\in R$.

\stage{v}  We prove that $\DS \{ {\cal G}^{[j]} \}_{j=0}^\infty$
converges to ${\cal G}$ in $\DS L^1(K)$ for all compact
...
$\DS t u_2^{[j]}(x,y) + (1-t) u_1^{[j]}(x,y) \in D$
for all $(x,y) \in \RR \times[0,\infty[$ and $t \in [0,1]$, and
$\DS m = \min_{u \in D} G''(u) > 0$ by hypothesis.

Let $\DS w_j(s,x,y) = \pdydx{x^{[j]}}{x}(s,x,y)$ for
$s\in ]0,\infty[$,
$(x,y) \in \RR \times ]0,\infty[$ and $j \in \NN$.\\
Since $\DS x^{[j]}(y,x,y) = x$ for all
$(x,y) \in \RR \times ]0,\infty[$ and $j \in \NN$, we get that
$\DS w_j(y,x,y) = 1$ for all $(x,y) \in \RR \times ]0,\infty[$
...
$\DS |w_j(s,x,y)| \leq e^{C\tilde{M}|s-y|/\alpha}
...
\]
Thus, for $\alpha \leq y \leq Y$, we have
...
$\DS P = \max_{(x,y)\in \RR\times [0,\infty[}
\left|\pdydx{\phi}{x}(x,y) \right|$.  Note that
(\ref{shock_uniqueEq10}) is also true for $y>Y$ because
$\DS \psi^{[j]}(x,y) = 0$ for $(x,y) \in \RR\times [Y,\infty[$
and $j \in \NN$.

\stage{vi.b}
We now consider $\DS \pdydx{\psi^{[j]}}{x}(x,y)$ for
$0 \leq y \leq \alpha$, $x\in \RR$ and $j \in \NN$.
Let $\DS V(f) \equiv \int_\RR |f'(x)|\dx{x}$ for all
functions $\DS f \in C^1(\RR)$.
Since $\DS \supp \psi^{[j]} \subset R$ for all $j \in \NN$,
we have that $\DS V(\psi^{[j]}_y) < \infty$
for all $y \geq 0$ and $ j \in \NN$.

We show that there exist a positive integer $N$ and a constant $Q$
such that $\DS V(\psi^{[j]}_y) \leq Q$ for all
...
$\DS \psi^{[j]}$ is constant along the section
$\DS \left\{ \left(x^{[j]}(s,x,y),s\right) : 0 < s \leq 1/N \right\}$
of the orbit $\DS \Gamma_{(x,y)}^{[j]}$ of
(\ref{shock_uniqueEq6}) for all
...
We now show that $\DS V(\psi^{[j]}_y) = V(\psi^{[j]}_{1/N})$ for all
$0 < y < 1/N$.   Given any sequence
$x_1 < x_2 < \ldots < x_n$ in $\RR$, we have
\[
...
for some
$\DS w_i \in
]x^{[j]}(s_{1/n,x_i,y},x_i,y),x^{[j]}(s_{1/n,x_{i+1},y},x_{i+1},y)[$
for $1\leq i < N$.

Taking the supremum over all possibles sequences
$x_1 < x_2 < \ldots < x_n$ in $\RR$ in the two previous series, we
find that $\DS V(\psi^{[j]}_y) = V(\psi^{[j]}_{1/N})$ for $0<y<1/N$ by
...
\end{align*}
if we assume that
$\DS \alpha < \min \{1/N, 1/(4 H M V(\psi^{[j]}_{1/N})) \}$.
Thus,
\[
\left| \int_0^\infty \int_{-\infty}^\infty
...
$\DS u \in C^1(\RR\times]0,\infty[)$ and
$\DS g,h \in C^1(\RR)$ and derive both sides of
(\ref{shock_burgers_sol}) with respect to $x$, then we get
\[
\pdydx{u}{x} = h'(x-g(u)\,y) \left( 1 - y\,g'(u) \pdydx{u}{x}\right) \ .
\]
Solving for $\DS \pdydx{u}{x}$, we find that
\[
\pdydx{u}{x} = \frac{h'(x-g(u)\,y)}{1 +y\, g'(u) h'(x-g(u)\,y)} \ .
\]
If $g'(u) \geq m > 0$ for all $u\in \RR$ and $h'(s) > 0$ for all $s
\in \RR$, then $\DS \pdydx{u}{x} \leq \frac{C}{y}$
...
Then
$\DS u^-(\VEC{p}) \equiv \lim_{x\to \gamma(y)} u(x,y) >
...
Suppose that $\DS G \in C^3(\RR)$ and $G''(u) >0$ for all $u\in \RR$.
...
$\DS \sup_{x\in\RR} |u(x,y)| = O(1/\sqrt{y})$ for all $y>0$.
In fact, $\DS \sqrt{y}\, \sup_{x\in\RR} |u(x,y)|$ converges to
a constant as $y \to \infty$
\end{theorem}

If $u$ is a periodic solution of (\ref{shock_burgers_g}) of period
$P$, then we have that $\DS \int_0^P u(x,y) \dx{x}$ is constant
...
mean value of $u$ as
$\DS M_u = \frac{1}{P}\int_0^P u(x,y) \dx{x}$ whatever $y>0$.

\begin{theorem} \label{asymptBehTh2}
Suppose that $\DS G \in C^3(\RR)$ and $G''(u) > k >0$
...
for all $y>0$, then
$\DS |u(x,y) - M_u| \leq 1/(ky)$ for all $y>0$.
...
$\DS p(v) = kv^{-\beta}$ for some constants $k$ and
...
equation.  If we derive $\DS \pdydx{u_2}{x} = \pdydx{u_1}{y}$
with respect to $y$ and
$\DS \pdydx{u_2}{y} + \pdfdx{p(u_1)}{x} = 0$ with respect to
$x$, then we get
$\DS \pdydxnm{u_2}{x}{y}{2}{}{} = \pdydxn{u_1}{y}{2}$
and
$\DS \pdydxnm{u_2}{y}{x}{2}{}{} + \pdfdxn{p(u_1)}{x}{2} = 0$
respectively.  Thus
$\DS \pdydxn{u_1}{y}{2} + \pdfdxn{p(u_1)}{x}{2} = 0$ with
$u_1(x,0) = h_1(x)$ and $\DS \pdydx{u_1}{y}(x,0) = h_2'(x)$ for
...
$\DS \pdydx{u_1}{y} = \pdydx{u_2}{x}$ implies that
$\DS u_2 = \pdydx{w}{y}$ and
$\DS u_1 = \pdydx{w}{x}$ for some function $w:\RR\times [0,\infty[$.
If we substitute these expressions in (\ref{psystEq1}), then we get
$\DS \pdydxn{w}{y}{2} +
...
nonlinear function $\DS -\dydx{p}{v}\left(\pdydx{w}{x}\right)$.
...
\end{equation}
where $\DS u:\RR\times [0,\infty[ \to \RR^n$ and
$\DS G:\RR^n \to \RR^n$
are sufficiently differentiable functions.

If we assume that $u$ is discontinuous along the curve
$\DS \Gamma = \left\{ (\gamma(y),y) : y \geq y_0 \right\}$
of class $\DS C^1$ for some $y_0 \geq 0$, we can proceed on
...
$\DS \lambda_1(u) = \dydx{G}{u}(u)$.  Hence,
...
$\DS \gamma'(y) < \lambda_1(u^-(\VEC{p}))
= \pdydx{G}{u}(u^-(\VEC{p}))$ and
$\DS \pdydx{G}{u}(u^+(\VEC{p})) = \lambda_1(u^+(\VEC{p}))
...
for all $\DS \VEC{v} \in \RR^n$
and $\lambda_i(\VEC{v}) = \lambda_i(v_i)$ for all $i$.
We assume that $\DS v(x,0) = \VEC{v}^-$ for $x<0$ and
$\DS v(x,0) = \VEC{v}^+$ for $x>0$, 

We have that $v$ is possibly discontinuous along the line
$\DS \Gamma = \left\{ (\eta y,y) : y \geq 0 \right\}$
where $\eta$ is given by the Rankine-Hugoniot formula
$\DS \lambda_i(\VEC{v}^+) - \lambda_i(\VEC{v}^-)
...
$\DS \lambda_i(v) = \lambda(\VEC{v}^-) = \lambda(v_i^-)$ on
...
$\DS v_i^+$ on the right of $\Gamma$ when $i\leq k_r$
...
\end{equation}
We have that $v_i$ is completely determined by the initial condition
$\DS v_i^-$ on the left of $\Gamma$ when $i > k_l$
...
$\DS G(\VEC{u}) = \begin{pmatrix} -u_2 \\ p(u_1) \end{pmatrix}$,
where $\DS p'(v) = -k\beta v^{-\beta-1} <0$ and
$\DS p''(v) = k\beta(\beta+1) v^{-\beta-2} >0$ for all $v \in \RR$
...
precisely, we assume that $\DS u(x,0) = \VEC{u}^-$ for $x<0$ and
$\DS u(x,0) = \VEC{u}^+$ for $x>0$.

The eigenvalues of
$\DS \diff G(\VEC{u}) = \begin{pmatrix} 0 & -1 \\
...
\end{equation}
Since $\DS \eta < \lambda_1(\VEC{u}^-) < 0$ for the $1$-shock
...
Similarly, since $\DS \eta > \lambda_2(\VEC{u}^+) > 0$ for the
...
$\DS \VEC{u}^+$ that may be connected to
$\DS \VEC{u}^-$ by a shock wave
\footnote{We could have reversed the statement and ask what are the
possible values of $\DS u^-(\VEC{p})$ that we may connected to
$\DS u^+(\VEC{p})$ by a shock wave.  This would not have changed the
...
\end{equation}
It follows from
$\DS -\eta ( u_1 - u^-_1) = u_2 - u^-_2 = (p(u_1) - p(u^-_1))/\eta$
that $\DS \eta^2 = (p(u_1) - p(u^-_1))/(u^-_1 - u_1)$.
...
$\DS -\sqrt{-p'(u_1)} < -\sqrt{-p'(u^-_1)}$.  Therefore,
$\DS p'(u_1) < p'(u^-_1)$.  Since
$p''(v) > 0$ for all $v \in \RR$, we have that $p'$ is increasing and
we must therefore have $\DS u_1 < u^-_1$.
...
< 0
\]
for $\DS u_1 < u^-_1$ and $u_1$ sufficient close to
$\DS u^-_1$.  A possible
...
=\lim_{u_1 \to u^-_1} \\
&\quad \frac{(u^-_1 -u_1)^{-3}\big(p(u_1) - p(u^-_1) +(u^-_1-u_1) p'(u_1)\big)^2
...
$\DS \big(p(u_1) - p(u^-_1) - (u_1-u^-_1) p'(u_1)\big)
= O(|u^-_1-u_1|^2)$ as
$\DS |u^-_1 - u_1| \to 0$ to compute the limit.
...
$\DS \VEC{u}^+$ that can be connected by a
$1$-shock to $\DS \VEC{u}^-$.  The points $\VEC{u}$ on the
curve $S_{2,\VEC{u}^-}$ are the possible values of 
$\DS \VEC{u}^+$ to that can be connected by a $2$-shock to
$\DS \VEC{u}^-$.}{shock_fig12}
...
to get $\DS \eta^2 = (p(u_1) - p(u^-_1))/(u^-_1 - u_1)$.
However, we now have that $\eta > 0$.  Therefore,
$\DS \eta = \big((p(u_1) - p(u^-_1))/(u^-_1 - u_1)\big)^{1/2}$.
It also follows from (\ref{psystEq3}) that
$\DS \sqrt{-p'(u_1)} < \sqrt{-p'(u^-_1)}$.  Therefore,
$\DS p'(u_1) > p'(u^-_1)$.  Since $p''(v) > 0$ for all
...
&\lim_{u_1 \to u^-_1} \pdydxn{u_2}{u_1}{2}(u_1)
=\lim_{u_1 \to u^-_1} \\
&\quad \frac{(u_1 -u^-_1)^{-3}\big(p(u_1) - p(u^-_1) +(u^-_1-u_1) p'(u_1)\big)^2
- 2 p''(u_1) (p(u_1)-p(u^-_1))/(u_1-u^-_1)}
{4\big(-(p(u_1) - p(u^-_1))/(u_1 - u^-_1)\big)^{3/2}} \\
&\qquad \qquad = \frac{0  - 2 p''(u^-_1) p'(u^-_1)}{4 \big(-p'(u^-_1)\big)^{3/2}}
= \frac{p''(u^-_1)}{2 \sqrt{-p'(u^-_1)}} > 0 \ .
\end{align*}

\stage{iii}  We would like to find out the values
of $\DS \VEC{u}^+$ that may be connected to a given
$\DS \VEC{u}^-$ by a
...
$\DS G(u) = \begin{pmatrix} -u_2 \\ p(u_1) \end{pmatrix}$
for all $\mu >0$, we seek a solution of the form $u(x,y) = w(x/y)$, where
$\DS w:\RR \to \RR^2$ is of class $\DS C^1$.  If we
...
\begin{pmatrix} \DS \dydx{w_1}{z} \\[0.6em]
\DS \dydx{w_2}{z} \end{pmatrix}
= -\sqrt{-p'(w_1)} \begin{pmatrix} \DS \dydx{w_1}{z} \\[0.6em]
\DS \dydx{w_2}{z} \end{pmatrix}
\]
that
$\DS \dydx{w_2}{z} - \sqrt{-p'(w_1)} \, \dydx{w_1}{z} = 0$.
...
= \sqrt{-p'(w_1)} \ .
\]
If we integrate from $\DS u_1^-$ to $u_1$, then we get
...
must have that
$\DS \lambda_1(\VEC{u}) > \lambda_1(\VEC{u}^-)$; namely,
$\DS -\sqrt{-p'(u_1)} > -\sqrt{-p'(u_1^-)}$.  Thus,
$\DS p'(u_1) > p'(u_1^-)$.  Since $p'$ is increasing, we get that
$\DS u_1 > u_1^-$.
Thus, the acceptable values of $\VEC{u}$ are the points on the curve
$R_{1,\VEC{u}^-}$ defined by (\ref{psystEq5}) for
$\DS u_1 > u_1^-$.  We have that
$\DS \dydx{u_2}{u_1} = \sqrt{-p'(u_1)}$ and
$\DS \dydxn{u_2}{u_1}{2} = \frac{-p''(u_1)}{2\sqrt{-p'(u_1)}}$.

A possible sketch of the curve $R_{1,\VEC{u}^-}$ is represented in
Figure~\ref{shock_fig14}.

The rarefaction waves connecting $\DS \VEC{u}^-$ to
...
$\DS \VEC{u}^+$ that can be connected by a back rarefaction wave
to $\DS \VEC{u}^-$.  The points $\VEC{u}$ on the curve
$R_{2,\VEC{u}^-}$ are the possible values of
$\DS \VEC{u}^+$ that can be connected by a front rarefaction
wave to $\DS \VEC{u}^-$.}{shock_fig19}
...
Since the speed of the wave should increase along the $x$ axis, we
must have that
$\DS \lambda_2(\VEC{u}) > \lambda_2(\VEC{u}^-)$; namely,
$\DS \sqrt{-p'(u_1)} > \sqrt{-p'(u_1^-)}$.  Thus,
$\DS p'(u_1) < p'(u_1^-)$.  Since $p'$ is increasing, we get that
$\DS u_1 < u_1^-$.
Thus, the acceptable values of $\VEC{u}$ are the points on the curve
$R_2$ defined by (\ref{psystEq5}) for $\DS u_1 < u_1^-$.  We
have that $\DS \dydx{u_2}{u_1} = -\sqrt{-p'(u_1)}$ and
$\DS \dydxn{u_2}{u_1}{2} = \frac{p''(u_1)}{2\sqrt{-p'(u_1)}}$.

A possible sketch of the curve $R_{2,\VEC{u}^-}$ is drawn in
Figure~\ref{shock_fig19}.
The rarefaction waves connecting $\DS \VEC{u}^-$
...
$R_{1,\VEC{u}^-}$ intersect at $\DS \VEC{u}^-$ where they have matching
...
$R_{2,\VEC{u}^-}$ intersect at $\DS \VEC{u}^-$ where they
...
an open neighbourhood of $\DS \VEC{u}^-$).  $W_1$ is the
...
rarefaction wave to connect $\DS \VEC{u}^-$ to $\VEC{u}$
...
$\DS \VEC{u}^-$ to $\VEC{u}_{bf}$ with a back rarefaction wave.
There may be another way to connect $\DS \VEC{u}^-$ to $\VEC{u}$.
...
$\DS \VEC{u}^-$ to $\VEC{u}_{fb}$ with a front rarefaction wave.

\pdfF{shock_waves/shock_fig15}{General case to connect
$\DS \VEC{u}^-$ to $\VEC{u}$}{The figure used to explain how
to connect $\DS \VEC{u}^-$ to $\VEC{u} \in W_2$ using
...
$\VEC{u}$ to $\DS \VEC{u}^-$ with shock and rarefaction
...
Namely, $\DS \bigcup_{\VEC{v}\in R_{1,\VEC{u}^-}} S_{2,\VEC{v}}$
...
Suppose $\DS P = \int_{u_1^-}^\infty \sqrt{-p'(v)}\dx{v} < \infty$.
The reader should verify that this last assumption is true in the
important case where $\DS p(v) = kv^{-\beta}$ and $\beta >1$.

Given $\VEC{v} \in R_{1,\VEC{u}^-}$, we have from (\ref{psystEq5}) that
$\DS v_2 - u_2^- = \int_{u_1^-}^{v_1} \sqrt{-p'(v)} \dx{v}$.
So, $\DS \lim_{v_1 \to \infty} v_2 = u_2^- + P$.  In
particular, $\DS v_2 \leq u_2^- + P$ for all
$\VEC{v} \in R_{1,\VEC{u}^-}$.

Given $\VEC{w} \in R_{2,\VEC{v}}$ with $\DS w_1 = u_1^-$, we have from
(\ref{psystEq6}) that
$\DS w_2 - v_2 = -\int_{v_1}^{w_1} \sqrt{-p'(v)} \dx{v}$.
So, $\DS w_2 \leq v_2 + P \leq u_2^- + 2P$ for all
$\VEC{v} \in R_{1,\VEC{u}^-}$.
Therefore, there is no curve $R_{2,\VEC{v}}$ that contains $\VEC{u}$ if
$\DS u_1 = u_1^-$ and $\DS u_2 > u_2^- + 2 P$.

\pdfF{shock_waves/shock_fig16}{Illustration of the problem to connect
$\VEC{u} \in W_2$ to $\DS \VEC{u}^-$ using a back
rarefaction wave first}{Illustration of the problem to connect
$\VEC{u} \in W_2$ to $\DS \VEC{u}^-$ using a back
rarefaction wave first.}{shock_fig16}

If $U$ is a small enough open neighbourhood of $\DS \VEC{u}^-$ as in
...
$(\diff G)(u(\rho(y),y))$ and $\DS \pdydx{u(\rho(y),y)}{x}$
...
$\lambda_1(\VEC{u}) < \lambda_2(\VEC{u}) < \ldots < \lambda_n(\VEC{u})$
are the eigenvalues of $\diff G(\VEC{u})$ for $\DS \VEC{u} \in \RR^n$,
and $\DS q_i(\VEC{u}) \in \RR^n$ is a (right) eigenvector of
$\diff G(\VEC{u})$
...
It is also going to be useful to assume that
$\DS p_i(\VEC{u}) \in \RR^n$
is a left eigenvector of $\diff G(\VEC{u})$ associated to the
eigenvalue $\lambda_i(\VEC{u})$ for $1 \leq i \leq n$; 
namely, $\DS (p_i(\VEC{u}))^\top \diff G(\VEC{u})
= \lambda_i(\VEC{u}) (p_i(\VEC{u}))^\top$.  It is well know that
$\DS (p_i(\VEC{u}))^\top q_j(\VEC{u}) = 0$ for $i \neq j$
and $\DS (p_i(\VEC{u}))^\top q_j(\VEC{u}) \neq 0$ for $i=j$.
So, after scaling, we may assume that
$\DS (p_i(\VEC{u}))^\top q_i(\VEC{u}) =1$
for $1\leq i \leq n$.

\begin{defn}
The pair $(\lambda_i,q_i)$ is called the
{\bfseries $\DS \mathbf{i^{th}}$ characteristic
field}\index{Conservation Laws!$\DS i^{th}$ Characteristic Field}.
\end{defn}
...
states $\DS \VEC{u}^-$ and $\DS \VEC{u}^+$.

\begin{defn}
We say that the $\DS i^{th}$ characteristic field of the system
...
$\lambda_1(\VEC{u})$ is
$\DS q_1(\VEC{u}) = \begin{pmatrix} 1 \\ \sqrt{-p'(u_1)}
\end{pmatrix}$ and an eigenvector associated to
$\lambda_2(\VEC{u})$ is
$\DS q_2(\VEC{u}) = \begin{pmatrix} -1 \\ \sqrt{-p'(u_1)}
\end{pmatrix}$.
...
(\ref{systConLawEq1}) with $\DS u(x,y) = \VEC{u}^-$
for $\DS x < y \lambda_i(\VEC{u}^-)$.  We seek
$\DS \VEC{u}^+$ such that $\DS \VEC{u}^-$
could be connected to $\DS \VEC{u}^+$ by
a rarefaction wave.

Let us assume that $(\graD \lambda_i)(\VEC{u}) \cdot q_i(\VEC{u}) \neq 0$
in a neighbourhood $U$ of $\DS \VEC{u}^-$.  By scaling the
...
\dydx{w}{z} = q_i(w)
\]
with the initial condition $\DS w(z^-) = \VEC{u}^-$
where $\DS z^- = \lambda_i(\VEC{u}^-)$.  The unique solution
$w$ is well defined and continuously differentiable (assuming that $G$
is continuously differentiable) on an interval $\DS [z^-,z^+]$.
...
\]
for all $\DS z \in [z^-,z^+]$. we have that
$\lambda_i(w(z)) = z + C$ for all $\DS z \in [z^-,z^+]$.
Moreover, since $\DS w(\lambda_i(\VEC{u}^-)) = \VEC{u}^-$,
we have that $C=0$.

Let $\DS \VEC{u}^+ = w(z^+)$.
The {\bfseries $\DS \mathbf{i^{th}}$-rarefaction
wave}\index{Conservation Laws!$\DS i^{th}$-Rarefaction Wave}
connecting $\DS \VEC{u}^-$ to $\DS \VEC{u}^+$
is defined by
\[
u(x,y) = \begin{cases}
...
{Example of an $\DS i^{th}$-rarefaction wave for a system of
conservation laws in higher dimension.  We assume that
$\DS u(x,y) = \VEC{u}^-$ for
$\DS x< y \lambda_i(\VEC{u}^-)$.  The lines where $u$
is constant are given by $x= y z$ for $\DS
\lambda_i(\VEC{u}^-) = z^- \leq z \leq z^+ = \lambda_i(\VEC{u}^+)$.}
{shock_fig18}

\begin{defn}
Let $\DS w:I \to \RR^n$ be a solution of
$\DS \dydx{w}{z} = q_i(w)$, where $I$ is an open interval
...
\end{split}
\end{equation}
for the solution $w$ of $\DS \dydx{w}{z} = q_i(w)$ and
...
\end{pmatrix}
\]
Thus $\DS w(z) = \begin{pmatrix} z + C \\ \tilde{F}(z) \end{pmatrix}$,
...
conservation laws in $\DS \RR^2$.  Moreover, suppose that
$\DS U \subset \RR^2$ is an open and connected set.
...
$\DS \VEC{u} \in \RR^2$, the integral curves associated to
$q_1$ intersect transversely the integral curves associated to $q_2$.
Hence, using standard results in ordinary differential equation, we
can show that
\begin{align*}
\Phi : U & \to \Phi(U) \\
\VEC{u} &\mapsto \big(v_1(\VEC{u}), v_2(\VEC{u})\big)
\end{align*}
defines an isomorphism and $\DS \Phi(U) \subset \RR^2$ is open and
connected.
...
$\DS \pdydx{v_1}{y} + \tilde{\lambda}_2(v_1,v_2) \pdydx{v_1}{x} =
0$, where $\tilde{\lambda}_2(v_1,v_2) = \lambda_2(u_1,u_2)$.
Similarly, we find that
$\DS \pdydx{v_2}{y} + \tilde{\lambda}_1(v_1,v_2) \pdydx{v_2}{x} = 0$.
...
constant on $\DS \{ w(z) : z \in u(W) = [z^-,z^+]\}$ where
$w$, used to define $u$ in Example~\ref{eggRareWaveN}, yields an integral
curve associated to $q_i$.
This property will become the base for the definition of 
$\DS i^{th}$-rarefaction wave.
\end{egg}
...
$u(x,y) \in U$, where $U$ is an open subset of $\DS \RR^n$.
We will say that $u$ is a
{\bfseries $\DS \mathbf{i^{th}}$-rarefaction
wave}\index{Conservation Laws!$\DS i^{th}$-Rarefaction Wave}
in $U$ if all $i$-Riemann invariant on $U$ are constant.
\end{defn}

\begin{prop}
Suppose that $u$ is an $\DS i^{th}$-rarefaction wave on $U$.
...
$\DS \pdfdx{u(\rho(y),y)}{y} = 0$ for all $y \in I$
...
Since
$\DS \pdydx{u}{y} + \diff G(u) \, \pdydx{u}{x} = \VEC{0}$
for $u \in U$, we have
\[
...
Moreover, since $w_j$ is constant on $U$, we have
\[
...
Therefore, $\DS \pdfdx{u(\rho(y),y)}{y}$ is orthogonal to
$p_i(u(\rho(y),y))$ and $\DS (\diff w_j)^\top(u(\rho(y),y))$
with $1 \leq j \leq n-1$ for all $y\in I$.  However, the vectors
$\DS (\diff w_j)^\top(u(\rho(y),y))$ for $1\leq j \leq n-1$ are
linearly independent and orthogonal to $q_i(u(\rho(y),y))$.  Since
$\DS (q_i(u(\rho(y),y)))^\top p_i(u(\rho(y),y)) \neq 0$, the vector
$p_i(u(\rho(y),y))$ is not orthogonal to $q_i(u(\rho(y),y))$
and so is linearly independent of
$\DS (\diff w_j)^\top(u(\rho(y),y))$ for $1\leq j \leq n-1$.

Thus, $\DS \pdfdx{u(\rho(y),y)}{y}$ is orthogonal to
a basis of $\DS \RR^n$ for all $y\in I$.  This is possible only if
$\DS \pdfdx{u(\rho(y),y)}{y} = \VEC{0}$
for all $y\in I$.
\end{proof}
...
$\DS U \subset \RR^n$.  For each $1 \leq i \leq n$ and
$\DS \VEC{u}^- \in U$,
...
$\DS R_i: [0,r_i] \to \RR^n$ such that
$\DS R_i(0) = \VEC{u}^-$ and
$\DS \VEC{u}^-$ can be connected to $R_i(r)$ by an
...
differential equation $\DS \dydx{w}{z} = q_i(w)$ with
$\DS w(\lambda_i(\VEC{u}^-)) = \VEC{u}^-$.
...
and $\DS \VEC{u}^-$, the exist an interval $[s_i,0]$ and a
smooth function $\DS S_i: [s_i,0] \to \RR^n$ such that
$\DS S_i(0) = \VEC{u}^-$,
$\DS \dydx{S_i}{s}(0) = q_i(\VEC{u}^-)$,
$\DS \dydxn{S_i}{s}{2}(0) = \dfdx{q_i(S_i(s))}{s}\Big|_{s=0}$
and $\DS \VEC{u}^-$ can be connected to $S_i(s)$ by a $i$-shock
for all $s \in [s_i,0]$.

Moreover, we have that
$\DS G(S_i(s))-G(\VEC{u}^-)
= \eta(s) \left(S_i(s) - \VEC{u}^-\right)$
with $\DS \eta(0) = \lambda_i(\VEC{u}^-)$ and $\eta'(0) = 1/2$.
\end{theorem}
...
curves $\DS S_i([s_i,0]) \subset \RR^n$ are the generalizations of the
curves $S_{1,\VEC{u}^-}$ and $S_{2,\VEC{u}^-}$ in
Example~\ref{eggPsystem2} about the $p$-system.
The curve $S_i([s_i,0])$ is called the
{\bfseries $\DS \mathbf{i^{th}}$ Hugoniot curve}\index{Conservation
Laws!$\DS i^{th}$ Hugoniot Curve} emanating from
$\DS \VEC{u}^-$.
...
order derivatives at the point $\DS \VEC{u}^-$ where they intersect.
...
$\DS \VEC{u} \in U \subset \RR^n$,
and $\DS q_i(\VEC{u}) \in \RR^n$ is an eigenvector of
$\diff G(\VEC{u})$
associated to the eigenvalue $\lambda_i(\VEC{u})$ for $1 \leq i \leq n$.
We say that the $\DS i^{th}$ characteristic field is
...
Suppose that $\DS i^{th}$ characteristic field is linearly
degenerate in an open set $\DS U \subset \RR^n$, then
...
$\DS \dydx{w}{s}(s) = q_i(w(s))$ with the
initial condition $\DS w(0) = \VEC{u}^- \in U$, we have that
$\DS \lambda_i(w(s)) = \lambda_i(w(0)) = \lambda_i(\VEC{u}^-)$
for $|s| < \epsilon$, where $\epsilon$ is such that $w(s) \in U$ for
$|s| < \epsilon$.
It follows that $\DS q_i(w(s)) = q_i(\VEC{u}^-)$ for
$|s| < \epsilon$.  Hence $\DS q_i(\VEC{u}^-)$ is an eigenvector
associated to the eigenvalue $\DS \lambda_i(\VEC{u}^-)$ of
$(\diff G)(w(s))$ for $|s| < \epsilon$.

For $|s| < \epsilon$, let $\DS z^- = \lambda_i(\VEC{u}^-)$ and
\[
...
$\DS s \mapsto G(w(s)) - \lambda_i(\VEC{u}^-) w(s)$
is constant.  Therefore,
$\DS G(w(s)) - \lambda_i(\VEC{u}^-) w(s)
...
$\gamma'(y)$  is equal to $\DS \lambda_i(\VEC{u}^-)$ with
$\DS \VEC{u}^- = u^-(\VEC{p})$ and $\VEC{p} = (\gamma(y),y)$.

As we did with the $p$-system, we can (in theory) combine
$i$-rarefaction waves, $i$-shocks and contact discontinuities to
create connection from $\DS \VEC{u}^-$ to other values of
...
may be used to connect $\DS \VEC{u}^-$ to 
$\DS \VEC{u}^+$ if it is closed enough to $\DS \VEC{u}^-$.
...
\]
Suppose that $\DS U \subset \RR^n$ is a {\bfseries sufficiently small}
...
Let $\DS R = \prod_{(j,k) \in I} [j\Delta x,(j+2) \Delta x]
\times \{k \Delta y\}$.  We have that $\Phi:\tilde{R} \to R$
defined by $\DS \Phi(\Theta) =
...
$\tilde{R}$ and a sequence $\DS \{ \Delta x_i\}_{i=1}^\infty$
converging to $0$ such that
$\DS \{u_{\Theta,\Delta x_i}\}_{i=1}^\infty$ converges in
$\DS L^1_{loc}(\RR\times [0,\infty[)$ to a solution of
...
\begin{defn}
Suppose that $\DS G:\RR^n \to \RR^n$ in
(\ref{systConLawEq1}) is of class $\DS C^1$, and that
$E,F:U\to \RR$ are two continuously differentiable
functions defined on an open set $\DS U \subset \RR^n$.  If
...
If $G$ is the gradient of a function $\DS g:\RR^n \to \RR$,
...
\end{equation}
We claim that $\DS E(\VEC{u}) = \|\VEC{u}\|^2/2$ and
$\DS F(\VEC{u}) = \VEC{u}^\top (\diff g)(\VEC{u}) - g(\VEC{u})$
define an entropy pair for the (\ref{eggGradSyst}) on
$\DS \RR^n$.  This follows
from
\[
...
(\ref{systConLawEq1}), then we have
\begin{align*}
...
A strong solution
$\DS u:\RR\times ]0,\infty[\to \RR^n$ of (\ref{systConLawEq1})
is called an {\bfseries entropy solution}\index{Conservation
  Laws!Entropy Solution} if there exists an entropy pair $(E,F)$ on
$\DS \RR^n$ such that
...
\begin{lemma}
Suppose that $\DS u:\RR\times [0,\infty[ \to \RR^n$ is a
strong solution of
(\ref{systConLawEq1}) which is discontinuous along the curve
$\DS \Gamma = \left\{ (\gamma(y),y) : y \geq y_0 \right\}$ of
class $\DS C^1$ for some $y_0\geq 0$.  If $u$ is an entropy
...
\footnotetext{As usual, we assume that $u$ is of class $\DS C^1$
on both sides of $\Gamma$.}
...
\]
for $\VEC{p} \in \Gamma$.
Since $\DS \pdfdx{E(u)}{y} + \pdfdx{F(u)}{x} = 0$ in
$V_l$ because $u$ is of class $\DS C^1$ on the left side of
$\Gamma$, we get that
...
(\ref{EntroPairRHeq1}), we get that
...
$\DS \RR^n$, is
{\bfseries strictly convex}\index{Strictly Convex} on $U$ if
$\DS \diff^2 f(\VEC{u})$ (the Hessian matrix of $f$) is definite
...
$(E,F)$ is an entropy pair on $\DS \RR^n$ with $E$ strictly convex on
$\DS \RR^n$.  In particular, we assume that $G$, $E$ and $F$ are
...
class $\DS C^1$ where the Rankine-Hugoniot formula
(\ref{shock_rank_hugND}) is satisfied with
$\DS \|u^+(\VEC{p})-u^-(\VEC{p})\|$ small enough for
...
$\DS \VEC{u}^- = u^-(\VEC{p})$ is connected to
$\DS \VEC{u}^+ = u^+(\VEC{p})$ 
by an $i$-shock (and therefore the Lax Shock Condition is satisfied)
if and only if $\DS u^-(\VEC{p}) = S_i(0)$ and
$\DS u^+(\VEC{p}) = S_i(s)$ for some $0 \leq i \leq 1$
...
required to conclude that it is true if $\DS \VEC{u}^+$ is
close enough to $\DS \VEC{u}^-$.  This is certainly what is
illustrated with the $p$-system.}.  Moreover, we have that
$\DS G(S_i(s))-G(S_i(0))
= \eta(s) \left(S_i(s) - S_i(0)\right)$
with $\DS \eta(0) = \lambda_i(S_i(0))$ and $\eta'(0) = 1/2$.
...
$\DS H(0) = \dydx{H}{s}(0) = \dydxn{H}{s}{2}(0) = 0$ and
$\DS \dydxn{H}{s}{3}(0) > 0$.
...
If we differentiate $\DS G(S_i(s))-G(S_i(0))
= \eta(s) \left(S_i(s) - S_i(0)\right)$ with respect to $s$, then we get
\[
...
\end{align*}
It follows that $\DS \dydx{E}{s}(0) = 0$.  Moreover, after
some computations, then we get
\begin{align*}
\dydxn{H}{s}{2}(s)
% &= \eta''(s) \big( E(S_i(0)) - E(S_i(s)) \big)
% - \eta'(s) (\diff E)(S_i(s)) \dydx{S_i}{s}(s) \\
...
$\DS \dydxn{E}{s}{2}(0) = 0$.  Finally, after a bit more
computations, we get
% \begin{align*}
% \dydxn{H}{s}{3}(s)
...
The term $\DS \epsilon A \pdydxn{u^{[\epsilon]}}{x}{2}$
...
Suppose that $(E,F)$ is an entropy pair on an open subset
$\DS U \subset \RR^n$
and that $\DS u^{[\epsilon]}$ is a classical solution of
(\ref{systConLawEq1Pert}).  If we multiply both sides of
(\ref{systConLawEq1Pert}) by
$\DS (\diff E)(u^{[\epsilon]})$, then we get
\begin{align*}
...
If we assume that $\DS (\diff^2 E)(u) \, A$ is
...
$\DS C^2(\RR^n)$.  Moreover, suppose that $E$ is such that
$\DS \int_{\RR} \int_{[0,\infty[}
\left( (\diff E)(u^{[\epsilon]}) \,
A \pdydx{u^{[\epsilon]}}{x} \right) \pdydx{\psi}{x} \dx{y}\dx{x}$ is
bounded independently of $\epsilon$.
If $\DS u^{[\epsilon]} \to u$ in the sense of distributions as
...
\]
for all $u$ between $\DS u^-(\VEC{p})$ and
$\DS u^+(\VEC{p})$, where
