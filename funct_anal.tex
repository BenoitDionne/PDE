...
$\DS \| x \| = \sqrt{\ps{x}{x}}$ for $x \in H$ defines a
...
$\DS \{ \VEC{x}_j \}_{j=0}^\infty$ in $H$ exactly has it is
...
$y\in H$, then
\begin{enumerate}
\item $\DS x \mapsto \ps{x}{y}$ is a continuous linear mapping.
\item $\DS x \mapsto \ps{y}{x}$ is a continuous
conjugate-linear mapping.
\item $\DS x \mapsto \|x\|$ is a continuous mapping.
\end{enumerate}
\end{prop}
...
functional on $H$ is denote $\DS H^\ast$.  This space is
...
A set $\DS S = \{ x_\alpha \}_{\alpha \in A} \subset H$,
...
\begin{theorem}
Let $H$ be an Hilbert space and let $\DS \{x_j\}_{j=1}^J$ be a finite
...
\end{align*}
in the space $\DS \ell^2(A)$ of square integrable functions
...
states that $\DS F_x \in \ell^2(A)$ for all $x\in H$.

Since $\DS F_x \in \ell^2(A)$, it follows that the set of all $\alpha$
...
$\DS \|x\|^2 = \sum_{\alpha \in A} |\ps{x}{x_\alpha}|^2$.
...
$\DS S = \{ x_\alpha \}_{\alpha \in A} \subset H$, where $A$ is
...
$\DS \{ \|x_\alpha\|^{-1} x_\alpha \}_{\alpha \in A}$ is an
orthonormal basis.
\end{defn}
...
If $\DS S = \{ x_\alpha \}_{\alpha \in A}$, where $A$ is some index
...
$\DS \ps{x}{y} = \int_A F_x(\alpha)F_y(\alpha) \dx{\nu(\alpha)}$,
...
\section{General $\DS L^2$ Spaces}\label{SectGenL2}

Suppose that $\mu$ is a measure on a measurable space $\Omega$.
Let $\DS L^2(\Omega) \equiv L^2(\Omega, \mu)$ be the space
of measurable functions $f:\Omega \rightarrow \CC$ such that
$\DS \int_\Omega |f|^2 \dx{\mu}$ is finite.  We can define
a scalar product on $\DS L^2(\Omega)$ by
\[
\ps{f}{g} = \int_\Omega f\, \overline{g} \dx{\mu} \quad , \quad
f, g \in L^2(\Omega) \ .
\]
The associated $\DS L^2$-norm is
\[
\| f \|_2 = \sqrt{\ps{f}{f}} = \left(\int_\Omega |f|^2 \dx{\mu}\right)^{1/2}
\quad , \quad  f \in L^2(\Omega) \ .
\]
Equipped with this norm, $\DS L^2(\Omega)$ is an Hilbert space.

\begin{rmk}
We use the term ``function'' for an element
$\DS f \in L^2(\Omega)$.  The
...
of functions $\DS \{ v_\alpha \}_{\alpha \in A}$, where $A$
is some index set, is complete if
$\ps{f}{v_\alpha} = 0$ for all $\alpha \in A$ implies that $f = 0$
almost everywhere on $\Omega$.

Let $\DS S= \{ v_\alpha \}_{\alpha \in A}$, where $A$ is some
index set, be an orthonormal basis of $\DS L^2(\Omega)$.  
The {\bfseries (generalized) Fourier series}%
\index{Generalized Fourier Series} of a function
$\DS f \in L^2(\Omega)$ with respect to the orthonormal basis $S$ is
\[
...
any ordering $\DS \{ \alpha_j\}_{j=0}^\infty$ of the set of
...
above is due to the fact that $\DS F_f \in \ell^2(\NN)$.
...
J \rightarrow \infty
\]
whatever the ordering $\DS \{ \alpha_j\}_{j=0}^\infty$ of
...
\subsection{$\DS L^2$ Spaces on the Real Line}

An important special case of the general theory of $\DS L^2$ spaces is
...
$\DS f(c^+) = \lim_{x\to c^+} f(x)$ and
$\DS f(c^-) = \lim_{x\to c^-} f(x)$ for all $c \in I$; in
particular at the points where $f$ is discontinuous.

$\DS L^2[a,b] \equiv L^2([a,b],\mu)$ is the space of
measurable functions $f:[a,b]\rightarrow \RR$ such that
$\DS \int_a^b f^2 \dx{\mu} = \int_a^b f^2(x) p(x) \dx{x}$
is finite.  The scalar product on $\DS L^2[a,b]$ is 
...
The $\DS L^2[a,b]$ spaces have countable orthonormal bases.
Suppose that $\DS \{v_n : n \in \NN \} \subset L^2[a,b]$
is an orthonormal base for $\DS L^2[a,b]$, the Fourier
series of a function $\DS f \in L^2[a,b]$ with respect to
this bases is
...
functions $\DS \{v_n : n \in \NN \} \subset L^2[a,b]$.
In this case, the Fourier series of $\DS f \in L^2[a,b]$
...
N \rightarrow \infty \ .
\]
We say that $\DS \sum_{n=0}^{N} \: a_n v_n$ converges in
$\DS L^2$ to $f$ as $N\rightarrow \infty$, and we write
\[
...
\subsection{$\DS L^2$ Spaces on the Unit Circle}

An important class of $\DS L^2$ spaces is the space of
measurable complex valued functions $\DS L^2(T) \equiv L^2(T,\nu)$,
...
\]
then we have an isometry between $\DS L^2(T,\nu)$ and $P$ defined by
\begin{align*}
Q: L^2(T,\nu) & \rightarrow P \\
  f & \mapsto f(e^{x\,i})
\end{align*}
...
$\DS B = e^{Ai} \equiv \{e^{x\,i} : x\in A\}$ where
$A \subset [-\pi,\pi]$
...
\begin{defn}
$P$ is denoted $\DS L^2(T) = L^2(T,\nu)$.
\end{defn}

The context will determine which of the two spaces we may be working with.

The set $\DS \left\{ e^{n x\, i} : n \in \ZZ \right\}$ is a
complete orthonormal set in $\DS L^2(T)$.  The
{\bfseries classical Fourier expansion}\index{Classical Fourier Expansion}
of a complex valued function $\DS f \in L^2(T)$ is
\[
...
\begin{theorem}
Let $f:\RR\to \CC$ be a $2\pi$-periodic,     { \label{faFsPwc} }
piecewise differentiable function.  Then
\[
\frac{f(x^+) + f(x^-)}{2} = \lim_{N\to \infty} \sum_{n=-N}^N c_n e^{n x\, i} \ ,
\]
where the $c_n$ are defined in (\ref{fa_coeff_cfs}),
$\DS f(x^+) = \lim_{s\to x^+} f(s)$ and
$\DS f(x^-) = \lim_{s\to x^-} f(s)$.
\end{theorem}
...
Value Theorem, we have that $\DS |f(x+t) - f(x^+)| \leq M t$
for $0 < t < \delta_0$.
...
\]
for $N > N_0$.  Hence, we have $|I_2(N)| < \epsilon$ for $N> N_0$.

We had to split the integral $I_2(N)$ because
$\DS (f(x+t) - f(x^+))/\sin(t/2)$ or is derivative may not
be bounded at the origin.  So, $\DS (f(x+t) - f(x^+))/\sin(t/2)$
...
We consider the space $\DS L^2[-L,L] = L^2([-L,L],m)$, where
...
\]
where
$\DS a_0 = \frac{1}{2L} \int_{-L}^{L} f(x) \dx{x}$,
$\DS a_n = \frac{1}{L} \int_{-L}^{L} f(x)
\cos\left(\frac{n\pi x}{L}\right) \dx{x}$ for $n \geq 1$, and\\
$\DS b_n = \frac{1}{L} \int_{-L}^{L} f(x)
\sin\left( \frac{n\pi x}{L}\right) \dx{x}$ for $n \geq 1$.

If $f:\RR\rightarrow \RR$ is periodic of period $T = 2L$ and
$\DS f\big|_{[-L,L]} \in L^2[-L,L]$, the coefficients of the Fourier
expansion are also given by
$\DS a_0 = \frac{1}{2L} \int_{\alpha}^{\alpha+2L} f(x) \dx{x}$,\\
$\DS a_n = \frac{1}{L} \int_{\alpha}^{\alpha+2L} f(x)
\cos\left(\frac{n\pi x}{L}\right) \dx{x}$ for $n \geq 1$ and
$\DS b_n = \frac{1}{L} \int_{\alpha}^{\alpha+2L} f(x)
\sin \left(\frac{n\pi x}{L}\right) \dx{x}$ for $n \geq 1$,
...
$\DS \int_{-L}^L g(t) \dx{t} = 0$ if $g$ is an odd
function and $\DS \int_{-L}^L g(t) \dx{t} = 2 \int_0^L g(t) \dx{t}$
if $g$ is an even function to obtain simpler formulae for the Fourier
series of odd and even functions.

If $\DS f \in L^2[-L,L]$ is an even function, its Fourier expansion is
\begin{equation} \label{OEfunctEq1}
a_0 + \sum_{n=1}^{\infty} \,  a_n \cos\left(\frac{n\pi x}{L}\right)  \ ,
\end{equation}
where
$\DS a_0 = \frac{1}{L} \int_{0}^{L} f(x) \dx{x}$ and
$\DS a_n = \frac{2}{L} \int_{0}^{L} \, f(x)
\cos\left( \frac{n\pi x}{L}\right) \dx{x}$ for $n \geq 1$.

If $\DS f\in L^2[-L,L]$ is an odd function, its Fourier expansion is
\begin{equation} \label{OEfunctEq2}
\sum_{n=1}^{\infty} \, b_n \sin \left( \frac{n\pi x}{L}\right) \ ,
\end{equation}
where $\DS b_n = \frac{2}{L} \int_{0}^{L} \, f(x)
\sin \left( \frac{n\pi x}{L}\right) \dx{x}$ for $n \geq 1$.
...
converge to $\DS f = F\big|_{[0,L]}$ in
$\DS L^2[0,L]$ because they converge to
$F$ in $\DS L^2[-L,L]$.
...
\begin{egg}
Find the Fourier cosine series of the function $\DS f(x) = e^x$ for
$0 \leq x <1$.
...
&= \begin{cases}
\DS \frac{2(e-1)}{(n\pi)^2+1} &\quad \text{if $n$ is even}\\
\DS \frac{-2(e+1)}{(n\pi)^2+1} &\quad \text{if $n$ is odd}\\
\end{cases}
\end{align*}
We get
\[
...
For instance, if we expand a function $\DS f \in L^2[0,L]$ to an even
...
\end{cases}
\]
then $\DS F\in L^2[-2L,2L]$ and the Fourier cosine series of $F$ on
...
If we expand a function $\DS f \in L^2[0,L]$ to an odd function
...
\end{cases}
\]
then $\DS F\in L^2[-2L,2L]$ and the Fourier sine series of $F$ on
$[0,2L]$ restricted to $[0,L]$ gives the following series
representation of $f$ on $[0,L]$.
...
converge to $\DS f = F\big|_{[0,L]}$ in $\DS L^2[0,L]$
because they converge to $F$ in $\DS L^2[-2L,2L]$.

\subsection{Fourier Series of Functions of Two Variables}

If, in the general definition of $\DS L^2$ spaces, we have
$\Omega =[a,b]\times[b,c]$ and $\mu$ is the Lebesgue measure on
$\Omega$, then we get the space
$\DS L^2([a,b]\times[b,c]) = L^2([a,b]\times[b,c],\mu)$ of
$\DS L^2$ integrable functions on the rectangle
...
linear space
$\DS \IMG(A) = \left\{ y \in Y : y = A(x) \ \text{for some} \ x\in X\right\}$.
The {\bfseries kernel}\index{Linear Mapping!Kernel} of $A$ is
the linear space
$\DS \KE(A) = \left\{ x \in X : A(x) = 0 \right\}$.
\end{defn}
...
$\DS \left\{ (x_n, A(x_n)) \right\}_{n=0}^\infty \subset \GR(A)$,
we have that
$\DS (x,y) = \lim_{n\to\infty} (x_n, A(x_n)) \in \GR(A)$; namely,
$y=A(x)$.
\end{defn}
...
$\DS (A-\lambda \Id)^{-1} \in \LL(X)$ if $\lambda$ is in the
resolvent set of $A$.

\begin{defn}
If $\lambda$ is in the resolvent set of $A$, then
$\DS R_\lambda(A) = (A-\lambda\Id)^{-1}$ is called the
...
$\DS r_\sigma(A) = \sup_{\lambda\in \sigma(A)} |\lambda|$.
\end{defn}
...
$\DS X^\ast$ of $X$ is the set of all bounded linear
functionals on $X$.
\end{defn}

\begin{egg}
If $H$ is an Hilbert space, then we have from the Riesz Representation
Theorem, Theorem~\ref{fu_an_RieszRT}, that there exists a mapping
$\DS T:H^\ast \to H$ such that $u(x) = \ps{x}{T(u)}$ for all
...
$\DS u\left( \|T(u)\|^{-1} T(u)\right) = \|T(u)\|$.
Hence, $\DS T:H^\ast \to H$ is an isometry from
$\DS H^\ast$ into $H$.
\end{egg}
...
$\DS A^\ast : Y^\ast \rightarrow X^\ast$ defined by
\[
...
\begin{theorem}
If $A \in \LL(X,Y)$, then $\DS A^\ast \in \LL(Y^\ast, X^\ast)$ and
$\DS \|A\| = \|A^\ast\|$
\end{theorem}
...
\]
with a similar definition for the norm on $\DS X^\ast$.
...
\item If $\lambda \neq 0$, then
$\DS \dim \KE(A-\lambda \Id) = \dim \KE(A^\ast-\lambda \Id)$.
...
the linear space
$\DS \IMG(A) = \left\{ y \in Y : y = A(x) \ \text{for some}
\ x\in \DO(A)\right\}$.
The {\bfseries kernel}\index{Linear Mapping!Kernel} of $A$ is the
linear space
$\DS \KE(A) = \left\{ x \in \DO(A) : A(x) = 0 \right\}$.
\end{defn}
...
$\DS \left\{ (x_n, A(x_n)) \right\}_{n=0}^\infty \subset \GR(A)$,
we have
$\DS (x,y) = \lim_{n\to \infty} (x_n, A(x_n)) \in \GR(A)$;
...
functionals $\DS w \in Y^\ast$ such that there exists a
bounded linear functional $\DS v \in X^\ast$ satisfying
$w(A(x)) = v(x)$ for all $x \in \DO(A)$.
Since $\DO(A)$ is dense, $v$ is uniquely defined.  The
{\bfseries adjoint}\index{Linear Mapping!Adjoint}
of $A$ is the mapping $\DS (S,A^\ast)$ from $\DS Y^\ast$
to $\DS X^\ast$ defined by $\DS A^\ast(w) = v$ for
$w\in S$.  We have that $\DS \DO(A^\ast) = S$.
\end{defn}

\begin{theorem}
Let $(\DO(A),A)$ be a linear mapping from $X$ into $Y$ such that
$\DO(A)$ is dense in $X$, then $\DS (\DO(A^\ast),A^\ast)$ is a closed
linear operator.  Moreover, if $X$ and $Y$ are
{\bfseries reflexive}\index{Reflexive Space}
(i.e.\ $\DS X^{\ast\ast}=X$ and $\DS Y^{\ast\ast} = Y$),
then $\DS \DO(A^\ast)$ is dense in
$\DS Y^\ast$ and $\DS A^{\ast\ast} = A$.
\end{theorem}
...
$\DS H^\ast$ and $H$ provided by the Riesz Representation
...
satisfying $\DS \ps{A(x)}{w}_{H_2} = \ps{x}{v}_{H_1}$ for $x
\in \DO(A)$.  Since $\DO(A)$ is dense, $v$ is uniquely defined.  The
{\bfseries (Hilbert space) adjoint}\index{Linear Mapping!Hilbert Space
Adjoint} of $A$ is the mapping $\DS (S,A^\bullet)$ from
$H_2$ to $H_1$ defined by $\DS A^\bullet(w) = v$ for $w\in S$.
We have $\DS \DO(A^\bullet) = S$.
\end{defn}
...
$\DS (\DO(A^\ast),A^\ast)$ be the
adjoint of $(\DO(A),A)$ in the sense of operators between Banach
spaces and $\DS (\DO(A^\bullet),A^\bullet)$ be the adjoint
...
\end{equation}
on $\DS \DD(A^\ast)$.  To prove this, we note that
\[
w(A(x)) = A^\ast(w)(x) \Leftrightarrow
\ps{A(x)}{\Lambda_{H_2}^{-1}(w)}_{H_2} =
\ps{x}{\Lambda_{H_1}^{-1}(A^\ast(w))}_{H_1}
\]
for all $x \in \DO(A)$ and $\DS w\in \DO(A^\ast)$.  Thus
$\DS \Lambda_{H_2}^{-1}(w) \in \DO(A^\bullet)$
for $\DS w \in \DD(A^\ast)$ and
\[
A^\bullet(\Lambda_{H_2}^{-1}(w)) = \Lambda_{H_1}^{-1}(A^\ast(w))
\]
for $\DS w\in \DO(A^\ast)$ by definition of
$\DS (\DO(A^\bullet),A^\bullet)$.  This proves (\ref{fu_an_dualHB}).

It follows from Definition~\ref{fu_an_dualHB_def} that
the mapping $\DS A \mapsto A^\bullet$ if conjugate-linear
because
\begin{align*}
A^\bullet(w) = v
...
however that the mapping $\DS A \mapsto A^\ast$ is linear.

From now on, we will follow the tradition in functional analysis and
use $\DS A (\DO(A^\ast),A^\ast)$ to also designate
$\DS A (\DO(A^\bullet),A^\bullet)$.  The context will determine if we are
...
$\DS \IMG(A)^\perp = \KE(A^\ast)$ and
$\DS \IMG(A^\ast)^\perp = \KE(A)$.
Moreover, $\DS \KE(A^\ast)^\perp = \overline{\IMG(A)}$ and
$\DS \KE(A)^\perp = \overline{\IMG(A^\ast)}$.
\end{theorem}
...
if $\DS (\DO(A^\ast),A^\ast)$ is an extension of $(\DO(A),A)$.  Thus,
$\DS \ps{A(x)}{y} = \ps{x}{A(y)}$ for $x,y \in \DO(A)$.

We say that $(\DO(A),A)$ is
{\bfseries self-adjoint}\index{Linear Mapping!Self Adjoint} if
$\DS (\DO(A^\ast),A^\ast) = (\DO(A),A)$; namely, if
$\DS \DO(A)=\DO(A^\ast)$ and
$\DS A(x)=A^\ast(x)$ for all $x \in \DO(A)$.
\end{defn}
...
\end{enumerate}
If $(\DO(A),A)$ is also self-adjoint, then we have
\begin{enumerate}
...
$\DS |\lambda_1| \geq |\lambda_2| \geq |\lambda_2| \geq \ldots$
...
basis and do the union of these basis, then we get an orthonormal set
$\DS \{ \phi_j \}_{j=1}^\infty$ such that $\phi_j \neq 0$
for only a finite number of $j$ when $\sigma(A)$ is finite.
...
Let $\DS S = \left\{ \phi_1, \phi_2, \phi_3, \ldots \right\}$ be
...
$\DS Au - \lambda u = f$ if and only if
$\DS \ps{f}{\phi_j} = 0$ for $\tilde{\lambda}_j = \lambda$.
...
$\DS K \in L^2(\Omega\times \Omega)$.  Then
$\DS T_K:L^2(\Omega)\rightarrow L^2(\Omega)$ defined by
\[
...
The function $\DS K \in L^2(\Omega\times \Omega)$ in the
...
= \| K \|_2^2 \|f\|_2^2 < \infty
\]
for all $\DS f\in L^2(\Omega)$.  Thus
$\DS T_K f \in L^2(\Omega)$ for all
$\DS f\in L^2(\Omega)$ and
...
Let $\DS \{\phi_j\}_{j=0}^\infty$ be an orthonormal basis of
$\DS L^2(\Omega)$.  We consider the functions
$\psi_{j,k}:\Omega \times \Omega \to \RR$ defined by
$\DS \psi_{j,k}(\VEC{x},\VEC{y}) = \phi_j(\VEC{x})\phi_k(\VEC{y})$ for
...
$\DS \{\psi_{j,k}\}_{(j,k)\in \NN\times \NN}$ is an
orthonormal basis of $\DS L^2(\Omega \times \Omega)$.

We can write $K$ as
$\DS K = \sum_{(j,k)\in \NN\times \NN} a_{j,k} \psi_{j,k}$
for some $a_{j,k} \in \RR$.  The convergence of the previous series is
in $\DS L^2(\Omega\times \Omega)$.  In particular,
$\DS \|K\|_2^2 = \sum_{(j,k)\in \NN\times \NN} a_{j,k}^2$
since $\DS \{\psi_{j,k}\}_{(j,k)\in \NN\times \NN}$ is an
...
exists $M>0$ such that $\DS \sum_{i+j > m} a_{j,k}^2 < \epsilon$
for $m\geq M$.  Thus $\DS \sum_{i+j > N} a_{j,k}^2 \to 0$ as
$N \to \infty$.

Let $\DS
K_N(\VEC{x}, \VEC{y}) = \sum_{i+j\leq N} a_{j,k} \psi_{j,k}(\VEC{x},\VEC{y})$
for $N \geq 0$.  Since the image of $T_{K_N}$ is included in the
span of $\DS \{\phi_j\}_{j=0}^N$, we have that
$\dim \IMG(T_{K_N}) \leq N +1 < \infty$.  Thus
$\DS T_{K_N}$ is a compact
...
$\DS \left| B(u,v) \right| \leq C \| u\| \, \| v\|$
for all $\DS (u,v) \in H \times H$, where $\|\cdot\|$ is the
...
$\DS B(u,v) = \overline{B(v,u)}$ for all $u,v \in H$.

The bounded sequilinear form $B$ is
{\bfseries coercive}\index{Sequilinear Form!Coercive} if there
exists a positive constant $c$ such that
$\DS \RE B(u,u) \geq c \|u||^2$ for all $u \in H$.  If
...
$\DS v_t = (1-t)u+t v$ for $0\leq t \leq 1$.  We have that
...
closed subset of $H$.  Then, for any $\DS \tilde{u} \in H^\ast$, there
...
$\DS B(v,u) = \ps{v}{Au}$ for all $v \in H$.
...
uniqueness of the representation of the elements in $\DS H^\ast$ by
...
$\DS \tilde{u}(v) = \ps{v}{\breve{u}}$ for all $v \in H$.
...
$\DS P(w) = u$ where $u$ satisfies (\ref{fu_an_lax4}).
...
Hence $\DS \|P(w_1) - P(w_2)\| \leq \| w_1 - w_2 \|$.
...
$\DS 0< p^2 C^2 - 2pc +1 <1$, then we
get that $F$ is a strict contraction on $K$.  Such a $p$ exists
because
\[
0 > p(pC^2 -2c) \geq -c^2/C^2 \geq -1
\]
for all $p$ is between the two roots, $0$ and $\DS 2c/C^2$, of the
quadratic polynomial $\DS p(pC^2 -2c)$.  The reader should verify
...
$\DS \tilde{u}(v) = \ps{v}{\check{u}}_B$ for all $v \in H$.
...
Then, for every $\DS \tilde{u} \in H^\ast$, there
...
$\DS \RE B(v-u,u) \geq \RE \tilde{u}(v-u)$ for all $v\in H$.
...
If $v = w$ in (\ref{fu_an_lax11}), then we get
...
$\DS \RE B(w,u) = \RE \tilde{u}(w)$ for all $w \in H$.
Thus, $w\mapsto B(w,u)$ and $\tilde{u}$ denote the same linear
functional \footnote{
$\DS \RE B(w,u) = \RE \tilde{u}(w)$ for all $w \in H$
if and only if
$\DS \RE B(i w,u) = \RE \tilde{u}(iw)$ for all $w \in H$
if and only if
$\DS \RE (i B(w,u)) = \RE (i\tilde{u}(w))$ for all $w \in H$
if and only if $\DS \IM B(w,u) = \IM \tilde{u}(w)$ for all $w \in H$.
...
$\DS A^{-1}(\breve{u})$.  The proof that $A$ is a bounded, one-to-one
...
$\DS \left\{A u_j\right\}_{j=1}^\infty$ converges to
$w\in H$, then we have
\[
...
\]
for $j \geq 1$.
Thus $\DS \|A(u_j)\| \geq c \|u_j\|$ for all $j$.  Since 
$\DS \left\{A u_j\right\}_{j=1}^\infty$ converges, it is a
Cauchy sequence.  Thus
$\DS \left\{u_j\right\}_{j=1}^\infty$ is a Cauchy sequence 
in the Hilbert space $H$.  Hence, the sequence
$\DS \left\{u_j\right\}_{k=1}^\infty$ converges.  Let $u$ be the
...
Let $\DS R:H^\ast \rightarrow H$ be the bounded linear
...
$\DS \tilde{u}\in H^\ast$.  Let $A:H\rightarrow H$ be the
...
Theorem that $\DS A^{-1}$ is continuous.  Finally, we have that
$\DS T=A^{-1}\circ R$.
\end{rmk}

\section{Exercises}

The problems that we suggest cover only $\DS L^2$ spaces and
