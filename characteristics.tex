...
\end{equation}
where $\DS a:\RR^3\rightarrow \RR$ and
$\DS b:\RR^3\rightarrow \RR$ are
two given differentiable functions, and
$\DS u:\RR^2\rightarrow \RR$ is an
...
$\DS p:\RR^2\rightarrow \RR$ and
$\DS q:\RR^2\rightarrow \RR$, then the
...
$\DS (a,b,c) \cdot \left(\pdydx{u}{x}, \pdydx{u}{y}, -1 \right) = 0$.
The vector $\DS \left(\pdydx{u}{x}, \pdydx{u}{y}, -1 \right)$
evaluated at $(x,y)$ is perpendicular to the surface $u=u(x,y)$ at
$\DS \left(x,y,u(x,y)\right)$.  Hence, the vector
$(a,b,c)$ evaluated at $(x,y,u(x,y))$ is in the tangent plan to the integral
surface $u=u(x,y)$ at the point $\DS \left(x,y,u(x,y)\right)$.

If $\DS \gamma = \left\{ (x(t),y(t),u(t)) : t \in \RR \right\}$
...
$\DS \left\{ \gamma_s \right\}_{s\in I}$.  If we assume that
\begin{equation} \label{char_IFTQL_cond}
\frac{\partial(x,y)}{\partial(t,s)}\bigg|_{(t,s)} = \det
\begin{pmatrix}
\DS \pdydx{x}{t} & \DS \pdydx{x}{s} \\[0.7em]
\DS \pdydx{y}{t} & \DS \pdydx{y}{s}
...
\right\} \ ,
\]
where $I$ is an open interval and $\DS x_0, y_0, u_0 \in C_b^1(I)$.
For $s \in I$, let
\[
...
\[
\det \begin{pmatrix}
a\left(x(t,s),y(t,s),u(t,s)\right) & \DS \pdydx{x}{s}(t,s) \\[0.7em]
b\left(x(t,s),y(t,s),u(t,s)\right) & \DS \pdydx{y}{s}(t,s)
\end{pmatrix} \neq 0
\]
for all $(t,s)$ \footnotemark, then the surface $S$ given by the
family of curves $\DS \left\{ \gamma_s \right\}_{s\in I}$ is the
...
\DS \pdydx{x}{s}(\tilde{t},\tilde{s}) \\[0.7em]
b\left(x(\tilde{t},\tilde{s}),y(\tilde{t},\tilde{s}),
 u(\tilde{t},\tilde{s})\right) &
\DS \pdydx{y}{s}(\tilde{t},\tilde{s})
\end{pmatrix} \neq 0
\]
...
& \DS \pdydx{x}{s}(0,\tilde{s}) \\[0.7em]
b(x(0,\tilde{s}), y(0,\tilde{s}), u(0,\tilde{s}))
& \DS \pdydx{y}{s}(0,\tilde{s})
\end{pmatrix} \nonumber \\
&= \det \begin{pmatrix}
...
integral surfaces, one for each initial curve $\DS \Gamma^\ast$
...
$\DS u = u(t,s) = \ln(s)e^t$.
We can solve $\DS x = se^t$ and $y=t$ in terms of $s$ and $t$ to get
$t=y$ and $\DS s= xe^{-y}$.  If we substitute these values
...
$\DS u = u(t,s) = \cos(s)e^t$.
We can solve $x = t+s$ and $y=t$ in terms of $s$ and $t$ to get
$t=y$ and $s= x-y$.  If we substitute these values of $s$ and $t$ in the
expression for $u$, we get the integral surface
$\DS u= u(x,y) = \cos(x-y) e^y$.
\end{egg}
...
$\DS x'(t) = x^2(t)$, $\DS y'(t) = y^2(t)$ and
$\DS u'(t) = u^2(t)$.
The initial curve is given by $x(0) = s$, $y(0) = 2s$ and $u(0) = 1$.

The equation $\DS x'(t) = x^2(t)$ with $x(0)=s$ gives
\begin{equation} \label{char_idt4}
x = x(t,s) = \frac{s}{1-st} \ .
\end{equation}
Similarly, the equation $\DS y'(t) = y^2(t)$ with $y(0)=2s$ gives
\begin{equation} \label{char_idt5}
y = y(t,s) = \frac{2s}{1-2st} \ .
\end{equation}
Finally, the equation $\DS u'(t) = u^2(t)$ with $u(0)=1$ gives
$\DS u = u(t,s) = \frac{1}{1-t}$.
We can solve (\ref{char_idt4}) and (\ref{char_idt5}) for $s$ and $t$
in terms of $x$ and $y$ to get
$\DS t = \frac{1}{x} - \frac{2}{y}$ and
$\DS s = \frac{xy}{2y-2x}$.
...
The set $\DS \left\{ (\alpha_s,\beta_s) : s \in I \right\}$
...
$\DS v_1:\RR^3\rightarrow \RR^3$ and
$\DS v_2:\RR^3\rightarrow \RR^3$ such that
...
$\DS \phi:\RR^3\rightarrow \RR$ and
$\DS \psi:\RR^3\rightarrow \RR$ such that $\graD \phi = v_1$ and
...
\]
with the condition $\DS u(x,x)=e^x$.
...
We have that $\DS \phi(x,y,u)=(x^2+y^2)/2$
...
F(x^2+y^2, u) = 0
\end{equation}
for functions $\DS F:\RR^2 \rightarrow \RR$.  

Suppose that $\DS \pdydx{F}{\beta}(\alpha,\beta) \neq 0$,
...
that $\DS u(x,y) =f(x^2+y^2)$.  From the initial condition
$\DS u(x,x)= e^x$, we
get $\DS e^x = f(2x^2)$.  With $\DS z = 2x^2$, we get
$\DS f(z) = e^{\sqrt{z/2}}$ for $z \geq 0$. 
We finally get the integral surface
$\DS u(x,y) = e^{\sqrt{(x^2+y^2)/2}}$.
\end{egg}
...
Suppose that
$\DS \pdydx{G}{\lambda}(x_1,y_1,\lambda_1) = 0$
and
$\DS \pdydxn{G}{\lambda}{2}(x_1,y_1,\lambda_1) \neq 0$, then
we may use the Implicit Function Theorem to find open neighbourhoods
$\DS V \subset \RR^2$ of $(x_1,y_1)$ and
$\DS W \subset \RR^3$ of
...
\end{equation}
Thus $\DS G(x,y,\lambda) = \sqrt{4-x^2-(y-\lambda)^2}$ in
...
(\ref{char_env4}), we get $\DS z = \sqrt{4-x^2}$.
Similarly, for $z<0$, we get $\DS z = -\sqrt{4-x^2}$.
Therefore, the envelope is the surface $\DS x^2 + z^2 = 4$.
...
\end{equation}
where $\DS F:\RR^5 \rightarrow \RR$ is sufficiently differentiable.
...
$\DS \left\{ \gamma_s \right\}_{s\in I}$ that
...
$\DS \pdydx{u}{x}(\tilde{x},\tilde{y})$ and
$\DS \pdydx{u}{y}(\tilde{x},\tilde{y})$; namely,
\[
...
$\DS \pdydx{F}{q}(\tilde{x},\tilde{y},\tilde{u},p,q) \neq 0$,
we have 
\[
\dydx{q}{p} =
- \frac{\DS \pdydx{F}{p}(\tilde{x},\tilde{y},\tilde{u},p,q)}
{\DS \pdydx{F}{q}(\tilde{x},\tilde{y},\tilde{u},p,q)} \ .
\]
Hence,
\begin{equation} \label{char_tp2}
\frac{(x-\tilde{x})}{\DS \pdydx{F}{p}
(\tilde{x},\tilde{y},\tilde{u},p,q)} = 
\frac{(y-\tilde{x})}{\DS \pdydx{F}{q}
(\tilde{x},\tilde{y},\tilde{u},p,q)} \ .
\end{equation}
Moreover, if we divide (\ref{char_tp1}) by $(y-\tilde{y})$, then we get
\[
\frac{u-\tilde{u}}{y-\tilde{y}} = p\frac{x-\tilde{x}}{y-\tilde{y}} + q = 
p\, \frac{\DS \pdydx{F}{p}(\tilde{x},\tilde{y},\tilde{u},p,q)}
{\DS \pdydx{F}{q}(\tilde{x},\tilde{y},\tilde{u},p,q)} + q \ .
\]
Thus
\[
...
$\DS \left\{\gamma_s\right\}_{s\in I}$, where
...
equations in (\ref{char_Gidx3}) with respect to $t$, then we get
\begin{align}
...
Moreover, if $u$ is an integral surface, then we have
$\DS F\left(x,y,u(x,y),p(x,y),q(x,y)\right) = 0$ with
$\DS p(x,y) = \pdydx{u}{x}(x,y)$ and
$\DS q(x,y) = \pdydx{u}{y}(x,y)$.  If we derive
$\DS F\left(x,y,u(x,y),p(x,y),q(x,y)\right) = 0$ 
with respect to $x$, then we get
\[
...
\det
\begin{pmatrix}
\DS \pdydx{F}{p}(x(t,s),y(t,s),u(t,s),p(t,s),q(t,s)) &
\DS \pdydx{x}{s}(t,s) \\[0.7em]
\DS \pdydx{F}{q}(x(t,s),y(t,s),u(t,s),p(t,s),q(t,s)) &
\DS \dydx{y}{s}(t,s)
\end{pmatrix} \neq 0
\end{equation}
for all $(t,s)$, then the surface $S$ given by the union of the family
of curves $\DS \left\{ \gamma_s \right\}_{s\in I}$ is the
...
\DS \pdydx{x}{s}(\tilde{t},\tilde{s}) \\[0.7em]
\DS \pdydx{F}{q}(x(\tilde{t},\tilde{s}),y(\tilde{t},\tilde{s}),
u(\tilde{t},\tilde{s}),p(\tilde{t},\tilde{s}),q(\tilde{t},\tilde{s})) &
\DS \pdydx{y}{s}(\tilde{t},\tilde{s})
\end{pmatrix} \neq 0
\end{align*}
...
$\DS \frac{\partial(x,y)}{\partial(t,s)}\bigg|_{(t,s)} \neq 0$.
...
\]
Its solutions is $\DS R(t,s) = C e^{H(t,s)}$, where $C$ is a
constant and
\[
H(t,s) = -\int_0^t
...
$s\in I$.  This proves (\ref{char_idxCON1}).
...
$u(t,s) = u(x,y)$, $\DS p(t,s) = p(x,y) = \pdydx{u}{x}(x,y)$
and $\DS q(t,s) = q(x,y) = \pdydx{u}{y}(x,y)$
...
\begin{pmatrix}
\DS -\dydx{x_0}{s}(s) &
\DS -\dydx{y_0}{s}(s) \\[0.7em]
\DS \pdydx{F}{p}(x_0(s),y_0(s),u_0(s),p,q) &
\DS \pdydx{F}{q}(x_0(s),y_0(s),u_0(s),p,q) \\
...
with $\DS F(x,y,u,p,q)=p^2+q$.  Hence, the characteristic
equations are
\begin{align*}
...
Note that $\DS \pdydx{u}{x}(x,y) = p(x,y) = 1$ and 
$\DS \pdydx{u}{y}(x,y) = q(x,y) = -1$ for all $x$ and $y$ as
expected.
\end{egg}
...
$\DS u_\omega = e^{i\omega t} \psi(x,y)$.
...
$\DS \psi(x,y) = \rho(x,y,k) e^{ik\phi(x,y)}$.
If we substitute this expression of $\psi$ in (\ref{char_eikon1}) and
divide the result by $\DS e^{ik\phi(x,y)}$, then we get
\[
...
$\DS F(x,y,u,p,q) = p^2 + q^2 - n^2=0$ for $u=\phi$.  Hence, the
...
$\DS t = (y-2x)/(2n\sqrt{5})$ and
$\DS s = (x+2y)/5$.
If we substitute these values of $s$ and $t$ in the
expression for $u$, we get the integral surface
$\DS u(x,y) = (n(y-2x))/\sqrt{5} +1$.
...
\]
The first equation says that $\DS \|(p_0(s),q_0(s))\|_2 = n$
...
s =
\begin{cases}
\DS \arctan\left(\frac{y}{x}\right) & \quad x>0 \\
\DS \frac{\pi}{2} & \quad x=0, y>0 \\
\DS \arctan\left(\frac{y}{x}\right) + \pi & \quad x<0 \\
\DS \frac{3\pi}{2} & \quad x=0, y<0
\end{cases}
\]
If we substitute these values of $s$ and $t$ in the
expression for $u$, we get the integral surface
...
We have that $\DS x^2+y^2 = (1 \pm 2nt)^2$ represents the
wavefront because $u$ is constant on these circles.
$\DS x^2+y^2 = (1 + 2nt)^2$ is the wavefront moving away of
the origin as $t$ increases while
$\DS x^2+y^2 = (1 - 2nt)^2$ is the wavefront moving toward
the origin to reach it at $t= 1/2n$.
\end{egg}

\begin{rmk}
The eikonal equation is of the form
$\DS F(x,y,u,p,q) = p^2 + q^2 - n^2=0$.  Because of the
...
satisfies $\DS \cot(\theta) = \sqrt{p^2 + q^2} = n$.
\end{rmk}
...
For each point
$\DS \left(\tilde{x},\tilde{y},\tilde{u}\right) \in E$, we have from
Remark~\ref{char_env_tgp} that the tangent plane to the envelope $E$
at $\DS \left(\tilde{x},\tilde{y},\tilde{u}\right)$ is also
the tangent plane of the integral surface given by
$u= f(x,y,\tilde{\lambda})$ at 
$\DS \left(\tilde{x},\tilde{y},\tilde{u}\right)$, where
$\DS \tilde{\lambda} = \lambda\left(\tilde{x},\tilde{y}\right)$.

Moreover, if
$\DS h(x,y) = f\left(x,y,\lambda(x,y)\right)$, then
\[
...
containing a curve $\DS \Gamma = \{x_0(s),y_0(s),u_0(s) : s \in I\}$
...
defined by $\DS u = G\left(x,y,\alpha(s), \beta(s)\right)$
...
$\DS u = G\left(x, y, \alpha(s), \beta(s)\right)$
that will contains $\Gamma$.

To prove that each point
$\DS \left(\tilde{x},\tilde{y},\tilde{u}\right) \in \Gamma$
...
$\DS \left(\tilde{x},\tilde{y},\tilde{u}\right)$
and $s=\tilde{s}$.

If we derive (\ref{char_compl1}) with respect to $s$, then we get
\begin{align*}
\dydx{u_0}{s}(s)
&= \pdydx{G}{x}\left(x_0(s), y_0(s), \alpha(s), \beta(s)\right)
...
$\DS p_0(s) = \pdydx{G}{x}(x_0(s),y_0(s),u_0(s),\alpha(s),\beta(s))$
and \\
$\DS q_0(s) = \pdydx{G}{y}(x_0(s),y_0(s),u_0(s),\alpha(s),\beta(s))$,
then
\begin{align*}
F\left(x_0(s),y_0(s),u_0(s),p_0(s),q_0(s)\right) &= 0
\intertext{and}
...
$\DS \left(p_0(s), q_0(s),-1\right)$ of the
...
\DS  \dydx{u_0}{s}(s) - \pdydx{G}{x}(x_0(s),y_0(s), \alpha, \beta)\,
\dydx{x_0}{s}(s) + \pdydx{G}{y}(x_0(s),y_0(s), \alpha, \beta)\,
\dydx{y_0}{s}(s)
\end{pmatrix} \ .
\end{align*}
Suppose that
$\DS \left(\tilde{s}, \tilde{\alpha}, \tilde{\beta}\right)
...
$\DS \left(\alpha\left(\tilde{s}\right),
\beta\left(\tilde{s}\right)\right)
...
= \det \begin{pmatrix}
\DS \pdydx{G}{\alpha} & \DS \pdydx{G}{\beta} \\[0.7em]
\DS \pdydxnm{G}{x}{\alpha}{2}{}{}\, \dydx{x_0}{s}
+ \pdydxnm{G}{y}{\alpha}{2}{}{}\, \dydx{y_0}{s} &
\DS \pdydxnm{G}{x}{\beta}{2}{}{}\, \dydx{x_0}{s}
...
and $\DS \dydx{y_0}{s}$ and $\DS \dydx{y_0}{s}$
...
and $\DS \left(\alpha\left(\tilde{s}\right)
\beta\left(\tilde{s}\right)\right)
...
$\DS F'(x) = \frac{1}{G'(y)}$.  Since the right hand side is
independent of $x$ and the left hand side is independent of $y$, we
must have $\DS F'(x) = \frac{1}{G'(y)} = a$, where $a$ is a
...
\det
\begin{pmatrix}
\DS \pdydxnm{G}{x}{\alpha}{2}{}{} &
\DS \pdydxnm{G}{x}{\beta}{2}{}{} \\[0.7em]
\DS \pdydxnm{G}{y}{\alpha}{2}{}{} &
\DS \pdydxnm{G}{y}{\beta}{2}{}{}
\end{pmatrix}
= \det
\begin{pmatrix}
1 & 0 \\ 0 & 1
...
functions of $x$ on one side and those of $y$ on the other side, then we
get
\[
...
$G'(y) + \alpha G(y) = 0$,
whose general solutions are $\DS F(x) = \beta_1 e^{\alpha x}$ and
$\DS G(y) = \beta_2 e^{-\alpha y}$ respectively.  A solution is
$\DS u(x,y) = F(x)G(y) = \beta e^{\alpha(x-y)}$, where
$\beta = \beta_1 \beta_2$.
...
\det
\begin{pmatrix}
\DS \pdydxnm{u}{x}{\alpha}{2}{}{} &
\DS \pdydxnm{u}{x}{\beta}{2}{}{} \\[0.7em]
\DS \pdydxnm{u}{y}{\alpha}{2}{}{} &
\DS \pdydxnm{u}{y}{\beta}{2}{}{}
\end{pmatrix}
= \det
\begin{pmatrix}
...
$\DS F(x) = \beta_1 |x|^\alpha$, where
$\DS \beta_1 = \pm e^{\gamma_1}$.  We may also accept
$\beta_1 = 0$ to get
the trivial solution $u(x,y)=0$ for all $x$ and $y$.  Similarly,
$\DS G(y) = \beta_2 |y|^\alpha$.   A solution is
...
\[
\begin{pmatrix}
\DS \pdydx{G}{\alpha} & \DS \pdydx{G}{\beta} \\
\DS \pdydxnm{G}{x}{\alpha}{2}{}{} &
\DS \pdydxnm{G}{x}{\beta}{2}{}{} \\
\DS \pdydxnm{G}{y}{\alpha}{2}{}{} &
\DS \pdydxnm{G}{y}{\beta}{2}{}{} \\
\end{pmatrix}
=
...
with the curve \\
$\DS \Gamma = \left\{ (x_0(s), y_0(s), u_0(s) : s \in I\right\}$,
...
$\DS \cos(s) = x/\sqrt{x^2+y^2}$ and
$\DS \sin(s) = y/\sqrt{x^2+y^2}$.
If we substitute these values of $\cos(s)$ and $\sin(s)$ in
(\ref{char_Eik1}), then we get
\[
...
The rest of these lecture notes is about linear partial differential equations.
...
these lecture notes.  The theory on linear partial differential
