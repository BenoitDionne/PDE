...
Let $\Omega$ be an open subset of $\DS \RR^n$ with a smooth
boundary (at least a boundary of class $\DS C^2$).  If $u$
and $v$ are two functions of class
$\DS C^2(\Omega) \cap C^1(\overline{\Omega})$, then
\begin{align}
...
$\DS C^k(\overline{\Omega})$ if $\DS D^{\VEC{\alpha}} f$
...
\]
for $\DS F:\Omega \rightarrow \RR^n$ of class
$\DS C^1(\Omega) \cap C(\overline{\Omega})$ and
$\nu$ a unite normal to $\partial \Omega$ pointing outside of
$\Omega$.

Moreover, $\DS \pdydx{f}{\nu} = \nabla f \cdot \nu$
for $\DS f \in C^1(\Omega)$.  Thus
...
Recall that, for $\DS \VEC{x} \in \RR^n$ and $r > 0$, the
open ball of radius $r$ centred at $\VEC{x}$ is the set
$\DS B_r(\VEC{x}) = \left\{ \VEC{y}\in \RR^n : 
\|\VEC{y}-\VEC{x}\|<r \right\}$.  The closure of this ball is thus
$\DS \overline{B_r(\VEC{x})} = \left\{ \VEC{y}\in \RR^n : 
...
where $\DS \omega_n = 2\pi^{n/2}\big/ \Gamma(n/2)$ is the area of the
surface of the ball of radius $1$ in $\DS \RR^n$.
\end{defn}

\begin{theorem}[Mean Value Theorem] \label{laplace_MVT}
Suppose that $\Omega$ is an open subset of $\DS \RR^n$ and that
$\DS u \in C^2(\Omega)$ satisfies $\Delta u = 0$
on $\Omega$.  If $\VEC{x} \in \Omega$ and $r>0$ are such
that $\DS \overline{B_r(\VEC{x})} \subset \Omega$, then
...
$\DS \overline{B_s(\VEC{x})}$ with $v \equiv 1$ that
...
$u:\Omega \rightarrow \RR$ be an harmonic function, then
$\DS u \in C^\infty(\Omega)$ and $\Delta u = 0$ on $\Omega$.
\end{theorem}

\begin{proof}
\stage{i} We first prove that $u$ is of class $\DS C^\infty(\Omega)$.

Choose $\psi \in \DD(]-1,1[)$ such that
\begin{equation} \label{laplace_int1}
\int_{\RR^n} \phi(\VEC{x}) \dx{\VEC{x}} = 1 \ ,
\end{equation}
where $\DS \phi(\VEC{x}) = \psi(\|\VEC{x}\|^2)$ for
$\DS \VEC{x} \in \RR^n$.
...
\psi(r^2) r^{n-1} \dx{r} \ ,
\end{align*}
where we have use the fact that $\DS \phi(r\VEC{y}) = \psi(r^2)$ for
...
\int_{\RR^n} u(\VEC{y}) \phi_\epsilon(\VEC{x}-\VEC{y}) \dx{\VEC{y}}
\end{align*}
is of class $\DS C^\infty$ on $\Omega_\epsilon$ because
$\DS \phi_\epsilon \in C_c^\infty(\RR^n)$ and, for any
...
$\DS \overline{B_\delta(\VEC{x}_0)} \times
...
$\DS r \mapsto \int_{\|\VEC{y}\|=1} u(\VEC{x}+r\VEC{y}) \dss{S}{y}$
is constant on its domain.  Hence, using the substitution
$\DS \VEC{y} = r^{-1} (\VEC{z}-\VEC{x})$, we get
\begin{align*}
...
Let $\DS \left\{ u_j \right\}_{j=1}^\infty$ be a sequence of
harmonic functions on an open set $\DS \Omega \subset \RR^n$.
If the sequence $\DS \left\{ u_j \right\}_{j=1}^\infty$
converges uniformly on compact subsets of $\Omega$ to a function $u$,
...
converges to $\DS \frac{1}{\omega_n} \int_{\|\VEC{y}\|=1}
...
functions on $\DS \RR^n$ in general.

\begin{theorem}[Liouville's Theorem]
If $\DS u:\RR^n \rightarrow \RR$ is a bounded harmonic
function, then $u$ is constant on $\DS \RR^n$.
\index{Liouville's Theorem}
\end{theorem}

\begin{proof}
Given $\DS \VEC{x} \in \RR^n$ and $R>\|\VEC{x}\|$, we
multiply both sides of
\[
u(\VEC{x}) = \frac{1}{\omega_n} \int_{\|\VEC{y}\|=1}
u(\VEC{x} + \rho R \VEC{y}) \dx{S_{\VEC{y}}}
\]
by $\DS \rho^{n-1}$ and integrate with respect to $\rho$
...
equality.
Since
$\DS \int_0^1 \rho^{n-1} = \frac{1}{n}$, we get from
(\ref{LouisEq1}) that
\[
...
\begin{prop}
A function $u:\Omega \to \RR$ of class $\DS C^2$ on an open set
$\DS \Omega \subset \RR^n$ is subharmonic if and only if
...
(\ref{MVTequ1}) shows that $\DS \int_{\partial B_s(\VEC{x})}
\pdydx{u}{\nu} \dss{S}{y} \geq 0$.
...
$\DS \overline{B_r(\VEC{z})} \subset \Omega$ and
$\Delta u(\VEC{x}) < 0 $ for all
$\DS \VEC{x} \in \overline{B_r(\VEC{z})}$.

Since $u$ and $\graD u$ are uniformly continuous on the compact set
$\DS \overline{B_r(\VEC{z})}$, we may write
\[
...
We now show that $\DS u^{-1}(\{A\})$ is also open in the
induce topology on $\Omega$.  If
$\DS \VEC{x} \in u^{-1}(\{A\})$, it follows that
\[
...
neighbourhood of $\VEC{x}$ inside $\DS u^{-1}(\{A\})$.
This shows that $\DS u^{-1}(\{A\})$ is open since
$\DS \VEC{x} \in u^{-1}(\{A\})$ is arbitrary

Since $\Omega$ is connected, either
$\DS u^{-1}(\{A\}) = \emptyset$ or
$\DS u^{-1}(\{A\}) = \Omega$ otherwise
$\DS \Omega = u^{-1}(\{A\}) \cup
...
Let $\Omega$ be a bounded open subset of $\DS \RR^n$.  If
...
$\DS u(\VEC{x}) = \sup_{\VEC{y}\in \overline{\Omega}}\, u(\VEC{y})$.
...
Let $\Omega$ be an open subset of $\DS \RR^n$ with compact
closure.  If $u_1 : \overline{\Omega} \rightarrow \RR$ and
...
= 0 \ .
\end{align*}
Thus $\DS u_1(\VEC{x}) - u_2(\VEC{x}) = 0$ for all
$\VEC{x} \in \Omega$.
\end{proof}
...
N(\VEC{x}) =
\begin{cases}
\DS \frac{1}{(2-n)\omega_n} \|\VEC{x}\|^{2-n} & \quad 
\text{if} \ n>2 \\[0.9em]
\DS \frac{1}{\omega_2} \ln\left(\|\VEC{x}\|\right) & \quad 
\text{if} \ n=2
\end{cases}
\]
...
Hence, $N$ is in $\DS L^1_{loc}(\RR^n)$ and thus in
$\DS \DD'(\RR^n)$.
...
\end{cases}
\]
Note that $N_\epsilon$ is of class $\DS C^{\infty}$ on
$\DS \RR^n$.

We first prove that $N_\epsilon \rightarrow N$ in $\DS L^1(K)$ as
$\epsilon \rightarrow 0$ for any compact set $\DS K \subset \RR^n$.
It is obvious that
$\DS \lim_{\epsilon \rightarrow 0} N_\epsilon(\VEC{x}) = N(\VEC{x})$
for $\VEC{x} \neq \VEC{0}$.  Moreover, for $\epsilon \leq 1$,
\[
\left| N_\epsilon(\VEC{x}) \right| <
\begin{cases}
\DS \left| N(\VEC{x})\right| & \quad \text{if} \quad n>2 \\[1em]
\DS \frac{1}{\omega_2} \left( \left|\ln(\|\VEC{x}\|)\right|+1 \right) 
& \quad \text{if} \quad n=2
\end{cases}
\]
for all $\VEC{x} \neq \VEC{0}$ \footnote{If
$\DS \|\VEC{x}\|^2+\epsilon^2 \leq 1$, then
$\DS (1/2) |\ln(\|\VEC{x}\|^2 + \epsilon^2)|  
...
\end{cases}
\end{align*}
It is easier to look at the graph of $|\ln(x)|$ to convince ourselves
...
We now prove that $N_\epsilon \rightarrow N$ in $\DS \DD'(\RR^n)$ as
$\epsilon \rightarrow 0$.  If $\DS \phi \in \DD(\RR^n)$,
let $K = \supp \phi$.
$K$ is a compact set by definition of $\DS \DD(\RR^n)$.  Hence
\begin{align*}
...
\rightarrow 0
\end{align*}
as $\epsilon \rightarrow 0$ because $N_\epsilon \rightarrow N$ in
$\DS L^1(K)$ as $\epsilon \rightarrow 0$ and
$\DS \sup_{\VEC{x}\in K} \left|\phi(\VEC{x})\right|$ is bounded.
...
We have that $\DS \psi \in L^1(\RR^n)$.  In fact,
...
\end{align*}
where we have used the substitution $\DS s = \frac{r^2}{r^2+1}$
and $\DS \dx{s} = \frac{2r}{(r^2+1)^2} \, \dx{r}$.

We may therefore use Proposition~\ref{distr_limit_convol} with
$\DS a = \int_{\RR^n} \psi(\VEC{x}) \dx{\VEC{x}} = 1$
to conclude that
\begin{equation} \label{laplace_pot1}
\ps{\Delta N_\epsilon}{\phi} = (\phi \ast \Delta N_\epsilon)(\VEC{0})
\rightarrow \phi(\VEC{0})
\end{equation}
as $\epsilon \rightarrow 0$ for any $\DS \phi \in \DD(\RR^n)$.
...
If $\DS u\in \DD'(\RR^n)$ satisfies
$\DS \Delta u = f \in C^\infty(\RR^n)$,
then $\DS u \in C^\infty(\RR^n)$.  In particular, we recover the
result that harmonic functions on $\DS \RR^n$ are of class
$\DS C^\infty(\RR^n)$.
\end{cor}
...
The distribution solution of $\DS \Delta u = f \in \EE'(\RR^n)$
is given by $u = N \ast f$.  The same conclusion may sometime be drawn when
$\DS f \not\in \EE'(\RR^n)$.

\begin{theorem} \label{laplace_not_EEp}
If $\DS f \in L^1(\RR^n)$ for $n>2$ or
$\DS \int_{\RR^n} \big|f(\VEC{y}) \ln(\|\VEC{y}\|)\big|
...
\end{cases}
\]
We have that $\DS \chi_r N \in L^1(\RR^n)$ because
\begin{align*}
...
\end{align*}
Moreover, $\DS (1-\chi_r)N \in L^{\infty}(\RR^n)$ because
...
that $\DS ( \chi_r N ) \ast f \in L^1(\RR^n)$ and
$\DS ( (1-\chi_r)N ) \ast f \in L^{\infty}(\RR^n)$.

We first show that $\chi_r f \to f$ and $N \ast (\chi_r f) \to N \ast f$ in
$\DS \DD'(\RR^n)$ as $r \to \infty$.

It follows from Lebesgue Dominated Convergence Theorem that
$\chi_r f$ converges to $f$ in $\DS L^1(\RR^n)$ as $r \to \infty$
because $\DS |\chi_r f| \leq |f| \in L^1(\RR^n)$ and
$\DS \lim_{r\to \infty}\chi_r(\VEC{x})f(\VEC{x}) = f(\VEC{x})$
for all $\VEC{x}$.
Therefore, $\ps{\chi_r f}{\phi} \to \ps{f}{\phi}$ as $r\to \infty$
for all test function $\DS \phi \in \DD(\RR^n)$; namely,
$\chi_r f \to f$ in $\DS \DD'(\RR^n)$ as $r\to \infty$. 
...
\]
as $r \to \infty$, we have for all $\DS \phi \in \DD(\RR^n)$ that
\begin{align*}
...
\end{equation}
for $\VEC{x}\neq \VEC{y}$ in $\DS \RR^n$ and
\[
  N_\epsilon(\VEC{x},\VEC{y}) = N_\epsilon(\VEC{x}-\VEC{y})
\]
for $\VEC{x},\VEC{y}$ in $\DS \RR^n$.

\begin{theorem} \label{laplace_uhexp}
Let $\DS \Omega \subset \RR^n$ be an open bounded set.  If
$\DS u\in C^1(\overline{\Omega})$ is harmonic in $\Omega$, then
...
compact subset of $\DS \RR^n$, we have that
\[
\left| u(\VEC{y})
...
$\DS \VEC{y}\mapsto u(\VEC{y})
\pdydx{N}{\nu_{\VEC{y}}}(\VEC{x},\VEC{y})$ in
$\DS L^1(\partial \Omega)$.
Moreover,
\[
...
$\DS \VEC{y}\mapsto h(\VEC{x},\VEC{y})$ in
$\DS L^1(\partial \Omega)$.
Moreover,
\[
\lim_{\epsilon \rightarrow 0}
...
\begin{rmk}
Replacing $\DS u\big|_{\partial \Omega}$ by $f$ and
...
$\DS u\big|_{\partial \Omega} = f$ and
$\DS \pdydx{u}{\nu}\bigg|_{\partial \Omega} = g$.
...
\begin{cor}
Let $\Omega$ be an open bounded subset of $\DS \RR^n$.  If
...
and $\DS w \in C^{\infty}(\overline{B})$.  Hence, from
...
$\DS B_{r/4}(\VEC{x}_0) = \RR^n \cap W$.  It
follows that $u = v+w$ is real analytic on $B_{r/4}(\VEC{x}_0)$.
\end{proof}

Let $\DS \Omega \subset \RR^n$ be an open subset with a
smooth boundary.
...
Choose $\DS \tilde{g} \in C^2(\Omega) \cap C(\overline{\Omega})$
...
Choose
$\DS \tilde{g} \in C^2(\Omega) \cap C^1(\overline{\Omega})$ such that
...
\]
and $\tilde{v} = N\ast \tilde{f}$.  Then
$\DS \Delta \tilde{v} = \tilde{f}$ as we have seen before.
...
Let $\Omega$ be an open subset of $\DS \RR^n$ with a
boundary of class $\DS C^2$.  Suppose that there exists a function
...
fundamental solution of $\DS \dydxn{u}{x}{2}$ because
\begin{align*}
...
$\DS \dydxn{U_x}{y}{2} = 0$ on $\Omega$, $U_x$ is continuous
on $\overline{\Omega}$, $U_x(0) = N(x,0) = N(x) = x/2$ and
$U_x(L) = N(x,L) = N(x-L) = (L-x)/2$.

Since $\DS \dydxn{U_x}{y}{2} = 0$, we get $U_x(y) = Ay + B$
...
Let $\Omega$ be any bounded open subset of $\DS \RR^n$ with
a boundary of class $\DS C^2$.  The Green's function $G$ for
$\Omega$ exists \footnotemark.  Moreover,
$\VEC{y} \mapsto G(\VEC{x},\VEC{y})$ is of class
$\DS C^\infty$ on $\overline{\Omega} \setminus \{\VEC{x}\}$.
\end{theorem}
...
$\DS u_{\VEC{x}} \in C(\overline{\Omega})$
...
is of class $\DS C^\infty$ on $\Omega \setminus \{\VEC{x}\}$.
...
with a boundary of class $\DS C^2$, then
...
$\DS C^\infty(\overline{\Omega_{\epsilon}})$.  We may therefore use
...
boundary of class $\DS C^2$ and let
$f:\Omega \rightarrow \RR$ be a function in
$\DS L^1(\Omega)$.  The solution of the Dirichlet problem
...
= \begin{cases}
\DS \max_{\VEC{y} \in \partial \Omega}
\frac{1}{(2-n)\omega_n}
...
&\leq C \equiv
\begin{cases}
\DS \frac{1}{(2-n)\omega_n d_m^{n-2}} & \quad \text{if}\ n > 2
\\[0.8em]
\DS \frac{1}{\omega_2} \max\{ |\ln(d_m)|, |\ln(d_M)| \} &
\quad \text{if}\ n = 2  
\end{cases}
\end{align*}
...
$\DS \tilde{G}_{\VEC{x}}: \RR^n \rightarrow \RR$ defined by
\[
...
for $\DS \VEC{y} \in \RR^n\setminus \{\VEC{x}\}$.
Then $\tilde{G}_{\VEC{x}} = N_{\VEC{x}} - \tilde{u}_{\VEC{x}}$
defines a distributions on $\DS \RR^n$ and
\[
...
Suppose that $\DS u \in C^1(\overline{\Omega}) \cap C^2(\Omega)$ is a
...
We have from Theorem~\ref{laplace_GreenExist} that
$\DS G_{\VEC{x}} \in C^\infty(\overline{\Omega_{\epsilon}})$.
...
$\DS \int_{\partial B_{\epsilon}(\VEC{x})}
G_{\VEC{x}}\,\pdydx{u}{\nu} \dx{S}$.
...
$\Omega$, then $u_{\VEC{x}}$ and $\DS \pdydx{u}{\nu}$
...
$\DS \ps{\Delta \tilde{G}_{\VEC{x}}}{\phi} = \phi(\VEC{x})$
...
Suppose that
$\DS u \in C^1(\overline{\Omega}) \cap C^2(\Omega)$ is the
...
$\DS G_{\VEC{x}} \in C^1(\overline{\Omega}) \cap C^2(\Omega)$.
We get
\[
...
$\DS G_{\VEC{x}}(\VEC{y}) = G(\VEC{x},\VEC{y})$ for
$\VEC{y} \in \overline{\Omega} \setminus \{\VEC{x}\}$.  The function
$\DS \VEC{y} \mapsto \pdydx{G_{\VEC{x}}}{\nu}(\VEC{y})$
...
$\DS \RR^n_+$ is defined by
$G(\VEC{x},\VEC{y}) = N(\VEC{x},\VEC{y}) - U(\VEC{x},\VEC{y})$
for $\DS \VEC{x} \in \RR_+^n$ and
$\DS \VEC{y} \in \overline{\RR^n_+}$ with
$\VEC{x} \neq \VEC{y}$, where
$U(\VEC{x},\VEC{y}) = N(\tilde{\VEC{x}}, \VEC{y})$
for $\DS \VEC{x} \in \RR_+^n$ and
$\DS \VEC{y} \in \overline{\RR^n_+}$.

\stage{i} The function
$\VEC{y} \mapsto U(\VEC{x},\VEC{y}) =
N(\tilde{\VEC{x}},\VEC{y})$ is harmonic in $\DS \RR^n_+$ and
continuous on $\DS \overline{\RR^n_+}$ because
$\DS \tilde{\VEC{x}} \not\in \overline{\RR^n_+}$.
In fact, it is harmonic in $\DS \RR^n \setminus \{\tilde{\VEC{x}}\}$.

\stage{ii} We have
$U(\VEC{x},\VEC{y}) = N(\tilde{\VEC{x}},\VEC{y}) = N(\VEC{x},\VEC{y})$ for
$\DS \VEC{y} \in \partial \RR^n_+$ because
...
\]
for $\DS \phi \in \DD(\RR^n_+)$ assuming that a function
$\DS \phi \in \DD(\RR^n_+)$ is expanded to $\DS \RR^n$
by $\phi(\VEC{y})=0$ for
$\DS \VEC{y} \in \RR^n \setminus \RR^n_+$.  Since
$\tilde{G}_{\VEC{x}}(\VEC{y}) = G_{\VEC{x}}(\VEC{y}) = G(\VEC{x},\VEC{y})$
for all $\DS \VEC{y} \in \overline{\RR^n_+}$ and
$\DS \VEC{x}\in \RR_+^n$, we get
$\Delta G_{\VEC{x}} = \delta_{\VEC{x}}$ in the sense of distribution
on $\DS \RR^n_+$ for $\DS \VEC{x}\in \RR_+^n$.
As mentioned in Remark~\ref{GreenFunctAltDefn}, this is expected from
the Green's function for $\DS \RR^n_+$.

Since our domain $\DS \RR^n_+$ is not bounded, we may not apply
...
\]
is the solution of $\Delta u = f$ in $\DS \RR^n_+$ with $u = 0$ on
$\DS \partial \RR^n_+$ if
$\DS f \in L^1(\RR^n_+)$ has compact support.  We
will not consider this problem but instead we will focus on the
Dirichlet Problem $\Delta u = 0$ in $\DS \RR^n_+$ with $u=g$ on
$\DS \partial \RR^n_+$ if
$\DS g \in C(\partial \RR^n_+)$.  More precisely, we
...
&=
\begin{cases}
\DS \frac{1}{(2-n)\omega_n} \|\VEC{x}-\VEC{y}\|^{2-n} -
\frac{1}{(2-n)\omega_n} \|\tilde{\VEC{x}}-\VEC{y}\|^{2-n} & \quad 
\text{if} \ n>2 \\[0.8em]
\DS \frac{1}{\omega_2} \ln\left(\|\VEC{x}-\VEC{y}\|\right) -
\frac{1}{\omega_2} \ln\left(\|\tilde{\VEC{x}}-\VEC{y}\|\right) & \quad 
\text{if} \ n=2
\end{cases}
\end{align*}
for $\DS \VEC{x} \in \RR^n_+$ and
$\DS \VEC{y}\in \overline{\RR^n_+}$ with
$\VEC{x} \neq \VEC{y}$.  The
...
$\DS s = \frac{r^2}{1+r^2}$ and
$\DS \dx{s} = \frac{2r}{(1+r^2)^2}\, \dx{r}$, and the
formulae $\DS \omega_n = 2\pi^{n/2}/\Gamma(n/2)$ and
$\Gamma(1/2) = \sqrt{\pi}$.
...
Suppose that $\DS g \in L^p(\partial \RR^n_+)$ \footnotemark,
for some integer $p\geq 1$.  Then
$u(\VEC{x}) = (g \ast P_{x_n})(\breve{\VEC{x}})$ for
$\DS \VEC{x} \in \RR^n_+$ is well defined and harmonic in
$\DS \RR^n_+$.
Moreover, $u_{x_n}(\breve{\VEC{x}}) \equiv u(\breve{\VEC{x}}, x_n)$ for
$\DS \breve{\VEC{x}} \in \RR^{n-1}$ satisfies
$u_{x_n} \rightarrow g$ in
$\DS L^p(\RR^{n-1})$ as $x_n \rightarrow 0$.

If $\DS g\in C(\partial \RR^n_+)$ is bounded, then $u$ can be
continuously extended to
$\DS R = \{\VEC{x}\in \RR^n : x_n \geq 0\}$ by
$u(\VEC{x}) = g(\VEC{x})$ for $\DS \VEC{x} \in \partial \RR^n_+$.
\end{theorem}

\footnotetext{In other words,
$\DS g \in L^p(\RR^{n-1})$ since
$\DS \partial \RR^n_+ \cong \RR^{n-1}$.}

\begin{proof}
\stage{i} We first show that $u$ is well defined.  We have that
$\DS P_{x_n} \in L^q(\RR^{n-1})$ for all integer $q \geq 1$.
This is a consequence of the following two facts.
$P_{x_n}$ is bounded at the origin; namely,
$\DS \left| P_{x_n}(\breve{\VEC{x}})\right| \leq 
\frac{2}{\omega_{n} x_n^{n-1}}$ for all
$\DS \breve{\VEC{x}} \in \RR^{n-1}$.
$P_{x_n}$ is $\DS L^q$-integrable away of the origin because
$\DS \left| P_{x_n}(\breve{\VEC{x}})\right| \leq
\frac{2 x_n}{\omega_{n} \|\breve{\VEC{x}}\|^n}$
for all $\DS \breve{\VEC{x}} \in \RR^{n-1}\setminus B_1(\VEC{0})$.
Hence, for all $\DS \VEC{x}\in \RR^n$,
\begin{align*}
\RR^{n-1} & \rightarrow \RR \\
\breve{\VEC{y}} & \mapsto g(\breve{\VEC{y}})\,
P_{x_n}\left(\breve{\VEC{x}} - \breve{\VEC{y}}\right)
\end{align*}
is in $\DS L^1(\RR^{n-1})$.  For instance, if $1<p<\infty$ we have
\[
...
< \infty \ .
\end{align*}
Thus $\DS u:\RR^n_+\rightarrow \RR$ is well defined.

\stage{ii} We show that $u$ is harmonic in $\DS \RR^n_+$.
The function $\VEC{x} \mapsto P_{x_n}( \breve{\VEC{x}} - \breve{\VEC{y}} )$
is of class $\DS C^\infty$ on $\DS \RR^n_+$ for all
$\DS \breve{\VEC{y}} \in \partial \RR^{n-1}$.
Choose $\delta>0$ and $b>a>0$.  Given $k\geq 0$ and a multi-index
$\DS \VEC{\alpha} \in \NN^{n-1}$, there exists $C>0$ such that
\[
...
\]
for all $\DS \breve{\VEC{y}} \in \RR^{n-1}$ and
$\DS \VEC{x} \in \RR^n_+$ such that
$\|\breve{\VEC{x}}\|\leq \delta$ and $a \leq x_n \leq b$.  Proceeding
as in (i), we can show that $\DS g h \in L^1(\RR^{n-1})$.
...
$\DS \RR^n_+$ for
$\DS \VEC{y} \in \partial \RR^n_+$, we have that
\[
\VEC{x} \mapsto -\pdydx{G}{y_n}(\VEC{x},\VEC{y})
= P_{x_n}(\breve{\VEC{x}}-\breve{\VEC{y}})
\]
is harmonic in $\DS \RR^n_+$ for
$\DS \VEC{y} \in \partial \RR^n_+$.
Hence,
\[
\Delta u(\VEC{x}) = 
...
\begin{theorem}
Suppose that $\DS g \in C(\partial \RR^n_+)$ and vanishes at infinity.
Then $u(\VEC{x}) = (g \ast P_{x_n})(\breve{\VEC{x}})$ for
$\DS \VEC{x} \in \RR^n_+$ vanishes at infinity on
$\DS R = \left\{ \VEC{x} \in \RR^n : x_n \geq 0 \right\}$.
Moreover, it is the unique solution that vanishes at infinity for the
Dirichlet problem $\Delta u=0$ on $\DS \RR^n_+$ with the
condition $u=g$ on $\DS \partial \RR^n_+$.
\end{theorem}

\begin{proof}
\stage{i} Suppose first that $g$ has a compact support inside
$\DS B_a(\VEC{0}) \subset \partial(\RR^n_+) \cong \RR^{n-1}$.  Since
$\DS g \in L^1(\RR^{n-1})$ and
$\DS \left| P_{x_n}(\breve{\VEC{x}})\right| \leq 
\frac{2}{\omega_{n} x_n^{n-1}}$
for $\DS \breve{\VEC{x}} \in \RR^{n-1}$ and $x_n >0$, we have
\begin{align*}
...
\end{align*}
uniformly for $\DS \breve{\VEC{x}} \in \RR^{n-1}$.  Moreover,
$\DS \left| P_{x_n}(\breve{\VEC{x}})\right| \leq
...
$\DS \RR^n_+$ when $g$ has a compact support.

\stage{ii} If $g$ vanished at infinity but does not have a compact
support, we choose functions $\DS g_k:\RR^{n-1}\to \RR$ with
compact supports such that $g_k \to g$ uniformly as $k \to \infty$.
For instance, just multiply $g$ by smooth functions
$\DS \psi_k:\RR^{n-1}\to \RR$ with compact support such that
$\psi(\VEC{y}) \leq 1$ for all $\DS \VEC{y} \in \RR^{n-1}$ and
$\psi(\VEC{y}) =1$ for $\VEC{y} \in B_k(\VEC{0}) \cap R$ with $k>0$.

Let $u_k(\VEC{x}) = (g_k \ast P_{x_n})(\breve{\VEC{x}})$ for
$\DS \VEC{x} \in \RR^n_+$.  We have shown in (i) that $u_k$
vanished at infinity on $\DS \RR^n_+$.  Moreover, $u_k$
converges uniformly on $\DS \RR^n_+$ to $u$ because
\begin{align*}
...
&\qquad \leq \sup_{\breve{\VEC{y}}\in \RR^{n-1}}
\big|g(\breve{\VEC{y}}) - g_k(\breve{\VEC{y}}) \big|
\, \sup_{\VEC{x}\in\RR^n_+} \left| \int_{\breve{y} \in \RR^{n-1}}
P_{x_n}(\breve{\VEC{y}}-\breve{\VEC{x}}) \dx{\breve{\VEC{y}}}\right| \\
&\qquad = \sup_{\breve{\VEC{y}}\in \RR^{n-1}}
\big|g(\breve{\VEC{y}}) - g_k(\breve{\VEC{y}}) \big|
\, \sup_{\VEC{x}\in\RR^n_+} \left| \int_{\breve{y} \in \RR^{n-1}}
...
Dirichlet problem $\Delta u=0$ on $\DS \RR^n_+$ with the
condition $u=g$ on $\DS \partial \RR^n_+$.  Suppose moreover
that they are continuous on $R$ and vanishing at infinity on $R$.
Then $w = v_1-v_2$ is a solution of $\Delta u=0$ on
$\DS \RR^n_+$ that vanishes at infinity on
$R$ and such that $w=0$ on $\DS \partial \RR^n_+$.
...
If $\DS \VEC{x}, \VEC{y} \in \RR^n$, $\VEC{x} \neq \VEC{0}$ and
$\|\VEC{y}\|=1$, then
$\DS \|\VEC{x}-\VEC{y}\|
...
\quad \text{if} \ \VEC{x} \neq \VEC{0} \\[0.8em]
\DS \frac{1}{(2-n)\omega_n} & \quad \text{if} \ \VEC{x} = \VEC{0}
\end{cases} \\
&= \begin{cases}
\DS \frac{1}{(2-n)\omega_n}
...
\text{if} \ \VEC{x} \neq \VEC{0} \\
\DS N(\VEC{y}) - \frac{1}{(2-n)\omega_n} & \quad \text{if}
\ \VEC{x} = \VEC{0}
\end{cases} \\
&= \begin{cases}
\DS \frac{1}{(2-n)\omega_n} \left( \|\VEC{x}-\VEC{y}\|^{2-n}
-\left\| \|\VEC{x}\|^{-1}\VEC{x} - \|\VEC{x}\|\VEC{y}\right\|^{2-n}
\right) & \quad \text{if} \ \VEC{x} \neq \VEC{0} \\[1em]
\DS \frac{1}{(2-n)\omega_n}\,\|\VEC{y}\|^{2-n} -
...
$\VEC{x} \rightarrow \VEC{0}$ and so $\DS
...
& \quad \text{if} \ \VEC{x} \neq \VEC{0} \\
\DS N(\VEC{y}) & \quad \text{if} \ \VEC{x} = \VEC{0}
\end{cases} \\
&= \begin{cases}
\DS \frac{1}{\omega_2} \left( \ln\left(\|\VEC{x}-\VEC{y}\|\right)
- \ln\left( \| \|\VEC{x}\|^{-1} \VEC{x}- \|\VEC{x}\| \VEC{y} \|\right) \right)
& \quad \text{if} \ \VEC{x} \neq \VEC{0} \\[1em]
\DS \frac{1}{\omega_2} \ln\left(\|\VEC{y}\|\right)
...
\]
provide that $\DS f\in L^1(B_1(\VEC{0}))$.
...
Suppose that $\DS g \in L^1(\partial B_1(\VEC{0}))$.  Then
\[
...
If we restate this formula in terms of $u$ and $g$, then we get
\[
...
{
\begin{lemma}
We have                 \label{laplaceLexistBall}
...
Since $\DS P(\VEC{0},\VEC{y}) = 1/\omega_n$ for all
...
$\DS \|r\VEC{x} - \VEC{y}\| = \|r^{-1}(r\VEC{x})- r \VEC{y}\|
= \|\VEC{x} - r \VEC{y}\|$ for $\VEC{y} \in \partial B_1(\VEC{0})$. 
Therefore, $\DS P(r\VEC{x},\VEC{y}) = P(r\VEC{y},\VEC{x})$ for
...
\]
for all $\DS \VEC{x}_0 \in \partial B_1(\VEC{0})$ and
$\DS \VEC{y} \in \partial B_1(\VEC{0})
...
$\DS 0 \leq P(r\VEC{x}_0,\VEC{y}) < \delta/M$
for all $\DS \VEC{x}_0 \in \partial B_1(\VEC{0})$,
$\DS \VEC{y} \in \partial B_1(\VEC{0}) \setminus
...
Lemma~\ref{laplaceLexistBall}.}{laplace_FIG3}
...
$\DS \VEC{\alpha} \in \NN^n$ with $|\VEC{\alpha}|\leq 2$.  Thus,
$\DS \VEC{y} \mapsto g(\VEC{y})\, D^{\VEC{\alpha}}_{\VEC{x}}
P(\VEC{x},\VEC{y})$
is bounded in absolute value by a function in
$\DS L^1(\partial B_1(\VEC{0}))$ for
$a \leq \|\VEC{x} \| \leq b$ and
$\VEC{y} \in \partial B_1(\VEC{0})$, and any multi-index
$\DS \VEC{\alpha} \in \NN^n$ with $|\VEC{\alpha}|<2$.  
...
that
$\DS |g(\VEC{y}) - g(\VEC{x})| < \epsilon/2$ for
...
\]
for all $\VEC{x} \in \partial B_1(\VEC{0})$.  We have that
$\DS |g(\VEC{y}) - g(\VEC{x})| < \epsilon/2$ for all
$\VEC{y} \in V_{\VEC{x}}$.
According to Lemma~\ref{laplaceLexistBall}, there exists $r_0<1$
...
$\DS \|g\|_{\infty} = \max_{\VEC{y}\in\partial B_1(\VEC{0})}
|g(\VEC{y})|$.
...
\dss{S}{y} + 2 \|g\|_{\infty}
\int_{\partial B_1(\VEC{0})\setminus V_{\VEC{x}}} P(r\VEC{x},\VEC{y})
\dss{S}{y} \\
...
\stage{iv}  Suppose that $\DS g\in L^p(\partial B_1(\VEC{0}))$ with
$1\leq p < \infty$.  Given $\epsilon>0$, choose
$f\in C(\partial B_1(\VEC{0}))$ such that
\begin{equation} \label{laplace_ident2}
\|g-f\|_p < \epsilon/3 \ , 
\end{equation}
where $\|\cdot\|_p$ is the $\DS L^p$-norm on
$\DS L^p(\partial B_1(\VEC{0}))$.
...
As seen in (iii), for $r$ closed enough to $1$, we have that
$\DS \| f - v_r \|_\infty < \frac{\epsilon}{3 B}$, where
$\DS B^p = \int_{\partial B_1(\VEC{0})} \dx{S}$.  Hence
...
Suppose that $\DS \Omega \subset \RR^n$ is an open set and that
$\DS u \in C^2(\Omega)\cap C^1(\overline{\Omega})$ is an
harmonic function in $\Omega$ such that $u(\VEC{x}) \geq 0$ for all
$\VEC{x} \in \Omega$.  Then, given $\VEC{x}_0 \in \Omega$ and $\rho>0$ such that
...
$\DS \RR^n$.  They will be quite useful later.  In particular, in the
...
$\DS P_k = H_k \oplus \|\VEC{x}\|_2^2 P_{k-2}$ and thus
$\DS P_k = \bigoplus_{j=0}^{\intpt{k/2}} \|\VEC{x}\|^{2j} H_{k-2j}$.
\end{prop}
...
for $\DS p = \sum_{\VEC{\alpha} \in \NN^n,\ |\VEC{\alpha}| = k}
a_{\VEC{\alpha}} \VEC{x}^{\VEC{\alpha}} \in P_k$, we set
$\DS p(\diff) = \sum_{\VEC{\alpha} \in \NN^n,\ |\VEC{\alpha}| = k}
...
\]
Hence, for
$\DS p = \sum_{\VEC{\alpha} \in \NN^n,\ |\VEC{\alpha}| = k}
a_{\VEC{\alpha}} \VEC{x}^{\VEC{\alpha}}$ and
$\DS q = \sum_{\VEC{\alpha} \in \NN^n,\ |\VEC{\alpha}| = k}
b_{\VEC{\alpha}} \VEC{x}^{\VEC{\alpha}}$, we have that\\
$\DS \psP{p}{q} = \sum_{\VEC{\alpha} \in \NN^n,\ |\VEC{\alpha}| = k}
a_{\VEC{\alpha}} b_{\VEC{\alpha}} \, \VEC{\alpha}!$.

We now show that the orthogonal complement of
$\DS \|\VEC{x}\|_2^2 P_{k-2}$
in $P_k$ is $H_k$.  Since $q \in P_k$ is in the orthogonal complement 
of $\DS \|\VEC{x}\|_2^2 P_{k-2}$ if and only if
$\DS 0 = \psP{\|\VEC{x}\|_2^2\, p}{q} =  p(\diff) \Delta q
= \psP{p}{\Delta q}$ for all $p \in P_{k-2}$, in particular for
$\DS p(\VEC{x}) = x^{\VEC{\alpha}}$ with $|\VEC{\alpha}| = k-2$,
we get that $\Delta q = 0$.  Thus $q \in H_k$.
\end{proof}

Since $\|\VEC{x}\|_2 = 1$ on
$\DS \partial B_1(\VEC{0}) \subset \RR^n$, we
get the following corollary.

\begin{cor}
If $\DS S = \partial B_1(\VEC{0}) \subset \RR^n$, then
$\DS P_k\big|_S = \bigoplus_{j=0}^{\intpt{k/2}} H_{k-2j}\big|_S$.
\end{cor}

\begin{theorem} \label{L2DecHk}
If $\DS S = \partial B_1(\VEC{0}) \subset \RR^n$, then
$\DS L^2(S) = \sum_{k=0}^\infty H_k\big|_S$, where the sum
is a direct sum with respect to the standard scalar product on
$\DS L^2(S)$.
\end{theorem}

\begin{proof}
Consider $\DS f \in L^2(S)$ and $\epsilon >0$.  Since $C(S)$
is dense in $\DS L^2(S)$, there exists a continuous function
...
\[
 ...
 = i p(\VEC{x})
\]
and similarly $\DS \pdydx{q}{\nu}(\VEC{x}) = j q(\VEC{x})$
for $\VEC{x} \in S$.  Thus $\ps{p}{q} = 0$.  Since $p$ and $q$ are
arbitrary, $H_i \perp H_j$.
\end{proof}

A basis of $P_k$ is formed of the monomials
$\DS \VEC{x}^{\VEC{\alpha}}$ with
$\DS \VEC{\alpha} \in \NN^n$ and $|\VEC{\alpha}| = k$.
Suppose that we have a line of $n+k-1$ balls.  The number of possible
...
$\DS \dim P_k = \frac{(n+k-1)!}{(n-1)!\,k!}$.  It follows
...
Let $\DS S = \partial B_1(\VEC{0}) \subset \RR^n$.
...
$\DS H_k\big|_S$ of $L^2(S)$.  From the finite dimensional
version of the Riesz Representation Theorem, there exists
$\DS p_{\VEC{x}}^{[k]}\in H_k\big|_S$ such that 
\[
  p(\VEC{x}) = \phi_{\VEC{x}}(p) = \ps{p}{p_{\VEC{x}}^{[k]}}
  = \int_S p(\VEC{y})\,p_{\VEC{x}}^{[k]}(\VEC{y}) \dss{S}{y}
\]
for all $\DS p \in H_k\big|_S$.

\begin{prop} \label{L2NormPx}
$\DS \left\| p_{\VEC{x}}^{[k]} \right\|_2 = \sqrt{d_k/\omega_n}$ and
$\DS \left\| p_{\VEC{x}}^{[k]} \right\|_\infty = d_k/\omega_n$.  
\end{prop}

\begin{proof}
\stage{i} Let $\DS R:\RR^n \to \RR^n$ be a rotation.  Using
...
\end{align*}
for all $\DS p \in H_k\big|_S$.  Since
$\DS p_{\VEC{x}}^{[k]}$ is the
unique element of $H_k\big|_S$ such that
$\DS p(\VEC{x}) = \ps{p}{p_{\VEC{x}}^{[k]}}$ for all
$\DS p \in H_k\big|_S$, we have that
$\DS p_{\VEC{x}}^{[k]} = p_{R(\VEC{x})}^{[k]}\circ R$ for
all rotation $R$.

It follows that
$\DS p_{\VEC{x}}^{[k]}(\VEC{x}) = p_{R(\VEC{x})}^{[k]}(R(\VEC{x}))$
for all $\VEC{x} \in S$.  Thus
$\DS p_{\VEC{x}}^{[k]}(\VEC{x})$ is constant for
$\VEC{x} \in S$.

\stage{ii}
Let $\DS \left\{ p_j\right\}_{j=1}^{d_k}$ be an orthonormal basis of
$\DS H_k\big|_S \subset L^2(S)$.  We note that
...
\]
We deduce that
$\DS p_{\VEC{x}}^{[k]}(\VEC{x}) = d_k/\omega_n$.
...
Let $\DS S = \partial B_1(\VEC{0}) \subset \RR^n$.
Given $\DS g \in L^2(S)$.  The function
...
convergent in $\DS L^2(S)$ for every $0 \leq r \leq 1$.
\item If we define
\[
...
$\DS \pi_k:L^2(S) \to H_k\big|_S$ is
given by $\DS \pi_k(g)(\VEC{x}) = \ps{g}{p_{\VEC{x}}^{[k]}}$
for $\VEC{x} \in S$.
According to Theorem~\ref{L2DecHk}, we have that $g = \pi_k(g) + h$
where $\DS h \in \left(H_k\big|_S\right)^\perp$.  Thus, since
$\DS p_{\VEC{x}}^{[k]} \in H_k\big|_S$, we have that
\[
...
$\DS \phi_k(\VEC{x}) = \ps{g}{p_{\VEC{x}}^{[k]}}$
...
$\DS \sum_{k=0}^\infty \phi_k$ is absolutely and uniformly
convergent on $\overline{B_r(\VEC{0})}$.  It follows from
Corollary~\ref{UnifConvHarm} that
$\DS u = \sum_{k=0}^\infty \phi_k$ is
harmonic on $B_r(\VEC{0})$.

\stage{ii} Since $\DS \phi_k = \pi_k(g)$ on $S$, it follows
from Theorem~\ref{L2DecHk} that
$\DS \sum_{k=0}^\infty \phi_k$ converges to $g$ in
$\DS L^2(S)$.  Moreover,
$\DS \|g\|_2^2 = \sum_{j=0}^\infty \left\|\phi_k\right\|_2^2 < \infty$
where $\DS \|\phi_k\|_2
= \left( \int_S |\phi_k(\VEC{x})|^2 \dss{S}{x}\right)^{1/2}$.
Since $\DS r^k\phi_k$ is in $\DS H_k\big|_S$ and
$\DS \sum_{j=0}^\infty \left\|r^k\phi_k\right\|_2^2
\leq \sum_{j=0}^\infty \left\|\phi_k\right\|_2^2 < \infty$ for
$0 \leq r \leq 1$, it follows again from Theorem~\ref{L2DecHk} that
$\DS \sum_{j=0}^\infty r^k\phi_k$ converges in $\DS L^2(S)$
for $0 \leq r \leq 1$.

\stage{iii} We have that $\DS u_r = \sum_{j=0}^\infty r^k\phi_k$
on $S$.  It follows from (ii) that $\DS u_r \in L^2(S)$ for
...
\ref{L2NormPx} that
$\DS \|p_{\VEC{x}}^{[k]}\|_\infty \leq C (1+k)^{n-2}$
for a constant $C$ \footnote{We have to use $(1+k)^{n-2}$ instead of
...
$\DS \overline{B_r(\VEC{0})}$ to interchange the sum
...
$\DS \VEC{y} \mapsto P(s\VEC{x},\VEC{y})$ and
$\DS \VEC{y} \mapsto
...
functions in $\DS \RR^n$.

\begin{theorem}[Reflection Principle] \label{laplace_refl_pr}
Let $\DS \Omega \subset \RR^n$ be an open subset, symmetric
...
$\DS \Omega_0 = \left\{ (\breve{\VEC{x}},0) \in \Omega \right\}$.
...
$\DS \|(\breve{\VEC{x}},x_n)-\VEC{x}_0\| =
\|(\breve{\VEC{x}},-x_n)-\VEC{x}_0\|$
and
$\DS \|(\breve{\VEC{x}},x_n)-(\breve{\VEC{y}},y_n)\| =
...
open subset of $\DS \RR^n$ using subharmonic functions.
...
Let $\Omega$ be an open subset of $\DS \RR^n$ and $u \in \Omega$ be a
...
Let $\Omega$ be an open subset of $\DS \RR^n$ and $u \in \Omega$ is a
subharmonic function.   Suppose that $\VEC{z} \in \Omega$
and $\overline{B_{\rho}(\VEC{z})} \subset \Omega$ for some $\rho >0$.  Then
...
\begin{lemma}
Let $\Omega$ be an open subset of $\DS \RR^n$ and $u \in C(\Omega)$.
...
Suppose that $\Omega$ is an bounded open subset of $\DS \RR^n$
and that $\DS f: \partial \Omega \to \RR$ is a continuous
...
that
$\DS m = \min_{\VEC{x} \in \partial \Omega} f(\VEC{x})
> -\infty$ and
$\DS M = \max_{\VEC{x} \in \partial \Omega} f(\VEC{x})
...
Suppose that $\DS u \in C(\overline{B_r(\VEC{x})})$
...
&\qquad = \frac{-2(z_j-x_j) \|\VEC{z}-\VEC{y}\|^n
...
$\DS \overline{B_{\rho}(\VEC{z})} \subset \Omega$,
and $0 < \eta < \rho$.

\stage{i} Given a sequence
$\DS \{\VEC{x}_j\}_{j=1}^\infty \subset B_\eta(\VEC{z})$.
By definition of $u_f$, we can find for each $j$ a sequence
$\DS \{v_{i,j}\}_{i=1}^\infty \subset \Sigma_f$ such that
$\DS \lim_{i \to \infty} v_{i,j}(\VEC{x}_j) = u_f(\VEC{x}_j)$
for $j \geq 1$.

Let $\DS v_i = \max_{1\leq j \leq i} v_{i,j}$.  It follows
from Lemma~\ref{PerronLem5} that $v_i \in \Sigma_f$.  Since
$\DS v_{i,j}(\VEC{x}_j) \leq v_i(\VEC{x}_j) \leq u_f(\VEC{x}_j)$
for all $i \geq j$ and
$\DS \lim_{i \to \infty} v_{i,j}(\VEC{x}_j) = u_f(\VEC{x}_j)$,
we get that
$\DS \lim_{i \to \infty} v_i(\VEC{x}_j) = u_f(\VEC{x}_j)$
for all $j$.
...
$\DS v_i(\VEC{x}_j) \leq v_{i,\VEC{z},\rho}(\VEC{x}_j)
\leq u_f(\VEC{x}_j)$ for all $i$ and $j$.  Thus
$\DS \lim_{i \to \infty} v_{i,\VEC{z},\rho}(\VEC{x}_j) = u_f(\VEC{x}_j)$
for all $j$.

\stage{ii} Given $\VEC{x} \in \overline{B_\eta(\VEC{z})}$, let 
$r = \rho - \|\VEC{x}-\VEC{z}\|$.  We have that
$\DS \overline{B_r(\VEC{x})} \subset \overline{B_\rho(\VEC{z})}$ and
...
$\DS \left\{ v_{i,\VEC{z},\rho} \right\}_{j=1}^\infty$ is
...
$\DS \left\{ v_{i_k,\VEC{z},\rho} \right\}_{k=1}^\infty$
...
$\DS \left\{v_{i_k,\VEC{z},\rho}\right\}_{k=1}^\infty$ is a
...
$\DS \lim_{i \to \infty} v_{i,\VEC{z},\rho}(\VEC{x}_j)
...
$\DS \{\VEC{x}_j\}_{j=1}^\infty \subset B_\eta(\VEC{z})$
such that $\VEC{x}_1 = \VEC{w}$ and $\DS \{\VEC{x}_j\}_{j=2}^\infty$
converges to $\VEC{w}$.  Since $v$ is continuous \footnote{Note that
the sequence $\DS \{v_{i,j}\}_{i=1}^\infty$ will change if
we change the sequence
$\DS \{\VEC{x}_j\}_{j=1}^\infty \subset B_\eta(\VEC{z})$ and
so may $v$.} on $\overline{B_\eta(\VEC{z})}$, we then have that
$\DS \lim_{j\to \infty} u_f(\VEC{x}_j)
...
\stage{v}
Let $\DS \{\VEC{x}_j\}_{j=1}^\infty$ be a dense sequence in 
...
$\DS \{\VEC{x}_j\}_{j=1}^\infty$, we have that
...
a manifold of class $\DS C^2$.  Using Perron's method to
solve the Dirichlet problem, we may impose less restrictive conditions
on the boundary of $\Omega$.

\begin{defn}[Barrier Condition]
The boundary $\partial \Omega$ of an open subset
$\DS \Omega \subset \RR^n$
...
$\DS C^2$ satisfies the barrier condition.   For each
$\VEC{z} \in \partial \Omega$, there exist $\VEC{w}$ on the line
$\DS \{ \VEC{z} + t\nu(\VEC{z})) : t > 0 \}$
and $s>0$ small enough such that
$\DS \overline{B_s(\VEC{w})} \cap \overline{\Omega} = \{\VEC{z}\}$
because $\partial \Omega$ is a
$\DS C^2$-manifold \footnote{This can be
...
g_{\VEC{z}}(\VEC{x})
= \begin{cases}
\DS \left\| \frac{1}{s}\,(\VEC{x}-\VEC{w}) \right\|^{2-n} - 1
...
$g_{\VEC{z}}$ is harmonic on $\DS \RR^n\setminus \{\VEC{w}\}$.
We have also by construction that
$\DS |g_{\VEC{z}}(\VEC{x})| < 0$ for
...
used to demonstrate that $\partial \Omega$ of class
$\DS C^2$ satisfies the barrier condition.}{Barrier1}

There are sets $\DS \Omega \subset \RR^n$ that do not have a
boundary of class $\DS C^2$ but satisfy the barrier
condition.  The simplest examples that come to mind are the sets
$\Omega$ which are strictly convex.
Therefore, for each point $\VEC{z} \in \partial \Omega$, there exists an
hyperplane
$\DS P_{\VEC{z}} = \left\{ \VEC{y} \in \RR^n :
\ps{\VEC{y}}{\VEC{w}} = 0 \right\}$ for some $\VEC{w}$ such that
$(\VEC{z} + P_{\VEC{z}}) \cap \overline{\Omega} = \{\VEC{z}\}$.
We may assume that $\VEC{w}$ is a unit vector pointing outside of $\Omega$.
Then $\DS g_{\VEC{z}}:\RR^n \to \RR$ defined by
...
open subset of $\DS \RR^n$ and satisfies the barrier condition.  

\begin{lemma}
$\DS \liminf_{\VEC{x}\to \VEC{z}} u_f(\VEC{x}) \geq f(\VEC{z})$
...
$\DS b_g = \max_{\VEC{x} \in \partial \Omega \setminus B_\delta(\VEC{z})}
...
$\partial \Omega \setminus B_\delta(\VEC{z})$, we have that
$\DS b_f = \min_{\VEC{x} \in \partial \Omega \setminus B_\delta(\VEC{z})}
f(\VEC{x}) > -\infty$.
...
\end{align*}
Since $\DS g_{\VEC{z}} \in C(\overline{\Omega})$
is subharmonic on $\Omega$, so is $u_\epsilon$.  Moreover,
$\DS u_\epsilon(\VEC{x}) \leq f(\VEC{z}) - \epsilon \leq f(\VEC{x})$
...
$\DS \lim_{\VEC{x}\to \VEC{z}} u_f(\VEC{x}) = f(\VEC{z})$
for all $\VEC{z} \in \partial \Omega$.
\end{lemma}

\begin{proof}
It follows from the previous lemma that we only have to prove that
$\DS \limsup_{\VEC{x}\to \VEC{z}} u_f(\VEC{x}) \leq f(\VEC{z})$
for all $\VEC{z} \in \partial \Omega$.

We have that
$\DS u_{-f}(\VEC{x}) = \sup_{u \in \Sigma_{-f}} u(\VEC{x})
...
$\DS u+v \in C(\overline{\Omega})$ is subharmonic
...
If $\DS \Omega \subset \RR^n$ is a bounded open set such that
$\partial \Omega$ satisfies the barrier condition, and if
$\DS f:\partial \Omega \to \RR$ is a continuous function, then there
exists a unique function
$\DS u \in C(\overline{\Omega})$ which is
...
If we substitute $u(x,y) = F(x)G(y)$ in (\ref{laplace_spv_A}), then we get
\[
...
$\DS F(x) = A e^{\sqrt{-k}\,x} + B e^{-\sqrt{-k}\, x}$.
Moreover, $A+B=0$ from $F(0)=0$ and 
$\DS A e^{a\sqrt{-k}} + B e^{-a\sqrt{-k}} = 0$ from $F(a)=0$.
...
$B \sin\left(a\sqrt{k}\right) = 0$.  If we take $B=0$, then we get the
null solution.  We must therefore have
$\sin\left(a\sqrt{k}\right) = 0$.  This implies that
$\DS k = k_n \equiv \left(n\pi/a\right)^2$ for any integer $n>0$.
The boundary value problem
(\ref{laplace_spv_C}) has non-null solutions only for
$\DS k = k_n = \left(n\pi/a\right)^2$ with
...
With $\DS k = k_n = \left(n\pi/a\right)^2$, the second
...
$\DS C\,(A\,e^{(k\,x)} + B\,e^{(-k\,x)})=0$ for $0<x<3$.
Thus $C=0$ because the expression between parentheses cannot be null
for all $x$ if we want to get a non-null solution.  The boundary
condition $u(x,7) = 0$ then gives
$\DS D (A\,e^{(k\,x)} + B\,e^{(-k\,x)})\sin(7k) = 0$.  Since $D$ and
...
From $\DS u(0,y) = 3\sin\left(4\pi y/7\right)$, we
get
\[
...
$\DS g'(y) = \sin\left(n\pi y/7\right)$.  Hence,
$f'(y) = 7-2y$ and
$\DS g(y) = -7/(n\pi)\,\cos\left(n\pi y/7\right)$.
We get
\begin{align*}
a_n\,e^{3n\pi /7} + b_n\,e^{-3n\pi /7}
...
$\DS g'(y) = \cos\left(n\pi y/7\right)$.  Hence,
$f'(y) = -2$ and $\DS g(y) =
...
$\DS a_4 = \frac{-3 e^{-12\pi/7} }{e^{12\pi/7}-e^{-12\pi/7}}
= \frac{-3 e^{-12\pi/7} }{2\sinh(12\pi/7)}$
and \\
$\DS b_4 = \frac{3\,e^{12\pi/7}}{e^{12\pi/7}-e^{-12\pi/7}}
= \frac{3\,e^{12\pi/7}}{2\sinh(12\pi/7)}$.
...
\]
From $u(x,0) = 0$, we get
$\DS C\,\left(A e^{kx} + B e^{-kx}\right)=0$ for
$-L<x<L$.  Since we are looking for non-null solutions, we must have
$C=0$ because $\DS A e^{kx} + B e^{-kx}$ cannot be equal to
$0$ for all $x$.  From $u(x,L) = 0$, we then get 
$\DS D\sin(kL)\left(A e^{kx} + B e^{-kx}\right)=0$ for $-L<x<L$.
...
From $\DS u(-L,y) = 50 \sin\left(\pi y/(2L)\right)$, we get
\[
\sum_{n=1}^\infty \sin\left(\frac{n\pi y}{L}\right)
...
From $\DS u(L,y) = 50 \sin\left(\pi y/(2L)\right)$,
we get
\[
\sum_{n=1}^\infty \sin\left(\frac{n\pi y}{L}\right)
\left(a_n e^{n\pi} + b_n e^{-n\pi} \right) =
...
\left( \frac{100}{(2n-1)\pi} + \frac{100}{(2n+1)\pi}\right) \ ,
\end{align*}
where we have used $\DS \sin\left(\frac{(2n-1)\pi}{2}\right)
...
\end{equation}
for $n > 0$.
From $\DS u(x,b) = 0.5\sin\left(7\pi x/a\right)$, we
get
\[
\sum_{n=1}^\infty \sin\left(\frac{n\pi x}{a}\right)
...
The solution of this system is
$\DS
a_7 = -b_7 =\frac{1}{\DS 2\left( e^{7\pi b/a} - e^{-7\pi b/a}\right)}
...
$\DS D\sin(kb)\left(A e^{kx} + B e^{-kx}\right)=0$ for
$0<x<a$.  Hence, $D \neq 0$ and $\sin(kb)=0$ because we are looking
for non-null solutions.  Therefore, $k = k_n \equiv n\pi/b$ for any
integer $n>0$.
...
From $\DS u(0,y) = 0.7 \sin\left(2\pi y/b\right)$,
we get
\[
\sum_{n=1}^\infty \sin\left(\frac{n\pi y}{b}\right)
...
$\DS g'(y) = \sin\left(n\pi y/b\right)$.  Thus,
$f'(y) = b-2y$ and
$\DS g(y) = -b/(n\pi)\,\cos\left(n\pi y/b\right)$.
We get
\begin{align*}
a_n e^{n\pi a/b} + b_n e^{-n\pi a/b}
...
$\DS g'(y) = \cos\left(n\pi y/b\right)$.  Thus,
$f'(y) = -2$ and $\DS g(y) =
...
\end{align*}
The solution of this system is
$\DS a_2 = \frac{-0.7\,e^{-2\pi a/b}}{2\sinh(2\pi a/b)}$
and $\DS b_2 = \frac{0.7\,e^{2\pi a/b}}{2\sinh(2\pi a/b)}$.
...
polynomial $\DS p:\RR^2\to \RR$ of degree two such that
$\Delta p = 0$ with
...
$\DS p(x,y) = a_{2,2}(x^2-y^2) + a_{1,1}xy + a_{1,0}x
+ a_{0,1}y + a_{0,0}$.
...
\]
with the boundary conditions
$\DS \pdydx{u}{x}(0,y) = \pdydx{u}{x}(\pi,y) = 0$
for $0<y<\pi$, and
$\DS \pdydx{u}{y}(x,0) =0$ and
$\DS \pdydx{u}{y}(x,\pi) = x - \frac{\pi}{2}$
for $0<x<\pi$.
...
\end{equation}
where
$\DS R = \{ (x,y) : 0 \leq y \leq \pi \ \text{and}
\ 0 \leq x \leq \pi\}$
...
\subI{Separation of Variables}
If we substitute $u(x,y) = F(x)G(y)$ in $\Delta u = 0$, then we get
\[
...
conditions.   From $\DS \pdydx{u}{x}(0,y) = 0$, we get
$\DS \dydx{F}{x}(0)G(y)=0$.  Since we are looking for
non-null solutions, $G$ must be non-null and we get
$\DS \dydx{F}{x}(0)=0$.  Similarly,
from $\DS \pdydx{u}{x}(\pi,y) = 0$, we
get $\DS \dydx{F}{x}(\pi)G(y)=0$.  Again, since we are
...
\]
However, $\DS \dydx{F}{x}(0)=0$ implies that $A - B=0$ and
$\DS \dydx{F}{x}(\pi)=0$ implies
$\DS A e^{\pi\sqrt{k}} - B e^{-\pi\sqrt{k}} = 0$.
The only solution of the system
\[
\begin{pmatrix}
...
However, both $\DS \dydx{F}{x}(0)=0$ and
$\DS \dydx{F}{x}(\pi)=0$ imply that $B_0=0$.  Hence, $A_0$
...
\]
However, we get $B =0$ from $\DS \dydx{F}{x}(0)=0$.  It then
follows from $\DS \dydx{F}{x}(\pi)=0$ that\\
$-A \sqrt{-k} \sin\left(\pi\sqrt{-k}\right) = 0$.  If we take $A=0$,
then we get the null solution.  We must therefore have
$\sin\left(\pi\sqrt{-k}\right) = 0$ with $k>0$.  This implies
that $\DS k = k_n \equiv -n^2$ for $n>0$.  The boundary value
problem (\ref{laplace_spv_egg1_bvpX}) has non-null 
solutions only for $\DS k=k_n = -n^2<0$ with any integer
$n>0$, and the solutions associated are of the form
$\DS F(x)=F_n(x) \equiv A_n \cos(nx)$ for $n > 0$.
...
$G_0(y) \equiv C_0 y + D_0$ and, for $\DS k= -n^2$ with
$n>0$, we get the general solution
$\DS G_n(y) \equiv C_n e^{ny} + D_n e^{-ny}$.

The simple functions obtained by separation of variables are
$\DS u_0(x,y) \equiv a_0 y + b_0$ and
$\DS u_n(x,y) \equiv \cos(n x)\left(a_n e^{n y} + b_n e^{-n y}\right)$
...
\]
we get from $\DS \pdydx{u}{y}(x,0) = 0$ that
\[
...
$\DS g(x) = (1/n) \sin(nx)$.  We end up with
\begin{align}
n a_n e^{n \pi} -n b_n e^{-n \pi}
...
=
\begin{cases}
\DS \frac{-2}{n^3\,\pi \sinh(n\pi)} & \quad \text{for $n$ odd} \\
0 & \quad \text{for $n$ even}
\end{cases}
\]
...
multiply by $\DS r^2$, then we get
\[
...
We have \\
$\DS \dydx{H}{\phi}(\phi) = \dydx{H}{w}(w)\, \dydx{w}{\phi}(\phi)
= -\sin(\phi) \dydx{H}{w}(w)$
and so
$\DS \frac{1}{\sin(\phi)} \dydx{H}{\phi}(\phi) = -\dydx{H}{w}(w)$
...
\]
If we expand this equation, then we get the Legendre equation
\[
...
substitute $\DS F(r) = r^\alpha$ in
(\ref{laplace_spv_pe_sphc1}), then we get the characteristic equation
...
$\DS F(r) = A r^{\alpha_1} + B r^{\alpha_2}$.  If there is
only one root $\alpha$ of algebraic multiplicity two, then the general
solution of (\ref{laplace_spv_pe_sphc1}) is given by
$\DS F(r) = A r^{\alpha} + B r^{\alpha}\ln(r)$.

If we substitute $k=n(n+1)$ in (\ref{laplace_spv_pe_sphc1}), then we get
...
$\DS u(R_2,\phi)= 3 \cos^2(\phi) + \cos(\phi) -1$.  Find the potential
$u(r,\phi)$:
...
\ .
\]
We find $\DS A_2 = -2 (27(9/64-16/27))^{-1}= 128/781$ and\\
$\DS B_2 = 18 (9/64 - 16/27)^{-1}=-31104/781$.   For all the
other values of $n$, $A_n = B_n = 0$.  The potential between the
spheres $S_1$ and $S_2$ is given by
\[
...
$\DS \VEC{v}\in \RR^n$ such that $A\VEC{v} = \lambda \VEC{v}$.
The vector $\VEC{v}$ is called an eigenvector associated to $\lambda$.

The same concept may be applied to the linear differential operator
$\Delta$.  Suppose that $\Omega$ is a open set in $\DS \RR^n$ with a
...
$\DS u \in C^2(\Omega)\cap C(\overline{\Omega})$ such that
...
\subI{Separation of Variables}
If we substitute $u(x,y) = F(x)G(y)$ in (\ref{laplace_e_eigenC}), then
we get
\[
...
For $k>0$, we get
$\DS G(y) = C \cos(\sqrt{k}\,y) + D \sin(\sqrt{k} \,y)$.
The boundary condition $G(0)=0$ forces $C$ to be $0$.  The boundary
condition $G(b) = 0$ implies that
$\DS k = k_n \equiv \left(n\pi/b\right)^2$ for all integers $n>0$
...
$\DS \lambda = \lambda_{m,n} \equiv \left(m\pi/a\right)^2 +
...
(\ref{laplace_e_eigenP}), then we get
\[
...
$\DS G(\theta) = A e^{\sqrt{-k}\theta} + B e^{-\sqrt{-k}\theta}$.
...
If $k>0$, the general solution of (\ref{laplace_e_eigenP2}) is of the form
$\DS G(\theta) = A \cos\left(\sqrt{k}\theta\right)
...
$\DS k = k_n = n^2$ for all integers $n > 0$.  We get the
non-null solution
\[
...
$\DS F(r) = C_n J_n\left(\sqrt{\lambda}\, r\right) + D_n
...
$\DS J_n\left(\sqrt{\lambda}\, R\right)= 0$.  Thus,
$\DS \lambda = \lambda_{m.n} \equiv \left(\alpha_{m,n}/R\right)^2$,
where $\alpha_{m,n}$ is the $\DS m^{th}$ zero of the Bessel
function $J_n$.  We get the non-null solutions
\[
...
$n>0$ because $\DS J_n\left(\alpha_{0,n}\,r/R\right) = J_n(0) = 0$
for all $r$.

We have found non-null solutions only for
$\DS \lambda = \lambda_{m.n} = \left(\alpha_{m,n}/R\right)^2$
...
$\DS \lambda_{m.n} = \left(\alpha_{m,n}/R\right)^2$ 
with $m>0$ for $n>0$ and $m\geq 0$ for $n=0$.
...
Note that for the smallest eigenvalue
$\DS \lambda_{0,0} = (\alpha_{0,0}/R)^2$,
the eigenfunctions are multiple of
$\DS J_0\left(\alpha_{0,0}\,r/R\right)$.  Thus, the
...
\]
We seek $\lambda$ and $\DS u \in C^2(\Omega)$ such that
...
(\ref{laplace_e_eigenS}), then we get 
...
\end{equation}
After multiplying both sides by
$\DS \frac{\sin^2(\phi)}{G(\theta)H(\phi)}$, we get
\[
...
We now solve (\ref{laplace_e_eigenS2}) with $\DS k=m^2$
for $m\geq 0$.  Namely, we consider
...
$0<\phi<\pi$.
We have that
$\DS \dydx{V}{\phi}(\phi) = \dydx{V}{w}(w)\, \dydx{w}{\phi}(\phi)
= -\sin(\phi) \dydx{V}{w}(w)$ and
$\DS \sin(\phi) \dydx{V}{\phi}(\phi) = - \sin^2(\phi) \dydx{V}{w}(w)
...
\]
We note that $\DS \dydxn{P_n}{w}{m}(w) =0$ for all $w$ when
...
$\DS H(\phi) = H_{m,n}(\phi) \equiv P_n^m(\cos(\phi))$.

We have found non-null solutions only for
$\DS \lambda = \lambda_n = n(n+1)$.  The solutions
...
$\DS \cos(m\theta)P_n^m(\cos(\phi))$ for $0 \leq m \leq n$
and
$\DS \sin(m\theta)P_n^m(\cos(\phi))$ for $1 \leq m \leq n$.
...
\end{align*}
After multiplying both sides by
$\DS \frac{r^2}{F(r)G(\theta)H(\phi)}$, we get
\begin{align*}
...
the form
$\DS G(\theta) H(\phi) = \cos(m\theta) P_n^m(\cos(\phi))$ for
$0 \leq m \leq n$ and
$\DS G(\theta) H(\phi) = \sin(m\theta) P_n^m(\cos(\phi))$
for $0 \leq m \leq n$.
...
Let $\DS F(r) = r^{-1/2}\,\rho(r)$.  We have that
$\DS \dydx{F}{r} = -\frac{1}{2}\,r^{-3/2} \, \rho(r) +
r^{-1/2}\,\dydx{\rho}{r}(r)$ and\\
$\DS \dydxn{F}{r}{2} = \frac{3}{4}\,r^{-5/2} \, \rho(r) -
r^{-3/2}\,\dydx{\rho}{r}(r) + r^{-1/2}\,\dydxn{\rho}{r}{2}$.
So, If we substitute $\DS F(r) = r^{-1/2}\,\rho(r)$
in (\ref{laplace_e_eigenB1}) and simplify, then we get
\[
r^2 \dydxn{\rho}{r}{2} + r \dydx{\rho}{r} + \left(\lambda r^2 -
  \left( n + \frac{1}{2}\right)^2\right)\rho = 0 \  .
\]
With $\DS z = \lambda^{1/2} r$, we have
$\DS
\dydx{V}{r} = \dydx{V}{z} \, \dydx{z}{r} = \lambda^{1/2} \dydx{V}{z}$
and \\
$\DS \dydxn{V}{r}{2} = \dfdx{ \left( \dydx{V}{r}\right) }{r} =
\lambda^{1/2} \dfdx{ \left(\lambda^{1/2} \dydx{V}{z} \right) }{z}
= \lambda \dydxn{V}{z}{2}$
for any twice differentiable function $V$.  Hence, with the
substitution $\DS z=\lambda^{1/2} r$, the previous ordinary
differential equation becomes
\[
z^2 \dydxn{\rho}{z}{2} + z \dydx{\rho}{z} + \left( z^2 - 
\left( n + \frac{1}{2}\right)^2\right)\rho = 0 \  .
\]
This is a Bessel Equation of order $\DS n + 1/2$.  Its
general solution is
$\DS \rho(z) = A J_{n+1/2}(z) + B J_{-n-1/2}(z)$.
...
$\DS 0 = F(R) = A R^{-1/2} J_{n+1/2}\left(\sqrt{\lambda}\, R\right)$.
Thus, $\DS \lambda = \lambda_{j,n} \equiv
\left(\alpha_{j,n+1/2}/R\right)^2$ for $j \geq 1$, where
$\alpha_{j,n+1/2}$ is the $\DS j^{th}$ root of the Bessel
function $J_{n+1/2}$.

We have shown that (\ref{laplace_e_eigenB}) has bounded solutions only when
$\DS \lambda = \lambda_{j,n} =
...
$\DS r^{-1/2} J_{n+1/2}\left(\alpha_{j,n+1/2} r/R\right)
