...
$\displaystyle u \in C^\infty(\Omega)$ and $\Delta u = 0$ on $\Omega$.
\end{theorem}

\begin{proof}
\stage{i} We first prove that $u$ is of class $\displaystyle C^\infty(\Omega)$.

Choose $\psi \in \DD(]-1,1[)$ such that
\begin{equation} \label{laplace_int1}
\int_{\RR^n} \phi(\VEC{x}) \dx{\VEC{x}} = 1 \ ,
\end{equation}
where $\displaystyle \phi(\VEC{x}) = \psi(\|\VEC{x}\|^2)$ for
...
\psi(r^2) r^{n-1} \dx{r} \ ,
\end{align*}
where we have use the fact that $\displaystyle \phi(r\VEC{y}) = \psi(r^2)$ for
...
$\displaystyle \VEC{y} = r^{-1} (\VEC{z}-\VEC{x})$, we get
\begin{align*}
...
harmonic functions on an open set $\displaystyle \Omega \subset \RR^n$.
If the sequence $\displaystyle \left\{ u_j \right\}_{j=1}^\infty$
...
Since
$\displaystyle D \subset D_R = \{ \VEC{y} \in \RR^n :
...
Let $\Omega$ be a bounded open subset of $\displaystyle \RR^n$.  If
...
Let $\Omega$ be an open subset of $\displaystyle \RR^n$ with compact
...
\end{cases}
\]
Note that $N_\epsilon$ is of class $\displaystyle C^{\infty}$ on
$\displaystyle \RR^n$.

We first prove that $N_\epsilon \rightarrow N$ in $\displaystyle L^1(K)$ as
$\epsilon \rightarrow 0$ for any compact set $\displaystyle K \subset \RR^n$.
It is obvious that
...
$\displaystyle \|\VEC{x}\|^2+\epsilon^2 \leq 1$, then
$\displaystyle (1/2) |\ln(\|\VEC{x}\|^2 + \epsilon^2)|  
...
We now prove that $N_\epsilon \rightarrow N$ in $\displaystyle \DD'(\RR^n)$ as
$\epsilon \rightarrow 0$.  If $\displaystyle \phi \in \DD(\RR^n)$,
let $K = \supp \phi$.
$K$ is a compact set by definition of $\displaystyle \DD(\RR^n)$.  Hence
\begin{align*}
...
$\displaystyle L^1(K)$ as $\epsilon \rightarrow 0$ and
$\displaystyle \sup_{\VEC{x}\in K} \left|\phi(\VEC{x})\right|$ is bounded.
...
If $\displaystyle u\in \DD'(\RR^n)$ satisfies
$\displaystyle \Delta u = f \in C^\infty(\RR^n)$,
then $\displaystyle u \in C^\infty(\RR^n)$.  In particular, we recover the
result that harmonic functions on $\displaystyle \RR^n$ are of class
...
Let $\Omega$ be an open subset of $\displaystyle \RR^n$ with a
boundary of class $\displaystyle C^2$.  Suppose that there exists a function
...
Suppose that
$\displaystyle u \in C^1(\overline{\Omega}) \cap C^2(\Omega)$ is the
...
$\displaystyle G_{\VEC{x}} \in C^1(\overline{\Omega}) \cap C^2(\Omega)$.
We get
\[
...
$\displaystyle \VEC{x} \in \RR^n_+$ is well defined and harmonic in
$\displaystyle \RR^n_+$.
Moreover, $u_{x_n}(\breve{\VEC{x}}) \equiv u(\breve{\VEC{x}}, x_n)$ for
$\displaystyle \breve{\VEC{x}} \in \RR^{n-1}$ satisfies
...
$\displaystyle \|r\VEC{x} - \VEC{y}\| = \|r^{-1}(r\VEC{x})- r \VEC{y}\|
...
$\displaystyle \VEC{\alpha} \in \NN^n$ with $|\VEC{\alpha}|\leq 2$.  Thus,
...
Suppose that $\displaystyle \Omega \subset \RR^n$ is an open set and that
$\displaystyle u \in C^2(\Omega)\cap C^1(\overline{\Omega})$ is an
...
$\displaystyle \RR^n$.  They will be quite useful later.  In particular, in the
...
$\displaystyle P_k = H_k \oplus \|\VEC{x}\|_2^2 P_{k-2}$ and thus
...
a manifold of class $\displaystyle C^2$.  Using Perron's method to
...
$\displaystyle C^2$ satisfies the barrier condition.   For each
...
$\displaystyle C^2$-manifold \footnote{This can be
...
$g_{\VEC{z}}$ is harmonic on $\displaystyle \RR^n\setminus \{\VEC{w}\}$.
...
$\displaystyle C^2$ satisfies the barrier condition.}{Barrier1}

There are sets $\displaystyle \Omega \subset \RR^n$ that do not have a
boundary of class $\displaystyle C^2$ but satisfy the barrier
...
hyperplane
$\displaystyle P_{\VEC{z}} = \left\{ \VEC{y} \in \RR^n :
...
Then $\displaystyle g_{\VEC{z}}:\RR^n \to \RR$ defined by
...
If $\displaystyle \Omega \subset \RR^n$ is a bounded open set such that
$\partial \Omega$ satisfies the barrier condition, and if
$\displaystyle f:\partial \Omega \to \RR$ is a continuous function, then there
...
$\displaystyle C\,(A\,e^{(k\,x)} + B\,e^{(-k\,x)})=0$ for $0<x<3$.
...
$\displaystyle D (A\,e^{(k\,x)} + B\,e^{(-k\,x)})\sin(7k) = 0$.  Since $D$ and
...
\]
From $u(x,0) = 0$, we get
$\displaystyle C\,\left(A e^{kx} + B e^{-kx}\right)=0$ for
$-L<x<L$.  Since we are looking for non-null solutions, we must have
$C=0$ because $\displaystyle A e^{kx} + B e^{-kx}$ cannot be equal to
$0$ for all $x$.  From $u(x,L) = 0$, we then get 
$\displaystyle D\sin(kL)\left(A e^{kx} + B e^{-kx}\right)=0$ for $-L<x<L$.
...
\]
From $u(0,y) = 0$, we get
$\displaystyle A\left(C e^{ky} + D e^{-ky}\right)=0$ for
$0<y<b$.  Thus $A=0$ because we are looking for non-null solutions.
From $u(a,y) = 0$, we then get
$\displaystyle B\sin(ka)\left(C e^{ky} + D e^{-ky}\right)=0$ for
...
$\displaystyle D\sin(kb)\left(A e^{kx} + B e^{-kx}\right)=0$ for
...
polynomial $\displaystyle p:\RR^2\to \RR$ of degree two such that
$\Delta p = 0$ with
...
$\displaystyle p(x,y) = a_{2,2}(x^2-y^2) + a_{1,1}xy + a_{1,0}x
+ a_{0,1}y + a_{0,0}$.
...
that $\displaystyle k = k_n \equiv -n^2$ for $n>0$.  The boundary value
problem (\ref{laplace_spv_egg1_bvpX}) has non-null 
solutions only for $\displaystyle k=k_n = -n^2<0$ with any integer
...
$G_0(y) \equiv C_0 y + D_0$ and, for $\displaystyle k= -n^2$ with
$n>0$, we get the general solution
$\displaystyle G_n(y) \equiv C_n e^{ny} + D_n e^{-ny}$.
...
multiply by $\displaystyle r^2$, then we get
\[
...
substitute $\displaystyle F(r) = r^\alpha$ in
...
$\displaystyle u(R_2,\phi)= 3 \cos^2(\phi) + \cos(\phi) -1$.  Find the potential
$u(r,\phi)$:
...
\ .
\]
We find $\displaystyle A_2 = -2 (27(9/64-16/27))^{-1}= 128/781$ and\\
$\displaystyle B_2 = 18 (9/64 - 16/27)^{-1}=-31104/781$.   For all the
...
$\displaystyle \VEC{v}\in \RR^n$ such that $A\VEC{v} = \lambda \VEC{v}$.
...
$\Delta$.  Suppose that $\Omega$ is a open set in $\displaystyle \RR^n$ with a
...
$\displaystyle u \in C^2(\Omega)\cap C(\overline{\Omega})$ such that
...
characteristic equation $\displaystyle \rho^2 +(\lambda-k_n)=0$.
...
where $\alpha_{m,n}$ is the $\displaystyle m^{th}$ zero of the Bessel
...
$\displaystyle \lambda_{m.n} = \left(\alpha_{m,n}/R\right)^2$ 
with $m>0$ for $n>0$ and $m\geq 0$ for $n=0$.
...
$\displaystyle \lambda_{0,0} = (\alpha_{0,0}/R)^2$,
...
\]
We seek $\lambda$ and $\displaystyle u \in C^2(\Omega)$ such that
...
$\displaystyle k = k_m \equiv m^2$ and
\[
...
We now solve (\ref{laplace_e_eigenS2}) with $\displaystyle k=m^2$
...
\]
With $\displaystyle z = \lambda^{1/2} r$, we have
$\displaystyle
...
substitution $\displaystyle z=\lambda^{1/2} r$, the previous ordinary
...
$\alpha_{j,n+1/2}$ is the $\displaystyle j^{th}$ root of the Bessel
function $J_{n+1/2}$.
