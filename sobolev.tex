...
\section{The $\DS \mathbf{W^{k,p}}$
spaces} \label{sobolev_def}

\begin{defn} \label{sob_1st_wkp}
Let $\Omega$ be an open subset of $\DS \RR^n$.  For $k\in \NN$ and
$p$ a positive integer or $\infty$, the
{\bfseries Sobolev space}\index{Sobolev Space!$\DS W^{k,p}(\Omega)$}
$\DS W^{k,p}(\Omega)$ is the space defined by
\[
W^{k,p}(\Omega) = \left\{ f \in L^p(\Omega) : \diff^{\VEC{\alpha}} f \in
L^p(\Omega) \ , \ \VEC{\alpha} \in \NN^n \ \text{and}
\ |\VEC{\alpha}| \leq k \right\} \ ,
\]
where the derivatives $\DS \diff^{\VEC{\alpha}}$ are derivatives in
the sense of distributions.
\end{defn}

We can define a norm on $\DS W^{k,p}(\Omega)$ by
\begin{equation} \label{sob_norm_wkp}
\| f \|_{k,p} =
\begin{cases}
\DS \left( \sum_{|\VEC{\alpha}|\leq k} \|
\diff^{\VEC{\alpha}} f \|_p^p \right)^{1/p}&
\quad \text{if} \quad 0 < p < \infty \\
\DS \max_{|\VEC{\alpha}|\leq k} \| \diff^{\VEC{\alpha}} f \|_\infty &
\quad \text{if} \quad p = \infty
\end{cases}
\end{equation}
for $\DS f \in W^{k,p}(\Omega)$, where $\|\cdot\|_p$ is the
$\DS L^p$-norm on $\DS L^p(\Omega)$.
We will sometime use the notation $\|f\|_{p,\Omega}$ and
$\|f\|_{k,p,\Omega}$ when we need to clearly identify the domain to be
considered for the norm.

Note that
$\DS \|f\|_\infty = \esssup_{\VEC{x} \in \Omega} |f(\VEC{x})|$.
However, to simplify the notation, we will only write
$\DS \|f\|_\infty = \sup_{\VEC{x} \in \Omega} |f(\VEC{x})|$
...
$\DS W^{k,p}(\Omega)$ equipped with
the norm (\ref{sob_norm_wkp}) is a Banach space.  If $p \neq \infty$,
$\DS W^{k,p}(\Omega)$ is also separable.
\end{theorem}

\begin{proof}
Let $\DS \left\{ \VEC{\alpha}_1, \VEC{\alpha}_2, \ldots,
\VEC{\alpha}_N \right\}$,
where $\DS N = \sum_{|\VEC{\alpha}|\leq k} 1$, be an ordering of
the multi-indices $\DS \VEC{\alpha} \in \NN^n$ such that
...
\end{align*}
This is an isometry if the norm on $\DS (L^p(\Omega))^N$ is
the norm
\[
\| \VEC{f} \|_{p} =
\begin{cases}
\DS \left( \sum_{j=0}^N \| f_{\VEC{\alpha}_j} \|_p^p \right)^{1/p} &
\quad \text{if} \ 0 < p < \infty \\
\DS \max_{1\leq j \leq N} \| f_{\VEC{\alpha}_j} \|_\infty &
\quad \text{if} \ p = \infty
\end{cases}
\]
for $\DS \VEC{f} \in (L^p(\Omega))^N$.

\stage{i}  We consider the case $1<p<\infty$ and leave the cases $p =1$ and
$p=\infty$ to the reader.

We prove that $\DS W^{k,p}(\Omega)$ is complete
by proving that $T$ is a closed mapping. Suppose that
$\DS \left\{f_j \right\}_{j=1}^\infty \subset W^{k,p}(\Omega)$
is such that $\DS \left\{T(f_j) \right\}_{j=1}^\infty$
converges to $\VEC{g}$ in $\DS (L^p(\Omega))^N$.  Then
$\DS \left\{ \diff^{\VEC{\alpha}_i} f_j \right\}_{j=1}^\infty$
converge to $g_{\VEC{\alpha}_i}$ in $\DS L^p(\Omega)$ for
$1\leq i \leq N$.  In particular, for $\VEC{\alpha}_1$, we have that
$\DS \left\{ f_j \right\}_{j=1}^\infty$
converges to $g \equiv g_{\VEC{\alpha}_1}$ in $\DS L^p(\Omega)$.

Since $\DS L^p$ convergence implies convergence in the
sense of distributions, we have that
$\DS g_{\VEC{\alpha}_i} = \diff^{\VEC{\alpha}_i} g$ almost
...
$\DS \int_\Omega g_{\VEC{\alpha}_i}(\VEC{x}) \phi(\VEC{x}) \dx{\VEC{x}}$
...
\stage{ii} To prove that $\DS W^{k,p}(\Omega)$ is separable for
$0\leq p < \infty$, it is enough to note that
$\DS T : W^{k,p}(\Omega) \rightarrow T\left(W^{k,p}(\Omega)\right)$ is
an isometry and that $\DS T\left(W^{k,p}(\Omega)\right)$ is
a subset of the separable space $\DS (L^p(\Omega))^N$.
\end{proof}
...
$\DS W^{k,2}(\RR^n)$ for $0\leq k \leq \infty$.

\begin{theorem} \label{sob_wk2_wk2}
For $k\in \NN$, $\DS f \in W^{k,2}(\RR^n)$ if and only if
$\DS \hat{f} \in L^2(\RR^n,\rho)$, where the measure $\rho$
is defined by
$\DS \dx{\rho(\VEC{x})} = (1+\|\VEC{x}\|_2^2)^k\dx{\VEC{x}}$.
...
\]
for $\DS \VEC{y} \in \RR^n$..

\stage{i.a} Since
$\DS \left| \VEC{y}^{\VEC{\alpha}} \right| \leq 1$
for $\|\VEC{y}\|_2 \leq 1$, and
$\DS \left| \VEC{y}^{\VEC{\alpha}} \right| \leq
...
\]
for $\DS \VEC{y} \in \RR^n$, where
$\DS C_2 = \sum_{|\VEC{\alpha}|\leq k} 1$.
...
\end{equation}
Since $\DS \sum_{|\VEC{\alpha}|= k}
\left|\VEC{y}^{2\VEC{\alpha}}\right|$ is 
continuous on the compact set $\{\VEC{y} : \|\VEC{y}\|_2=1\}$, the
minimum is reached at a point $\VEC{w} \neq \VEC{0}$ in $\DS \RR^n$.
If $w_j \neq 0$, then $\DS \left| w_j^{2k}\right| >0$ and thus
$\DS C_1' = \sum_{|\VEC{\alpha}|= k}
\left|\VEC{w}^{2\VEC{\alpha}}\right| >0$.
Given $\VEC{y} \neq \VEC{0}$, let $\VEC{z} = \|\VEC{y}\|_2^{-1} \VEC{y}$.
Since $\|\VEC{z}\|_2 = 1$, we get from
$\DS \sum_{|\VEC{\alpha}|= k} \left|\VEC{z}^{2\VEC{\alpha}}\right| \geq
C_1' >0$ that
$\DS \sum_{|\VEC{\alpha}|= k} \left|\VEC{y}^{2\VEC{\alpha}}\right|
\geq C_1' \|\VEC{y}\|_2^{2k}$
because $\DS \sum_{|\VEC{\alpha}|= k}
\left|\VEC{z}^{2\VEC{\alpha}}\right|$
is an homogeneous function of degree $2k$.  We have
\begin{align*}
...
$\DS (\diff^{\VEC{\alpha}} u)^\wedge
= (i \VEC{y})^{\VEC{\alpha}} \hat{u}$ in the
sense of distributions for all $\DS u \in \SS'(\RR^n)$ and
multi-index $\VEC{\alpha}$.
Therefore, $\DS (\diff^{\VEC{\alpha}} u)^\wedge \in L^2(\RR^n)$
if and only if
$\DS (i \VEC{y})^{\VEC{\alpha}} \hat{u} \in L^2(\RR^n)$.  It follows
from Plancherel theorem that there exists $\DS g \in L^2(\RR^n)$
such that $\DS (\diff^{\VEC{\alpha}} f)^\wedge = \hat{g}$ in
the sense of distributions if and only if
$\DS (i \VEC{y})^{\VEC{\alpha}} \hat{f} \in L^2(\RR^n)$.
But $\DS (\diff^{\VEC{\alpha}} f)^\wedge = \hat{g}$ in the sense of
distributions means that
$\DS (\diff^{\VEC{\alpha}} f)(\hat{\phi}) = g(\hat{\phi})$
for all $\phi \in \SS(\RR^n)$.  From
Proposition~\ref{distr_frr_tempT}, this means that
$\DS (\diff^{\VEC{\alpha}} f)(\phi) = g(\phi)$
for all $\phi \in \SS(\RR^n)$.  Therefore,
$\DS \diff^{\VEC{\alpha}} f = g \in L^2(\RR^n)$ in the sense of
distributions.
Thus, the two norms describe the same space $\DS W^{k,2}(\RR^n)$.

Moreover, since
$\DS (\diff^{\VEC{\alpha}} f)^\wedge(\VEC{y})
=(i\VEC{y})^{\VEC{\alpha}} \hat{f}(\VEC{y})$
for $\DS \VEC{y}\in \RR^n$ and
$\DS f \in W^{k,2}(\RR^n)$, we get from Plancherel theorem that
\[
...
\]
Thus, it follows from (i) that $\DS C_1 \|f\|_{k,\rho}^2
\leq \|f\|_{k,2}^2 \leq C_2 \|f\|_{k,\rho}^2$.
\end{proof}

Hence, we can expand the definition of
$\DS W^{k,2}(\RR^n)$ from $k \in\NN$ to $k\in \RR$ as follows.

\begin{defn} \label{sob_2nd_wk2}
For $s\in \RR$, the
{\bfseries Sobolev space}\index{Sobolev Space!$W^{s,2}(\RR^n)$}
$\DS W^{s,2}(\RR^n)$ is the space defined by
\[
W^{s,2}(\RR^n) = \left\{ f \in L^2(\RR^n) : \hat{f} \in
  L^2(\RR^n,\rho) \right\} \ ,
\]
where the measure $\rho$ is defined by
$\DS \dx{\rho(\VEC{x})} = (1+\|\VEC{x}\|^2)^s\dx{\VEC{x}}$.
\end{defn}

If follows from Theorem~\ref{sob_wk2_wk2} that the space
$\DS W^{k,2}(\RR^n)$ for $k \in \NN$ is an Hilbert space
equipped with the scalar product
\begin{equation} \label{sob_sp_wkmu}
\ps{f}{g}_{k,\rho} = \int_{\RR^n} \hat{f}(\VEC{y})\, \overline{\hat{g}(\VEC{y})}
\dx{\rho(\VEC{y})}
\end{equation}
for $\DS f,g \in W^{k,2}(\RR^n)$.

A direct consequence of Definition~\ref{sob_2nd_wk2} is the following
proposition.

\begin{prop} \label{sob_DinDm}
For $s\in \RR$, $\DS f\in W^{s,2}(\RR^n)$ if and only if
$\DS \diff^{\VEC{\alpha}} f \in W^{s-|\VEC{\alpha}|,2}(\RR^n)$ for all
multi-indices $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq s$.  Moreover,
$\DS \diff^{\VEC{\alpha}}$ is a bounded operator
from $\DS W^{s,2}(\RR^n)$ to
$\DS W^{s-|\VEC{\alpha}|,2}(\RR^n)$ for $|\VEC{\alpha}|\leq s$.  To be
more precise
$\DS \|\diff^{\VEC{\alpha}} f\|_{s-|\VEC{\alpha}|,\rho}
\leq \|f\|_{s,\rho}$ for $\DS f\in W^{s,2}(\RR^n)$.
\end{prop}

\begin{proof}
Note that the result is trivial if $s$ is a non-negative integer
because it is obvious that
$\DS \|\diff^{\VEC{\alpha}} f\|_{s-|\VEC{\alpha}|,2} \leq
\|\diff^{\VEC{\alpha}} f\|_{s,2}$ for all $\DS f \in W^{s,2}(\RR^n)$
by definition of the norm $\|\cdot\|_{s,2}$.

For $\DS f \in W^{s,2}(\RR^n)$ and a multi-index $\VEC{\alpha}$
such that $|\VEC{\alpha}| \leq s$, we have
\begin{align}
...
$\DS f\in W^{s,2}(\RR^n)$ implies
$\DS \diff^{\VEC{\alpha}} f \in W^{s-|\VEC{\alpha}|,2}(\RR^n)$
for all multi-indices $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq s$.

Moreover, (\ref{sob_DinDmEQ}) proves that
$\DS \diff^{\VEC{\alpha}}$ is a bounded operator
of norm less than $1$ from $\DS W^{s,2}(\RR^n)$ to
$\DS W^{s-|\VEC{\alpha}|,2}(\RR^n)$ for $|\VEC{\alpha}|\leq s$.

Conversely, if $\DS \diff^{\VEC{\alpha}} f \in
W^{s-|\VEC{\alpha}|,2}(\RR^n)$ for
all multi-indices $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq s$, then
$\DS f\in W^{s,2}(\RR^n)$ if we take $\VEC{\alpha} = (0,0,\ldots,0)$.
\end{proof}

\subsection{Dense Subsets}

\begin{defn}
An open set $\DS \Omega \subset \RR^n$ satisfies
...
$\DS U_{\VEC{x}} \subset \RR^n$ of $\VEC{x}$ and an
element $\DS \VEC{y} \in \RR^n$ such that
...
$\DS \bigcup_{0\leq \delta \leq 1} \big(\overline{\Omega}\cap U +
\delta \VEC{y} \big)$.}{SEGMPROP}

If $\DS \Omega \subset \RR^n$ is an open set with a bounded
boundary of class $\DS C^2$, then $\Omega$ has the segment
...
simple example is a square in $\DS \RR^2$.
...
$\DS \RR^n$, $k\in \NN$ and $p$ is a positive integer.
Moreover, suppose that $\DS f \in W^{k,p}(\Omega_1)$ has a
...
\end{cases}
\]
Then $\DS \underline{f} \in W^{k,p}(\Omega_2)$.  In particular,
$\DS \diff^{\VEC{\alpha}} \underline{f}
...
\footnote{
To obtain such a function $\psi$, let $\DS \{V_j\}_{j=1}^J$
...
$\DS \{\psi_j\}_{j=1}^J$ for $\supp f$
subordinate to $\DS \{V_j\}_{j=1}^J$ and set
$\DS \psi = \sum_{j=1}^J \psi_j$. \label{FOOTNOTE1}}.
...
$\DS \underline{\diff^{\VEC{\alpha}} f}$ defines a
distribution in $\DS L^p(\Omega_2)$ for $|\VEC{\alpha}|\leq k$.
Hence,
\begin{align*}
...
equality comes from $\DS f \in W^{k,p}(\Omega_1)$ and
$\psi \phi \in \DD(\Omega_1)$.
Thus $\DS \diff^{\VEC{\alpha}} \underline{f}
= \underline{\diff^{\VEC{\alpha}} f}$
almost everywhere in $\DS \RR^n$.
\end{proof}

It is also interesting to note that, as a distribution,
$\DS \diff^{\VEC{\alpha}} \underline{f}\,\big|_{\Omega_1}
= \diff^{\VEC{\alpha}} f$.

\begin{lemma} \label{sob_denselem4}
Suppose that $\Omega$ is an open subset of $\DS \RR^n$,
...
Let $\DS \underline{f}:\RR^n \to \RR$ be the function defined by
\[
\underline{f}(\VEC{x}) =
\begin{cases}
...
\end{cases}
\]
Then $\DS \underline{f} \in W^{k,p}(V)$, where
$\DS V = \RR^n \setminus S$.
In particular,
$\DS \diff^{\VEC{\alpha}} \underline{f}
= \underline{\diff^{\VEC{\alpha}} f}$
...
for $|\VEC{\alpha}|\leq k$, we get that
$\DS \underline{\diff^{\VEC{\alpha}} f}$ defines a
distribution included in $\DS L^p(V)$ for $|\VEC{\alpha}|\leq k$.
...
Choose a function $\DS \psi \in \DD(\RR^n)$ such that
$\psi(\VEC{x}) = 1$ for $\VEC{x} \in K$ and
$\psi(\VEC{x}) = 0$ for
$\DS \VEC{x} \in \RR^n \setminus U$ \footnote{See
...
from $\DS f \in W^{k,p}(\Omega)$ and $\psi \phi \in \DD(\Omega)$.
Since $\phi \in \DD(V)$ and $|\alpha|\leq k$ are arbitrary,
we get that $\DS \diff^{\VEC{\alpha}} \underline{f}
= \underline{\diff^{\VEC{\alpha}} f}$
...
Given $\DS \psi \in \DD(\RR^n)$ such that
$\psi(\VEC{x}) = 0$ for $\|\VEC{x}\|>1$ and
$\DS \int_{\RR^n} \psi(\VEC{x})\dx{\VEC{x}} = 1$ \footnotemark, let
$\DS \psi_\epsilon(\VEC{x}) = \epsilon^{-n} \psi(\epsilon^{-1}\VEC{x})$
for $\DS \VEC{x} \in \RR^n$ and $\epsilon > 0$.
Let $\DS \underline{f}:\RR^n \to \RR$ be the function defined by
\[
\underline{f}(\VEC{x}) =
\begin{cases}
...
\end{cases}
\]
Then, $\DS \underline{f} \ast \psi_{\epsilon} \in C^\infty(\RR^n)$
for all $\epsilon$
and $\DS \left(\underline{f} \ast \psi_{\epsilon}\right)\big|_U \in
...
$\DS \underline{f} \ast \psi_{\epsilon} \in \DD(\RR^n)$ for
all $\epsilon$.  In particular, $\DS \supp (\underline{f} \ast
...
\psi(\VEC{x}) =
\begin{cases}
\DS ke^{-1/(1-\|\VEC{x}\|^2)} & \quad \text{for} \quad \|\VEC{x}\|<1 \\
0 & \quad  \text{for} \quad \|\VEC{x}\|\geq 1
\end{cases}
\]
where $\DS k^{-1} = \int_{\|\VEC{x}\|<1} e^{-1/(1-\|\VEC{x}\|^2)}
\dx{\VEC{x}}$.}

\begin{proof}
We have that $\underline{f}$ is a distribution on $\DS \RR^n$ because
$\DS \underline{f} \in L^p(\RR^n)$.
It follows from Proposition~\ref{distr_smooth_convol} that 
$\DS \underline{f} \ast \psi_{\epsilon} \in C^\infty(\RR^n)$.

Given $|\VEC{\alpha}|\leq k$, since
$\DS \underline{\diff^{\VEC{\alpha}} f} \in L^p(\RR^n)$ because
$\DS \supp\, \underline{\diff^{\VEC{\alpha}} f} \subset
\supp f \subset \Omega$ and
$\DS \diff^{\VEC{\alpha}} f \in L^p(\Omega)$, we get from
Theorem~\ref{distr_sp_young} that
$\DS \underline{\diff^{\VEC{\alpha}} f} \ast
\psi_\epsilon \in L^p(\RR^n)$.

For $\epsilon < \dist{\partial \Omega}{U}$, $\phi \in \DD(U)$ and
$|\VEC{\alpha}|\leq k$, we have
\begin{align*}
...
\dx{\VEC{x}} \\
&= (-1)^{|\VEC{\alpha}|} \int_{\RR^n} \left( \int_{\RR^n}
\underline{f}(\VEC{y}-\VEC{x})\, \diff^{\VEC{\alpha}} \phi(\VEC{x}) \dx{\VEC{x}}
\right) \psi_\epsilon (\VEC{y}) \dx{\VEC{y}} \\
&= (-1)^{|\VEC{\alpha}|} \int_{B_\epsilon(\VEC{0})}
\left( \int_{U} f(\VEC{y}-\VEC{x})\,
\diff^{\VEC{\alpha}} \phi(\VEC{x}) \dx{\VEC{x}}
\right) \psi_\epsilon (\VEC{y}) \dx{\VEC{y}} \\
&= \int_{B_\epsilon(\VEC{0})} \left( \int_{U} \diff^{\VEC{\alpha}}
...
&= \int_U \left( \int_{\RR^n} \underline{\diff^{\VEC{\alpha}} f}
(\VEC{y}-\VEC{x})\,
\psi_\epsilon (\VEC{y}) \dx{\VEC{y}} \right) \phi(\VEC{x}) \dx{\VEC{x}}
...
$\DS \underline{f}(\VEC{y} - \VEC{x}) = f(\VEC{y}-\VEC{x})$
and\\
$\DS \underline{\diff^{\VEC{\alpha}} f}(\VEC{y} - \VEC{x})
...
\end{align*}
Since this is true for all $|\VEC{\alpha}| \leq k$, we get that
$\DS \left\| \underline{f} \ast \psi_\epsilon - f \right\|_{p,k,U}$
as $\DS \epsilon \to 0^+$.

If $f$ has a compact support, then
$\DS \underline{f} \ast \psi_\epsilon$ has a compact support
because
$\DS \supp (\underline{f} \ast \psi_\epsilon)
...
$\DS f \in W^{k,p}(\Omega)$.
Given $\DS \VEC{y} \in \RR^n$, suppose that there exists
...
$\DS f_\delta \in W^{k,p}(U)$ for $0 \leq \delta < d$ and
$f_\delta \to f_0 = f$ in $\DS W^{k,p}(U)$ as
$\DS \delta \to 0^+$.
\end{lemma}

\begin{proof}
\stage{i}
We first note that if $\DS g \in L^p(\Omega)$, then
$\DS g_\delta \in L^p(U)$ for $0 \leq \delta <d$ where
...
$\DS g = \diff^{\VEC{\alpha}} f$.  Since this is true for all
multi-indices $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq k$, we get that
$\DS f_\delta \in W^{k,p}(U)$.

\stage{ii}
We fix the multi-index $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq k$.
Given $\epsilon >0$, since $\DD(\Omega)$ is dense in
$\DS L^p(\Omega)$, there exists $\phi \in \DD(\Omega)$
such that
$\DS \|\diff^{\VEC{\alpha}} f - \phi\|_{p,\Omega} < \epsilon/3$.
We then have that
$\DS \|\diff^{\VEC{\alpha}} f - \phi\|_{p,U} \leq
\|\diff^{\VEC{\alpha}} f - \phi\|_{p,\Omega} < \epsilon/3$
and
$\DS \|\diff^{\VEC{\alpha}} f_\delta - \phi_\delta \|_{p,U}
...
$\DS Q^p = \int_U \dx{\VEC{x}}$.  This implies that
...
$\DS \delta \to 0^+$.  Since this is true for all
multi-indices $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq k$, we get that
$\DS \|\diff^{\VEC{\alpha}} f_\delta
...
If $\DS \Omega \subset \RR^n$ satisfies the segment property, then 
$\DS \DD(\RR^n)\big|_{\Omega} =
\left\{ \phi\big|_{\Omega} : \phi \in \DD(\RR^n) \right\}$
is dense in $\DS W^{k,p}(\Omega)$ with respect to the norm
$\|\cdot\|_{k,p,\Omega}$.
\end{theorem}

\begin{rmk}
Since elements of $\DS W^{k,p}(\RR^n)$ are equivalence class
of functions which are equal almost everywhere, the statement
$\DS \DD(\RR^n) \subset W^{k,p}(\RR^n)$ has a special meaning.
When view as an element of $\DS W^{k,p}(\RR^n)$, 
a function $\DS \phi \in \DD(\RR^n)$ is associated to its
equivalence class in $\DS W^{k,p}(\RR^n)$.  We will regularly make
this type of interpretation from now on.
\end{rmk}

\begin{proof}
\stage{i} We first shown that every member of
$\DS W^{k,p}(\Omega)$ can be approximated by members of
$\DS W^{k,p}(\Omega)$ with bounded support.

Choose $\DS \phi\in \DD(\RR^n)$ such that
$\phi(\VEC{x}) = 1$ for $\|\VEC{x}\|_2 \leq 1$ and
$\phi(\VEC{x}) = 0$ for $\|\VEC{x}\|_2 > 2$, let $M$ be a constant such
that $\DS |\diff^{\VEC{\alpha}}\phi(\VEC{x})| \leq M$ for all
$\DS \VEC{x} \in \RR^n$ and
$|\VEC{\alpha}| \leq k$.

Given $\delta >0$, let $\DS \phi_\delta \in \DD(\RR^n)$ be
the function defined
$\DS \phi_\delta(\VEC{x}) = \phi\left(\delta \VEC{x}\right)$
for $\DS \VEC{x} \in \RR^n$.  For $\delta \leq 1$, we have that
$\DS |\diff^{\VEC{\alpha}} \phi_\delta(\VEC{x})| \leq
\delta^{|\VEC{\alpha}|} M \leq M$ for all
$\DS \VEC{x} \in \RR^n$ and $|\VEC{\alpha}| \leq k$.

Given $\DS f \in W^{k,p}(\Omega)$, let
$f_\delta = \phi_\delta f$ on $\Omega$.  We have that $f_\delta$ has a bounded
support; namely,
$\DS \phi_\delta f \subset \Omega \cap B_{2/\delta}(\VEC{0})$.
Moreover, $\DS \phi_\delta f \in W^{k,p}(\Omega)$ because
\[
...
and $\DS \binom{\VEC{\alpha}}{\VEC{\beta}} = 
...
\]
because $\DS \diff^{\VEC{\alpha}}f \in L^p(\Omega)$ for all
$|\VEC{\alpha}| \leq k$.  Hence, we have shown that every member of
$\DS W^{k,p}(\Omega)$ can be approximated by members of
$\DS W^{k,p}(\Omega)$ with bounded support.

\stage{ii} From now on, we assume that $\DS f \in W^{k,p}(\Omega)$
has a bounded support.  Let
$\DS K = \supp f \subset \overline{\Omega}$.
For each bounded open set $U_{\VEC{x}}$ provided in the definition of
segment property, choose an open set
$V_{\VEC{x}}$ such that
$\DS \overline{V_{\VEC{x}}} \subset U_{\VEC{x}}$.
Let
$\DS Q = K \setminus \bigcup_{\VEC{x} \in \partial \Omega} V_{\VEC{x}}$.
We have that $Q \subset \Omega$ because
$\DS \partial \Omega \subset \bigcup_{\VEC{x}
...
We get an open cover $\DS \left\{U\right\} \cup
\big\{ \bigcup_{\VEC{x} \in \partial \Omega} V_{\VEC{x}} \big\}$ of $K$.
Being closed and bounded, $K$ is compact.  So, there exists a finite
subcover of $K$ that we denote $\DS \left\{ V_i \right\}_{i=1}^I$.
At most one of the $V_i$ may have an empty intersection with $\partial \Omega$;
namely, the open set $U$ introduced before.

Let $\DS \left\{ \phi_i \right\}_{i=1}^I$ be a partition
of unity of $K$ subordinate to $\DS \left\{ V_i \right\}_{i=1}^I$
and let $\DS f_i = \phi_i f$.  We have that
$\DS f_i \in W^{k,p}(\Omega)$ with
$\supp f_i \subset V_i \cap \overline{\Omega}$.
Given $\epsilon >0$, if we can find $\DS \xi_i \in \DD(\RR^n)$
such that $\DS \|f_i - \xi_i\|_{k,p,\Omega} < \epsilon/I$ for all $i$,
then we will have that $\DS \xi = \sum_{i=1}^I \xi_i \in \DD(\RR^n)$
and $\DS \|f - \xi\|_{k,p,\Omega}
\leq \sum_{i=1}^I \|f_i - \xi_i\|_{k,p,\Omega} < \epsilon$.  Because
$\epsilon$ is arbitrary, this will prove that
$\DS \DD(\RR^n)\big|_{\Omega}$ is dense in
$\DS W^{k,p}(\Omega)$.

\stage{iii}
Consider $\DS \underline{f}_i : \RR^n \to \RR$ defined by
\[
...
$\DS \underline{f}_i \in W^{k,p}(\RR^n)$
and from\\
Lemma~\ref{sob_denselem2} (with $\Omega$ and $U$ replaced by
$\DS \RR^n$ and $\Omega$ respectively) that
$\DS \underline{f}_i \ast \psi_\epsilon \in \DD(\RR^n)$ and
\[
...
for $\epsilon$ small enough to get that
$\DS \left\|\xi_i - f_i\right\|_{k,p,\Omega}
= \left\| \underline{f}_i \ast \psi_\epsilon - f_i \right\|_{k,p,\Omega}
< \epsilon/I$.

We now suppose that $S = \supp f_i \cap \partial \Omega \neq \emptyset$.
It follows from Lemma~\ref{sob_denselem4} that
$\DS \underline{f}_i \in W^{k,p}(V)$, where
$\DS V = \RR^n \setminus S$.
...
$\DS \underline{f}_{i,\delta} \in W^{k,p}(V-\delta \VEC{y}_i)$ by
...
$\DS (V_i \cap \partial \Omega) - \delta \VEC{y}_i
\subset U_i \setminus \overline{\Omega}$ for $0 < \delta < d$.
Thus
$\DS S - \delta \VEC{y}_i \subset U_i \setminus \overline{\Omega}$ for
...
$\DS \underline{f}_{i,\delta} \to \underline{f}_i = f_i$
in $\DS W^{k,p}(\Omega)$ as $\delta \to 0$.
So, there exists $\delta_i$ such that
$\DS \left\|\underline{f}_{i,\delta_i} - f_i \right\|_{k,p,\Omega}
< \epsilon/(2I)$.
...
$\DS \underline{f}_{i,\delta_i} \ast \psi_\epsilon \to
\underline{f}_{i,\delta_i}$ in $\DS W^{k,p}(\Omega)$ 
as $\epsilon \to 0$.  Choose $\xi_i$ to be
$\underline{f}_{i,\delta_i} \ast \psi_\epsilon$
for $\epsilon$ small enough to get that
$\DS \left\|\underline{f}_{i,\delta_i} - \xi_i\right\|_{k,p,\Omega}
< \epsilon/(2I)$.
Then
$\DS \|f_i - \xi_i\|_{k,p,\Omega} \leq
...
$\DS W^{k,2}(\Omega)$ for $k$ a positive integer.

\begin{egg}
Let $k$ be a positive integer, and $p$ be a positive integer or $\infty$. 
If $\DS \Omega \subset \RR^n$ is a bounded open set, then
$\DD(\Omega)$ is not dense in $\DS W^{k,p}(\Omega)$ with
...
\end{align*}
where $\DS u \in C^\infty(\overline{\Omega})$ is a
...
$\DS T(v)=0$ for all $v\in \DD(\Omega)$.
However, $\DS u \in W^{k,p}(\Omega)$ because $\Omega$ is bounded and
$\DS u \in C^\infty(\overline{\Omega})$.  Thus,
$T$ is not the trivial functional because $T(u) >0$.

If $\DD(\Omega)$ was dense in $\DS W^{k,p}(\Omega)$, we
would have that $T=0$ on $\DS W^{k,p}(\Omega)$ because $T=0$ on
$\DD(\Omega)$.  But this is not the case.
\end{egg}

The following lemma is a special case of Lemma~\ref{sob_denselem2}
with $\DS \Omega = U = \RR^n$ if we assume that the distance
...
Given $\DS \psi \in \DD(\RR^n)$ such that
$\psi(\VEC{x}) = 0$ for $\|\VEC{x}\|>1$ and
$\DS \int_{\RR^n} \psi(\VEC{x})\dx{\VEC{x}} = 1$, let
$\DS \psi_\epsilon(\VEC{x}) = \epsilon^{-n} \psi(\epsilon^{-1}\VEC{x})$
for $\DS \VEC{x} \in \RR^n$ and $\epsilon > 0$.
Then
$\DS f\ast \psi_{\epsilon} \in W^{k,p}(\RR^n) \cap C^\infty(\RR^n)$
for all
$\epsilon$ and $\DS \| \left( f \ast \psi_{\epsilon} \right)
- f\|_{k,p} \rightarrow 0$ as $\DS \epsilon \rightarrow 0^+$.

If $f$ has a compact support, then
$\DS f \ast \psi_{\epsilon} \in \DD(\RR^n)$ for all $\epsilon$;
more precisely,
...
Theorem~\ref{distr_sp_young}, that
$\DS \diff^{\VEC{\alpha}} f \ast \psi_\epsilon \in L^p(\RR^n)$ for all
$|\VEC{\alpha}|\leq k$ because $\DS f \in W^{k,p}(\RR^n)$ implies that
$\DS \diff^{\VEC{\alpha}} f \in L^p(\RR^n)$ for all
$|\VEC{\alpha}|\leq k$.
Moreover, from Proposition~\ref{distr_comm_der}, we have that
$\DS \diff^{\VEC{\alpha}}(f \ast \psi_\epsilon) = \diff^{\VEC{\alpha}} f
\ast \psi_\epsilon$ in the sense of distributions.  Thus
$\DS f \ast \psi_\epsilon \in W^{k,p}(\RR^n)$.
...
$\DS \| (f \ast \psi_\epsilon) - f \|_{k,p} \rightarrow 0$
as $\epsilon \rightarrow 0^+$.

It follows from Proposition~\ref{distr_smooth_convol} that
$\DS f \ast \phi_{\epsilon} \in C^\infty(\RR^n)$ for all $\epsilon>0$.
...
$\DS \Omega = \RR^n$ and if we assume that a set with an
...
For $k\in \NN$ and $0 < p < \infty$, $\DS \DD(\RR^n)$ is dense in
$\DS W^{k,p}(\RR^n)$ with respect to the norm $\|\cdot\|_{k,p}$.
\end{prop}

\begin{proof}
As in the proof of Theorem~\ref{DDRRnDWkp}, we have that every member of
$\DS W^{k,p}(\RR^n)$ can be approximated by members of
$\DS W^{k,p}(\RR^n)$ with bounded support.

We now show that every member of $\DS W^{k,p}(\RR^n)$ with
compact support can be approximated by members of $\DS \DD(\RR^n)$.

Suppose that $\DS f \in W^{k,p}(\RR^n)$ has compact
support.  Choose $\DS \phi \in \DD(\RR^n)$ such that\\
$\DS \int_{\RR^n} \phi(\VEC{x}) \dx{\VEC{x}} = 1$
and set
$\DS \phi_\epsilon(\VEC{x}) = \epsilon^{-n}\phi(\epsilon^{-1} \VEC{x})$
for all $\DS \VEC{x} \in \RR^n$.
It follows from Proposition~\ref{distr_smooth_convol} that
$\DS f \ast \phi_{\epsilon} \in C^\infty(\RR^n)$ for all $\epsilon>0$.
Since both $f$ and $\phi_\epsilon$ have compact supports, 
$f \ast \phi_{\epsilon}$ has compact support.  Thus,
$\DS f \ast \phi_{\epsilon} \in \DD(\RR^n)$.  Moreover, from 
...
\quad \text{as} \quad \epsilon \to 0^+
\]
in $\DS L^p(\RR^n)$ for all $|\VEC{\alpha}| \leq k$.  Thus,
$f \ast \phi_{\epsilon} \to f$ in $\DS W^{k,p}(\RR^n)$ as
$\DS \epsilon \to 0^+$.
\end{proof}

The next theorem is a version of Proposition~\ref{DDRRnDWkpV2} for
$\DS W^{s,2}(\RR^n)$ with $s$ real.  Its proof is remarkably simple.

\begin{theorem} \label{DDRRnDWs2}
For $s\in \RR$, $\DS \DD(\RR^n)$ is dense in
$\DS W^{s,2}(\RR^n)$ with
...
transform is a homeomorphism from $\DS \SS(\RR^n)$ onto
itself.  It is also clear that
$\DS \phi \mapsto (1+\|\VEC{y}\|_2^2)^{s/2} \phi$ is a
homeomorphism from $\DS \SS(\RR^n)$ onto itself.  Thus,
$\DS T^{[s]}$ is a homeomorphism from $\DS \SS(\RR^n)$
onto itself.

Since $\DS \SS(\RR^n)$ is dense in $\DS L^2(\RR^n)$
according to the norm
$\DS \|g\|_2 = \left(\int_{\RR^n} |g(\VEC{x})|^2 \dx{\VEC{x}}
\right)^{1/2}$
for $\DS g \in L^2(\RR^n)$, it
follow from the isometry $\DS T^{[s]}$ that
$\DS \SS(\RR^n)$ is dense in $\DS W^{s,2}(\RR^n)$.

Moreover, $\DS \DD(\RR^n)$ is dense in
$\DS \SS(\RR^n)$ according to the norm
$\|\cdot\|_{s,\rho}$.  To prove this, choose $\DS \phi \in \DD(\RR^n)$
such that $|\phi(\VEC{x})| \leq 1$ for all $\DS \VEC{x} \in \RR^n$,
$\phi(\VEC{x}) = 1$ for $\|\VEC{x}\|<1$ and
$\phi(\VEC{x}) = 0$ for $\|\VEC{x}\|>2$.  Let
$\DS \phi_\delta(\VEC{x}) = \phi\left(\delta \VEC{x}\right)$
for all $\DS \VEC{x} \in \RR^n$ and $\delta >0$.  If
$\DS \psi \in \SS(\RR^n)$, then
$\DS \psi_\delta = \psi \phi_\delta \in \DD(\RR^n)$ and
$\psi_\delta - \psi \to 0$ as $\DS \delta \to 0^+$ in
$\DS L^2(\RR^n)$
follows from the Lebesgue Dominate Convergence Theorem
since $\psi_{\delta}(\VEC{x}) \to \psi(\VEC{x})$ for all
$\DS \VEC{x} \in \RR^n$ as $\DS \delta \to 0^+$ and
...
$P(\psi_\delta - \psi) \to 0$ in $\DS \SS(\RR^n)$ as $\delta \to 0^+$
according to the norm $\|\cdot\|_2$.
This implies that
$\DS \| \psi - \psi_\delta \|_{s,\rho}
= \left\| P(\psi - \psi_\delta) \right\|_2 \to 0$ as $\delta \to 0^+$.
\end{proof}

\begin{prop} \label{sob_T_dd_wkp}
Let $\DS \Omega \subset \RR^n$ be an open set and
$\phi \in \DD(\Omega)$.
...
$\DS \RR^n$.  Then,
$\DS W^{k,p}(\Omega_2)\big|_{\Omega_1} \subset W^{k,p}(\Omega_1)$.  More
precisely, if $\DS f \in W^{k,p}(\Omega_2)$, then
$\DS f\big|_{\Omega_1} \in W^{k,p}(\Omega_1)$ and
$\DS \diff^{\VEC{\alpha}} \left(f\big|_{\Omega_1}\right) =
\left( \diff^{\VEC{\alpha}} f \right)\big|_{\Omega_1}$ for
$|\VEC{\alpha}| \leq k$.
\end{lemma}

\begin{proof}
Given $\DS f \in W^{k,p}(\Omega_2)$, we have that
$\DS (\diff^{\VEC{\alpha}} f)\big|_{\Omega_1} \in L^p(\Omega_1)$ for
all multi-indices $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq k$ because
$\DS \int_{\Omega_1} |\diff^{\VEC{\alpha}} f|^p \dx{\VEC{x}} \leq 
\int_{\Omega_2} |\diff^{\VEC{\alpha}} f|^p \dx{\VEC{x}} < \infty$
for all $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq k$.  In
particular, this implies that 
$\DS f\big|_{\Omega_1}$ defines a proper distribution on $\Omega_1$.

Moreover, for $|\VEC{\alpha}| \leq k$, we have
\begin{align*}
...
Let $\Omega$ be an open subset of $\DS \RR^n$, $k\in \NN$
and $0<p<\infty$.
Then $\DS C^\infty(\Omega) \cap W^{k,p}(\Omega)$ is dense in
$\DS W^{k,p}(\Omega)$.
\end{theorem}
...
is dense in $\DS W^{k,p}(\Omega)$.  Consider
$\DS f\in W^{k,p}(\Omega)$ and $\delta>0$.

\stage{i} Let $\DS \Omega_{-1} = \Omega_0 = \emptyset$ and
\[
\Omega_j = \left\{ \VEC{x} \in \Omega : \|\VEC{x}\| < j \ \text{and} \ 
\dist{\VEC{x}}{\partial \Omega} > \frac{1}{j} \right\}
\]
for $j>0$.  Moreover, let
$\DS V_j = \Omega_{j+1} \setminus \overline{\Omega_{j-1}}$
for $j>0$.
The collection $\DS \left\{ V_j \right\}_{j=1}^\infty$ is an
open cover of $\Omega$.  Let
$\DS \left\{ \phi_j \right\}_{j=1}^\infty$ be a partition
of unity of $\Omega$ subordinate to
$\DS \left\{ V_j \right\}_{j=1}^\infty$.  We are using here
the Definition~\ref{pot_part_unity_V2} of a partition of unity.

Choose $\DS \psi \in \DD(\RR^n)$ such that
$\psi(\VEC{x}) = 0$ for $\|\VEC{x}\|>1$ and
$\DS \int_{\RR^n} \psi(\VEC{x}) \dx{\VEC{x}} = 1$.
We consider
$\DS \psi_\epsilon(\VEC{x}) = \epsilon^{-n}
\psi(\epsilon^{-1} \VEC{x})$ for $\VEC{x} \in \RR^n$ and $\epsilon>0$.

\stage{ii} Choose $\delta >0$.  Consider
$\DS \underline{f}: \RR^n \to \RR$ defined by
\[
\underline{f}(\VEC{x}) =
\begin{cases}
f(\VEC{x}) & \quad \text{if} \ \VEC{x} \in \Omega \\
...
$\DS \{\epsilon_j\}_{j=1}^\infty \subset \RR$ to be
specified later,
\begin{equation} \label{sob_HequalsW_phi}
\phi = \sum_{j=1}^\infty (\phi_j\,\underline{f}) \ast \psi_{\epsilon_j}
\end{equation}
is in $\DS C^\infty(\Omega) \cap W^{k,p}(\Omega)$ and satisfies
$\|f-\phi\|_{k,p} \leq \delta$ on $\Omega$.  As usual, we may assume
that $\phi_j$ is defined on $\DS \RR^n$ by $\phi_j(\VEC{x}) = 0$ for
$\DS \VEC{x} \in \RR^n\setminus \Omega$.
It follows from Proposition~\ref{sob_T_dd_wkp} that
$\DS \phi_j \underline{f} \in W^{k,p}(\RR^n)$ and from
Lemma~\ref{sob_denselem5} that
$\DS (\phi_j\,\underline{f}) \ast \psi_{\epsilon_j} \in
W^{k,p}(\RR^n) \cap C^\infty(\RR^n)$.

If $\DS \epsilon_j < \frac{1}{(j+1)(j+2)}$ and 
$\DS U_j = \Omega_{j+2} \setminus \overline{\Omega_{j-2}}$
for $j>0$, then
\[
...
$\DS C^\infty$ on the interior of $K$ since each term of the
series is in $\DS C^\infty(\RR^n)$.  Finally, since this is true
for any compact subset of $\Omega$, then
$\DS \phi \in C^\infty(\Omega)$.
Since $\DS \phi \in L^1_{loc}(\Omega)$, we also get that
$\phi \in \DD'(\Omega)$.

We use Lemma~\ref{sob_denselem2} to choose
$\DS \epsilon_j < \frac{1}{(j+1)(j+2)}$ such that
\[
\left\| \left( (\phi_j\,\underline{f}) \ast \psi_{\epsilon_j}\right)
- (\phi_j\,f) \right\|_{k,p,\Omega}  < \frac{\delta}{2^j Q}
\]
for $j>0$, where $\DS Q^p = \sum_{|\VEC{\alpha}|\leq k} 1$.
...
\]
on $\Omega_{j-1}$ because $\DS \sum_{i=1}^j \phi_j(\VEC{x}) =1$ for
...
Thus $\DS h_{j-1} \in W^{k,p}(\Omega)$ for all $j$.
Note that
$\DS \sum_{i=1}^j (\phi_i\,\underline{f}) \ast \psi_{\epsilon_i}
...
$\DS \left\| \diff^{\VEC{\alpha}} (f - \phi)
\right\|_{p,\Omega} \leq \delta/Q$ for all multi-indices $\VEC{\alpha}$ with
$|\VEC{\alpha}| \leq k$.  Hence,
$\DS \left\| f -\phi \right\|_{k,p,\Omega} \leq \delta$.

\stage{iii} We have that $\phi \in S$ because
$\DS f\in W^{k,p}(\Omega)$
and $\DS \phi-f \in W^{k,p}(\Omega)$ by (ii), thus
$\DS \phi = (\phi-f) + f \in W^{k,p}(\Omega)$.
In particular,
\[
...
there exists $\DS g\in W^{k,p}(\RR^n)$ such that $u=g$ on
$V$ in the sense of distributions \footnotemark.\addtocounter{footnote}{-1}

Similarly, if $\DS s \in \RR$, the {\bfseries localized Sobolev space}
$\DS W^{s,2}_{loc}(\Omega)$ is the set
of all distributions $u \in \DD'(\Omega)$ with the following property.
For every bounded open set $V$ such that $\overline{V}\subset \Omega$,
there exists $\DS g\in W^{s,2}(\RR^n)$ such that $u=g$ on
$V$ in the sense of distributions \footnotemark.
\end{defn}

\footnotetext{$u=g$ on $V$ means that
$\DS u(\phi) = g(\phi) = \int_{\Omega} g \phi \dx{\VEC{x}}$
for all $\phi \in \DD(V)$.  If $\DS u \in L^1_{loc}(\Omega)$, this
implies that $u=g$ almost everywhere on $V$.}

Proposition~\ref{sob_T_dd_wkp} is also true for the Sobolev spaces
$\DS W^{s,2}(\RR^n)$ with $s\in [0,\infty[$.  To prove it, the
following technical lemma is needed first.

\begin{lemma} \label{sob_tri_equ}
For all $\DS \VEC{x}, \VEC{y} \in \RR^n$ and $m \in \RR$, we have
that
$\DS \left(\frac{1+\|\VEC{y}\|^2}{1+\|\VEC{x}\|^2} \right)^m
...
It follows from
$\DS \left(\|\VEC{y} - \VEC{x}\| - \|\VEC{x}\|\right)^2 \geq 0$
that $\DS 2\|\VEC{x}\|\,\|\VEC{y}-\VEC{x}\|
\leq \|\VEC{y} - \VEC{x}\|^2 + \|\VEC{x}\|^2$.  Hence, from
$\DS \|\VEC{y}\| \leq \|\VEC{y}-\VEC{x}\| +\|\VEC{x}\|$, we
get
\[
...
For $\DS \phi \in \SS(\RR^n)$ and $s\in [0,\infty[$, the mapping
\begin{align*}
...
$\DS \|\phi f \|_{s,\rho} \leq C \|f\|_{s,\rho}$ for
$\DS f \in W^{s,2}(\RR^n)$, we first prove that the mapping 
$\DS T : L^2(\RR^n,\rho) \rightarrow L^2(\RR^n,\rho)$
defined by
\[
T(h)(\VEC{y}) =
...
\]
for $\DS h \in L^2(\RR^n,\rho)$ is a continuous linear
mapping, where the measure $\rho$ is defined by
$\DS \dx{\rho(\VEC{x})} = (1+\|\VEC{x}\|^2)^s\dx{\VEC{x}}$.
In particular,
$\DS \|h\|_{2,\rho} \equiv \left(
\int_{\RR^n} |h(\VEC{z})|^2 \dx{\rho(\VEC{z})}\right)^{1/2}$ for
$\DS h \in L^2(\RR^n,\rho)$
...
\]
for some constant $C$ to get that
$\DS \|T(h) \|_{2,\rho} \leq C \|h\|_{2,\rho}$ for
$\DS h \in L^2(\RR^n,\rho)$.  From
...
\end{align*}
for all $\DS \VEC{z}\in \RR^n$ because
$\DS \hat{\phi} \in \SS(\RR^n)$ by
...
< \infty
\]
for all $\DS \VEC{y}\in \RR^n$ because
$\DS \hat{\phi} \in \SS(\RR^n)$ by
...
$\DS W^{s,2}(\Omega)$, then we also denote this function as
$u$.

\begin{prop} \label{sob_Hsloc_cond}
Let $\Omega$ be an open subset of $\DS \RR^n$ and $k$ be a
non-negative integer.  Suppose that $u \in \DD'(\Omega)$.  Then
$\DS u \in W^{k,p}_{loc}(\Omega)$ if and only if
$\DS \phi \, u \in W^{k,p}(\RR^n)$
\footnotemark\addtocounter{footnote}{-1}
for all $\phi \in \DD(\Omega)$.
Similarly, if $s \in \RR$, then $\DS u \in W^{s,2}_{loc}(\Omega)$
if and only if $\DS \phi \, u \in W^{s,2}(\RR^n)$
\footnotemark\ for all $\phi \in \DD(\Omega)$.
\end{prop}

\begin{rmk}
As usual, $\phi \in \DD(\Omega)$ can be expanded to
$\DS \phi \in \DD(\RR^n)$ with $\supp \phi \subset \Omega$
by setting $\phi(\VEC{x}) = 0$ for
$\DS \VEC{x} \in \RR^n\setminus \Omega$.
\end{rmk}

\footnotetext{ Recall that $\DS \phi \, u \in \DD'(\RR^n)$
is defined by $(\phi \, u)(\psi) = u(\phi \psi)$ for all
$\DS \psi \in \DD(\RR^n)$
because $\supp (\phi \psi) \subset \supp \phi \subset \Omega$ for all
$\DS \psi \in \DD(\RR^n)$ and so we may assume that
$\psi \psi \in \DD(\Omega)$.
Hence $\DS \phi u \in W^{k,p}(\RR^n)$ (resp.
$\DS \phi u \in W^{s,2}(\RR^n)$) means that there exists
$\DS g \in W^{k,p}(\RR^n)$ (resp.
$\DS g \in W^{s,2}(\RR^n)$) such that
$\phi u = g$ in the sense of distributions on $\DS \RR^n$.}

\begin{proof}
\stage{i}
Suppose that $\DS u \in W^{s,2}_{loc}(\Omega)$ and
$\phi \in \DD(\Omega)$.  Choose an open set $V$ such that
$\DS \supp \phi \subset V \subset \overline{V}\subset \Omega$.
We get from the definition of $\DS W^{s,2}_{loc}(\Omega)$
that there exists $\DS g\in W^{s,2}(\RR^n)$ such that $u=g$ on
...
\]
for all $\DS \psi \in \DD'(\RR^n)$ since
$\supp(\phi\psi) \subset \supp
\phi \subset V$.  Therefore,
$\DS \phi\, u = \phi \, g \in W^{s,2}(\RR^n)$ by
Proposition~\ref{sob_T_dd_wk2}.

Suppose that $\DS \phi\,u \in W^{s,2}(\RR^n)$ for all
$\DS \phi\in \DD(\Omega)$.  Let $V$ be an open
subset of $\Omega$ such that $\overline{V}\subset \Omega$.  Choose
$\DS \phi \in \DD(\Omega)$ such that $\phi(\VEC{x})=1$ for all
$\VEC{x} \in V$, then $u = \phi \, u$ on $V$ in the sense of
distributions with $\DS \phi \, u \in W^{s,2}(\RR^n)$.

\stage{ii} The proof in the case of $\DS W^{k,p}(\RR^n)$ is
basically identical.
\end{proof}

It follows from Propositions~\ref{sob_T_dd_wk2} and
\ref{sob_Hsloc_cond} that
$\DS W^{s,2}(\RR^n) \subset W^{s,2}_{loc}(\Omega)$
for every open set $\DS \Omega \subset \RR^n$.

\section{The $\DS \mathbf{W^{k,p}_0}$ spaces}

\begin{defn}
Let $\Omega$ be an open subset of $\DS \RR^n$.
For $k\in \NN$ and $p$ a positive integer. the
{\bfseries Sobolev space}\index{Sobolev Space!$W^{k,p}_0(\Omega)$}
$\DS W^{k,p}_0(\Omega)$ is the closure in 
$\DS W^{k,p}(\Omega)$ of $\DD(\Omega)$ with respect to the norm
$\|\cdot\|_{k,p}$.

For $s \in \RR$. the
{\bfseries Sobolev space}\index{Sobolev Space!$W^{s,2}_0(\Omega)$}
$\DS W^{s,2}_0(\Omega)$ is the closure in 
$\DS W^{s,2}(\Omega)$ of $\DD(\Omega)$ with respect to the norm
$\|\cdot\|_{s,\rho}$.
\end{defn}

If $\DS \Omega \subset \RR^n$ is a bounded open set, we will
show when studying the trace operators in Section~\ref{sectTraceOp}
that the space $\DS W^{k,p}_0(\Omega)$ generalizes the space
of functions $\DS f \in C^k(\Omega) \cap C^{k-1}(\overline{\Omega})$
such that $\DS \pdydxn{f}{\nu}{j}(\VEC{x}) = 0$ for all
...
open subset of $\DS \RR^n$.  Given a function
$f:\Omega \rightarrow \CC$,
we define a function $\DS \underline{f}:\RR^n\rightarrow \CC$ by
\[
\underline{f}(\VEC{x}) =
\begin{cases}
f(\VEC{x}) & \quad \text{if} \ \VEC{x} \in \Omega \\
...
\end{cases}
\]
If $\DS f \in W^{k,p}_0(\Omega)$, then
$\DS \diff^{\VEC{\alpha}} \underline{f}
= \underline{\diff^{\VEC{\alpha}} f}$
in the sense of distributions on $\DS \RR^n$ for
$|\VEC{\alpha}|\leq k$.  In particular,
$\DS \underline{f} \in W^{k,p}(\RR^n)$.
\end{prop}

\begin{proof}
By definition, there exists a sequence
$\DS \{\phi_i\}_{i=0}^\infty$ in
...
i\rightarrow \infty \ ,
\end{align*}
where $\DS \frac{1}{p}+ \frac{1}{q} = 1$ and
$\DS C = \left(
\int_\Omega \left| \psi(\VEC{x}) \right|^q \dx{\VEC{x}} \right)^{1/q}
< \infty$.
...
\]
the function $\underline{f}$ defines a distribution on $\DS \RR^n$.
...
= (\underline{\diff^{\VEC{\alpha}} f}) (\phi)
\end{align*}
for $\DS \phi \in \DD(\RR^n)$.
Thus $\DS \diff^{\VEC{\alpha}} \underline{f}
= \underline{\diff^{\VEC{\alpha}} f}$
almost everywhere in $\DS \RR^n$.

\stage{iii} We have that $\DS \diff^{\VEC{\alpha}} \underline{f}
...
\]
since $\DS f \in W^{k,p}(\Omega)$.  Thus,
$\DS \underline{f} \in W^{k,p}(\RR^n)$.

It is also interesting to note that, as a distribution,
$\DS \diff^{\VEC{\alpha}} \underline{f}\,\big|_{\Omega}
= \diff^{\VEC{\alpha}} f$.
\end{proof}

\begin{lemma} \label{sob_w0_trad1}
Let $\Omega$ be an open subset of $\DS \RR^n$.
If $\DS f \in W^{k,p}(\Omega)$ has a compact support in
$\Omega$, then there exists a sequence
$\DS \left\{ \psi_i \right\}_{i=1}^\infty \subset \DD(\Omega)$
such that $\psi_i \to f$ in $\DS W^{k,p}(\Omega)$ as
$i \to \infty$; namely, $\DS f\in W^{k,p}_0(\Omega)$.
\end{lemma}
...
$\DS d = \dist{K}{\partial U} > 0$. 

Choose $\DS \phi \in \DD(\RR^n)$ such that
$\phi(\VEC{x}) = 0$ for $\|\VEC{x}\|>1$ and
$\DS \int_{\RR^n} \phi(\VEC{x}) \dx{\VEC{x}} = 1$.
Let $\DS \phi_i(\VEC{x}) = i^n\phi(i \VEC{x})$ for all
$\DS \VEC{x} \in \RR^n$.
It follows from Lemmas~\ref{sob_denselem1} and
\ref{sob_denselem5} that 
$\DS \underline{f} \ast \phi_i \in \DD(\RR^n)$ for all $i>0$
and
$\underline{f} \ast \phi_i \to \underline{f}$ in
$\DS W^{k,p}(\RR^n)$ as $i \to \infty$.
...
We expand $f$ to $\DS \underline{f}:\RR^n\rightarrow \CC$ by
\[
\underline{f}(\VEC{x}) =
\begin{cases}
f(\VEC{x}) & \quad \text{if} \ \VEC{x} \in \Omega \\
...
$\DS \underline{f} \in W^{k,p}(\RR^n)$.  Since
$\DS \DD(\RR^n)$
is dense in $\DS W^{k,p}(\RR^n)$, there exists a sequence
$\DS \{\phi_i\}_{i=0}^\infty$ in 
$\DS \DD(\RR^n)$ converging to $\underline{f}$ with respect
to the norm $\DS \|\cdot\|_{k,p,\RR^n}$.

Choose $\phi \in \DD(\Omega)$ such that
$\phi(\VEC{x}) = 1$ for all $\VEC{x} \in \supp f$ \footnote{
See footnote on page~\pageref{FOOTNOTE1}.}.
It follows from Proposition~\ref{sob_T_dd_wkp} that
$\DS \{\phi \, \phi_i\}_{i=0}^\infty$
converges to $\phi \,\underline{f}$ with respect to the norm
$\|\cdot\|_{k,p,\RR^n}$.  Thus,
$\DS \{\phi \, \phi_i\}_{i=0}^\infty$
is a sequence in $\DD(\Omega)$ converging to
$\DS \phi \,\underline{f}\big|_{\Omega} = f$ with respect to the norm
$\|\cdot\|_{k,p,\Omega}$.  Therefore
$\DS f\in W^{k,p}_0(\Omega)$ by definition.
\end{rmk}
...
Consider $k\in \NN$ and $p$ a positive integer.  Let $\Omega$ be an
open subset of $\DS \RR^n$ that satisfies the segment
...
\end{cases}
\]
Then, $\DS f \in W^{k,p}_0(\Omega)$ if and only if
$\DS \underline{f} \in W^{k,p}(\RR^n)$.
\end{prop}

\begin{proof}
We have proved in Proposition~\ref{sob_expand_WKP} that
$\DS \underline{f} \in W^{k,p}(\RR^n)$ if
$\DS f \in W^{k,p}_0(\Omega)$.  We now prove the converse.

\stage{i} We first assume that
$\DS K = \supp f \subset \overline{\Omega}$ is compact.
Let
$\DS Q = K \setminus \bigcup_{\VEC{x} \in \partial \Omega} U_{\VEC{x}}$,
...
$\DS \left\{ U_i \right\}_{i=1}^I$.
At most one of the $U_i$ may have an empty intersection with $\partial \Omega$;
namely, the open set $U$ introduced before.

Let $\DS \left\{ \phi_i \right\}_{i=1}^I$ be a partition
of unity of $K$ subordinate to $\DS \left\{ U_i \right\}_{i=1}^I$

Suppose that $\DS \underline{f} \in W^{k,p}(\RR^n)$.
Let $h_i \equiv \phi_i f$. Then
$\DS \underline{h}_i = \phi_i \underline{f} \in W^{k,p}(\RR^n)$
according to Proposition~\ref{sob_T_dd_wkp}.
We can write $\underline{f}$ as
$\DS \underline{f} = \sum_{i=0}^I \phi_i \underline{f}
...
$\DS h_i \in W^{k,p}_0(\Omega)$ for all $i$, then we will
have that
$\DS f = \sum_{i=0}^I h_i \in W^{k,p}_0(\Omega)$.

If $\supp h_i \subset \Omega$, we have that
$\DS h_i \in W^{k,p}_0(\Omega)$ according to
Lemma~\ref{sob_w0_trad1}.

if $\supp \underline{h}_i \subset U_i \cap \overline{\Omega}$, we consider
$\DS h_{i,t} \in W^{k,p}(\RR^n)$ for $0 < t < 1$ defined by
...
$\DS h_{i,t} \in W^{k,p}_0(\Omega)$ for all $t$.

Since $h_{i,t} \to h_i$ in $\DS W^{k,p}_0(\Omega)$ as $t \to 0^+$
according to Lemma~\ref{sob_denselem3}, and since 
$\DS W^{k,p}_0(\Omega)$ is closed, we get that
$\DS h_i \in W^{k,p}_0(\Omega)$.

\stage{ii}  If $\supp f$ is not bounded, we construct a sequence of functions
$\DS \left\{ f_i \right\}_{i=1}^\infty \subset W^{k,p}(\Omega)$
...
$f_i \to f$ in $\DS W^{k,p}(\Omega)$ as $i \to \infty$.

From (i), we have that $\DS f_i \in W^{k,p}_0(\Omega)$.  Since
$f_i \to f$ in $\DS W^{k,p}(\Omega)$ as $i \to \infty$, and since 
$\DS W^{k,p}_0(\Omega)$ is closed, we get that
$\DS f \in W^{k,p}_0(\Omega)$.
\end{proof}

\begin{egg}
As the following example shows,
$\DS W^{k,2}_0(\Omega) \subsetneqq W^{k,2}(\Omega)$ when $\Omega$ is a
bounded open subset of $\DS \RR^n$ and $k\geq 1$.

If $\Omega$ is bounded, we obviously have that $f:\Omega \to \RR$
defined by $f(\VEC{x}) = 1$ for all $\VEC{x} \in \Omega$ is in 
$\DS W^{k,2}(\Omega)$.  We now show that
$\DS f \not\in W_0^{k,2}(\Omega)$.  Since
$\DS W_0^{k,2}(\Omega) \subset W_0^{1,1}(\Omega)$ because
$\Omega$ is bounded, it is in fact enough to
show that $\DS f \not\in W_0^{1,1}(\Omega)$.

Suppose that $\DS f \in W_0^{1,1}(\Omega)$.  Then, there
exists a sequence
$\DS \{\phi_i\|_{i=1}^\infty \subset \DD(\Omega)$ such that
$\phi_i \to f$ with respect to the norm $\|\cdot\|_{1,1}$.
Since $\DS \int_{\Omega} |\phi_i| \dx{\VEC{x}} < \infty$ and
$\DS \sum_{j=1}^n \int_{\Omega} \left| \pdydx{\phi_i}{x_j}
\right| \dx{\VEC{x}} < \infty$ because
$\DS \phi_i \in \DD(\Omega)$, there exists a positive constant $C_i$
such that
$\DS \int_{\Omega} |\phi_i| \dx{\VEC{x}} < C_i
...
$\DS W^{k,p}_0(\Omega) \subsetneqq W^{k,p}(\Omega)$ when $\Omega$ is a
bounded open subset of $\RR^n$ and $p,k\geq 1$.
\end{egg}

\section{The $\DS \mathbf{H^{k,p}}$ spaces} \label{WequalH}

We introduce in this section the spaces $\DS H^{k,p}(\Omega)$ and
$\DS H^{k,p}_0(\Omega)$ for $k \in \NN$ and
$1 \leq p \leq \infty$, and the spaces
$\DS H^{s,2}(\Omega)$ and $\DS H^{s,2}_0(\Omega)$
for $s\in \RR$.  We also prove that, except for $k=\infty$, the spaces
$\DS H^{k,p}(\Omega)$ and $\DS H^{k,p}_0(\Omega)$ are
none other than the the spaces
$\DS W^{k,p}(\Omega)$ and $\DS W^{k,p}_0(\Omega)$
respectively.

\begin{defn} \label{sob_H_spaces}
Assume that $k\in \NN$ and $p$ is a positive integer or $\infty$.
Let $\Omega$ be an open set in $\DS \RR^n$ and
\[
...
$\DS H^{k,p}(\Omega)$ is the closure of $S$ in
$\DS L^p(\Omega)$ with respect to the norm $\|\cdot\|_{k,p}$.

The {\bfseries Sobolev space}\index{Sobolev Space!$H^{k,p}_0(\Omega)$}
$\DS H^{k,p}_0(\Omega)$ is the closure of $\DD(\Omega)$ in
$\DS L^p(\Omega)$ with respect to the norm $\|\cdot\|_{k,p}$.

Since the case $p=2$ is predominant in the study of partial
differential equations, we define
$\DS H^k(\Omega) \equiv H^{k,2}(\Omega)$ and
$\DS H^k_0(\Omega) \equiv H^{k,2}_0(\Omega)$.
\end{defn}

\begin{rmkList} \label{sob_compl_Hk2}
\begin{enumerate}
\item Let $\Omega$ be a bounded open subset of $\DS \RR^n$.
An equivalent definition of $\DS H^{k,p}(\Omega)$ for $p$ a
positive integer is that $\DS H^{k,p}(\Omega)$ is the closure of
$\DS C^\infty(\overline{\Omega})$ in $\DS L^p(\Omega)$
with respect to the norm $\|\cdot\|_{k,p}$ defined in
(\ref{sob_norm_wkp}).

Let $H$ be the closure of $\DS C^\infty(\overline{\Omega})$ in
$\DS L^p(\Omega)$ with respect to the norm $\|\cdot\|_{k,p}$.
If $\DS f\in C^\infty(\overline{\Omega})$, then
$\DS \diff^{\VEC{\alpha}} f$ is a
continuous function on the compact set $\overline{\Omega}$ for all
multi-indices $\VEC{\alpha}$.  It follows that
$\DS \|\diff^{\VEC{\alpha}} f\big|_{\Omega}\|_p \leq
\|\diff^{\VEC{\alpha}} f\big|_{\overline{\Omega}}\|_p < \infty$ for all
multi-indices $\VEC{\alpha}$.  Thus,
$\DS \left\|f\big|_{\Omega} \right\|_{k,p} < \infty$ for all
$\DS f\in C^\infty(\overline{\Omega})$.  Hence,
$\DS C^\infty(\overline{\Omega}) \subset S$, where $S$ is given in
Definition~\ref{sob_H_spaces}.

However, it follows from Theorem~\ref{DDRRnDWkp} that
$\DS C^\infty(\overline{\Omega})$ is dense in $S$ with
respect to the norm $\|\cdot\|_{k,p}$ \footnote{In fact
$\DS C^\infty(\overline{\Omega})$
is dense in $\DS W^{k,p}(\Omega) \supset S$ with respect to
$\|\cdot\|_{k,p}$.}.

Thus, the closure of $\DS C^\infty(\overline{\Omega})$ in
$\DS L^p(\Omega)$ with respect to $\|\cdot\|_{k,p}$
is the closure of $S$ in $\DS L^p(\Omega)$
with respect to $\|\cdot\|_{k,p}$.

\item As well, an equivalent definition of
$\DS H^{k,p}_0(\Omega)$ is that
$\DS H^{k,p}_0(\Omega)$ is the
closure of $\DS C^k_c(\Omega)$ in $\DS L^p(\Omega)$
with respect to the norm $\|\cdot\|_{k,p}$ defined in
(\ref{sob_norm_wkp}).

Given $\DS f \in C^k_c(\Omega)$, it follows from
Lemma~\ref{sob_w0_trad1} that there exists a sequence
$\DS \left\{ \psi_i \right\}_{i=1}^\infty \subset \DD(\Omega)$
such that $\psi_i \to f$ in $\DS W^{k,p}(\Omega)$ as $i \to \infty$.
Thus, $\DS C^k_c(\Omega) \subset \overline{\DD(\Omega)}$,
where the closure is with respect to the norm $\|\cdot\|_{k,p}$.
It follows that the closure of $\DS C^k_c(\Omega)$ in
$\DS L^p(\Omega)$ with respect to $\|\cdot\|_{k,p}$
is included in $\DS H^{k,p}_0(\Omega)$, the closure of
$\DD(\Omega)$ in $\DS L^p(\Omega)$
with respect to $\|\cdot\|_{k,p}$.

But since $\DS \DD(\Omega) \subset C^k_c(\Omega$, we also have that
$\DS H^{k,p}_0(\Omega)$, the closure of
$\DS \DD(\Omega)$ in $\DS L^p(\Omega)$ with
respect to $\|\cdot\|_{k,p}$, is included in the closure of
$\DS C^k_c(\Omega)$ in
$\DS L^p(\Omega)$ with respect to $\|\cdot\|_{k,p}$ 
Thus, the closure of $\DS C^k_c(\Omega)$ in
$\DS L^p(\Omega)$ with respect to $\|\cdot\|_{k,p}$
is $\DS H^{k,p}_0(\Omega)$.
\end{enumerate}
\end{rmkList}

\begin{prop} \label{sob_WcontainsH}
Let $\Omega$ be an open subset of $\DS \RR^n$.  If
$k\in \NN$ and $p$ is a positive integer or $\infty$, then
$\DS H^{k,p}(\Omega) \subset W^{k,p}(\Omega)$ and
$\DS H^{k,p}_0(\Omega) = W^{k,p}_0(\Omega)$.
\end{prop}

\begin{proof}
That $\DS H^{k,p}_0(\Omega) = W^{k,p}_0(\Omega)$ is simple
to proof because the closure of $\DD(\Omega)$ in $\DS L^p(\Omega)$ or
$\DS W^{k,p}(\Omega)\subset L^p(\Omega)$ with respect to the
norm $\|\cdot\|_{k,p}$ give the same
set since $\DS W^{k,p}(\Omega)$ is closed with respect to this norm.
Recall that $\DS W^{k,p}(\Omega)$ is a Banach space with
respect to the norm 
$\|\cdot\|_{k,p}$, so $\DS W^{k,p}(\Omega)$ is complete and
thus closed with respect to this norm.

To prove that $\DS H^{k,p}(\Omega) \subset W^{k,p}(\Omega)$,
we first note that $S$ in the definition of
$\DS H^{k,p}(\Omega)$ is a subset of
$\DS W^{k,p}(\Omega)$.  Again, Since
$\DS W^{k,p}(\Omega)$ is closed, we have that
$\DS H^{k,p}(\Omega) = \overline{S} \subset W^{k,p}(\Omega)$,
where the closure is with respect to the norm $\|\cdot\|_{k,p}$.
\end{proof}

The first question that comes to mind is whether
$\DS W^{k,p}(\Omega) = H^{k,p}(\Omega)$ whatever the open set
$\DS \Omega \subset \RR^n$.  The answer is positive for
$1\leq p < \infty$.  However, it is not always true for $p=\infty$.

\begin{theorem} \label{sob_HequalsW}
If $\DS \Omega \subset \RR^n$ is an open set, $k\in \NN$ and $p$ is a
positive integer, then $\DS H^{k,p}(\Omega) = W^{k,p}(\Omega)$.
\end{theorem}

\begin{proof}
$\DS H^{k,p}(\Omega) \subset W^{k,p}(\Omega)$ follows from
...
$\DS H^{k,p}(\Omega)$ contains
$\DS C^\infty(\Omega) \cap W^{k,p}(\Omega)$.
\end{proof}

\begin{egg}
We have that
$\DS H^{1,\infty}(]-1,1[) \subsetneqq W^{1,\infty}(]-1,1[)$.

The function $f(x) = |x|$ is in $\DS W^{1,\infty}(]-1,1[)$.
In particular,
\[
\diff f(x) =
\begin{cases}
...
\end{cases}
\]
where $c$ can be any constant of our choice.  However, it is
impossible to find
\[
\phi \in
...
$\DS \lim_{x\rightarrow 0^-}|\diff f(x)-\diff \phi(x)|
= |-1-b| \geq 1$ or
$\DS \lim_{x\rightarrow 0^+}|\diff f(x)-\diff \phi(x)| =|1-b| \geq 1$.
Recall also that the uniform limit of continuous functions on
$]-1,1[$ is a continuous function.  Thus, if
$\DS \left\{\phi_j\right\}_{j=0}^\infty$ was to converge in
$\DS W^{1,\infty}(]-1,1[)$ to $f$, then
$\DS \{\diff \phi_j\}_{j=1}^\infty$ would be converging
...
$\DS H^{s,2}(\Omega) = W^{s,2}(\Omega)$ for $s \in \RR$.
Moreover, the {\bfseries Sobolev space}\index{Sobolev Space!$H^{s,2}_0(\Omega)$}
$\DS H^{s,2}_0(\Omega)$ is the completion of
$\DD(\Omega)$ in $\DS H^{s,2}(\Omega)$.
\end{defn}

Because of Theorem~\ref{sob_wk2_wk2}, the definition of 
$\DS H^{k,2}_0(\Omega)$ for $k \in \NN$ above is equivalent
to the definition of $\DS H^{k,2}_0(\Omega)$ given in
Definition~\ref{sob_H_spaces}.

\section{Sobolev Spaces on Manifolds}

We can define Sobolev spaces on subset of $\DS \RR^n$ other
than open sets.

\begin{defn} \label{sob_manifold}
Let $\DS M\subset \RR^n$ be a $\DS C^k$-manifold
of dimension $m$ (without a boundary).  Let
$\DS \BB = \{ (,U_j,\psi_j) \}_{j=1}^\infty$ be an atlas for
$M$ as defined in Section~\ref{pot_int_man}.
Let $\DS \{\phi_i\}_{i=1}^\infty$ be a partition of unity
for $M$ subordinate to $\BB$.  The space
$\DS W^{k,2}(M)$ is defined by 
\[
...
$\DS \RR^n$, and that $\chi$ is a
$\DS C^k$-diffeomorphism of $\Omega_1$ onto $\Omega_2$ \footnotemark.
\begin{enumerate}
\item Then, $\DS T:W_{loc}^{s,2}(\Omega_1) \to
 W_{loc}^{s,2}(\Omega_2)$ defined by
$\DS T(u) = u \circ \chi^{-1}$ for
$\DS u \in W_{loc}^{s,2}(\Omega_1)$ is an
isomorphism with inverse defined by
$\DS T^{-1}(v) = v \circ \chi$ for all
$\DS v \in W_{loc}^{s,2}(\Omega_2)$.
\item If in addition all the partial derivatives of $\chi$ and
$\DS \chi^{-1}$ of order less or equal to $k$ are bounded, then
$\DS T:W^{k,p}(\Omega_1) \to W^{k,p}(\Omega_2)$ defined by
$\DS T(u) = u \circ \chi^{-1}$ for
$\DS u \in W^{k,p}(\Omega_1)$
is an isomorphism with inverse defined by
$\DS T^{-1}(v) = v \circ \chi$ for all
$\DS v \in W^{k,p}(\Omega_2)$.
This result is also true if 
$\DS W^{k,p}(\Omega_1)$ and $\DS v \in W^{k,p}(\Omega_2)$
are respectively replaced by $\DS v \in W^{k,p}_0(\Omega_1)$
and $\DS v \in W^{k,p}_0(\Omega_2)$.
\end{enumerate}
\end{prop}

\footnotetext{Namely, $\chi:\Omega_1 \to \Omega_2$ is an homeomorphism,
and $\chi:\Omega_1 \to \Omega_2$ and
$\DS \chi^{-1}:\Omega_2 \to \Omega_1$
are of class $\DS C^k$.}
...
However, we will make very little use of Sobolev spaces on manifolds
in these lecture notes.  For more details about Sobolev spaces on
Manifold, the reader should consult Chapter~25 of \cite{Tr}.

\section{Dual Spaces} \label{sobDualSp}

\begin{defn}
Let $\Omega$ be an open subset of $\DS \RR^n$.  The
{\bfseries dual}\index{Sobolev Space!Dual Space} of the space
$\DS H^{s,2}_0(\Omega)$ is the space
$\DS H^{-s,2}(\Omega)$ of all bounded
linear functionals on $\DS H^{s,2}_0(\Omega)$.
\end{defn}

\begin{rmk}
If $\DS \Omega = \RR^n$ or $\Omega$ is a compact manifold
(without boundary), we then have
that $\DS H^{k,2}(\Omega) = H^{k,2}_0(\Omega)$ and
$\DS H^{-k,2}(\Omega)$ is
the dual space of $\DS H^{k,2}(\Omega)$; namely,
$\DS H^{-k,2}(\Omega)$ is the
set of all bounded linear functionals on $\DS H^{k,2}(\Omega)$
\end{rmk}

It may seem from the definition above that we have two definitions
of $\DS H^{s,2}(\RR^n)$ for $s < 0$: the definition above
and Definition~\ref{sob_2nd_wk2}.  We now show that it is not so and,
in this process, show that the dual space of $\DS H^{s,2}(\RR^n)$
for $s\geq 0$ is the space $\DS H^{-s,2}(\RR^n)$.

\begin{prop}
For $\DS s \in \RR$, the dual of
$\DS H^{s,2}(\RR^n)$ is canonically isomorphic 
to $\DS H^{-s,2}(\RR^n)$.
\end{prop}
...
\end{CD}
\]
because the dual of $\DS L^2(\RR^n)$ is $\DS L^2(\RR^n)$
and $\DS \DD'(\RR^n)$ is the dual of
$\DS \DD(\RR^n)$ by definition; namely,
$\DS \DD'(\RR^n)$ is the space of continuous linear
functionals on $\DS \DD(\RR^n)$.

For $\DS \phi \in \SS(\RR^n) \subset H^{s,2}(\RR^n)$ and
$\psi \in \SS(\RR^n) \subset (L^2(\RR^n))^\ast = L^2(\RR^n)$, we have
\begin{align*}
...
Since $\DS \SS(\RR^n)$ is dense in $\DS H^{s,2}(\RR^n)$ and
in $\DS L^2(\RR^n)$, we get that 
$\DS (T^{[s]})^\ast = T^{[s]}$.

Since $\DS T^{[s]}$ is an isometry of $\DS H^{s,2}(\RR^n)$
onto $\DS L^2(\RR^n)$, we have that
$\DS (T^{[s]})^\ast$ is an isometry of 
$\DS L^2(\RR^n)$ onto
$\DS \big((H^{s,2}(\RR^n)\big)^\ast$
\footnote{We leave it to the reader to verify that the adjoint of a
surjective isometry is a surjective isometry.}.  So,
$\DS T^{[s]} = (T^{[s]})^\ast$ is an isometry of 
$\DS L^2(\RR^n)$ onto $\DS \big((H^{s,2}(\RR^n)\big)^\ast$.

For $\DS \phi \in \SS(\RR^n)$, we have
\begin{align*}
\left\| T^{[s]}(\phi)\right\|_{-s,\rho}^2
&= \int_{\RR^n} (1+\|\VEC{z}\|_2^2)^{-s}
\left| \F\left( \F^{-1}\left(
(1+\|\VEC{y}\|_2^2)^{s/2} \F(\phi)\right)\right)(\VEC{z})
\right|^2 \dx{\VEC{z}} \\
...
Hence, $\DS T^{[s]}\left(L^2(\RR^n)\right) \subset H^{-s,2}(\RR^n)
\subset \big(H^{s,2}(\RR^n)\big)^\ast$ since $\DS
\SS(\RR^n)$ is dense in $\DS L^2(\RR^n)$.  It follows that
$\DS T^{[s]}$ is an isometry of 
$\DS L^2(\RR^n)$ onto $\DS H^{-s,2}(\RR^n)$.

To be thorough, we shall probably prove our previous claim that
$\DS H^{-s,2}(\RR^n) \subset \big(H^{s,2}(\RR^n)\big)^\ast$.
This is a consequence of the following observation.
Given $\DS f \in H^{-s,2}(\RR^n)$, we have
\begin{align*}
...
\end{align*}
for all $\DS g \in H^{s,2}(\RR^n)$, where the second
...
\begin{theorem}
Let $\Omega$ be an open subset of $\DS \RR^n$.  Every
$\DS F\in H^{-k,2}(\Omega)$ can be expressed as
\begin{equation} \label{sob_dual_form}
\ps{F}{f} = F(f) = \sum_{|\VEC{\alpha}|\leq k} \int_\Omega
F_{\VEC{\alpha}}\,\diff^{\VEC{\alpha}} f
\end{equation}
for all $\DS f \in H^{k,2}_0(\Omega)$, where the $F_{\VEC{\alpha}}$
are some functions in $\DS L^2(\Omega)$.
\end{theorem}

\begin{rmk}
Since $\DD(\Omega)$ is dense in $\DS H^{k,2}_0(\Omega)$,
(\ref{sob_dual_form}) is equivalent to
$\DS F = \sum_{|\VEC{\alpha}|\leq k} (-1)^{|\VEC{\alpha}|}
...
\diff^{\VEC{\alpha}_N} f \right)
\end{align*}
where $\DS (L^2(\Omega))^N$ is an Hilbert space with the
scalar product defined by
$\DS \ps{\VEC{f}}{\VEC{g}}_2 =
\sum_{j=0}^N \int_\Omega f_{\VEC{\alpha}_j} g_{\VEC{\alpha}_j}$
for all $\DS \VEC{f}, \VEC{g} \in (L^2(\Omega))^N$.
$T$ is a linear mapping preserving the norm if the norm on
$\DS (L^2(\Omega))^N$ is the norm induced by the scalar
product; namely,
$\DS \| \VEC{f} \|_2 =
\left( \sum_{j=0}^N \| f_{\VEC{\alpha}_j} \|_2^2 \right)^{1/2}$
for all $\DS \VEC{f} \in (L^2(\Omega))^N$.
Let $\DS R = T(H^{k,2}_0(\Omega))$.  Since $T$ preserves the
norm (so $T$ is a closed mapping), $R$ is a closed subspace of
$\DS (L^2(\Omega))^N$.  Moreover,
$\DS T:H^{k,2}_0(\Omega) \rightarrow R$ is invertible.  Let
...
$\DS Q: (L^2(\Omega))^N \rightarrow \RR$ such that
$\DS Q\big|_R = P$ and $\|Q\| = \|P\|$; the norm of the
operators is induced by the norm $\|\cdot\|_2$ on
$\DS (L^2(\Omega))^N$.

Using Riesz representation theorem, there exist
$\DS F_{\VEC{\alpha}} \in L^2(\Omega)$ for
$|\VEC{\alpha}|\leq k$ such that
\[
\ps{F}{f} = \ps{Q}{T(f)} = Q(T(f))
= \sum_{j=0}^N \int_\Omega F_{\alpha_j} \diff^{\VEC{\alpha}} f 
\]
for all $\DS f \in H^{k,2}_0(\Omega)$.
\end{proof}

\section{Extension}

Let $\Omega$ be an open subset of $\DS \RR^n$.  The space
$\DS W_0^{k,2}(\Omega)$ can always be extended to
$\DS W^{k,2}(\RR^n)$.  If 
$\DS f\in W_0^{k,2}(\Omega)$, we define $f(\VEC{x}) =0$ for
$\DS \VEC{x} \in \RR^n\setminus \Omega$ to get that
 $\DS f \in W^{k,2}(\RR)$.
The element of $\DS W_0^{k,2}(\Omega)$ are the elements of
$\DS W^{k,2}(\RR)$
which are the limit with respect to the norm $\|\cdot\|_{k,2}$ of test
functions with compact support in $\Omega$.

Since it is generally simpler to work with $\DS W^{k,2}(\RR^n)$
than with $\DS W^{k,2}(\Omega)$, where $\Omega$ is a bounded
domain in $\DS \RR^n$, it should be useful to determine under which
conditions $\DS W^{k,2}(\Omega)$ 
may be extended to a subspace of $\DS W^{k,2}(\RR^n)$.
...
functions defined on $\DS \RR^n$ instead of functions
defined on a bounded open set $\Omega$.

\begin{defn} \label{sob_k_ext_prop}
Let $\Omega$ be an open subset of $\DS \RR^n$.  Let $E$ be a linear
operator from the space of measurable functions on $\Omega$ to the
space of measurable function on $\DS \RR^n$.
\begin{enumerate}
\item $E$ is called a
{\bfseries $\mathbf{(k,p)}$-extension operator}%
\index{Sobolev Space!$\mathbf{(k,p)}$-Extension Operator}
on $\Omega$ if the restriction of $E$ to $\DS W^{k,p}(\Omega)$
is a bounded linear mapping
$\DS E:W^{k,p}(\Omega) \rightarrow W^{k,p}(\RR^n)$ such that
$\DS E(f)\big|_{\Omega} = f$ for all
$\DS f\in W^{k,p}(\Omega)$.
...
restriction of $E$ to $\DS W^{m,p}(\Omega)$ is a
...
Let $\Omega$ be an open subset of $\DS \RR^n$ with the
$(k,2)$-extension property.  Then
$\DS \DD(\RR^n)\big|_{\Omega}$ is dense in
$\DS W^{k,2}(\Omega)$ with respect to the norm $\|\cdot\|_{k,2}$.
\end{theorem}

\begin{proof}
We have from Theorem~\ref{DDRRnDWs2} that $\DD(\RR^n)$ is dense in
$\DS W^{k,2}(\RR^n) \supset E(W^{k,2}(\Omega))$.

Given $\DS f\in W^{k,2}(\Omega)$, suppose that
$\DS \{\phi_j\}_{j=0}^\infty \subset \DD(\RR^n)$ converges
to $E(f)$ with respect to the norm $\|\cdot\|_{k,2,\RR^n}$.  Then,
$\phi_j\big|_\Omega \rightarrow f$ in
$\DS W^{k,2}(\Omega)$ with respect
to the norm $\|\cdot\|_{k,2,\Omega}$ as $j\rightarrow \infty$
according to Lemma~\ref{sobVsubsetU}.
\end{proof}

A first example of an open subset of $\DS \RR^n$ with a
...
$\DS \RR^n_+ \equiv \{ \VEC{x} \in \RR^n : x_n>0 \}$.
Given $\DS f \in W^{k,p}(\RR^n_+)$,
\[
...
$E_{\VEC{\alpha}}$ on
$\DS \RR^n_+$ such that
$\DS \diff^{\VEC{\alpha}} (E(f)) = E_{\VEC{\alpha}}
(\diff^{\VEC{\alpha}} f)$ for all $\DS f \in W^{k,p}(\RR^n_+)$.
\end{theorem}

\begin{rmk}
It is also useful to define for later use the sets
$\DS \RR^n_0 = \{ \VEC{x} \in \RR^n : x_n=0 \}$ and
$\DS \RR^n_- = \{ \VEC{x} \in \RR^n : x_n<0 \}$.
\end{rmk}

\begin{proof}
Let $\DS \VEC{a} \in \RR^{k+1}$ be the unique solution of
the linear equation $A \VEC{a} = \VEC{u}$, where $A$ is the
Vandermonde matrix defined by $\DS A_{i,j} = (-j)^{i-1}$ for
...
sufficiently differentiable functions on $\DS \RR^n$.
...
\DS \sum_{j=1}^{k+1} a_j \phi(\breve{\VEC{x}},-jx_n)
...
= \begin{cases}
\phi(\VEC{x}) & \quad \text{if} \ \VEC{x} \in \RR^n_+ \cup \RR^n_0 \\
\DS \sum_{j=1}^{k+1} (-j)^{\alpha_n} a_j \phi(\breve{\VEC{x}},-jx_n)
& \quad \text{if} \ \VEC{x} \in \RR^n_-
\end{cases}
\]
for $\DS \phi \in S = \{ \phi\big|_{\RR^n_+ \cup \RR^n_0} :
\phi \in \DD(\RR^n) \}$,
...
We have that $E(\phi)$ is of class $\DS C^m$ on
$\DS \RR^n_+ \cup \RR^n_-$ for all $\phi \in S$.
Moreover,
\[
...
\]
for $|\VEC{\alpha}| \leq m$ and $\DS \VEC{x} \in \RR^n_-$
and obviously
\[
...
\]
for $|\VEC{\alpha}| \leq m$
because $\DS \sum_{j=1}^{k+1} (-j)^{\alpha_n}a_j = 1$ for
$0 \leq \alpha_n \leq m \leq k$.  Thus,
$\DS E(\phi) \in C^m(\RR^n)$ for $\phi \in S$.
...
$\DS \sum_{i=1}^p \frac{1}{p/s_i} = 1$, we get
\begin{align*}
&\int_{\RR^n_+} \prod_{i=1}^p \left| (-j_i)^{\alpha_n} \diff^{\VEC{\alpha}}
\phi(\breve{\VEC{x}},-j_i x_n)\right|^{s_i} \dx{\VEC{x}}
...
\begin{align}
&\| E(\phi) \|_{m,p,\RR^n} \nonumber \\
&\quad \leq \bigg( \sum_{|\VEC{\alpha}|\leq m} \bigg( 1
...
\int_{\RR^n_+} \left| \diff^{\VEC{\alpha}} \phi(\VEC{x}) \right|^p \dx{\VEC{x}}
  \bigg)^{1/p} \nonumber \\
&\quad \leq C \| \phi \|_{m,p,\RR^n_+} \label{sobRpExtEq1}
\end{align}
...
\]
where $\DS N = \sum_{|\VEC{\alpha}|\leq m} 1$.

Since $E$ is defined by extension from the dense set $S$ in
$\DS W^{m,p}(\RR^n_+)$ according to Theorem~\ref{DDRRnDWkp},
we have that (\ref{sobRpExtEq1}) is true for all
$\DS \phi \in W^{m,p}(\RR^n_+)$.  Thus, proving that
$\DS E: W^{m,p}(\RR^n_+) \to W^{m,p}(\RR^n)$ is a $(m,p)$-extension
operator on $\DS \RR^n_+$ for all
$0\leq m \leq k$ and $0<p<\infty$.

To prove that $E_{\VEC{\alpha}}$ is a strong
$(k-|\VEC{\alpha}|)$-extension operator on $\DS \RR^n_+$, we note that
$\DS \diff^{\VEC{\beta}} (E_{\VEC{\alpha}}(\phi))
...
$\DS W^{m,p}(\RR^n_+)$ for $0 \leq m \leq k-|\VEC{\alpha}|$.
\end{proof}
...
Since
$\DS S = \left\{ \phi\big|_{\RR^n_+} : \phi \in \DD(\RR^n) \right\}$
is dense in $\DS W^{k,p}(\RR^n_+)$ according to
Theorem~\ref{DDRRnDWkp}, it is enough to define the extension operators
on $S$.

Let $\psi:\RR \to [0,1]$ be a function of class $\DS C^\infty$
...
= \begin{cases}
\phi(\VEC{x}) & \quad \text{if} \ \VEC{x} \in \RR^n_+ \cup \RR^n_0 \\
\DS \sum_{j=1}^\infty a_j \psi(-2^jx_n)\phi(\breve{\VEC{x}},-2^jx_n)
& \quad \text{if} \ \VEC{x} \in \RR^n_-
\end{cases}
\]
where $\breve{\VEC{x}} = (x_1,x_2,\ldots, x_{n-1})$ for all
$\DS \VEC{x} \in \RR^n$ and
$\DS \{a_j\}_{j=1}^\infty \subset \RR$ is a
sequence satisfying
...
$\DS \Omega \subset \RR^n$
that ensure the existence of a strong $k$-extension operator on $\Omega$.
But first, we need some definitions.

\begin{defn}
A collection $\DS \{ U_j \}_{j=1}^\infty$ of open subsets of
$\DS \RR^n$ is a {\bfseries locally finite open cover}%
\index{Locally Finite Open Cover} of a set
$\DS \Omega \subset \RR^n$ if
$\DS \Omega \subset \bigcup_{j=1}^\infty U_j$ and,
for any compact set $\DS K \subset \RR^n$, the set $K$
intersects at most a finite number of $U_j$.
\end{defn}

\begin{defn} \label{UnifKregDefn}
A open set $\DS \Omega \subset \RR^n$ satisfies the
{\bfseries uniform $\DS \mathbf C^k$-regularity condition}%
\index{Uniform $\mathbf C^k$-Regularity Condition} if there exists
a locally finite open cover
$\DS \{ U_j \}_{j=1}^\infty$ for $\partial \Omega$ and a collection 
$\DS \{ \psi_j \}_{j=1}^\infty$ of $\DS C^k$-diffeomorphisms 
$\DS \psi_i: U_i \to B_{1}(\VEC{0}) \subset \RR^n$ with the following
properties:
\begin{enumerate}
...
\]
\item For each $j$, we have that
$\DS \psi_j(U_j\cap \Omega) = B_{1}(\VEC{0}) \cap \RR^n_+$.
...
$\DS \{ U_j \}_{j=1}^J$ for some positive integer $J$.
\end{defn}

\begin{theorem} \label{sob_Omega_Ext}
Let $\Omega$ be an open subset of $\DS \RR^n$ with a bounded boundary
\footnotemark and suppose that $\Omega$ satisfies the uniform
$\DS C^k$-regularity condition, then there exists a strong
...
\begin{proof}[Proof (Sketch)]
Let $\DS \{ U_j \}_{j=1}^J$ be the finite cover
of $\partial \Omega$ from the definition of uniform
$\DS C^k$-regularity, and 
$\DS \{ \psi_j \}_{j=1}^J$ be the associated
collection of $\DS C^k$-diffeomorphisms with the properties
listed in the definition of uniform $\DS C^k$-regularity. 

Let $\DS V_j = \psi_j^{-1}(B_{1/2}(\VEC{0}))$
and $\DS W_j = \psi_j^{-1}(B)$ for $1\leq j \leq J$, where
\[
...
As stated in the definition of uniform $\DS C^k$-regularity,
...
\bigcup_{j=1}^J V_j \ .
\]
Let $\DS W_0 = \left\{ \VEC{x} \in \Omega :
\dist{\VEC{x}}{\partial \Omega}  > \delta/2 \right\}$.
We then have that $\DS \{ W_j \}_{j=0}^J$ is an open cover
of $\Omega$.
Let $\DS \{ \phi_j \}_{j=0}^J$ be a partition of unity for
$\Omega$ subordinated to $\DS \{ W_j \}_{j=0}^J$.

Since $\Omega$ satisfies the uniform $\DS C^k$-regularity
condition, we have that $\partial \Omega$ is a boundary of class
$\DS C^k$.  Thus, $\partial \Omega$ satisfies the segment
property.  It then follows from Theorem~\ref{DDRRnDWkp} that
$\DS S = \left\{ \phi\big|_{\Omega} : \phi \in \DD(\RR^n) \right\}$
is dense in $\DS W^{m,p}(\Omega)$ for $m\leq k$ arbitrary
but fixed.  Hence, as we have done
before, it is enough to define the extension operators on $S$.  

Given $\phi \in S$, we have that
$\DS \phi = \sum_{j=0}^J \omega_j$, where
$\DS \omega_j = \phi\, \phi_j \in C^\infty(\RR^n)$ has its
support in $W_j$.
...
0 & \quad  \text{if} \ \VEC{x} \in \RR^n\setminus W_0
  \end{cases}
\]
So, $\rho_0$ is an extension of $\omega_0$ to $\DS \RR^n$ and,
according to Lemma~\ref{sob_denselem1},
$\DS \rho_0 \in W^{m,p}(\RR^n)$
since $\supp \omega_0 \subset \Omega$.

For $0< j \leq J$, we consider
$\DS \tilde{\omega}_j = \omega_j \circ \psi_j^{-1}$ on $B$.
...
$\DS \VEC{x} \in \RR^n \setminus B$.  We have that
$\DS \tilde{\omega}_j \in W^{m,p}(\RR^n_+)$
according to Lemma~\ref{sobVsubsetU}.
Using the strong $k$-extension operator $E$
provided by Theorem~\ref{sob_Rp_ext}, we have that
$\DS E(\tilde{\omega}_j) \in W^{m,p}(\RR^n)$,
$E(\tilde{\omega}_j)\big|_{\RR^n_+} = \tilde{\omega}_j$ and
$\DS \|E(\tilde{\omega}_j)\|_{m,p,\RR^n} \leq
...
function in $\DS W^{m,p}(\RR^n)$ by setting
$\rho_j(\VEC{x}) = 0$ for $\DS \VEC{x} \in \RR^n \setminus W_j$.
...
$\DS C^k$-regularity.  We may also assume that $K \geq 1$.  Consult
Theorem 5.22 in \cite{Ad} for the details.

The operator $F$ is defined on $S$ by
$\DS F(\phi) = \sum_{j=0}^J \rho_j$.
We have that $\DS F(\phi) \in W^{m,p}(\RR^n)$ by construction,
\[
...
$\DS W^{m,p}(\Omega)$, we have that
(\ref{sobOmegaExtEq1}) is true for all
$\DS \phi \in W^{m,p}(\Omega)$.  Thus, proving that
$\DS F: W^{m,p}(\Omega) \to W^{m,p}(\RR^n)$ is a $(m,p)$-extension
operator on $\DS \Omega$ for all
$0\leq m \leq k$ and $0<p<\infty$.
\end{proof}
...
If $\Omega$ is an open subset of $\DS \RR^n$ with a compact
boundary of class $\DS C^k$ (so $\partial \Omega$ is a
compact oriented manifold of class $\DS C^k$),  then
$\Omega$ satisfies the uniform $\DS C^k$-regularity 
...
consider $\DS H^{1,p}(I)$, where $I$ is an open interval
...
$\DS f \in L^1_{loc}(I)$.  For $x_0 \in I$, let
\[
...
Given $\DS u \in H^{1,p}(I)$, where $I = ]a,b[ \subset \RR$,
...
$\DS u \in H^{1,p}(I)$, so
$\DS u' \in L^p(I) \subset L^p(J)$
where $J$ is the bounded interval with endpoints $x_0$ and
$x$.  Moreover, $\DS L^p(J) \subset L^1(J)$ because $J$ is bounded.

From Lemma~\ref{sob_ftc}, we have
\[
...
$\DS H^{1,p}(I) \subset C(\overline{I})$.  We prove more
general and stronger results in the next section.

\subsection{In $\DS \mathbf{\RR^n}$}

To illustrate the complexity of the results about imbeddings, we
summarize some of the results listed in \cite{Br}.
\begin{center}
\begin{tabular}{c|c|c|c}
& $\DS \frac{1}{p'} = \frac{1}{p} - \frac{1}{n} > 0$
& $p\leq q < \infty$ or $p = n$ & $ p > n$ \\
\hline  
\rule{0pt}{1.2em} $\RR^n$ & $\DS H^{1,p}(\RR^n)
 \rightarrow L^{q}(\RR^n)$
& $\DS H^{1,p}(\RR^n) \rightarrow L^{q}(\RR^n)$
& $\DS H^{1,p}(\RR^n) \rightarrow L^{\infty}(\RR^n)$ \\
& for $p \leq q \leq p'$ & & \\
& is continuous. & is continuous. & is continuous. \\
\hline
\rule{0pt}{1.2em} $\Omega$ open, and $\partial \Omega$
& $\DS H^{1,p}(\Omega) \rightarrow L^{p'}(\Omega)$
& $\DS H^{1,p}(\Omega) \rightarrow L^{q}(\Omega)$
& $\DS H^{1,p}(\Omega) \rightarrow L^{\infty}(\Omega)$ \\
...
& $\DS H^{1,p}(\Omega) \rightarrow L^{q}(\Omega)$
& $\DS H^{1,p}(\Omega) \rightarrow L^{q}(\Omega)$
& $\DS H^{1,p}(\Omega) \rightarrow C(\overline{\Omega})$ \\
bounded, and & for $1 \leq q < p'$ & & \\
$\partial \Omega$ of class $\DS C^1$ & is compact. & is compact.
& is compact. \\
\hline
\end{tabular}
\end{center}
There are similar results for $\DS H^{m,p}$ with $m>1$.
...
$\partial \Omega$ of class $\DS C^k$ for $k\geq 1$, $\Omega$
satisfying the uniform $\DS C^k$-regularity conditions, and
...
If $s >k +n/2$, then $\DS H^{s,2}(\RR^n) \subset C^k(\RR^n)$ and there
exists a constant $C$ depending on $s$ and $k$ only such that
\begin{equation} \label{sob_sob_lemma_max}
\|f\|_{k,\infty} \leq C \|f\|_{s,\rho}
\end{equation}
for $\DS f \in H^{s,2}(\RR^n)$.
In particular, $\DS H^{s,2}(\RR^n) \subset C_b^k(\RR^n)$.
Moreover, the imbedding $\DS i:H^{s,2}(\RR^n) \to C_b^k(\RR^n)$
is continuous.  \index{Sobolev's Lemma}
\end{theorem}

\begin{proof}
Let $f$ be an element of $\DS \SS(\RR^n) \subset H^{s,2}(\RR^n)$.
Note that $\DS \DD(\RR^n) \subset H^{s,2}(\RR^n)$ could also
have been used.

Assume that $|\VEC{\alpha}|\leq k$.
Since $\DS (\diff^{\VEC{\alpha}} f)^\wedge(\VEC{y}) =
...
$f$ but only on $s$ and $k$.  Since $\DS \diff^{\VEC{\alpha}} f$, and
$\DS (\diff^{\VEC{\alpha}} f)^\wedge$ are in
$\DS L^1(\RR^n)$, we
get from Proposition~\ref{distr_inv_four_transf} that
$\DS ((\diff^{\VEC{\alpha}} f)^\wedge)^\vee \in C_0(\RR^n)$,
$\DS ((\diff^{\VEC{\alpha}} f)^\wedge)^\vee
...
\leq C \| f \|_{s,\rho}
\end{equation}
for all $\DS f \in \SS(\RR^n)$.  Since
$\DS \SS(\RR^n)$ is dense in
$\DS H^{s,2}(\RR^n)$, (\ref{sob_sob_inequ}) is true for all
$\DS f \in H^{s,2}(\RR^n)$.  More precisely, if
$\DS \{f_i\}_{i=1}^\infty \subset \SS(\RR^n)$ converges in
$\DS H^{s,2}(\RR^n)$ to $\DS f \in H^{s,2}(\RR^n)$, then
$\DS \{f_i\}_{i=1}^\infty$ is a Cauchy sequence in
$\DS H^{s,2}(\RR^n)$.
From (\ref{sob_sob_inequ}), $\DS \{f_i\}_{i=1}^\infty$ is a Cauchy
sequence in the complete space $\DS C_b^k(\RR^n)$.  Thus, there exists
$\DS g \in C_b^k(\RR^n)$ such that
$\DS \{f_i\}_{i=1}^\infty$ converges in
$\DS C_b^k(\RR^n)$ to $g$.  Moreover, since
$\DS \{f_i\}_{i=1}^\infty $ converges in
$\DS H^{s,2}(\RR^n)$ to $f$, we have
that $\DS \{f_i\}_{i=1}^\infty $ converges pointwise to $f$ almost
everywhere.  Thus $f = g$ almost everywhere.

The continuity of the injection
$\DS i:H^{m,2}(\RR^n) \to C_b^k(\RR^n)$ is a direct
consequence of (\ref{sob_sob_lemma_max}).
\end{proof}

\begin{rmk}
In the previous theorem, we may have proved directly that 
$\DS H^{s,2}(\RR^n) \subset C^k(\RR^n)$.

Let $f$ be any element of $\DS H^{s,2}(\RR^n)$.  Let $V$ be
a bounded open subset of $\DS \RR^n$.  Choose
$\DS \psi \in \DD(\RR^n)$ such that
...
$\DS \diff^{\VEC{\alpha}} F \in L^2(\RR^n)$
has a compact support, we also have that
$\DS \diff^{\VEC{\alpha}} F \in L^1(\RR^n)$.
Since $\DS (\diff^{\VEC{\alpha}} F)^\wedge(\VEC{y}) =
...
Since
$\DS \diff^{\VEC{\alpha}} F$ and
$\DS (\diff^{\VEC{\alpha}} F)^\wedge$
are in $\DS L^1(\RR^n)$, we
get from Proposition~\ref{distr_inv_four_transf} that the inverse Fourier
transform of $\DS (\diff^{\VEC{\alpha}} F)^\wedge$ is in
$\DS C_0(\RR^n)$
and is equal to $\DS \diff^{\VEC{\alpha}} F$ almost everywhere
for $|\VEC{\alpha}| \leq k$.  From Proposition~\ref{distr_cdistr_cder}, we
get that $\DS F\in C^k(\RR^n)$.  Since this is true for any
bounded open subset $V$ of $\DS \RR^n$ and $f=F$ on $V$, then
$\DS f\in C^k(\RR^n)$.
\end{rmk}

\begin{cor}
If $\DS f\in H^{s,2}(\RR^n)$ for all $s>n/2$, then
$\DS f\in C^\infty(\RR^n)$.
\end{cor}

\begin{cor} \label{sob_TheSobLemma_Cor1}
Every distribution on $\DS \RR^n$ with a compact support belongs to
$\DS H^{s,2}(\RR^n)$ for some $s\in \RR$.
\end{cor}
...
$\DS \left| u(\phi) \right| \leq C_1 \max_{|\VEC{\alpha}|\leq N}
\left\| \diff^{\VEC{\alpha}} \phi \right\|_{K,\infty}$ for all $\phi \in \DD_K$.
Thus,
$\DS \left| u(\phi) \right| \leq C_1 \max_{|\VEC{\alpha}|\leq N}
\left\| \diff^{\VEC{\alpha}} \phi \right\|_{\infty}$
for all $\DS \phi \in \DD(\RR^n)$ because $K = \supp u$.

From Theorem~\ref{sob_TheSobLemma}, Sobolev's Lemma, there exists a
constant $C$ such that\\
$\DS \max_{|\VEC{\alpha}|\leq N}
\left\| \diff^{\VEC{\alpha}} \phi \right\|_{\infty} \leq C \|\phi\|_{s,2}$
for all $\DS \phi \in \DD(\RR^n)$ if $s > N+ n/2$.  Thus,
$\DS \left| u(\phi) \right| \leq C C_1 \|\phi\|_{s,2}$
for all $\DS \phi \in \DD(\RR^n)$.

Since $\DS \DD(\RR^n)$ is dense in
$\DS H^{s,2}(\RR)$, we may
therefore extend $u$ to a bounded linear functional on
$\DS H^{s,2}(\RR)$.  Thus,
$\DS u \in \left( H^{s,2}(\RR^n) \right)^\ast = H^{-s,2}(\RR^n)$.
\end{proof}

\begin{cor} \label{sob_TheSobLemma_cor1}
Let $\DS \Omega \subset \RR^n$ be an open set with the
$(m,2)$-extension property. If $m>k +n/2$, then
$\DS H^{m,2}(\Omega) \subset C_b^k(\overline{\Omega})$.
Moreover, the imbedding
$\DS i:H^{m,2}(\Omega) \to C_b^k(\overline{\Omega})$
is continuous.
\end{cor}

\begin{proof}
Let $\DS E:H^{m,2}(\Omega) \rightarrow H^{m,2}(\RR^n)$ be a bounded
linear mapping provided by the $(m,2)$-extension property.  Given
$\DS f\in H^{m,2}(\Omega)$, we have from the Sobolev's Lemma that
$\DS E(f) \in H^{m,2}(\RR^n) \subset C^k_b(\RR^n)$ and the
injection $\DS i_0:H^{m,2}(\RR^n) \to C_b^k(\RR^n)$ is
continuous.

Let $\DS R: C_b^k(\RR^n) \to C_b^k\left(\overline{\Omega}\right)$
be the mapping defined by $\DS R(f) = f\big|_{\overline{\Omega}}$.
This is a continuous mapping.
Since $i = R\circ i_0 \circ E$, the composition of continuous mappings,
we have that $i$ is continuous.

Note that $\DS f = i(f)\big|_{\Omega}$, so
$\DS i(f) \in C^k_b(\overline{\Omega})$
...
Exponent $\mu$} on the open set $\DS \Omega \subset \RR^n$,
denoted $\DS C^{k,\mu}(\overline{\Omega})$, is
the set of all functions $\DS f \in C_{bu}^k(\overline{\Omega})$
...
\begin{enumerate}
\item $\DS C^{k,\mu}(\overline{\Omega})$ is a Banach space
...
$\DS H^{m,2}(\Omega) \subset C^{k,\mu}(\overline{\Omega})$
and the imbedding
$\DS i_3: H^{m,2}(\Omega) \to C^{k,\mu}(\overline{\Omega})$
...
Let $\DS \Omega \subset \RR^n$ be a bounded open set with
the $(m,2)$-extension property. If $m>k +n/2$, then the imbedding
$\DS i:H^{m,2}(\Omega) \to C_{bu}^k(\overline{\Omega})$
is compact.
\end{cor}

\begin{proof}
\stage{i} To proof that $\DS I: H^{m,2}(\Omega) \to
C^k_{bu}(\overline{\Omega})$ is compact when $\Omega$ is bounded, we first
show that it is enough to prove that the imbedding
$\DS i: H^{m-k,2}(\Omega) \to C^0_{bu}(\overline{\Omega})$ is
compact. Suppose that
$\DS i: H^{m-k,2}(\Omega) \to C_{bu}^0(\overline{\Omega})$
...
Let $\DS \left\{ f_i \right\}_{i=1}^\infty$ be a bounded
sequence in $\DS H^{m,2}(\Omega)$.
\begin{itemize}
\item Then,
$\DS \left\{ \diff^{\VEC{\alpha}_1} f_i \right\}_{i=1}^\infty$
is a bounded sequence in
$\DS H^{m-|\VEC{\alpha}_0|,2}(\Omega) \subset H^{m-k,2}(\Omega)$.
Since we assume that
$\DS i: H^{m-k,2}(\Omega) \to C_{bu}^0(\overline{\Omega})$
is compact, there exists a subsequence
$\DS \left\{ f_{\VEC{\alpha}_1,i} \right\}_{i=1}^\infty$
of $\DS \left\{ f_i \right\}_{i=1}^\infty$
such that
$\DS \left\{ \diff^{\VEC{\alpha}_1} f_{\VEC{\alpha}_1,i}
\right\}_{i=1}^\infty$
converges in $\DS C_{bu}^0(\overline{\Omega})$.
\item We now have that
$\DS \left\{ \diff^{\VEC{\alpha}_2} f_{\VEC{\alpha}_1,i}
\right\}_{i=1}^\infty$ is
a bounded sequence in
$\DS H^{m-|\alpha_1|,2}(\Omega) \subset H^{m-k,2}(\Omega)$.
Since we assume that
$\DS i: H^{m-k,2}(\Omega) \to C_{bu}^0(\overline{\Omega})$
is compact, there exists a subsequence
$\DS \left\{ f_{\VEC{\alpha}_2,i} \right\}_{i=1}^\infty$
of $\DS \left\{ f_{\VEC{\alpha}_1,i} \right\}_{i=1}^\infty$
such that
$\DS \left\{ \diff^{\VEC{\alpha}_2} f_{\alpha_2,i}
\right\}_{i=1}^\infty$
converges in $\DS C_{bu}^0(\overline{\Omega})$.
\item and so on until we reach $\alpha_N$.
\end{itemize}
The sequence
$\DS \left\{ f_{\VEC{\alpha}_N,i} \right\}_{i=1}^\infty$ is such
that
$\DS \left\{ \diff^{\VEC{\alpha}} f_{\VEC{\alpha}_N,i}
\right\}_{i=1}^\infty$
converges in $\DS C_{bu}^0(\overline{\Omega})$ for all
multi-indices $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq k$.  Thus,
$\DS \left\{ f_{\VEC{\alpha}_N,i} \right\}_{i=1}^\infty$
converges in $\DS C_{bu}^k(\overline{\Omega})$.
...
Let $\DS \Omega \subset \RR^n$ be a bounded open set with the
$(k_2,2)$-extension property.  Then, the inclusion
$\DS i:H^{k_2,2}(\Omega) \to H^{k_1,2}(\Omega)$ is a
compact mapping.  In fact, every bounded sequence in
$\DS H^{k_2,2}(\Omega)$ has a subsequence which converges in
$\DS H^{k_1,2}(\Omega)$ for all $k_1 < k_2$.
\index{Rellich's Theorem}
\end{theorem}

\begin{proof}
Suppose that $\DS \{f_i\}_{i=1}^\infty$ is a bounded sequence in
$\DS H^{k_2,2}(\Omega)$.

Choose $\DS \psi\in \DD(\RR^n)$ such that $\psi(\VEC{x})=1$ for all
$\VEC{x}\in \Omega$ and $0\leq \psi(\VEC{x}) \leq 1$ for all
$\DS \VEC{x}\in \RR^n$.  If
$\DS E:H^{k_2,2}(\Omega) \rightarrow H^{k_2,2}(\RR^n)$
is the bounded linear operator satisfying
Definition~\ref{sob_k_ext_prop}, then
$\DS F=\psi E:H^{k_2,2}(\Omega) \rightarrow H^{k_2,2}(\RR^n)$
is a bounded linear mapping such that $F(f)\big|_{\Omega} = f$ for all
$\DS f\in H^{k_2,2}(\Omega)$ and
$\supp F(f) \subset K \equiv \supp \psi$ for all
$\DS f\in H^{k_2,2}(\Omega)$.

Consider the sequence $\DS \{g_i \}_{i=1}^\infty$, where
$g_i = F(f_i)$ for all $i$.  We have that
$\DS g_i \in H^{k_2,2}(\RR^n)$,
$\supp g_i \subset K$ for all $i$, $g_i\big|_{\Omega} = f_i$ for all
$i$, and there exists $C \in \RR$ such that
$\DS \|g_i\|_{k_2,\rho} < C$ for
all $i$ because the sequence $\DS \{f_i\}_{i=1}^\infty$ is
bounded and $F$ is a bounded linear mapping.

\stage{i}
Choose $\DS \phi \in \DD(\RR^n)$ such that $\phi(\VEC{x})=1$ for all
$\VEC{x}\in K$.  For all $i$, we have that
$\hat{g}_i = \hat{g}_i \ast \hat{\phi}$
because $g_i = g_i \, \phi$.  Note that $\hat{g}_i \ast \hat{\phi}$ is
defined by the standard integration formula because
$\DS g_i \in H^{k_2,2}(\RR^n) \subset L^2(\RR^n)$ implies
$\DS \hat{g}_i \in L^2(\RR^n)$, and $\phi \in \SS(\RR^n)$
implies $\DS \hat{\phi} \in \SS(\RR^n) \subset L^2(\RR^n)$
by Proposition~\ref{distr_frr_tempT}.  Moreover, it follows from
item 3 of Proposition~\ref{distr_frr_tempD}
that $\DS \hat{g}_i = \hat{g}_i \ast \hat{\phi} \in C^\infty(\RR^n)$
because
$\DS \hat{\phi} \in \SS(\RR^n)$ and
$\DS \hat{g}_i \in L^2(\RR^n) \subset \SS'(\RR^n)$.
...
\end{align*}
for all $\DS \VEC{y}\in \RR^n$ and all $i$.  Similarly,
since $\DS \diff^{\VEC{e}_j} \hat{g}_i
...
\end{align*}
for all $\DS \VEC{y}\in \RR^n$ and all $i$, and $1 \leq j \leq n$.

Hence, the sequence of functions $\DS \{\hat{g}_i\}_{i=1}^\infty$ is
uniformly bounded and equicontinuous on $\DS \RR^n$ because
\[
\| \graD_{\VEC{y}} \hat{g}_i(\VEC{y}) \| \leq
\sqrt{n}\,2^{k_2/2}\,C
\,\max_{1\leq j\leq n} \left\|\pdydx{\phi}{y_j}\right\|_{k_2,\rho}
\]
for $\DS \VEC{y}\in \RR^n$ and $i > 0$,
where $\|\cdot\|$ is the Euclidean norm in $\DS \RR^n$.  The
equicontinuity comes from the Mean Value Theorem.

It follows from Arzel\'a-Ascoli Theorem that there exists a subsequence
$\DS \left\{ \hat{g}_i^{[1]}\right\}_{i=1}^\infty$ of
$\DS \{\hat{g}_i\}_{i=1}^\infty$ that
...
$\DS \left\{ \hat{g}_{i}^{[2]}\right\}_{i=1}^\infty$ of
$\DS \left\{ \hat{g}_i^{[1]}\right\}_{i=1}^\infty$ that
converges uniformly on the compact set of $\overline{B_2(\VEC{0})}$.
In general,
having the $\DS \left\{\hat{g}_i^{[N]}\right\}_{i=1}^\infty$
...
$\DS \left\{ \hat{g}_i^{[N+1]}\right\}_{i=1}^\infty$ of
$\DS \left\{\hat{g}_i^{[N]}\right\}_{i=1}^\infty$ that
converges uniformly on the compact set of $\overline{B_{N+1}(\VEC{0})}$.
The sequence $\DS \left\{ \hat{g}_i^{[i]}\right\}_{i=1}^\infty$ is a
subsequence of $\DS \{\hat{g}_i\}_{i=1}^\infty$ that
...
$\DS \left\{g_i^{[i]}\right\}_{i=1}^\infty$ converges in
$\DS H^{k_1,2}(\RR^n)$ since
$\DS g_i^{[i]}\big|_{\Omega} = f_i^{[i]}$ for all $i$.
...
$\DS 2C \left(1+N^2\right)^{k_1-k_2} < \epsilon/2$ because $k_1-k_2<0$.
For this $N$, since $\DS \left\{ \hat{g}_i^{[i]} \right\}_{i=1}^\infty$
...
\]
for $j,k > J$.  Thus,
$\DS \left\|g_i^{[i]} - g_j^{[j]}\right\|_{k_1,\rho}^2 < \epsilon$
for $j,k > J$.
This prove that $\DS \left\{g_i^{[i]} \right\}_{i=1}^\infty$
is a Cauchy sequence in
$\DS H^{k_1,2}(\RR^n)$.  Since $\DS H^{k_1,2}(\RR^n)$
is a Banach space, this series converges in
$\DS H^{k_1,2}(\RR^n)$.
\end{proof}

\begin{theorem}[Poincaré's Ineguality] \label{sob_pt_carre}
Let $\DS \Omega \subset \RR^n$ be an open subset contained
in the region $\DS R= \{ \VEC{x} \in \RR^n : a < x_n < b \}$.  Let
\begin{equation} \label{sob_pc_norm}
\|f \|_{k,2,\ast} = \left( \sum_{|\VEC{\alpha}|=k}
\| \diff^{\VEC{\alpha}} f \|_{2,\Omega}^2 \right)^{1/2}
\end{equation}
for $\DS f \in H^{k,2}_0(\Omega)$.
Then, there exists a constant $C$ such that
$\DS \|f\|_{k,2,\Omega} \leq C \| f \|_{k,2,\ast}$ for
all $\DS f \in H^{k,2}_0(\Omega)$.
\index{Poincaré's Ineguality}
\end{theorem}

\begin{rmk}
If follows from Poincaré's Inequality that, if
$\DS \Omega \subset \RR^n$ is an open subset contained between two
parallel hyperplanes, then $\|\cdot\|_{k,2,\ast}$ is a norm on
$\DS H^{k,2}_0(\Omega)$ equivalent to the norm
...
\end{equation}
for $\DS f \in H^{k,2}_0(\Omega)$ and $|\alpha|<k$.

Recall that the $\DS \diff^{\VEC{\alpha}}$ are derivatives
...
$\DS \| \diff^{\VEC{\alpha}} g \|_{2,\Omega} \leq \|g\|_{k,2}$
and $\DS \|g\|_{k,2,\ast} \leq \|g\|_{k,2,\Omega}$ for
$\DS g \in H^{k,2}_0(\Omega)$ and $0\leq |\VEC{\alpha}|<k$.  Hence, if
$\DS \left\{\phi_i\right\}_{i=1}^\infty$ converges to 
$\DS f \in H^{k,2}_0(\Omega)$ according to the norm
$\|\cdot\|_{k,2,\Omega}$, then
$\DS \| \diff^{\VEC{\alpha}} \phi_i \|_{2,\Omega}
\rightarrow \| \diff^{\VEC{\alpha}} f \|_{2,\Omega}$
and $\DS \|\phi_i\|_{k,2,\ast} \rightarrow \|f\|_{k,2,\ast}$
as $i\rightarrow \infty$.  So,
$\DS \| \diff^{\VEC{\alpha}} \phi_i \|_{2,\Omega}
\leq C \|\phi_i\|_{k,2,\ast}$ for
all $i$ implies that
$\DS \| \diff^{\VEC{\alpha}} f \|_{2,\Omega} \leq C \|f\|_{k,2,\ast}$.

The proof of (\ref{sobPtCarreEq1}) is by induction on $k$.  Consider $k=1$.

We write $\DS \VEC{x}\in \RR^n$ as
$\VEC{x} = (\breve{\VEC{x}}, x_n)$ with
$\DS \breve{\VEC{x}} \in \RR^{n-1}$ and $x_n \in \RR$.
Since \\
$\DS \phi(\VEC{x})
...
&\quad \leq (b-a) \int_a^b \left( \int_{\RR^{n-1}}
...
for $k=j+1$.  Note that
$\DS H^{j+1,2}_0(\Omega) \subset H^{j,2}_0(\Omega)$.
By induction, there exists a constant $C_1$ such that
$\DS \| \diff^{\VEC{\alpha}} \phi \|_{2,\Omega}
...
The reasoning for $k=1$ applied to $\DS \diff^{\VEC{\alpha}} \phi$
instead of $\phi$ shows
that $\DS \|\diff^{\VEC{\alpha}} \phi\|_{2,\Omega} \leq
...
= \|\phi\|_{j+1,2,\ast}^2
\]
for all $\phi \in \DD(\Omega)$.  Thus, with $C_2=b-a$, we have that
$\DS \| \diff^{\VEC{\alpha}} \phi \|_{2,\Omega}
\leq C_2 \|\phi\|_{j+1,2,\ast}$ for
$|\VEC{\alpha}|=j$ and $\phi \in \DD(\Omega)$.
...
the fractional Sobolev spaces $\DS H^{s,2}(\RR^n)$ to define
the trace operators.
...
theorem for $\DS u \in H^{1,2}(\Omega)$, where $\Omega$ is a
bounded open subset of $\DS \RR^n$ with a nice boundary.

\begin{prop}
Suppose that $\Omega$ is a bounded open subset of $\DS \RR^n$ and that
$\partial \Omega$ is a manifold of class $\DS C^2$.  There
...
\leq C \| u\|_{1,2,\Omega}
\]
for all $\DS u \in C^1(\overline{\Omega})$.
\end{prop}
...
\end{align*}
with $\DS C^2 = 2(n+1)C_1$.  Note that we have used the inequality
$\DS 2ab \leq a^2+b^2$ to get the third inequality above.
\end{proof}

\begin{cor}[Elementary trace operator]
Suppose that $\Omega$ is a bounded open subset of $\DS \RR^n$ and that
$\partial \Omega$ is a manifold of class $\DS C^2$.  The
restriction map
\begin{align*}
...
$\DS T:H^{1,2}(\Omega) \rightarrow L^2(\partial \Omega)$.
\end{cor}
...
\end{equation}
for all $\DS u \in C^1(\overline{\Omega})$, 
and $\DS C^1(\overline{\Omega}) \supset C^\infty(\overline{\Omega})$
is dense in $\DS H^{1,2}(\Omega)$ with
respect to the norm $\|\cdot\|_{1,2,\Omega}$, we may
extend the restriction map $T$ to $\DS H^{1,2}(\Omega)$ as follows.

Given $\DS u\in H^{1,2}(\Omega)$, choose
$\DS \left\{ u_j\right\}_{j=0}^\infty
\subset C^1(\overline{\Omega})$ such that
$\|u_j - u\|_{1,2,\Omega} \rightarrow 0$ as $j\rightarrow \infty$.
From (\ref{sobolev_easy_trace}),
$\DS \left\{ u_j\big|_{\partial \Omega} \right\}_{j=0}^\infty$ is a
Cauchy sequence in $\DS L^2(\partial \Omega)$.  Thus there exists
$\DS f \in L^2(\partial \Omega)$ such
that $\DS \left\{ u_j\big|_{\partial \Omega} \right\}_{j=0}^\infty$
converges to $f$ in $\DS L^2(\partial \Omega)$.  We set $T(u) = f$.

The procedure is independent of the chosen sequence 
$\DS \left\{ u_j\right\}_{j=0}^\infty \subset C^1(\overline{\Omega})$.
If
$\DS \left\{ v_j\right\}_{j=0}^\infty \subset C^1(\overline{\Omega})$
is another sequence such that
$\|v_j - u\|_{1,2,\Omega} \rightarrow 0$ as $j\rightarrow \infty$ and
$\DS g \in L^2(\partial \Omega)$ is the limit of
$\DS \left\{ v_j\big|_{\partial \Omega} \right\}_{j=0}^\infty$ in
$\DS L^2(\partial \Omega)$, then (\ref{sobolev_easy_trace}) yields
\[
...
$\DS H^k(\Omega)$ is $\DS \Omega = \RR^n_+$.
Its boundary is the hyperplane
$\DS \RR^n_0 \equiv \{ \VEC{x}\in \RR^n : x_n = 0 \} \cong \RR^{n-1}$.

\begin{prop} \label{TraceRnRnp}
There exists a bounded linear mapping
$\DS T:H^k(\RR^n) \rightarrow H^{k-1/2}(\RR^{n-1})$
such that
$\DS T(\phi)(\VEC{z}) = \phi(\VEC{z},0)$
for all $\DS \phi\in\SS(\RR^n)$ and all
$\DS \VEC{z} \in \RR^{n-1}$.
The operator $T$ is called a
{\bfseries trace operator}\index{Sobolev Space!Trace Operator},

Moreover, if $\DS f \in H^k(\RR^n)$ and
$\DS W \subset \RR^n$ is an open set
such that $\supp f \subset W$, then we may assume that
$\DS \supp T(f) \subset W \cap \RR^n_0$.
\end{prop}
...
&\qquad = \int_{-\infty}^\infty \left( 1 + z^2\right)^{-k} \dx{\mu(z)}
...
for all $\DS \phi \in \SS(\RR^n)$.  Since
$\DS \SS(\RR^n)$ is dense in 
$\DS H^{k,2}(\RR^n)$, we can extend $T$ to
$\DS H^{k,2}(\RR^n)$ and still
have $\DS \|T(f)\|_{k-1/2,\rho} \leq \sqrt{C}\, \|f\|_{k,\rho}$ for all
$\DS f \in H^{k,2}(\RR^n)$.

For the last statement of the theorem, choose
$\DS \phi \in \DD(\RR^n)$
such that $\phi(\VEC{x}) = 1$ for $\VEC{x} \in \supp f$ and
$\phi(\VEC{x}) = 0$ for $\DS \VEC{x} \in \RR^n \setminus W$.
Then $f = \phi f$ and $T(f) = T(\phi) T(f)$
\footnote{We have by construction that
$T(\phi \phi_i) = T(\phi) T(\phi_i)$ for
$\DS \phi \in \DD(\RR^n)$ and
$\DS \phi_i \in \SS(\RR^n)$.  If $\phi_i \to f$ in
$\DS H^k(\RR^n)$ as $i \to \infty$,
then $\phi \phi_i \to \phi f$ in $\DS H^k(\RR^n)$
according to Proposition~\ref{sob_T_dd_wk2}, and
$\DS T(\phi f) = \lim_{i\to \infty} T(\phi \phi_i) =
...
$\DS E: H^k(\RR^n_+) \to H^k(\RR^n)$ be a strong
$k$-extension operator.  Such an operator exists according to
Theorem~\ref{sob_Rp_ext}.
Then, $\DS T_0 = T \circ E:H^k(\RR^n_+)
\rightarrow H^{k-1/2}(\RR^{n-1})$ is continuous and onto.
Moreover, if $\DS f = \phi\big|_{\RR^n_+}$ for some
$\DS \phi \in \SS(\RR^n)$, then
$T_0(f)(\VEC{z}) = f(\VEC{z},0)$ for $\DS \VEC{z} \in \RR^{n-1}$.
...
$\DS S_0:H^{k-1/2}(\RR^{n-1}) \to H^k(\RR^n_+)$.
Since $\DS \SS(\RR^{n-1})$ is dense in
$\DS H^{k-1/2}(\RR^{n-1})$,
we only have to defined the mapping $S_0$ on $\DS \SS(\RR^{n-1})$.

We consider the mapping $\DS S:\SS(\RR^{n-1}) \to \SS(\RR^n)$
defined by
\[
...
\]
for $\DS \phi \in \SS(\RR^{n-1})$ and
$\DS \VEC{x} \in \RR^n$, where
$\DS C = (2\pi)^{-1/2} \int_{\RR} (1+x^2)^{-k}\dx{x}$ and
$\DS \VEC{y} = (\breve{\VEC{y}},y_n) \in \RR^n$.  Since the Fourier
...
$\DS H^{k-1/2}(\RR^{n-1})$ to $\DS H^k(\RR^n)$.
We have
\begin{align*}
\|S(\phi)\|_{k,\rho}^2
...
$\DS y_n = \left(1 + \|\breve{\VEC{y}}\|^2\right)^{1/2} u$.

We have shown that
$\DS \|S(\phi)\|_{k,\rho}
= (2\pi)^{1/4} C^{-1/2} \|\phi\|_{k-1/2,\rho}$
for all $\DS \phi \in \SS(\RR^{n-1})$.
Since $\DS \SS(\RR^{n-1})$ is dense in
$\DS H^{k-1/2}(\RR^{n-1})$.  We can
extend $S$ to $\DS H^{k-1/2}(\RR^{n-1})$ and still
have that
$\DS \|S(f)\|_{k,\rho} = (2\pi)^{1/4} C^{-1/2} \|f\|_{k-1/2,\rho}$
for all $\DS f \in H^{k-1/2}(\RR^{n-1})$.

The operator $S_0$ is defined by $S_0 = i \circ S$ where
$\DS i: H^{k}(\RR^n) \to H^{k}(\RR^n_+)$ is the
continuous mapping defined by $\DS i(f) = f\big|_{\RR^n_+}$ for all
$\DS f \in H^{k}(\RR^n)$.  We obviously have that
$\DS \|i(f)\|_{k,2,\RR^n_+} = \left\|f\right\|_{k,2,\RR^n_+}
...
= \phi(\VEC{z})  
\end{align*}
for all $\DS \VEC{z} \in \RR^{n-1}$.  Therefore,
$T_0(S_0(\phi))(\VEC{z}) = T(S(\phi))(\VEC{z}) = S(\phi)(\VEC{z},0)
= \phi(\VEC{z})$ for all $\DS \VEC{z} \in \RR^{n-1}$ and all
$\DS \phi \in \SS(\RR^{n-1})$.  Hence,
$T_0(S_0(\phi)) = \phi$ for all $\DS \phi \in \SS(\RR^{n-1})$.
We get that $T_0(S_0(f)) = f$ for all
$f \in \DS H^{k-1/2}(\RR^{n-1})$
because $\DS \SS(\RR^{n-1})$ is dense in
$\DS H^{k-1/2}(\RR^{n-1})$.
\end{proof}

Since $\DS \pdydxn{}{x_n}{j}$ is a bounded operator from
$\DS H^k(\RR^n)$ to $\DS H^{k-j}(\RR^n)$ for
$j\leq k$, we may define
$\DS T_j : H^k(\RR^n_+) \to H^{k-j -1/2}(\RR^{n+1})$ 
as $\DS T_j = T \circ \pdydxn{}{x_n}{j} \circ E$, where $T$
...
integer, then
$\DS T_j = T \circ \pdydxn{}{x_n}{j} \circ E:H^k(\RR^n_+)
\rightarrow H^{k-j-1/2}(\RR^{n-1})$
is continuous and onto.
Moreover, if $\DS f = \phi\big|_{\RR^n_+}$ for
$\DS \phi \in \SS(\RR^n)$, then
$\DS T_j(f)(\VEC{z}) = \pdydxn{f}{x_n}{j}(\VEC{z},0)$ for
$\DS \VEC{z} \in \RR^{n-1}$.
...
\begin{proof}
Since $T$, $\DS \pdydxn{}{x_n}{j}$ and $E$ are continuous,
we obviously have that their composition
$\DS T_j = T \circ \pdydxn{}{x_n}{j} \circ E$ is continuous.

To prove that $T_j$ is onto, we do as we did for $T_0$ in the proof
of Theorem~\ref{TraceT0}; namely, we build a right inverse
$\DS S_j : H^{k-j-1/2}(\RR^{n-1}) \to H^k(\RR^n_+)$ for $T_j$.
Since $\DS \SS(\RR^{n-1})$ is dense in
$\DS H^{k-j-1/2}(\RR^{n-1})$,
we only have to define $S_j$ on $\DS \SS(\RR^{n-1})$.

We first consider the mapping $\DS S:\SS(\RR^{n-1}) \to \SS(\RR^n)$
defined by
\[
...
for $\DS \phi \in \SS(\RR^{n-1})$ and
$\DS \VEC{x} \in \RR^n$, where
$\DS C = (2\pi)^{-1/2} j! \int_{\RR} (1+x^2)^{-k+j}\dx{x}$ and
$\VEC{y} = (\breve{\VEC{y}},y_n) \in \RR^n$.

To prove that $S$ is a continuous mapping from
$\DS H^{k-j-1/2}(\RR^{n-1})$ to $\DS H^k(\RR^n)$, we
note that
\begin{align*}
&\|S(\phi)\|_{k,\rho}^2 = \int_{\RR^n} \left| \F(S(\phi))(\VEC{y}) \right|^2
\left( 1 + \|\VEC{y}\|^2\right)^k \dx{\VEC{y}} \\
& = \int_{\RR^n} \left|\pdfdxn{\left(
\frac{(1+\|\breve{\VEC{y}}\|^2)^{k-j-1/2}}{C (1+ \|\VEC{y}\|^2)^{k-j}}
...
(1+\|\breve{\VEC{y}}\|^2)^{2k-2j-1} \left| \F(\phi)(\breve{\VEC{y}}) \right|^2
  \dx{\breve{\VEC{y}}} \\
& = \int_{\RR^{n-1}} \frac{1}{C^2} \left( \frac{1}{(1+\|\breve{y}\|_2)^{k-j-1/2}}
...
also note that $\DS \pdydx{}{y_n} =
\left(1 + \|\breve{\VEC{y}}\|^2\right)^{-1/2} \pdydx{}{u}$.

We have shown that $\|S(\phi)\|_{k,\rho} = Q_0 \|\phi\|_{k-1/2,\rho}$
for all $\DS \phi \in \SS(\RR^{n-1})$ and some constant $Q_0$.
Since $\DS \SS(\RR^{n-1})$ is dense in
$\DS H^{k-j-1/2}(\RR^{n-1})$.  We can
extend $S$ to $\DS H^{k-j-1/2}(\RR^{n-1})$ and still
have $\DS \|S(f)\|_{k,\rho} = Q_0 \|f\|_{k-j-1/2,\rho}$ for all
$\DS f \in H^{k-j-1/2}(\RR^{n-1})$.

The operator $S_j$ is defined by $S_j = i \circ S$ where
$\DS i: H^{k}(\RR^n) \to H^{k}(\RR^n_+)$ is the
continuous mapping defined by $\DS i(f) = f\big|_{\RR^n_+}$ for all
$\DS f \in H^{k}(\RR^n)$.  Because of the equivalence between the
norms $\|\cdot\|_{k,2,\RR^n}$ and $\|\cdot\|_{k,\rho}$, there exists a
constant $Q_1$ such that
$\DS \|S_j(f)\|_{k,2,\RR^n_+} \leq Q_1 \|f\|_{k-j-1/2,\rho}$ for all
$\DS f \in H^{k-j-1/2}(\RR^{n-1})$.

To complete the proof of the theorem, we now show
that $S_j$ is the right inverse of $T_j$.
Using the substitution
$\DS y_n = \left(1 + \|\breve{\VEC{y}}\|^2\right)^{1/2} u$, we
obtain
\begin{align*}
...
= \phi(\VEC{z})
\end{align*}
for all $\DS \VEC{z} \in \RR^{n-1}$, we get
\[
  T_j(S_j(\phi))(\VEC{z}) = T \left( \pdfdxn{S(\phi)}{x_n}{j} \right)
  = \pdfdxn{S(\phi)}{x_n}{j}(\VEC{z},0) = \phi(\VEC{z})
\]
for all $\DS \VEC{z} \in \RR^{n-1}$ and all
$\DS \phi \in \SS(\RR^{n-1})$.

Hence, $T_j(S_j(\phi)) = \phi$ for all
$\DS \phi \in \SS(\RR^{n-1})$.  We get
that $T_j(S_j(f)) = f$ for all $f \in \DS H^{k-j-1/2}(\RR^{n-1})$
because $\DS \SS(\RR^{n-1})$ is dense in
$\DS H^{k-j-1/2}(\RR^{n-1})$.
\end{proof}

\begin{prop} \label{sobTrace0K}
Given $\DS f \in H^k(\RR^n_+)$ and
$\DS T_j:H^k\left(\RR^n_+\right) \to
H^{k-j-1/2}\left(\RR^{n-1}\right)$
defined in Theorem~\ref{TraceT0} for $j=0$ and Theorem~\ref{TraceTj}
for $j>0$.  If $\DS \supp f \subset \RR^n_+$ is compact,
then $T_j(f) = 0$.  
\end{prop}

\begin{proof}
Since $\supp f \subset \RR^n_+$ is compact, we have that
$\DS f \in H^k_0(\RR^n_+)$ according to Lemma~\ref{sob_w0_trad1}.
So, there exists a sequence
$\DS \left\{ \xi_i \right\}_{i=1}^\infty \subset \DD(\RR^n_+)$
that converges to $f$ in $\DS H^k_0(\RR^n_+)$.

Since
$\DS T_j(\xi_i)(\VEC{z}) = \pdydxn{\xi_i}{x_n}{j}(\VEC{z},0) = 0$
for $\VEC{z} \in \RR^{n-1}$, we get from the continuity of $T_j$ on
$\DS H^k(\RR^n_+)$ that
$\DS T_j(f) = \lim_{i\to \infty} T_j(\xi_i) = 0$.
\end{proof}

\begin{prop} \label{sobTjDomain}
Given $\DS f \in H^k(\RR^n_+)$ and
$\DS T_j:H^k\left(\RR^n_+\right) \to H^{k-j-1/2}\left(\RR^{n-1}\right)$
defined in Theorem~\ref{TraceT0} for $j=0$ and Theorem~\ref{TraceTj}
for $j>0$.  If $\DS V \subset \RR^n$ is an open set such that
$V \supset K = \{ (\VEC{z},0) : \VEC{z} \in \supp T_j(f)$, then there
exists $\DS \tilde{f} \in H^k(\RR^n_+)$ such that
...
Since $\DS \DD(\RR^n)\big|_{\RR^n_+}$ is dense in
$\DS H^k(\RR^n_+)$, there exists a sequence
$\DS \left\{\xi_i\right\}_{i=1}^\infty \subset \DD(\RR^n)$
such that $\xi_i \to f$ in $\DS H^k(\RR^n_+)$.
Since $\|g\|_{j,2,\RR^n_+} \leq \|g\|_{k,2,\RR^n_+}$ for $g \in H^k(\RR^n_+)$
and $0 \leq j \leq k$, we also have that $\xi_i \to f$ in
$\DS H^j(\RR^n_+)$ as $i \to \infty$ for $j<k$.

Since $\DS \phi \in \DD(\RR^n)$, we have from
Lemma~\ref{sob_T_dd_wkp} that 
$\phi \xi_i \to \phi f$ in $\DS H^j(\RR^n_+)$ as $i \to \infty$
...
T_m(\xi_i)(\VEC{z})
\end{align*}
for all $\DS \VEC{z} \in \RR^{n-1}$.
Since $\xi_i \to f$ in $\DS H^m(\RR^n_+)$ as $i \to \infty$ for
$0 \leq m \leq j$, we get that
$\DS T_m(\xi_i) = \pdydxn{\xi_i}{x_n}{m} \to T_m(f)$
in $\DS H^{k-m-1/2}(\RR^{n-1})$
as $i \to \infty$ for $0 \leq m \leq j$.  It follows from
Lemma~\ref{sob_T_dd_wk2} that
$\DS \pdydxn{\phi}{x_n}{j-m}\, T_m(\xi_i) 
\to \pdydxn{\phi}{x_n}{j-m}\, T_m(f)$ in
$\DS H^{k-j-1/2}(\RR^{n-1})$ as $i \to \infty$ for
$0 \leq m \leq j$.
...
\]
Since $\DS \pdydxn{\phi}{x_n}{j-m}(\VEC{z},0) = 0$
for $\VEC{z} \in W \supset K$ and $0 \leq m<j$, and
$\DS \phi(\VEC{z},0) = 1$ for $\VEC{z} \in W \supset K$, we
get that $\DS T_j(\phi f) = T_j(f)$.  We can take
$\tilde{f} = \phi f$.
\end{proof}
...
$\DS T_m(f) = g_m \in H^{k-m-1/2}(\RR^{n-1})$ for
$0 \leq m \leq j$ if we assume that $k > 1/2 +j$.

Because $T_m$ is onto, we know that there exists
$\DS f_m \in H^k(\RR^n_+)$ such that $\DS T_m(f_m) = g_m$
for each $m$.  But, we want something stronger.  We want $f_m$ to be
the same for all $m$.  How can we find $\DS f \in H^k(\RR^n_+)$ such
that $\DS T_m(f) = g_m$ for $0 \leq m \leq j$?  To develop a
...
$\DS \pdfdxn{S(f)}{x_n}{m}(\VEC{z},0) = 0$ for $0 \leq m < j$
and $\DS f \in H^{k-j-1/2}(\RR^{n-1})$ because it is true
for $\DS \phi \in \SS(\RR^{n-1})$.
...
subsets $\Omega$ of $\DS \RR^n$ with a smooth boundary.

\begin{theorem} \label{TraceTjOmega}
Let $\Omega$ be an open subset of $\DS \RR^n$ with a bounded boundary
$\partial \Omega$.  Assume that $\Omega$ satisfies the uniform
$\DS C^k$-regularity condition.  If $k > 1/2 +j$ with
$j \in \NN$, then there exists a continuous and surjective linear operator
$\DS \TT_j:H^k(\Omega) \to H^{k-j-1/2}(\partial \Omega)$ such
that $\DS \TT_j(f)(\VEC{z}) = \pdydxn{f}{\nu}{j}(\VEC{z})$ for
$\VEC{z} \in \partial \Omega$ and
$\DS T(f) = \phi\big|_{\partial \Omega}$ for
$\DS \phi \in \DD(\RR^n)$.
...
the uniform $\DS C^k$-regularity condition given in
Definition~\ref{UnifKregDefn} is given for a finite atlas
$\DS \{ (U_i,\psi_i) \}_{i=1}^I$ for $\partial \Omega$.

We have from item (3) in Definition~\ref{UnifKregDefn} that
$\DS \psi_i(U_i \cap \partial \Omega)
= B_0 \equiv B_1(\VEC{0}) \cap \RR^n_0$ for all $i$.

As stated in the definition of uniform $C^k$-regularity, there exists
$\delta>0$ such that\\
$\DS \left\{ \VEC{x} \in \RR^n :
\dist{\VEC{x}}{\partial \Omega}  < \delta \right\} \subset
\bigcup_{i=1}^I U_i$.
Let $\DS U_0 = \left\{ \VEC{x} \in \Omega :
\dist{\VEC{x}}{\partial \Omega}  > \delta/2 \right\}$.
We then have that $\DS \{ U_i \}_{i=0}^I$ is an open cover of
$\Omega$.
Let $\DS \{ \phi_i \}_{i=0}^I$ be a partition of unity for
$\Omega$ subordinated to $\DS \{ U_i \}_{i=0}^I$.

\subI{Definition of $\TT_j$}
Given $\DS f \in H^k(\Omega)$, we have that
$\DS f = \sum_{i=0}^I f_i$ where
$\DS f_i \equiv \phi_i f \in H^k(\Omega)$ and
$\supp f_i \subset U_i \cap \overline{\Omega}$.

Since $\supp f_0 \subset U_0 \subset \Omega$, we have that
$\DS \TT_j(f_0) = 0 \in H^{k-j-1/2}(\partial \Omega)$
...
set $\DS W \subset \RR^n_+$ such that $\psi_0$ and
$\psi_0^{-1}$ are of class $\DS C^k$.  Then
$\DS h_0 \equiv f_0 \circ \psi_0^{-1} \in H^k(W)$
can be extended to $\DS H^k(\RR_+^n)$ by
$h_0(\VEC{y}) = 0$ for $\VEC{y} \in \RR_+^n \setminus W$
because $\DS \supp h_0 \subset W$.
...
Lemma~\ref{sobVsubsetU} that $\DS f_i \in H^k(U_i \cap \Omega)$.

Let $\DS B_+ = B_1(\VEC{0}) \cap \RR^n_+$.
We have from Proposition~\ref{sob_cv} that
$\DS h_i \equiv f_i \circ \psi_i^{-1} \in H^k(B_+)$ and
$\DS \supp h_i \subset B_+ \cup B_0$.  Therefore, we may
extend $h_i$ to get $\DS h_i \in H^k(\RR^n_+)$ by defining
$h_i(\VEC{y}) = 0$ for $\DS \VEC{y} \in \RR^n_+ \setminus B_+$.
This can be justified by slightly modifying the proof of
Lemma~\ref{sob_denselem1}.

Let $\DS q_i = T_j(h_i) \in H^{k-j-1/2}(\RR^{n-1})$,
where $\DS T_j:H^k(\RR^n_+) \to H^{k-j-1/2}(\RR^{n-1})$ is
the trace operator given in Theorem~\ref{TraceT0} for $j=0$ or
Theorem~\ref{TraceTj} for $j>0$.  We have that
$\DS q_i\big|_{B_1(\VEC{0})} \in H^{k-j-1/2}(B_1(\VEC{0}))$.
It follows from the definitions $E$ in Theorem~\ref{sob_Rp_ext}
that $\DS \supp E(h_i) \subset B_1(\VEC{0}) \subset \RR^n$,
...
We get that
$\DS g_i \equiv q_i \circ \psi_i \in
H^{k-j-1/2}(U_i \cap \partial \Omega)$.
Since $\supp g_i \subset U_i \cap \partial \Omega$, we
may extend $g_i$ to get
$\DS g_i \in H^{k-j-1/2}(\partial \Omega)$ by setting
$g_i(\VEC{x}) = 0$ for $\VEC{x} \in \partial \Omega \setminus U_i$.

A trace operator $\TT_j$ acting on $f$ is defined by
$\DS \TT_j(f) = \sum_{i=0}^I g_i$.
Recall that $g_0 = \TT_j(f_0) = 0$.

\subI{Surjectivity of $\TT_j$}
Given $\DS g \in H^{k-j-1/2}(\partial \Omega)$, we have that
$\DS g = \sum_{i=0}^I (\phi_i g)$ where
$\DS \phi_i g \in H^{k-j-1/2}(\partial \Omega)$ and
$\supp (\phi_i g) \subset U_i \cap \partial \Omega \subset \partial \Omega$.
Hence,
$\DS \phi_i g \in H^{k-j-1/2}(U_i \cap \partial \Omega)$.

It follows from Proposition~\ref{sob_cv} that
$\DS h_i \equiv (\phi_i g) \circ \psi_i^{-1} \in H^{k-j-1/2}(B_0)$.
Since $\DS \supp h_i \subset B_0$, we may extend
$h_i$ to get $h_i \in \DS H^{k-j-1/2}(\RR^n_0)$ by defining
$\DS h_i(\VEC{y}) = 0$ for
$\DS \VEC{y} \in \RR^n_0 \setminus B_0$.

Since $\DS T_j: H^k(\RR^n_+) \to H^{k-j-1/2}(\RR^{n-1})$ is onto, there
exists $\DS q_i \in H^k(\RR^n_+)$ such that 
$\DS T_j (q_i) = h_i$.  According to
Proposition~\ref{sobTjDomain}, we may assume that
$\DS \supp q_i \subset B_1(\VEC{0}) \subset \RR^n$.

Let $f_i = q_i \circ \psi_i$ on $U_i\cap \Omega$.  We have that
$\supp f_i \subset U_i$ and $\DS f_i \in H^k(U_i\cap \Omega)$.
...
If $\DS f = \sum_{i=0}^I f_i$, then
$\DS \TT_j(f) = \sum_{i=0}^I \TT(f_i) = \sum_{i=0}^I \phi_i g = g$ as
required.

\subI{$\TT_j$ for $\DS \mathbf{\phi \in \DD(\RR^n)}$}
For this part of the proof, we assume that the $\psi_i$ and
$\DS \psi_j^{-1}$ are of class $\DS C^\infty$.  We
basically go through the step to define $\TT_j(\phi)$ and show what happen when
$\DS \phi \in \DD(\RR^n)$.

We have that $\DS \phi = \sum_{i=0}^I (\phi_i \phi)$ where
$\DS \phi_i \phi \in \DD\left(U_i \cap \overline{\Omega}\right)$;
in particular, $\supp (\phi_i \phi) \cap \partial U_i = \emptyset$.
We may have that
$\supp (\phi_i \phi) \cap \partial \Omega \neq \emptyset$.

We have that
$\DS h_i \equiv (\phi_i \phi) \circ \psi_i^{-1}
\in \DD\left(\overline{B_+}\right)$; in particular,
$\DS \supp h_i \cap \partial B_1(\VEC{0}) = \emptyset$.  We
extend $h_i$ to get
$\DS h_i \in \DD\left(\overline{\RR^n_+}\right)$ by defining
$h_i(\VEC{y}) = 0$ for
$\DS \VEC{y} \in \overline{\RR^n_+} \setminus B_1(\VEC{0})$.

We have that $\DS T_j(h_i)
= \pdfdxn{((\phi_i \phi) \circ \psi_i^{-1})}{y_n}{j}$ on
$\DS \RR^n_0 \cong \RR^{n-1}$.  Hence
\[
\TT_j(\phi)(\VEC{z})
...
We get the wrong sign because $\DS \pdydx{}{y_n}$ refers to
the normal to $\DS \RR^n_0$ pointing inside the region
$\DS \RR^n_+$ while
...
$\DS C^k$, one needs to prove that
$\DS T_j(f)(\VEC{z}) = \pdfdxn{f}{x_n}{j}(\VEC{z},0)$
for $\DS f \in C_c^k(\RR^n_+)$.  This can easily be proved
because $\DS \DD(\RR^n_+)$ is dense in
$\DS C_c^k(\RR^n_+)$ according to the norm on
$\DS \|\cdot\|_{k,2,\RR^n_+}$.
\end{proof}
...
We can use a trace operator to define $\DS H^{k,2}_0(\Omega)$ where
$\partial \Omega$ is a bounded $\DS C^k$-manifold in
$\DS \RR^n$.  Before doing that, we need the following result.
...
\begin{proof}
\subI{$\mathbf{\subset}$}
Given $\DS f \in H^k_0(\RR^n_+)$, choose a sequence
$\DS \left\{\xi_i\right\}_{i=1}^\infty \subset \DD(\RR^n_+)$
that converges to $f$ in $\DS H^k(\RR^n_+)$.
Since $\|f\|_{j,2,\RR^n_+} \leq \|f\|_{k,2,\RR^n_+}$ for $0\leq j < k$,
we also have that $\DS \left\{\xi_i\right\}_{i=1}^\infty$
converges to $f$ in $\DS H^j(\RR^n_+)$ for $0\leq j < k$.
We have that $\DS \TT_j(\xi_i)(\VEC{z}) =
\pdydxn{\xi_i}{x_n}{j}(\VEC{z},0) = 0$ for
$\DS \VEC{z} \in \RR^{n-1}$, $0 \leq j < k$
and all $i$ because $\DS \supp \xi_i \subset \RR^n_+$.
Hence, we get by continuity of $\TT_j$ on $\DS H^j(\RR^n_+)$
that $\DS \TT_j(f) = \lim_{i\to \infty} \TT_j(\xi_i) = 0$
for $0 \leq j < k$.

\subI{$\mathbf{\supset}$}
Consider $\DS f \in H^{k}(\RR^n_+)$ such that
$T_j(f) = 0$ for $0 \leq j < k$.
According to Proposition~\ref{sob_expand_WKP_Conv}, we 
will have that $\DS f \in H^{k}_0(\RR^n_+)$ if we show that
\[
\underline{f}(\VEC{x}) =
\begin{cases}
...
\end{cases}
\]
is in $\DS H^{k}(\RR^n)$.  We have that $\underline{f}$
defines a distribution on $\DS \RR^n$ because
\[
\int_{\RR^n} \left|\underline{f}\right|^2 \dx{\VEC{x}}
= \int_{\RR^n_+} \left|f\right|^2 \dx{\VEC{x}} < \infty
\]
since $\DS f \in H^k(\RR^n_+)$.

Since $\DS \DD\left(\RR^n\right)\big|_{\RR^n_+}$ is dense in
$\DS H^k(\RR^n_+)$, there exists a sequence
$\DS \left\{\xi_i\right\}_{i=1}^\infty \subset
\DD\left(\RR^n\right)$ such that
$\DS \tilde{\xi}_i \equiv \xi_i\big|_{\overline{\RR^n_+}} \to f$ in
$\DS H^k(\RR^n_+)$ as $i \to \infty$.

Suppose that $\DS \VEC{\alpha} \in \NN^n$ is a multi-index such
...
\end{align}
for all $\DS \phi \in \DD(\RR^n)$.  If $m=0$, the sum in the previous
...
\]
in $\DS H^{k-|\VEC{\alpha}'|-j -1/2}(\RR^{n-1})$ as $i \to \infty$
because
$\DS T_j\left( \tilde{\xi}_i \right) \to  T_j(f)$ in
$\DS H^{k-j -1/2}(\RR^{n-1})$ and
$\DS \diff^{\VEC{\alpha}'}$ is a bounded operator from
$\DS H^{k-j -1/2}(\RR^{n-1})$ to
$\DS H^{k-|\VEC{\alpha}'|-j -1/2}(\RR^{n-1})$ according to 
Theorem~\ref{sob_DinDm}.

Since $\DS H^{k-j -1/2}(\RR^{n-1}) \subset H^0(\RR^{n-1})$
because $\|g\|_{0,\rho} \leq \|g\|_{k-j -1/2,\rho}$ for all\\
$\DS g \in H^{k-j -1/2}(\RR^{n-1})$, we have that
$\DS \pdfdxn{\diff^{\VEC{\alpha}'}
\tilde{\xi}_i}{x_n}{j}(\VEC{z},0) \to 0$
in $\DS H^0(\RR^{n-1})$ as $i \to \infty$ for $0 \leq j <m$.
From Plancherel theorem, we get that
$\DS \pdfdxn{\diff^{\VEC{\alpha}'}
\tilde{\xi}_i}{x_n}{j}(\VEC{z},0) \to 0$
in $\DS L^2(\RR^{n-1})$ as $i \to \infty$ for $0 \leq j <m$.
...
Since $\DS \xi_i\big|_{\overline{\RR^n_+}} \to f$ in
$\DS H^k(\RR^n_+)$ as $i \to \infty$, it also follows from Schwarz
inequality that
$\DS \int_{\RR^n_+} \left(\diff^{\VEC{\alpha}}
\tilde{\xi}_i\right)\, \phi \dx{\VEC{x}}
\to \int_{\RR^n_+} \left(\diff^{\VEC{\alpha}} f\right) \, \phi \dx{\VEC{x}}$
as $i \to \infty$.

If $i \to \infty$ in (\ref{H0TraceEq1}), then we get
\[
\diff^{\VEC{\alpha}} \underline{f} (\phi)
= (-1)^{|\VEC{\alpha}|} \int_{\RR^n}
...
\]
for all $\DS \phi \in \DD(\RR^n)$ \footnote{Note that
$\underline{\tilde{\xi}}_i \to \underline{f}$ as distributions on
$\DS \RR^n$, not necessarily in $\DS H^k(\RR^n)$.}.
Thus, we have for all $|\VEC{\alpha}| \leq k$
that $\DS \diff^{\VEC{\alpha}} \underline{f}
= \underline{\diff^{\VEC{\alpha}} f}$
and $\DS \diff^{\VEC{\alpha}} \underline{f} \in L^2(\RR^n)$ because
\[
\int_{\RR^n} \left|\underline{\diff^{\VEC{\alpha}} f}\right|^2 \dx{\VEC{x}}
= \int_{\RR^n_+} \left|\diff^{\VEC{\alpha}} f\right|^2 \dx{\VEC{x}} < \infty
\]
since $\DS f \in H^k(\RR^n_+)$.
Therefore, $\DS \underline{f} \in H^k(\RR^n)$.
\end{proof}

\begin{theorem} \label{H0TraceOmega}
Let $\DS \Omega \subset \RR^n$ be an open set that satisfies
the uniform $\DS C^k$-regularity condition.  Then
\[
...
Given $\DS f \in H^k_0(\Omega)$, choose a sequence
$\DS \left\{\xi_i\right\}_{i=1}^\infty \subset \DD(\Omega)$
that converges to $f$ in $\DS H^k(\Omega)$.
Since $\|f\|_{j,2,\Omega} \leq \|f\|_{k,2,\Omega}$ for $0\leq j < k$,
we also have that $\DS \left\{\xi_i\right\}_{i=1}^\infty$
converges to $f$ in $\DS H^j(\Omega)$ 
for $0\leq j < k$.  We known that $\TT_j(\xi_i) = 0$ for $0 \leq j < k$
and all $i$.  Hence, we get by continuity of $\TT_j$ on
$\DS H^j(\Omega)$ that $\TT_j(f) = 0$ for $0 \leq j < k$.

\subI{$\mathbf{\supset}$}
Suppose that $\DS f \in H^k(\Omega)$ and
$\TT_j(f) = 0$ for $0 \leq j < k$.

As in the proof of Theorem~\ref{TraceTjOmega}, we may assume that
$\DS \{ (U_i,\psi_i) \}_{i=1}^I$ is a finite atlas for
$\partial \Omega$ with $\psi_i(U_i) = B_1(\VEC{0})$,
$\DS \psi_i(U_i \cap \Omega) = B_+ = B_1(\VEC{0}) \cap \RR^n_+$
and
$\DS \psi_i(U_i \cap \partial \Omega) = B_0 = B_1(\VEC{0})
\cap \RR^n_0$ for all $i$.  We also consider the open set
$\DS U_0 = \left\{ \VEC{x} \in \Omega :
\dist{\VEC{x}}{\partial \Omega}  > \delta/2 \right\}$
associated to the constant $\delta$ provided by the definition
of uniform $\DS C^k$-regularity.
Let $\DS \{ \phi_i \}_{i=0}^I$ be a partition of unity for
$\Omega$ subordinated to the open cover $\DS \{ U_i \}_{i=0}^I$
of $\Omega$.

We have that $\DS f = \sum_{i=0}^I f_i$ where
$\DS f_i \equiv \phi_i f \in H^k(\Omega)$.  If we prove that
$\DS f_i \in H^k_0(\Omega)$ for all $i$, then we will
have that $\DS f \in H^k_0(\Omega)$.
Since $\supp f_0 \subset U_0 \subset \overline{U_0} \subset \Omega$, we
have that $\DS f_0 \in H^k_0(\Omega)$ according to
Lemma~\ref{sob_w0_trad1}.  So, we only have to prove
that $\DS f_i \in H^k_0(\Omega)$ for $0 < i \leq I$.

It can be shown that $T_j(f) = 0$ for
$\DS f \in H^k(\RR_+^n)$ and $0 \leq j <k$
implies that $\DS T_j(\phi f) = 0$ for
$\DS \phi \in \DD(\RR^n)$ and $0 \leq j <k$.   This can be
generalized to show that $\TT_j(f) = 0$ for
$\DS f \in H^k(\Omega)$ and $0 \leq j <k$
implies that $\DS \TT_j(\phi_i f) = \TT_j(f_i) = 0$ for
...
definition of manifold which we are trying to avoid in these lecture notes.}.
...
$\DS T_j(h_i) = 0$ for $0 \leq j <k$, where
$\DS h_i \in H^k(\RR^n_+)$ is defined by
$h_i \equiv f_i \circ \psi_i^{-1}$ on
$\DS B_+ = B_1(\VEC{0}) \cap \RR^n_+$
and $0$ on $\DS \RR^n_+ \setminus B_1(\VEC{0})$.  We also have that
$\supp h_i \subset B_+ \cup B_0$.

It follows from Proposition~\ref{H0Trace} that
$\DS h_i \in H^k_0(\RR^n_+)$.
Therefore, there exists a sequence
$\DS \left\{\xi_m\right\}_{m=1}^\infty \subset \DD(\RR^n_+)$
converging to $h_i$ in $\DS H^k(\RR^n_+)$.  Since
$\supp h_i \subset B_1(\VEC{0})$, we may assume that
$\supp\, \xi_m \subset B_+$.  If it is not
so, choose $\DS \phi \in \DD(\RR^n)$ such that
$\phi(\VEC{x}) = 1$ for $\VEC{x} \in \supp h_i$ and $\phi(\VEC{x}) = 0$ for
$\DS \VEC{x} \in \RR^n \setminus B_1(\VEC{0})$.
Then,
$\DS \supp\, \phi \xi_m \subset B_1(\VEC{0}) \cap \RR^n_+ = B_+$.
It follows from Proposition~\ref{sob_T_dd_wkp} that
$\phi \xi_m \to \phi h_i = h_i$ in $\DS H^k(\RR^n_+)$.
Hence, $\DS \tilde{\xi}_m \equiv (\phi \xi_m) \circ
\psi_i \in C_c^k(\Omega \cap U_i)$
and $\tilde{\xi}_m \to h_i \circ \psi_i = f_i$
in $\DS H^k(\Omega \cap U_i)$.
Hence, $\DS f_i \in H^k_0(U_i \cap \Omega)$
according to the second item of Remark~\ref{sob_compl_Hk2}
and thus $\DS f_i \in H^k_0(\Omega)$ because $f_i = \phi_i f = 0$ on
$\Omega \setminus U_i$.
\end{proof}
...
Suppose $\DS f \in W^{k,p}(\Omega)$ with $k\in \NN$ and let
$U$ be an open subset of $\RR$ such that
$\DS \overline{ f(\Omega)} \subset U$.  If
$\DS \psi \in C^k(U)$ is such that the derivatives
$\DS \psi^{(j)}$ for $0\leq j \leq k$ are bounded on
$\DS \overline{ f(\Omega)}$,
then $\DS \psi \circ f \in W^{k,p}(\Omega)$.
\end{lemma}

\begin{proof}[Proof (Sketch).]
The proof that $\DS \psi \circ f \in W^{k,p}$ requires the
...
$\DS \RR^n$ that satisfies the uniform
$\DS C^k$-regularity condition.  If
$\DS f \in C^{k-1}\left(\overline{\Omega}\right) \cap H^k(\Omega)$,
then
$\DS f \in H^k_0(\Omega)$ if and only if
$\DS \diff^{\VEC{\alpha}} f =0$ on $\partial \Omega$ for
$|\VEC{\alpha}|<k$.
\end{prop}
...
$\DS \TT_j(f)(\VEC{z}) = \pdydxn{f}{\nu}{j}(\VEC{z})$ for
$\VEC{z} \in \partial \Omega$ and $0 \leq j < k$ when
$\DS f \in C^{k-1}\left(\overline{\Omega}\right)$.
...
$\DS \{ U_i \}_{i=1}^I$ for $\partial \Omega$.
As stated in the definition of uniform $\DS C^k$-regularity,
there exists $\delta>0$ such that
$\DS \left\{ \VEC{x} \in \RR^n :
\dist{\VEC{x}}{\partial \Omega}  < \delta \right\} \subset
...
Given $\DS f \in C(\overline{\Omega})$, we can write 
$\DS f = \sum_{j=0}^N f_j$ on $\overline{\Omega}$,
where $f_j \equiv \phi_j f :\overline{\Omega} \rightarrow \RR$ is such that
$\supp f_j \subset U_i \cap \overline{\Omega}$.
We obviously have that $\DS f_j \in C\left(\overline{\Omega}\right)$
and it follows that $\DS f_j \in H^1(\Omega)$.

\stage{$\mathbf{\Leftarrow}$}
Choose $\DS \psi \in \DD(\RR)$ such that
...
by $\DS g_m(\VEC{x}) = \frac{1}{m}\xi(mf_j(\VEC{x}))$ for
$\VEC{x}\in \overline{\Omega}$ is therefore in $\DS H^1(\Omega)$
...
$\DS g_m \in H^1_0(\Omega)$ for all positive integer $m$.

We now show that $g_m \rightarrow f_j$ in $\DS H^1_0(\Omega)$.
Since $\DS H^1_0(\Omega)$ is closed, this will prove that
$\DS f_j \in h^1_0(\Omega)$.

Let $\VEC{\alpha}$ be a multi-index such that $|\VEC{\alpha}|\leq 1$.
We first prove that $\DS \diff^{\VEC{\alpha}} g_m(\VEC{x}) \rightarrow
\diff^{\VEC{\alpha}} f_j(\VEC{x})$ as $m\rightarrow \infty$ for almost all
$\VEC{x}\in \Omega$.
Since $\DS \diff^{\VEC{\alpha}} g_m(\VEC{x})=0$ for all $m$ and
$\VEC{x} \in \Omega \setminus \supp f_j$ because
$\DS \diff^{\VEC{\alpha}} f_j(\VEC{x})=0$ for all
$\VEC{x} \in \Omega \setminus \supp f_j$,
we trivially have that
$\DS \diff^{\VEC{\alpha}} g_m(\VEC{x})
...
We now prove that $\DS |\diff^{\VEC{\alpha}} g_m|^2$ is bounded by
the integrable function $\DS K |\diff^{\VEC{\alpha}} f_j|^2$ for all
...
$m \geq 1$ and $\VEC{x}\in \Omega$, we have
\[
...
Since the sequence
$\DS \left\{ (\diff^{\VEC{\alpha}} g_m)^2 \right\}_{m=1}^\infty$
converges pointwise almost everywhere to
$\DS (\diff^{\VEC{\alpha}} f_j)^2$
and is bounded by the integrable function
$\DS K (\diff^{\VEC{\alpha}} f_j)^2$ on $\Omega$, we get from
the Lebesgue Dominated Convergence Theorem that
$\DS \left\{ \diff^{\VEC{\alpha}} g_m \right\}_{m=1}^\infty$ converges
to $\DS \diff^{\VEC{\alpha}} f_j$ in $\DS L^2(\Omega)$.
Since $\VEC{\alpha}$ such that $|\VEC{\alpha}|\leq 1$ is arbitrary, we
have that $\|g_m - f_j\|_{1,2,\Omega} \rightarrow 0$ as $m\rightarrow \infty$.

\stage{$\mathbf{\Rightarrow}$}
Using the expansion
$\DS f = \sum_{j=0}^N f_j$ given at the beginning of the proof, it
is enough to prove that $\DS f_j \big|_{\partial \Omega} = 0$.
We have that 
$\DS f_j \in W^{1,2}_0(\Omega)$ because $f_j = f \phi_j$ with
$\DS f \in W^{1,2}_0(\Omega)$ and
$\DS \phi_j \in \DD(\RR^n)$.  In fact, for the previous
statement, the readers should convince themselves that if
$\DS \{ \eta_i \}_{i=1}^\infty \subset \DD(\Omega)$
satisfies $\DS \lim_{i\rightarrow \infty} \|\eta_i -f \|_{1,2} = 0$
then
$\DS \{ \phi_j \eta_i \}_{i=1}^\infty \subset \DD(\Omega)$
and
$\DS \lim_{i\rightarrow \infty} \|\phi_j \eta_i -f_j \|_{1,2} = 0$.
Thus $\DS f_j \in W^{1,2}_0(\Omega)$.

Recall that we have from Definition~\ref{UnifKregDefn} that
the $\DS C^1$-diffeomorphism
...
\end{cases}
\]
We have that $\DS \tilde{f}_j \in C\left(\overline{\RR^n_+}\right)$
because $\DS f_j \in C\left(U_j\cap \overline{\Omega}\right)$ and
$\supp f_j \subset U_j\cap \overline{\Omega}$ (Figure~\ref{TraceFig1}).

The problem is reduced to proving that
$\DS \tilde{f}_j \big|_{\RR^n_0} = 0$.
Let $\DS \{ \eta_i \}_{i=1}^\infty \subset \DD(\Omega)$
be a sequence such that
$\DS \lim_{i\rightarrow \infty} \|\eta_i -f_j \|_{1,2,\Omega} = 0$.
Since $\DS \supp f_j \subset U_j \cap \overline{\Omega}$, we
...
\]
We get a sequence
$\DS \{ \tilde{\eta}_i \}_{i=1}^\infty \subset
C^1_c\left(\overline{\RR^n}\right)$
such that $\supp \tilde{\eta}_i \subset B$ and
$\DS \lim_{i\rightarrow \infty}
\|\tilde{\eta}_i -\tilde{f}_j \|_{1,2,\overline{\RR^n_+}} = 0$.

Consider $\DS \VEC{x} = (\breve{\VEC{x}}, x_n) \in \RR^n_+$ where, as
usual, $\DS \breve{\VEC{x}} \in \RR^{n-1}$.  We have
\[
...
respect to the norm $\DS \|\cdot\|_{1,2,\overline{\RR^n_+}}$,
...
\right)^{1/2} \rightarrow 0 \quad \text{as} \quad i\rightarrow \infty
\end{align*}
for $1 \leq m \leq n$, and $\DS \diff^{\VEC{e}_m}$ is as
...
\leq 0
\end{equation}
because $\DS \tilde{f}_j \in C(\overline{\RR^n_+})$ and
$\DS \diff^{e_m}\tilde{f}_j \in L^1(\RR^n_+)$ since
...
Since $\DS \tilde{f}_j \in C(\overline{\RR^n_+})$, we get from
(\ref{sob_Wz_Cz_two}) that $\DS \tilde{f}_j\big|_{\RR^n_0} = 0$.
\end{proof}
...
\begin{defn}
A linear differential operator $L$ with $\DS C^\infty$ coefficients in
$\DS \Omega \subset \RR^n$ is
...
$\DS C^\infty$ coefficients
in $\DS \Omega \subset \RR^n$ and let
$Q(\VEC{x}, \VEC{\xi})$ for $\VEC{x} \in \Omega$ and
$\DS \VEC{\xi} \in \RR^n$ be the principal symbol associated to $L$.
Let
\[
...
then there exists $\DS f \in C^\infty(\Omega)$ such that
$Lu = f$ has no 
