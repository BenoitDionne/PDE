...
Let $\Gamma$ be a curve in $\DS \RR^2$, and let
...
\begin{pmatrix}
\DS \pdydx{\eta}{x} & \DS \pdydx{\eta}{y} \\[0.7em]
\DS \pdydx{\xi}{x} & \DS \pdydx{\xi}{y}
\end{pmatrix} \neq 0
\end{equation}
...
\end{align*}
because
$\DS \left(-\pdydx{\xi}{y}(\VEC{q}), \pdydx{\xi}{x}(\VEC{q})\right)$
is also orthogonal to
$\DS \left(\pdydx{\xi}{x}(\VEC{q}),\pdydx{\xi}{y}(\VEC{q})\right)$,
...
\end{align*}
in (\ref{classif_PDE1}), then we get
...
$\DS \pdydxn{u}{\xi}{j}$ for $j\geq 2$ at the origin.  For 
that, we assume that (\ref{classif_PDE3}) can be solved for
$\DS \pdydxn{u}{\xi}{2}$ at $(\eta,\xi)=(0,0)$.  This is
possible only if
...
(\ref{classif_class2}) determine $\DS \pdydx{u}{x}$ and 
$\DS \pdydx{u}{y}$ uniquely along $\Gamma$ because
\begin{align*}
&\det
\begin{pmatrix}
x_0'(s) & y_0'(s) \\
...
The derivatives $\DS \pdydxn{u}{x}{2}$,
$\DS \pdydxnm{u}{x}{y}{2}{}{}$ and
$\DS \pdydxn{u}{y}{2}$ along $\Gamma$ are the solutions (if
...
\end{pmatrix} \ .
\end{align*}
for $(x,y)$ and $(\zeta_1,\zeta_2)$ in $\DS \RR^2$.
\end{defn}
...
$\DS Q(x_0(s),y_0(s), -y_0'(s) , x_0'(s)) = 0$
for all $s \in I$.
\end{prop}

Since $\DS \left(-y_0'(s) , x_0'(s)\right)$ is orthogonal
to the curve $\Gamma$ at $\DS \left(x_0(s), y_0(s)\right)$.
...
the condition $\DS \pdydx{u}{\nu}\bigg|_\Gamma = g$ may not
be reduced to the simple expression
$\DS \pdydx{u}{\xi}(\eta,0) = g\left(x(\eta,0),y(\eta,0)\right)$.
...
$\DS (\zeta_1,\zeta_2) \in \RR^2 \setminus \{(0,0)\}$ or
$Q(x,y,\zeta_1,\zeta_2) < 0$ for all
$\DS (\zeta_1,\zeta_2) \in \RR^2 \setminus \{ (0,0)\}$
respectively.  There is no point other than the origin where the
principal symbol is null. Therefore, there are not characteristic
curves and, in particular, any curve $\Gamma$ can be used for the conditions
...
\]
If we solve $\DS \dydx{y}{x} = \phi_{+}(x,y)$, then we get a family
of curves $\DS \psi(x,y) = c \in \CC$.
Since
\[
...
positively defined) and $A_{1,1} = 0$.  If we divide by $A_{2,0}$,
...
\beta (\eta,\xi) = 0 \ , 
\end{equation}
where $\DS \alpha_{1,0} = A_{1,0}/A_{2,0}$, etc.
...
\]
is an example of an elliptic equation on all $\DS \RR^2$.
\end{egg}
...
$\DS \RR^2$ along which the principal symbol is null.
...
$\DS \dydx{y}{x} = \phi_{+}(x,y)$ and the second family of
curves is given by the implicit solution of
$\DS \dydx{y}{x} = \phi_{-}(x,y)$.
...
If we substitute $\phi_{\pm}$ in (\ref{classif_PDE6}), then we get
\begin{align*}
...
If we divide by $A_{1,1}$, then we get an equation of the form
...
\beta (\eta,\xi) = 0 \  , 
\end{equation}
where $\DS \alpha_{1,0} = A_{1,0}/A_{1,1}$, etc.
...
\]
is an example of an hyperbolic equation on all $\DS \RR^2$.
...
$\DS p = \pdydx{u}{x}$, (\ref{classif_k3}) becomes
\[
\pdydx{u}{y} + 2 p = 0 \ .
\]
For $x$ fixed, this is a separable ordinary differential equation
whose solution is $\DS p = A(x) e^{-2y}$, where $A$ is an
...
solution of $\DS \dydx{x}{y} = -1$ and another family of curves
$\xi(x,y) = k$ from the implicit solution of
$\DS \dydx{x}{y} = 0$.  We respectively get
$x = -y + \eta$ and $x=\xi$.  Hence $\eta(x,y)=x+y$ and $\xi(x,y) = x$.

Since $\DS \pdydx{\eta}{x} = 1$, $\DS \pdydx{\eta}{y} = 1$,
$\DS \pdydx{\xi}{x} = 1$ and $\DS \pdydx{\xi}{y} = 0$,
we get
\begin{align*}
...
(\ref{classif_egg_hypk1}), then we get
$\DS \pdydxnm{u}{\xi}{\eta}{2}{}{} = 0$.
We integrate with respect to $\xi$ to get
$\DS \pdydx{u}{\eta} = H(\eta)$ for some function
...
\]
We get a family of curves $\eta(x,y) = h$ from the implicit
solution of $\DS \dydx{y}{x} = -\frac{y}{x}$ and another family
of curves $\xi(x,y) = k$ from the implicit solution of
$\DS \dydx{y}{x} = 0$.  From
$\DS \dydx{y}{x} = -\frac{y}{x}$, we get $\eta = xy$.  We get
$\xi = y$ from $\DS \dydx{y}{x} = 0$.
Since $\DS \pdydx{\eta}{x} = y = \xi$,
$\DS \pdydx{\eta}{y} = x = \frac{\eta}{\xi}$,
$\DS \pdydx{\xi}{x} = 0$ and $\DS \pdydx{\xi}{y} = 1$,
we get
\begin{align*}
...
\end{align*}
If we substitute these expressions for the derivatives in
(\ref{classif_egg_hypk2}), then we get
\[
...
$a_{2,0}(x,y) = x$, $\DS a_{1,1}(x,y) = x^2$ and $a_{0,2}(x,y) = 0$.
Hence
\[
\det \begin{pmatrix}
a_{2,0}(x,y) & a_{1,1}(x,y) \\
...
\]
we get a family of curves $\eta(x,y) = h$ from the implicit
solution of $\DS \dydx{y}{x} = 2x$ and another family of curves
$\xi(x,y) = k$ from the implicit solution of
$\DS \dydx{y}{x} = 0$.  The solution of the first equation
is $\DS y = x^2 + \eta$ and the solution of the second is
$y=\xi$.  Hence, $\DS \eta(x,y)=y-x^2$ and $\xi(x,y) = y$.
...
Since $\DS \pdydx{\eta}{x} = -2x = -2\sqrt{\xi-\eta}$,
$\DS \pdydx{\eta}{y} = 1$,
$\DS \pdydx{\xi}{x} = 0$ and $\DS \pdydx{\xi}{y} = 1$,
we get
\begin{align*}
...
\end{align*}
If we substitute these expressions for the derivatives in
(\ref{classif_egg_hypb}), then we get
\begin{align*}
...
implicit solution of $\DS \dydx{y}{x} = \phi(x,y)$.
...
\end{equation}
This equation is
$\DS Q\left(x,y,\pdydx{\eta}{x}(x,y),\pdydx{\eta}{y}(x,y)\right) = 0$.
...
If we divide by $A_{0,2}$, then we get an equation of the form
...
\beta (\eta,\xi) = 0 \  , 
\end{equation}
where $\DS \alpha_{1,0} = A_{1,0}/A_{0,2}$, etc.
...
\]
is an example of a parabolic equation on all $\DS \RR^2$.
\end{egg}
...
Let $\DS v_1 = \pdydx{u}{x}$ and
$\DS v_2 = \pdydx{u}{y}$.  Then,
\[
...
Let $\DS \LL(\RR^n)$ be the space of \nn matrices.  A
...
\end{equation}
where $\DS A:\RR^2\rightarrow \LL(\RR^n)$,
$\DS B:\RR^2\rightarrow \LL(\RR^n)$ and
$\DS c:\RR^{2+n}\rightarrow \RR^n$ are continuous functions.
If we combine the condition
\begin{equation} \label{classif_SPDE1_cond}
v\big|_\Gamma = f
\end{equation}
for some curve $\Gamma$ in the plane and some function
$\DS f:\Gamma \rightarrow \RR^n$ with
...
$\DS \pdydx{v}{x}(x_0(s),y_0(s))$ and the second for
$\DS \pdydx{v}{y}(x_0(s),y_0(s))$ if the matrix
...
for $(x,y)$ and $\DS \xi_1,\xi_2)$ in $\RR^2$.
\end{defn}
...
\diff^{\VEC{\alpha}} u = f \ ,
\end{equation}
where $\DS u:\RR^n \rightarrow \RR$ is at least of class
$\DS C^m$, $\DS f:\RR^n \rightarrow \RR$ is continuous and
$\DS a_{\VEC{\alpha}}:\RR^n \rightarrow \RR$ is of class
$\DS C^\infty$ for all multi-indices involved in the sum.
We will relax the conditions on $f$
and $a_{\VEC{\alpha}}$ starting in Chapter~\ref{elliptic_PDEs}.

\begin{defn}
A subset $S$ of $\DS \RR^n$ is an
{\bfseries hyper-surface}\index{Hyper-Surface} 
if $S$ is an orientable manifold of dimension $n-1$.  We will say that
$S$ is an hyper-surface of class $\DS C^k$ (resp.\ an analytic
hyper-surface) if $S$ is a manifold of class $\DS C^k$
...
$\DS \phi:\RR^n \to \RR$.  If $S$ is of class
$\DS C^k$ (resp. analytic), we may assume that
$\phi$ is of class $\DS C^k$ (resp. analytic).  We will
...
\end{equation}
for $\VEC{x}$ and $\VEC{\xi}$ in $\DS \RR^n$.
\end{defn}
...
\end{equation}
where $\DS u:\RR^n \rightarrow \RR$,
$\DS a_{i,j} \in C^2(\RR^n)$,
$\DS b_i \in C^1(\RR^n)$ and $\DS c \in C(\RR^n)$.
...
In $\DS \RR^2$, this definition is equivalent to
...
$L(\VEC{x}, \diff)$ is
{\bfseries elliptic}\index{Linear Partial Differential Operator!Elliptic}
at $\VEC{x}$ if $\DS Q(\VEC{x}, \VEC{\xi}) \neq 0$ for all
$\DS \VEC{\xi} \in \RR^n\setminus \{\VEC{0}\}$.
\end{defn}
...
operator of order $m$ at $\DS \VEC{x} \in \RR^n$, $n>1$.
\begin{enumerate}
\item If $\DS a_{\VEC{\alpha}}:\RR^n \rightarrow \RR$ for all
...
$\VEC{\xi} \neq \VEC{0}$.
\item If $\DS a_{\VEC{\alpha}}:\RR^n \rightarrow \CC$ for
...
with $\DS (1-s_0)\VEC{\xi}_1 + s_0\VEC{\xi}_2 \neq \VEC{0}$.
This is a contradiction that $L(\VEC{x}, \diff)$ is elliptic.

Therefore, for
$\DS \VEC{\xi} \in \RR^n \setminus \{\VEC{0}\}$, we have
that $\DS Q(\VEC{x}, -\VEC{\xi}) = (-1)^mQ(\VEC{x}, \VEC{\xi})$
and $\DS Q(\VEC{x}, \VEC{\xi})$ are of the same sign.  This
is possible only if $m$ is even.

\stage{ii} Suppose that
$\DS a_{\VEC{\alpha}}:\RR^n\rightarrow \CC$ for some
multi-indices $\VEC{\alpha}$ such that $|\VEC{\alpha}|=m$ and $n>2$.

For each $\DS \VEC{y} \in \RR^{n-1}$, let
\[
...
$\DS \VEC{y} \in \RR^{n-1}\setminus \{\VEC{0}\}$ because
$L(\VEC{x},\diff)$ is elliptic, we have $n_+(\VEC{y})+n_-(\VEC{y})=m$
for $\DS \VEC{y} \in \RR^{n-1}\setminus \{\VEC{0}\}$.

Moreover, it follows from Rounch\'e's theorem (see \cite{Co}) that
$n_+$ and $n_-$ are locally constant.  So, they are constant on the
largest connected component of $\DS \RR^{n-1}$ that does not
contain the origin (the only value of $\VEC{y}$ for which
$p_{\VEC{y}}$ may have real roots).  Since
$\DS \RR^{n-1}\setminus\{\VEC{0}\}$ is connected for
$n>2$,  $n_+$ and $n_-$ are constant on
$\DS \RR^{n-1}\setminus\{\VEC{0}\}$.

Finally, $n_+(-\VEC{y}) = n_-(\VEC{y})$ for
$\DS \RR^{n-1}\setminus\{\VEC{0}\}$ because
$\DS p_{-\VEC{y}}(-z) = (-1)^m p_{\VEC{y}}(z)$.  Since $n_+$
and $n_-$ are constant on $\DS \RR^{n-1}\setminus\{\VEC{0}\}$, we get
$n_-(\VEC{y}) = n_+(-\VEC{y}) = n_+(\VEC{y})$ for
$\DS \VEC{y} \in \RR^{n-1}\setminus\{\VEC{0}\}$.
...
on an open subset $\DS \Omega \subset \RR^n$ if there exists
a constant $C>0$ such that
\begin{equation} \label{strong_ell_formEq1}
(-1)^{m/2} \RE Q(\VEC{x}, \VEC{\xi}) \geq C \|\VEC{\xi}\|^m
\end{equation}
for all $\DS \VEC{\xi} \in \RR^n$ and $\VEC{x}\in \Omega$.
\end{defn}
...
compact set $\DS B= \{\VEC{\xi}\in \RR^n : \|\VEC{\xi}\| = 1\}$
...
$L(\VEC{x},\diff)$ are functions from $\DS \RR^n$ to $\RR$, then
...
set $\DS \Omega \subset \RR^n$ if there exist a complex
valued function
$\DS \gamma \in C^\infty(\overline{\Omega})$ and a constant $C>0$ such
...
equation by $\DS (-1)^{m/2}/\gamma$ to obtain a linear partial
...
$\DS (-1)^{m/2} a_{\VEC{\alpha}}/\gamma$ in the new linear partial
