...
\]
with $a_{2,0} = 1$, $a_{1,1} = 0$ and $\DS a_{0,2}=-c^2$.
...
\end{pmatrix} \ .
\]
The wave equation is hyperbolic on $\DS \RR^2$ because
$\DS \det
\begin{pmatrix}
a_{2,0} & a_{1,1} \\
a_{1,1} & a_{0,2}
...
respect to $\xi$ yields $\DS \pdydx{u}{\eta} = H(\eta)$ for
some function $H:\RR\rightarrow \RR$.  Integrating
$\DS \pdydx{u}{\eta} = H(\eta)$ with respect to $\eta$ yields
$\DS u(\eta,\xi) = \int H(\eta) \dx{\eta} + G(\xi)$ for some
function $G:\RR\rightarrow \RR$.  If we introduce the function $F$
defined by
$\DS F(\eta) = \int H(\eta) \dx{\eta}$, then we get the solution
\[
u(\eta,\xi) = F(\eta) + G(\xi)
\]
...
If $\DS f \in C^2(\RR)$ and $\DS g \in C^1(\RR)$, then
(\ref{wave_oneD_sol}) provides a solution of class $\DS C^2(\RR)$ for
(\ref{wave_oneD}) with the initial conditions (\ref{wave_oneD_init}).

If $g = 0$, then the solution of the wave equation is
$\DS u(x,t) = \frac{1}{2} \left(f(x-ct) + f(x+ct)\right)$.
...
\begin{egg}
Sketch the solution $u$ of the wave equation $\DS u_{tt} = c^2 u_{xx}$
with the initial conditions
\[
...
\begin{egg}
Sketch the solution of the wave equation $\DS u_{tt} = c^2 u_{xx}$ for
...
and $\DS g\in L^1(]0,L[)$.  We only have to consider $u$ along the
...
$A,B \in C([0,\infty[)$, $\DS g \in L^1(]0,L[)$ and $f\in C([0,L])$
with $B(0)=f(L)$ and $A(0) = f(0)$ are sufficient to guarantee that
$u$ is continuous on the domain
$\{ (x,t) : t\geq 0, \ 0 \leq x \leq L \}$.

The solution $u$ will be of class $\DS C^1$ inside the $R_i$'s if
$\DS f\in C^1(]0,L[)$ and $g \in C(]0,L[)$.  The solution
$u$ will even be of class $\DS C^2$ inside the $R_i$'s if
$\DS f\in C^2(]0,L[)$ and $\DS g \in C^1(]0,L[)$.
...
If $\DS A,B \in C^2(]0,\infty[)\cap C([0,\infty[)$,
$\DS g\in C^1(]0,L[)\cap C([0,L])$,
$\DS f\in C^2(]0,L[)\cap C([0,L])$,
$A(0)=f(0)$, $A'(0)=g(0)$, $\DS A''(0)= c^2 f''(0)$, $B(0)=f(L)$,
$B'(0)=g(L)$ and $\DS B''(0)= c^2 f''(L)$, then
the solution $u$ will be of class $\DS C^2$ on all the domain.
...
set containing $\DS K \in \RR^2$, and $\partial K$ is positively
...
\]
because $\DS \pdydx{u}{t}(x,0) = g(x)$ for $x\in \RR$.
...
\end{align*}
because $\DS u(x,0) = f(x)$ for $x \in \RR$.
...
\end{align*}
If we solved for $u(x_0,t_0)$, then we get
\[
...
$\DS u(x,0)= 0$ and $\DS \pdydx{u}{t}(x,0) = 0$
...
\end{align*}
Thus, $\DS \pdydx{u_2}{t}(x,0) = 0$ as desired.
Furthermore,
...
with the initial conditions
$\DS u(x,0) = 0$ and
$\DS \pdydx{u}{t}(x,0) = \frac{1}{1+x^2}$ for $x \in \RR$.
...
$\DS g(x) = 1/(1+x^2)$ for all
$x\in \RR$, and $\DS F(x,t) = e^x + \sin(t)$.  We get
\begin{align*}
...
x\in \RR \text{ and } t>0 \ ,
\]
and $\DS u_2(x,t)= - e^x/4$ is a solution of
\[
...
\]
for $\DS \phi \in \DD(\RR^2)$. 

$u:\Omega \to \RR$ is a strong solution of $L(\eta,\xi,\diff)u=0$
over an open set $\DS \Omega \subset \RR^2$ if
...
$\DS u \in C^2(\Omega)$ is a solution of
...
$\DS \Omega_+ = \left\{ (\eta,\xi) \in \Omega : \xi>0 \right\}$,
$\DS \Omega_- = \left\{ (\eta,\xi) \in \Omega : \xi<0 \right\}$
...
in $\DS \DD(\RR^2)$ and has a compact support in $\Omega$.  This is
something that we will often do to simplify the discussion when we
have to work on an open subset of $\DS \RR^n$.
...
\end{align*}
where $\DS u_\pm(\eta,0) = \lim_{\xi\rightarrow 0^\pm} u(\eta,\xi)$
and $\DS \pdydx{u_\pm}{\eta}(\eta,0) =
\lim_{\xi\rightarrow 0^\pm} \pdydx{u}{\eta}(\eta,\xi)$.  We assume
that these limits exist.

To get $\DS \int_R u(\eta,\xi)\, L^\ast(\eta,\xi,\diff)\phi(\eta,\xi)
...
$\DS L(\eta,\xi,\diff)u = \pdydxnm{u}{\eta}{\xi}{2}{}{} = 0$, the
...
\]
for almost every $\eta \in \RR$.  If $u$ and $\DS
\pdydx{u}{\eta}$ can be continuously extended to the $\xi$ axis, then
this shows that $\DS u_+(\eta,0) - u_-(\eta,0)$ is constant
for $\eta \in \RR$.

\begin{egg}
In Example~\ref{wave_egg1}, if the functions $f$, $g$, $A$ and $B$ are
$\DS C^\infty$. then the solution $u$ constructed in the example is a
...
(\ref{wave_geo2}) that 
$\DS \lim_{\substack{a \to d\\ a \in E_3}} u(a) = u(d)+ B(0) - f(L)$
...
Suppose that $\DS f \in C^3([0,L])$,
$\DS g\in C^2([0,L])$, $f(0)=f(L)=0$,
...
derivatives $\DS \pdydx{}{t}$, $\DS \pdydxn{}{t}{2}$,
$\DS \pdydx{}{x}$, $\DS \pdydxn{}{x}{2}$ and
$\DS \pdydxnm{}{x}{t}{2}{}{}$.  Since each term of the
series in (\ref{wave_class_sol1}) is twice differentiable, it follows
that the same is true for $u$.  Thus
$\DS u\in C^2(]0,L[\times]0,\infty[)$.
...
prove that
$\DS \sum_{n=1}^\infty n^2|a_n|$ and
$\DS \sum_{n=1}^\infty n^2|b_n|$ converge.
...
\]
for all $N>0$.  Since
$\DS \left\{ S_N \right\}_{N=1}^\infty$ is a bounded increasing 
sequence, it converges.  Namely, $\DS \sum_{n=1}^\infty n^6 a_n^2$
...
terms to conclude that $\DS \sum_{n=1}^\infty n^2 |a_n|$
converges.  Similarly, we have that
$\DS n^2 |b_n| \leq \frac{1}{2} \left( \frac{1}{n^2} + n^6
  b_n^2\right)$ and $\DS \sum_{n=1}^\infty n^2 |b_n|$ converges.
\end{proof}
...
\begin{rmk}
The function
$\DS u_n(x,t) = \left(a_n\cos\left(\frac{n\pi t}{L}\right)
+ b_n \sin\left(\frac{n\pi t}{L}\right)\right)
...
$\DS c^2 = T/\rho = 1$.  The initial deflection is
...
from $\DS g(x) = \pdydx{u}{t}(x,0)$, we have
\[
...
and the initial conditions
$u(x,0) = f(x)$ and $\DS \pdydx{u}{t}(x,0) = g(x)$ for
$0 < x < L$.              \label{wave_spv_egg1}

\subI{Separation of Variables}
If we substitute $u(x,t) = F(x)G(t)$ in (\ref{wave_spv_egg1_e}), then we get
\[
F(x)\,\dydxn{G}{t}{2}(t) = c^2 \dydxn{F}{x}{2}(x)\, G(t) \ .
\]
Thus, after dividing both sides by $\DS c^2\,F(x)G(t)$, we get
\[
...
satisfies two boundary conditions.   From $\DS u(0,t) = 0$, we get
$F(0)G(t)=0$.  Since we assume that $G$ is non-null, we get
$F(0)=0$.  Similarly, from $\DS u(L,t) = 0$, we
...
$\DS A e^{L\sqrt{k}} + B e^{-L\sqrt{k}} = 0$.
...
$\DS \lambda^2 +\left(n c\pi/L\right)^2=0$.  The two
roots of this equation are the complex numbers
$\DS \lambda_{\pm} = \pm (c n\pi/L)i$.
...
From $\DS \pdydx{u}{t}(x,0) = g(x)$, we get
\[
...
\end{equation}
with the boundary conditions
$\DS \pdydx{u}{x}(0,t) = \pdydx{u}{x}(L,t) = 0$ for
$t>0$, and the initial conditions
$u(x,0) = f(x)$ and $\DS \pdydx{u}{t}(x,0) = g(x)$
for $0<x<L$.           \label{wave_spv_egg2}

\subI{Separation of Variables}
If we substitute $u(x,t) = F(x)G(t)$ in (\ref{wave_spv_egg2_e}), then we get
\[
F(x)\,\dydxn{G}{t}{2}(t) = c^2 \dydxn{F}{x}{2}(x)\, G(t) \ .
\]
After dividing both sides by $\DS c^2\,F(x)G(t)$, we get
\[
\frac{1}{c^2\,G(t)} \, \dydxn{G}{t}{2}(t) =
\frac{1}{F(x)}\, \dydxn{F}{x}{2}(x)
\]
...
$\DS \pdydx{u}{x}(0,t) = 0$, we get
$F'(0)G(t)=0$.  Since we assume that $G$ is non-null, we get
$F'(0)=0$.  Similarly, from $\DS \pdydx{u}{x}(L,t) = 0$, we
get $F'(L)G(t)=0$.  Again, since we assume that $G$ is non-null, we
get $F'(L)=0$.  The boundary conditions for 
...
$F'(L)=0$ implies
$\DS A \sqrt{k}\, e^{L\sqrt{k}} - B \sqrt{k}\, e^{-L\sqrt{k}} = 0$.
The only solution of the system
\[
\begin{pmatrix}
...
$A=0$, then we get the null solution.  We must therefore have
...
for $n>0$.  For each $n$, this is a second order ordinary differential
equation with the characteristic equation
$\DS \lambda^2 +\left(c n\pi/L\right)^2=0$.  The two
roots of this equation are the complex numbers
$\DS \lambda_{\pm} = \pm (c n\pi/L)i$.
The general solution of this ordinary differential equation is therefore
\[
...
\end{equation}
with the boundary conditions
$u(0,t) = 0$ and $\DS \pdydx{u}{x}(5,t) = 0$ for $t > 0$,
and the initial conditions
$\DS u(x,0) = 2 \sin\left(\frac{7\pi x}{10}\right)$
and
$\DS \pdydx{u}{t}(x,0) = 5 \sin\left(\frac{11 \pi x}{10}\right)$
for $0 < x < 5$.

\subI{Separation of Variables}
If we substitute $u(x,t) = F(x)G(t)$ in (\ref{wave_spv_egg3_a}), then we get
\[
...
$\DS \dydx{F}{x}(5)=0$ implies that $A = 0$.
...
$\DS 5\sqrt{-k} = (2n-1)\pi/2$ for $n >0$. Thus
$\DS k = k_n \equiv -\left((2n-1)\pi/10\right)^2$
for $n>0$.
...
$\DS k = k_n = -\left((2n-1)\pi/10\right)^2$
becomes
\[
...
From $\DS u(x,0) = 2 \sin\left(\frac{7 \pi x}{10}\right)$, we get
\[
...
b_n =
\begin{cases}
\DS \frac{50}{11\pi} & \quad \text{if} \ n=6 \\
0 & \quad \text{otherwise}
\end{cases}
\]
...
\end{equation}
with the boundary conditions
$u(0,t) = 0$ and $\DS \pdydx{u}{x}\left(\frac{\pi}{2},t\right) = 0$
for $t > 0$, and the initial conditions
$\DS u(x,0) = 3 \sin\left(7 x\right) + 5 \sin\left(13 x\right)$
and
$\DS \pdydx{u}{t}(x,0) = 3 \sin\left(7 x\right)$
for $0 < x < \pi/2$.

\subI{Separation of Variables}
If we substitute $u(x,t) = F(x)G(t)$ in (\ref{wave_spv_egg3_e}), then we get
\[
...
$\DS \pdydx{u}{x}\left(\frac{\pi}{2},t\right) = 0$, we get
$\DS \dydx{F}{x}\left(\frac{\pi}{2}\right)G(t)=0$.
Again, since we assume that $G$ is non-null, we get
$\DS \dydx{F}{x}\left(\frac{\pi}{2}\right)=0$.  The
boundary conditions for the first ordinary differential equation in
(\ref{wave_spv_egg3_ode}) are
$\DS F(0)=\dydx{F}{x}\left(\frac{\pi}{2}\right)=0$. 
...
$\DS F(x) = A e^{\sqrt{k}\,x} + B e^{-\sqrt{k}\, x}$.
However, $F(0)=0$ implies that $A+B=0$ and
$\DS \dydx{F}{x}\left(\frac{\pi}{2}\right)=0$ implies
$\DS A\sqrt{k} e^{\pi\sqrt{k}/2} - B\sqrt{k} e^{-\pi\sqrt{k}/2} = 0$.
The only solution of the system
\[
\begin{pmatrix}
...
$\DS \dydx{F}{x}\left(\frac{\pi}{2}\right)=0$ implies that
...
$\DS \pi \sqrt{-k}/2 = (2n-1)\pi/2$ for
$n\in \NN$. Thus $\DS k = k_n \equiv -(2n-1)^2$
...
\end{cases}
\]
From $\DS \pdydx{u}{t}(x,0) = 3 \sin\left(7 x\right)$, we get
\[
...
$\DS \pdydx{u}{x}(0,t) = \pdydx{u}{x}(L,t) = 0$ for
$t>0$, and the initial conditions
$\DS u(x,0) = x^2(3L-2x)$ and
$\DS \pdydx{u}{t}(x,0) = \sin^2\left(\pi x/L\right)$ for
$0<x<L$.
...
infinite summations, then we get
\[
...
$\DS h(x,t) = 2Lx-x^2$ for $0<x<L$.  We have
\[
...
From $\DS u(x,0) = x^2(3L-2x)$, we get
\[
u(x,0) = A_0
+ \sum_{n=1}^\infty 
...
= x^2(3L-2x) \ .
\]
This is the Fourier cosine series of $\DS x^2(3L-2x)$.  Thus,
\[
...
= \frac{1}{2} - \frac{1}{2}\cos\left(\frac{2 \pi x}{L}\right) \ .
\]
This is the Fourier cosine series of $\DS
\frac{1}{2} - \frac{1}{2}\cos\left(\frac{2 \pi x}{L}\right)$.  By
uniqueness of the Fourier series, we have
\[
B_n =
\begin{cases}
\DS \frac{1}{2} & \quad \text{if} \ n=0 \\[0.7em]
\DS -\frac{1}{2} & \quad \text{if} \ n=2 \\[0.7em]
0 & \quad \text{otherwise}
\end{cases}
\]
