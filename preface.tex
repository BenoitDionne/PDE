...
These lecture notes are for an advanced undergraduate course on partial
...
present lecture notes cover must of the important topics found in books on
...
knowledge of these topics.  The present lecture notes are accessible to
...
These lecture notes contain must of the material on partial differential
...
with all the concept of vector calculus in $\DS \RR^n$.
These topics are still accessible to readers with a more standard
vector calculus course in $\DS \RR^2$ and
$\DS \RR^3$ if they assume that $n$ is $2$ or
...
Except for Chapters~\ref{ChapCaract} and \ref{ChapShock}, these
lecture notes are about linear partial differential equations.  So,
there is no mention of reaction diffusion equations, solitons, inverse
scattering, etc.  In particular, there is no mention of semi-group
theory.  On this subject, we strongly recommend \cite{He}.  Energy
methods are used a few times in these lecture notes but not as
extensively as we will find in some books on partial differential
equation.
...
$\DS \cos(\theta_1) = \frac{1}{\sqrt{1+u_x^2(x_1,t)}}$ and
$\DS \cos(\theta_2) = \frac{1}{\sqrt{1+u_x^2(x_2,t)}}$, we
get
\[
% \begin{equation} \label{prefaceEq1}
\frac{T(x_1,t)}{\sqrt{1+u_x^2(x_1,t)}}
= \frac{T(x_2,t)}{\sqrt{1+u_x^2(x_2,t)}} \ .  \tag{1} \label{prefaceEq1}
% \end{equation}
\]
...
$\DS T(x_2,t) \sin(\theta_2) - T(x_1,t) \sin(\theta_1) =
\int_{x_1}^{x_2} \rho u_{tt}(x,t) \dx{x}$.  Since
$\DS \sin(\theta_1) = \frac{u_x(x_1,t)}{\sqrt{1+u_x^2(x_1,t)}}$ 
and
$\DS \sin(\theta_2) = \frac{u_x(x_2,t)}{\sqrt{1+u_x^2(x_2,t)}}$,
we get
\[
% \begin{equation} \label{prefaceEq2}
\frac{T(x_2,t) u_x(x_2,t)}{\sqrt{1+u_x^2(x_2,t)}} -
\frac{T(x_1,t) u_x(x_1,t)}{\sqrt{1+u_x^2(x_1,t)}} = 
\int_{x_1}^{x_2} \rho u_{tt}(x,t) \dx{x} \ . \tag{2}  \label{prefaceEq2}
% \end{equation}
\]
...
u_{tt}(x,t) \dx{x} \ .  \tag{3}  \label{prefaceEq3}
% \end{equation}
\]
...
% \begin{equation} \label{prefaceEq4}
  u_{tt} = c^2 u_{xx} \tag{4}  \label{prefaceEq4}
% \end{equation}
\]
...
the region representing the membrane in $\DS \RR^2$.
...
$\DS M(x_1,x_2,t) = \int_{x_1}^{x_2} u(x,t) \dx{x}$.
...
$\DS \RR^2$.  The solutions of this
equation are called ``harmonic functions.''\  We will study these
functions intensively in these lecture notes.  The Laplace equation is
usually accompanied of boundary conditions.  For instance, $u=0$ on the
...
the related chapter in these lecture notes and without having to read
the book used as the source for the exercises.  We have tried to avoid
exercises that refer specifically to the content of the textbook used
as the source for the exercises.  There are a few exceptions where
we have selected exercises that cover important material that has not
been included in the present lecture notes.  As for any book on partial
...
The open ball of radius $r$ centred at $\DS \VEC{y} \in \RR^n$
is denoted $\DS B_r(\VEC{y})$; namely,\\
$\DS B_r(\VEC{y}) = \left\{ \VEC{x} \in \RR^n :
...
{\bfseries supremum norm}\index{Supremum Norm of a Function}
...
If $V$ be an open subset of $\DS \RR^n$ and $f:V \to \RR$ is
sufficiently differentiable, we define the differential operator
$\DS \diff^{\VEC{\alpha}}$ for $\DS \VEC{\alpha} \in \NN^n$
by
\[
...
$\DS |\VEC{\alpha}| = \sum_{j=1}^n \alpha_j$.

\begin{defn*}
Let $V$ be an open subset of $\DS \RR^n$.  For $m\in \NN$,
$\DS C^m(\overline{V})$ is the space of all
$\DS f \in C^m(V) \cap C(\overline{V})$ such that
$\DS \diff^{\VEC{\alpha}} f$ can be continuously extended
to $\overline{V}$ for
all multi-index $\DS \VEC{\alpha} \in \NN^n$ such that
$|\VEC{\alpha}|\leq m$.

$\DS C^\infty(\overline{V})$ is the space of all
$\DS f \in C^\infty(V) \cap C(\overline{V})$ such that
$\DS \diff^{\VEC{\alpha}} f$ can be continuously extended to
$\overline{V}$ for all multi-index $\DS \VEC{\alpha} \in \NN^n$.
\end{defn*}

\begin{defn*}
Let $V$ be an open subset of $\DS \RR^n$ and $k$ be a
positive integer.  The space of {\bfseries bounded continuous
functions}\index{Bounded Continuous Functions}, denoted
$\DS C_b^k(\overline{V})$, is the set of all functions
$\DS f\in C^k\left(\overline{V}\right)$ such that
\[
...
$\DS C^k_b(\overline{V})$ is a Banach space
with respect to the norm $\DS \|f\|_{k,\infty,V}$.
...
$\DS C_{bu}^k(\overline{V})$, is the set of all functions
$\DS f\in C_b^k\left(\overline{V}\right)$ such that
$\DS \diff^{\VEC{\alpha}} f:\overline{V} \to \RR$ is uniformly
continuous for $|\VEC{\alpha}| \leq k$.
\end{defn*}

It is also not hard to prove that
$\DS C_{bu}^k(\overline{V})$ is a closed subset of
$\DS C_b^k\left(\overline{V}\right)$ with respect to the
norm $\|\cdot\|_{k,\infty,V}$.

\begin{defn}
If $V$ is an open subset of $\DS \RR^n$ and $f:V \to \RR$ is a
...
If $\DS F:V \to \RR^n$ is a sufficiently differentiable
function, then the {\bfseries divergence}\index{Divergence} of $F$ is
defined by $\DS \diV F(\VEC{x}) = \nabla \cdot F(\VEC{x})
...
\subI{Warning} In these lecture notes, $\NN = \{0,1,2,3, \ldots\}$.
