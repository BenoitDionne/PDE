...
\]
on $\Omega$.  The constant $\DS c^2$ represent the heat
...
implies in particular that $\DS \lim_{x\to \pm \infty} f(x) = 0$.
...
of the form
$\DS Q(x,t) = q\left(\frac{x}{2c\sqrt{t}}\right)$.
We introduce the factor $\DS \frac{1}{2c}$ for aesthetic
...
u(x,t) = \begin{cases}
\DS \int_{-\infty}^{\infty} K(x-y,t) f(y) \dx{y}
...
$C$ such that
$\DS |K(x-y,t) f(y)| \leq C |f(y)|$,
$\DS \left| \pdydx{K}{t}(x-y,t) f(y)\right| \leq C |f(y)|$,
$\DS \left| \pdfdx{K}{x}(x-y,t) f(y)\right| \leq C |f(y)|$
and
$\DS \left| \pdfdxn{K}{x}{2}(x-y,t) f(y)\right| \leq C |f(y)|$
...
We first note that
$\DS \int_{-\infty}^{\infty} K(y,t) \dx{y} = 1$ for all $t>0$.
Let $\DS M = \sup_{x \in \RR} |f(x)|$.  We have that
$M < \infty$ because $f$ is a continuous function in $\DS L^1(\RR)$.
...
\begin{rmk}
If $f$ is differentiable and $\DS f' \in L^1(\RR)$, there is
an easy to prove that
$\DS \lim_{t\to 0^+} u(x,t) = f(x)$.  Note that this is not
...
\end{align*}
Since $\DS \lim_{x\to \pm\infty}f(x) = 0$, we get
\[
...
\]
where we have again used $\DS \lim_{x\to -\infty}f(x) = 0$.
We should note that $|Q(x-y,t) f'(y)| < |f'(y)|$ for all $x$ and
$t\geq 0$.  Since $\DS f' \in L'(\RR)$, we may use the
...
particular, this implies that $\DS \lim_{x\to \pm \infty} f(x) = 0$.
...
Again, this implies that $\DS \lim_{x\to \pm \infty} g(x,t) = 0$
for all $t$.   With these assumptions, all the
following integrals involving $g$ and $\DS \pdydx{g}{x}$
...
$\DS \lim_{x\to \pm\infty} g(x,t) = 0$ for all $t>0$.
...
\label{heat_cauchyB} 
\end{align}
where $\DS f:\RR^n\rightarrow \RR$ is a function to be
specified later.

\begin{rmk}
The hyperplane $t=0$ is a characteristic surface because the outward
unit normal to this hyperplane is $\nu(\VEC{x},0) = (\VEC{0},-1)$ for
all $\DS \VEC{x} \in \RR^n$, where $\VEC{0}$ is the origin
in $\DS \RR^n$, and the principal symbol for the heat equation is
\[
...
Let us assume that $\DS f \in \SS(\RR^n)$.  Moreover, in addition of
$u$ being of class $\DS C^2$ on
$\DS \RR^n \times ]0,\infty[$ and
continuous on $\DS \RR^n \times [0,\infty[$, let us assume that
$\VEC{x} \mapsto u(\VEC{x},t)$ is and
$\DS \VEC{x} \mapsto \pdydx{u}{t}(\VEC{x},t)$ are absolutely
integrable on $\DS \RR^n$ for all $t \geq 0$.  We may then use Fourier
...
\]
for $\DS \VEC{y} \in \RR^n$ and $t \geq 0$.
...
\]
for $\DS \VEC{x} \in \RR^n$ and $t > 0$.  Proceeding as we
...
K(\VEC{x},t) =
\begin{cases}
\DS (4\pi t)^{-n/2} e^{-\|\VEC{x}\|^2/(4t)} & \quad 
\text{if} \quad t>0 \\
...
Assume that $\DS f \in L^p(\RR^n)$, where $1\leq p \leq \infty$.  Then
...
continuously to $\DS \RR^n \times [0,\infty[$ with
$u(\VEC{x},0) = f(\VEC{x})$ for $\VEC{x} \in \RR^n$.

If $\DS f \in L^p(\RR^n)$, where $1\leq p < \infty$, then
$v_t(\VEC{x}) = u(\VEC{x},t)$ for $\DS \VEC{x} \in \RR^n$ satisfies
$v_t \rightarrow f$ in $\DS L^p(\RR^n)$ as $t\rightarrow 0$.
\end{theorem}
...
We have that $\DS K_t \in L^q(\RR^n)$ for
$1\leq q \leq \infty$ and $t>0$.
Hence,  $\DS \VEC{y} \mapsto f(\VEC{y})\,K_t(\VEC{x}-\VEC{y})$
for $\DS \VEC{y} \in \RR^n$ is in
$\DS L^1(\RR^n)$.  For instance, if
$1<p<\infty$, then we have
\[
...
\]
because
$\DS \left(\pdydx{}{t} - \Delta_{\VEC{x}}\right) K_t\left(\VEC{x}
...
If $\DS f\in L^p(\RR^n)$ with $1\leq p < \infty$, it follows from
Proposition~\ref{distr_limit_convol} that
$f \ast K_t \rightarrow f$ in $\DS L^p(\RR^n)$ as $t \rightarrow 0$.

If $f$ is continuous and bounded on $\DS \RR^n$, then $f$ is uniformly
continuous on any bounded open set $V$ because $\overline{V}$ is a
compact subset of $\DS \RR^n$.  It follows from
Proposition~\ref{distr_limit_convol} that
$f \ast K_t \rightarrow f$ uniformly on compact subsets of
$\DS \RR^n$ as $t \rightarrow 0$.  In particular,
$u(\VEC{x},0) = f(\VEC{x})$ for $\DS \VEC{x} \in \RR^n$ defines a
continuous extension of $u$ to $\DS \RR^n \times [0,\infty[$.
\end{proof}

\begin{prop}
Assume that $\DS f \in L^p(\RR^n)$, where $1\leq p \leq \infty$.  The
solution $u$ of (\ref{heat_cauchyA}) given in (\ref{heat_sol_cauch}) is
of class $\DS C^\infty(\RR^n\times ]0,\infty[)$.
\end{prop}
...
and $t$.  Since $\DS K \in C^\infty(\RR^n \times ]0,\infty[)$,
we get that $\DS u \in C^\infty(\RR^n \times ]0,\infty[)$.
\end{proof}
...
\end{cases}
\]
The function $g$ is in $\DS C^\infty(\RR)$.  The function
...
$\DS \pdydx{u}{t} - \pdydxn{u}{x}{2} = 0$.  This follows
from the fact that the series (\ref{heat_not_uno}),
$\DS \sum_{k=0}^\infty \frac{g^{(k+1)}(t)}{(2k)!}\,x^{2k}$
and $\DS \sum_{k=1}^\infty \frac{g^{(k)}(t)}{(2k-2)!}\,x^{2k-2}$
converge uniformly on compact sets.  Hence, we may interchange
summation and partial derivatives in (\ref{heat_not_uno}). 
Since $\DS g^{(k)}(0)=0$ for all $k$, $u(x,0)=0$ for all
...
\begin{prop}
Suppose that
$\DS u \in C(\RR^n \times [0,\infty[) \cap
C^2(\RR^n \times ]0,\infty[)$
is a solution of
$\DS \pdydx{u}{t} - \Delta u = 0$ on
$\DS \RR^n \times ]0,\infty[$
and $u(\VEC{x},0)=0$ for $\DS \VEC{x} \in \RR^n$.  If, for every
$\epsilon >0$, there exists $C>0$ such that
$\DS |u(\VEC{x},t)| \leq C e^{\epsilon \|\VEC{x}\|^2}$
and
$\DS \|\graD_{\VEC{x}} u(\VEC{x},t)\| \leq C e^{\epsilon \|\VEC{x}\|^2}$
for all $\VEC{x}$ and $t$, then $u \equiv 0$.
\end{prop}

\begin{proof}
Given $\DS (\VEC{x}_0, t_0) \in \RR^n \times ]0,\infty[$, we
prove that $u(\VEC{x}_0,t_0) = 0$.

We defined two functions, $\DS f:\RR^n\times ]0,\infty[ \to \RR$ and
$\DS g:\RR^n\times ]-\infty,t_0[ \to \RR$, as follows.
$f(\VEC{x},t) = u(\VEC{x},t)$ for
$\DS (\VEC{x},t)\in \RR^n\times ]0,\infty[$
and $g(\VEC{x},t) = K(\VEC{x}-\VEC{x}_0, t_0-t)$ for
$\DS (\VEC{x},t)\in \RR^n\times ]-\infty, t_0[$.

We have that $\DS \pdydx{f}{t} - \Delta f = 0$
on $\DS \RR^n \times ]0,\infty[$ and
$\DS \pdydx{g}{t} + \Delta g = 0$ on $\RR^n \times ]-\infty,t_0[$.

Given $0< a < b < t_0$ and $r>0$, let
$\DS \Omega =
\left\{ (\VEC{x},t) : \|\VEC{x}\| < r, \ a<t<b \right\}$.
Moreover, let $\DS F:\RR^n \times ]0,t_0[ \to \RR^{n+1}$ be
...
= 0
\end{align*}
in $\DS \RR^n \times ]0,t_0[$, we get from the divergence theorem that
...
$\DS \big(u(\cdot, a) \ast K_{t_0-a}(\cdot)\big)(\VEC{x}_0) \to 0$
as $\DS a \to 0^+$.  We have
\[
\lim_{a\to 0^+} u(\VEC{x},a) K(\VEC{x}-\VEC{x}_0, t_0 - a) =
u(\VEC{x},0) K(\VEC{x}-\VEC{x}_0, t_0) = 0
\]
for all $\DS \VEC{x} \in \RR^n$ because $u(\VEC{x},0) = 0$
for all $\VEC{x} \in \RR^n$.  Moreover, given
$\epsilon < 1/(4t_0)$, there exists $C>0$ such that
\[
...
$\DS \big(u(\cdot, b) \ast K_{t_0-b}(\cdot)\big)(\VEC{x}_0) \to
u(\VEC{x}_0,t_0)$ as $\DS b \to t_0^-$.
...
\end{split}
\end{equation}
Given $\DS \VEC{x} \in \RR^n$, there exists
...
\stage{iii} If we let $\DS a\to 0^+$ and
$\DS b\to t_0^-$ in (\ref{heat_unicB}), then we get
...
As before, let $\DS K:\RR^n \times \RR \rightarrow \RR$ be
...
$\DS \pdydx{u}{t} - \Delta u = 0$ on $\DS \RR^n\times \RR$.
\end{theorem}

\begin{proof}
\stage{i} We prove that $\DS K\in \DD'(\RR^n\times \RR)$ by
proving that $\DS K \in L^1_{loc}(\RR^n \times \RR)$.  We
prove in fact a stronger result.  Since
$\DS \int_{\RR^n} \left| K(\VEC{x},t) \right| \dx{\VEC{x}} = 1$
for all $t>0$, we get
\[
...
distribution on $\DS \RR^n \times \RR$.

\stage{ii} We show that
$\DS \pdydx{K}{t} - \Delta K = \delta$.  For $\epsilon>0$,
let
\[
K^{[\epsilon]}(\VEC{x},t) =
\begin{cases}
...
$\DS K^{[\epsilon]}\in \DD'(\RR^n\times \RR)$.

Given $\DS \phi \in \DD(\RR^n \times \RR)$,
we have that $\DS K^{[\epsilon]}(\VEC{x},t) \phi(\VEC{x},t)
\rightarrow K(\VEC{x},t) \phi(\VEC{x},t)$ as
$\DS \epsilon \rightarrow 0^+$ for all
$\DS (\VEC{x},t) \in \RR^n\times \RR$.
Moreover, since $\phi$ has a compact support,
$\DS M = \sup_{(\VEC{x},t) \in \RR^n \times \RR} |\phi(\VEC{x},t)|
< \infty$ and there exists $I \subset \RR$ such that
$\DS \supp \phi \subset \RR^n \times I$. 
Since
\[
\left| K^{[\epsilon]}(\VEC{x},t) \phi(\VEC{x},t) \right|
\leq M \Chi_{\RR^n \times I}(\VEC{x},t) K(\VEC{x},t)
\]
for all $\DS (\VEC{x},t) \in \RR^n\times \RR$ and
$\DS M \Chi_{\RR^n \times I} K \in L^1(\RR^n \times \RR)$,
...
\]
Since $\DS \phi \in \DD(\RR^n \times \RR)$ is arbitrary,
we have that $\DS K^{[\epsilon]} \rightarrow K$ in
$\DS \DD'(\RR^n\times \RR)$ as $\epsilon \rightarrow 0^+$.

To complete the proof, we only have to show that
$\DS \pdfdx{K^{[\epsilon]}}{t} - \Delta_{\VEC{x}} K^{[\epsilon]}
\rightarrow \delta$ in $\DS \DD'(\RR^n\times \RR)$ as
$\DS \epsilon \to 0^+$.
Namely, for $\DS \phi \in \DD(\RR^n\times \RR)$,
...
\end{align*}
because $\phi$ has a compact support in $\DS \RR^n\times \RR$.  Since
$\DS \pdydx{K}{t}(\VEC{x},t) -\Delta_{\VEC{x}} K(\VEC{x},t) = 0$
and $K(\VEC{x},t) = K(-\VEC{x},t)$ for
$\DS \VEC{x}\in \RR^n$ and $t>0$, we have
that
\[
\ps{K^{[\epsilon]}}{-\pdfdx{\phi}{t} - \Delta_{\VEC{x}} \phi}
...
\quad \text{as} \quad \epsilon \to 0^+ \  .
\]
Since $\DS \phi:\RR^n\times \RR$ is a continuous function with
compact support, $\phi$ is uniformly continuous.  In particular,
$\phi(\cdot, \epsilon) - \phi(\cdot, 0) \rightarrow 0$ 
uniformly on $\DS \RR^n$.  Since $K(\VEC{x},\epsilon) >0$ for all
$\DS \VEC{x}\in \RR^n$, it follows from (\ref{heat_convol1}) that
...
\begin{cor}
The heat operator
$\DS L(\VEC{x},y,D) = \pdydx{}{t} - \Delta_{\VEC{x}}$ is hypoelliptic.
\end{cor}

\begin{proof}
This follows from Theorem~\ref{distr_hypoTH} because $\DS K \in
C^\infty\left((\RR^n\times \RR) \setminus \{ (\VEC{0},0) \}\right)$.
\end{proof}

If $\DS f \in \EE'(\RR^n\times \RR)$, then $u = K\ast f$ is
a solution (in the sense of distributions) of the non-homogeneous heat
equation $\DS \pdydx{u}{t} - \Delta_{\VEC{x}} u = f$ on
$\DS \RR^n\times \RR$.
The conclusion of the next theorem states that $K\ast f$ can define a
function almost everywhere if $f$ is nice enough.

\begin{theorem}
If $\DS f \in L^1(\RR^n\times \RR)$, then $u = f\ast K$ is
well defined almost everywhere and
$\DS \pdydx{u}{t} - \Delta_{\VEC{x}} u = f$ in the sense of
distribution.
\end{theorem}

\begin{proof}
Since $\DS f\in L^1(\RR^n\times \RR)$, we have by Fubini's
Theorem that $\DS f(\cdot, s) \in L^1(\RR^n)$ for almost all
...
$\DS \VEC{x} \mapsto \left( |f|(\cdot,s) \ast K(\cdot, t-s)
\right)(\VEC{x})$ for $\DS \VEC{x} \in \RR^n$ is in
$\DS L^1(\RR^n)$ for almost all $s$ such that $s<t$.  The
convolution is obviously on $\DS \RR^n$.  We then have
\begin{align*}
...
\right)(\VEC{x}) \dx{s}
\end{align*}
because $K(\VEC{z}, t-s) = 0$ for $\DS \VEC{z}\in \RR^n$
and $s\geq t$.  Hence,
\begin{align*}
...
\leq \|f\|_1 < \infty \  ,
\end{align*}
where the last $\DS L^1$-norm is on
$\DS L^1(\RR^n\times \RR)$.  It follows
...
replaced by $K$ and $\DS \pdydx{}{t} - \Delta_{\VEC{x}}$
respectively.
\end{proof}
...
equation $\DS \pdydx{u}{t} - \Delta u = 0$ in
$\Omega \times ]0,T[$.  Then, $u$ reaches its maximum on
$(\Omega \times \{0\}) \cup ( \partial \Omega \times [0,T])$.
\end{theorem}

\begin{proof}
Since $\DS \pdydx{}{t}-\Delta_{\VEC{x}}$ is hypoelliptic, we
have that $\DS u \in C^\infty(\Omega\times ]0,T[)$.

Consider $\epsilon>0$ and $T'$ such that $0<T'<T$.  let
$\DS v(\VEC{x},t) = u(\VEC{x},t) + \epsilon \|\VEC{x}\|^2$
...
derivatives $\DS \pdydxn{u}{x_j}{2}(\VEC{x},t)$ exist for
$t=T$.  Hence, $\DS \pdydx{v}{t}(\VEC{x}_0,T') \geq 0$ and
$\Delta_{\VEC{x}} v(\VEC{x}_0,T') \leq 0$ implies that
$\DS \pdydx{v}{t} - \Delta_{\VEC{x}} v \geq 0$.  This again
...
\end{align*}
If $\epsilon \rightarrow 0$, then we get
\[
\max \{ u(\VEC{x},t) : (\VEC{x},t)\in \overline{\Omega}\times [0,T'] \}
...
If we substitute $u(x,t) = F(x)G(t)$ in (\ref{heat_spv_he}), then we get
\[
...
characteristic equation $\DS \lambda^2-k=0$.
...
$B=0$, then we get the null solution.  We must therefore have
$\sin\left(\pi\sqrt{-k}\right) = 0$.  This implies that
$\DS k = k_n \equiv -n^2$ for $n>0$.  The boundary value problem
(\ref{heat_spv_bvp}) has non-null solutions only for
$\DS k=k_n=-n^2<0$
...
$\DS G(t) = G_n(t) \equiv C_n e^{-4n^2t}$.
...
$\DS f(x) = \sum_{n=1}^\infty b_n\,\sin(n\,x)$
...
\DS (-1)^{(n+3)/2}\frac{4}{n^2 \pi} & \quad \text{if $n$ is odd}
\end{cases}
\]
...
$k\geq -2$.  Since
$\DS e^{-4(2m-1)^2\,t} \leq e^{-4(2m-1)^2\epsilon}$
...
\frac{ \DS (2m+1)^{k} e^{-4(2m+1)^2\epsilon}}
{ \DS (2m-1)^{k} e^{-4(2m-1)^2\epsilon}}
...
$\DS \pdydx{}{t} - \Delta_{\VEC{x}}$ is hypoelliptic.
...
\ \text{and} \ t > 0 \ ,
\end{equation}
with $\DS c^2 = 4$, the boundary conditions
$u(0,t) = 0$ and $\DS \pdydx{u}{x}(5,t) = 0$ for $t>0$, and
the following initial condition.
\begin{center}
\begin{tabular}{l@{\hspace{0.5em}}l@{\hspace{5em}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS u(x,0) = 8 \sin\left(\frac{13\pi x}{10}\right)$ &
\subQ{b} & $u(x,0) = x(5-x)^2$
\end{tabular}
\end{center}

\subI{Separation of Variables}
If we substitute $u(x,t) = F(x)G(t)$ in (\ref{heat_spv_diffa}), then we get
\[
F(x)\,\dydx{G}{t}(t) = 4 \dydxn{F}{x}{2}(x)\, G(t)
...
$F(0)=0$.  Similarly, from $\DS \pdydx{u}{x}(5,t) = 0$, we
...
$\DS A \sqrt{k} e^{5\sqrt{k}} - B \sqrt{k} e^{-5\sqrt{k}} = 0$.
...
where $n$ is a positive integer.  For each $n$, the general solution
of this first order separable ordinary differential equation is
$\DS G(t) = G_n(t) \equiv C_n e^{-t ((2n-1)\pi/5)^2}$.
...
\subQ{a} From
$\DS u(x,0) = 8 \sin\left(\frac{13\pi x}{10}\right)$,
we get
\[
...
\subQ{b} From $\DS u(x,0) = x(5-x)^2$, we get
\[
x(5-x)^2 = \sum_{n=1}^\infty b_n\,\sin\left(\frac{(2n-1)\pi x}{10}\right)
\]
for $0\leq x\leq 5$.  Expanding $\DS f(x) = x(5-x)^2$ on the interval
...
(C \cos (\eta y) + D \sin (\eta y)) \ ,
\]
where $\DS k^2 = \mu^2 + \eta^2$.
From $u(0,y,t) = 0$, we get
$\DS A e^{-c^2 k^2 t} (C \cos (\eta y) + D \sin (\eta y)) = 0$.
...
Since 
$\DS \pdydx{u}{y} = B \sin\left(\frac{m\pi x}{5}\right) e^{-c^2 k^2 t}
(-C \eta \sin (\eta y) + D \eta \cos (\eta y))$,
we get from $\DS \pdydx{u}{y}(x,0,t) = 0$ that
$\DS B D \eta \sin\left(\frac{m\pi x}{5}\right) e^{-c^2 k^2 t} = 0$
for all $0 \leq x \leq 5$ and $t>0$, where $B\neq 0$.  Thus $D=0$.  From
$\DS \pdydx{u}{y}(x,5,t) = 0$, we then get
$\DS
...
\end{cases}
\]
Since $\DS k_{n.m} = 9 \pi^2/25$ for $m=3$ and $n=0$, and
$\DS k_{n.m} = 149 \pi^2/25$ for $m=10$ and $n=7$, we get
the solution
\[
...
\pdydx{u}{t} - c^2 \pdydxn{u}{x}{2} = 0 \quad , \quad 0 < x < L
  \ \text{and} \ t > 0 \ ,
\end{equation}
with the boundary conditions $\DS u(0,t) = t^2$ and $u(L,t) = 1$ for
$t>0$, and the initial condition $\DS u(x,0) = x/L$
for $0 \leq x \leq L$.
...
Let $\DS \phi(x,t) = (1-t^2)x/L + t^2 = (x+(L-x)t^2)/L$.
This is the equation of the line in the $x,u$-plane that contains the
points $\DS (0,t^2)$ and $(L,1)$.  The substitution
...
the form
$\DS B_n e^{-c^2n^2\pi^2t/L^2} \sin\left(\frac{n\pi x}{L}\right)$
...
\]
for all $n$.  Thus
$\DS C_n = -\left(\frac{4L^4}{c^4n^5\pi^5}\right)$ for $n>0$
and
\[
...
be presented in these lecture notes.  A good reference for the semi-group
