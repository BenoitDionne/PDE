...
\begin{rmk}
If $f$ is differentiable and $\displaystyle f' \in L^1(\RR)$, there is
...
\label{heat_cauchyB} 
\end{align}
where $\displaystyle f:\RR^n\rightarrow \RR$ is a function to be
specified later.
...
in $\displaystyle \RR^n$, and the principal symbol for the heat equation is
\[
...
continuously to $\displaystyle \RR^n \times [0,\infty[$ with
$u(\VEC{x},0) = f(\VEC{x})$ for $\VEC{x} \in \RR^n$.

If $\displaystyle f \in L^p(\RR^n)$, where $1\leq p < \infty$, then
$v_t(\VEC{x}) = u(\VEC{x},t)$ for $\displaystyle \VEC{x} \in \RR^n$ satisfies
$v_t \rightarrow f$ in $\displaystyle L^p(\RR^n)$ as $t\rightarrow 0$.
\end{theorem}
...
\end{cases}
\]
The function $g$ is in $\displaystyle C^\infty(\RR)$.  The function
...
Since $\displaystyle g^{(k)}(0)=0$ for all $k$, $u(x,0)=0$ for all
...
is a solution of
$\displaystyle \pdydx{u}{t} - \Delta u = 0$ on
$\displaystyle \RR^n \times ]0,\infty[$
...
Moreover, let $\displaystyle F:\RR^n \times ]0,t_0[ \to \RR^{n+1}$ be
...
u(\VEC{x}_0,t_0)$ as $\displaystyle b \to t_0^-$.
...
\stage{iii} If we let $\displaystyle a\to 0^+$ and
$\displaystyle b\to t_0^-$ in (\ref{heat_unicB}), then we get
...
$\displaystyle \epsilon \rightarrow 0^+$ for all
$\displaystyle (\VEC{x},t) \in \RR^n\times \RR$.
...
\]
for all $\displaystyle (\VEC{x},t) \in \RR^n\times \RR$ and
...
$\displaystyle \VEC{x}\in \RR^n$, it follows from (\ref{heat_convol1}) that
...
characteristic equation $\displaystyle \lambda^2-k=0$.
...
$\displaystyle k = k_n \equiv -n^2$ for $n>0$.  The boundary value problem
(\ref{heat_spv_bvp}) has non-null solutions only for
$\displaystyle k=k_n=-n^2<0$
...
\ \text{and} \ t > 0 \ ,
\end{equation}
with $\displaystyle c^2 = 4$, the boundary conditions
...
\subQ{b} From $\displaystyle u(x,0) = x(5-x)^2$, we get
\[
x(5-x)^2 = \sum_{n=1}^\infty b_n\,\sin\left(\frac{(2n-1)\pi x}{10}\right)
\]
for $0\leq x\leq 5$.  Expanding $\displaystyle f(x) = x(5-x)^2$ on the interval
...
\]
with $\displaystyle k^2 = \mu^2 + \eta^2$.
\end{rmk}
...
(C \cos (\eta y) + D \sin (\eta y)) \ ,
\]
where $\displaystyle k^2 = \mu^2 + \eta^2$.
...
\end{cases}
\]
Since $\displaystyle k_{n.m} = 9 \pi^2/25$ for $m=3$ and $n=0$, and
$\displaystyle k_{n.m} = 149 \pi^2/25$ for $m=10$ and $n=7$, we get
the solution
\[
...
\pdydx{u}{t} - c^2 \pdydxn{u}{x}{2} = 0 \quad , \quad 0 < x < L
  \ \text{and} \ t > 0 \ ,
\end{equation}
with the boundary conditions $\displaystyle u(0,t) = t^2$ and $u(L,t) = 1$ for
...
Let $\displaystyle \phi(x,t) = (1-t^2)x/L + t^2 = (x+(L-x)t^2)/L$.
This is the equation of the line in the $x,u$-plane that contains the
points $\displaystyle (0,t^2)$ and $(L,1)$.  The substitution
