...
\end{equation}
where $\DS u \in C^m(\RR^n)$, $\DS f \in C(\RR^n)$
and $\DS a_{\VEC{\alpha}} \in C^{|\VEC{\alpha}|}(\RR^n)$ for all
multi-indices involved in the sum.

If we multiply both sides of the equation in (\ref{distr_lPDE}) by a
function $\DS \phi \in C_c^\infty(\RR^n)$
(functions in $\DS C^\infty(\RR^n)$ with compact supports) and
use integration by parts, then we get
\begin{align*}
...
Conversely, if there exists a function $\DS u \in C^m(\RR^n)$
such that
...
\left(a_{\VEC{\alpha}}(\VEC{x})\phi(\VEC{x})\right) \dx{\VEC{x}}
\end{equation}
for all functions $\DS \phi \in C_c^\infty(\RR^n)$,
...
functions over $\DS \RR^n$, (\ref{distr_intE}) implies that
...
function $\DS u:\RR^n \to \CC$ that satisfies (\ref{distr_intE}).
...
\cite{ReeSim,RuFA}.  The reader will find in these references the proofs of
the results that we state without proofs in the presentation below.

\section{Test Functions} \label{SectTestFnct}

From now on, unless otherwise stated, $\DS \Omega \subset \RR^n$ is an
open set.

The space $\DS C_c^\infty(\Omega)$ of infinitely
...
$\DS \lim_{j\rightarrow \infty}|u(\phi_j)-u(\phi)| = 0$
for all sequences $\DS \left\{ \phi_j\right\}_{j=0}^\infty$ in
$\DD(\Omega)$ converging toward $\phi$ in $\DD(\Omega)$.
The space of all distributions on $\Omega$ is denoted $\DD'(\Omega)$.

Let $K$ be a compact subset of $\DS \RR^n$.  The space
$\DD_K$ is defined by\\
$\DS \DD_K = \left\{ u \in \DD(\RR^n) : \supp u \subset K \right\}$.
...
$\DS \left| u(\phi) \right| \leq C_K \max_{|\VEC{\alpha}|\leq N_K}
...
\begin{egg}
Let $u$ be a function in $\DS L^1_{loc}(\RR^n)$.  The
function $u$ defines a distribution on $\DS \RR^n$ by
\[
\ps{u}{\phi} = u(\phi) =
...
$\DS u:\DD(\RR^n)\rightarrow \CC$ is well defined, $u$ is
linear, and $u$ is continuous.

If $\DS \phi \in \DD(\RR^n)$ and $K = \supp \phi$, then 
\begin{align*}
\left|u(\phi)\right|
...
\end{align*}
because
$\DS \left\|\phi(\VEC{x})\right\|_{\infty,K} < \infty$ since
$\phi$ is continuous on the compact set $K$, and
$\DS \int_{K} \left| u(\VEC{x})\right| \dx{\VEC{x}}$ is bounded
since $\DS u\in L^1_{loc}(\RR^n)$.  Thus,
$\DS u:\DD(\RR^n)\rightarrow \CC$ is well defined.

$\DS u:\DD(\RR^n)\rightarrow \CC$ is linear because the integral is a
linear mapping.

To show that $\DS u:\DD(\RR^n)\rightarrow \CC$ is
continuous, let $\phi$ be any element of $\DS \DD(\RR^n)$ and
$\DS \left\{ \phi_j\right\}_{j=0}^\infty$ be any sequence
$\DS \DD(\RR^n)$ converging toward $\phi$ in
$\DS \DD(\RR^n)$.  By definition
of convergence in $\DS \DD(\RR^n)$, there exists a compact set
$\DS K \subset \RR^n$ such that $\supp \phi_j \subset K$ for all $j$,
...
- \diff^{\VEC{\alpha}} \phi \right\|_{\infty,K} = 0
\]
for all multi-indices $\DS \VEC{\alpha} \in \NN^n$.
...
$\DS \int_K \left| u(\VEC{x}) \right| \dx{\VEC{x}}$ is finite
because $\DS u\in L^1_{loc}(\RR^n)$.
\end{egg}

\begin{egg}
The {\bfseries Dirac delta function}\index{Dirac Delta Function} is
the distribution on $\DS \RR^n$ defined by
\[
...
$\DS \int_{-\infty}^\infty \delta(x)f(x) \dx{x} = f(0)$ for
...
Since most of the distributions $\DS u \in \DD'(\RR)$ used in
engineering comes from integrable functions, the integral notation
$\DS \int_{-\infty}^\infty u(x) \phi(x)\dx{x}$ is often used
...
$\DS f \in C^\infty(\Omega)$ as the distribution $f u$ defined by
\[
(f u)(\phi) = u(f \phi) \quad , \quad \phi \in \DD(\Omega) \ .
\]

\begin{prop} \label{distr_product}
If $\DS f\in C^\infty(\Omega)$ and $u \in \DD'(\Omega)$, then
\[
...
depends only of
$\DS \left\|\diff^{\VEC{\alpha}-\VEC{\beta}} f\right\|_{\infty,K}$
...
\]
defines a distribution on $\Omega$.  We denote this distribution by
$\DS \diff^{\VEC{\alpha}} u$, the
{\bfseries partial derivative}\index{Distribution!Partial Derivative}
of $u\in \DD'(\Omega)$.
\end{prop}

\begin{proof}
\stage{i} $\DS (\diff^{\VEC{\alpha}} u)(\phi)$ is well defined for all
$\phi \in \DD(\Omega)$ because
$\DS \diff^{\VEC{\alpha}} \phi \in \DD(\Omega)$
for all $\phi\in \DD(\Omega)$.

\stage{ii} It is easy to show that $\DS \diff^{\VEC{\alpha}} u$ is a
linear functional.

\stage{iii} To prove that $\DS \diff^{\VEC{\alpha}} u$ is
continuous, we show that $\DS \diff^{\VEC{\alpha}} u$ satisfies
Proposition~\ref{distr_cont_cond}.

Let $K$ be a compact subset of $\Omega$.  Since $u\in \DD'(\Omega)$,
it follows from Proposition~\ref{distr_cont_cond} that there exist a
constant $C_K$ and an integer $N_K$ such that
$\DS \left| u(\phi) \right| \leq C_K \max_{|\VEC{\beta}|\leq N_K}
...
\begin{egg}
Suppose that $\DS u \in C^k(\RR^n)$.  Show that the partial
derivatives $\DS \diff^{\VEC{\alpha}} u$ of the distribution $u$ on
$\DS \RR^n$ with $|\VEC{\alpha}|\leq k$
...
$\DS \RR^n$ is the distribution given by the standard
derivative of the function $u$.

Note that $\DS u \in C^k(\RR^n)$ implies that standard derivative
$\DS \diff^{\VEC{\alpha}} u$ of $u$ is in $\DS C(\RR^n)$
for all $|\VEC{\alpha}|\leq k$ and
so it is in $\DS L_{loc}^1(\RR^n)$ for all $|\VEC{\alpha}|\leq k$.
Thus, the standard derivative $\DS \diff^{\VEC{\alpha}} u$
...
distribution on $\DS \RR^n$ because
$\DS u\in L_{loc}^1(\RR^n)$.  Hence, the 
...
\stage{ii} The function $\DS \diff^{\VEC{\alpha}} u$ can be
used to define a distribution on $\DS \RR^n$ because
$\DS \diff^{\VEC{\alpha}} u\in L_{loc}^1(\RR^n)$.
Hence, by definition,
\[
...
However, for all $\DS \phi \in \DD(\RR^n)$, we have
\begin{align*}
&-\int_{\RR^n} u(\VEC{x}) \pdydx{\phi}{x_1}(\VEC{x}) \dx{\VEC{x}}
...
\begin{egg}
Let $\DS k(x_1,x_2) = \frac{1}{2\pi(x_1 + i x_2)}$ and
$L = \diff_{x_1} + i \diff_{x_2}$.

Given $\DS \phi \in \DD(\RR^2)$, we have
\begin{align*}
...
the counterclockwise orientation of the circle, then we get
\begin{align*}
&\int_{\|\VEC{x}\|_2\geq r}
\left( \pdydx{\phi}{x_1} + i \pdydx{\phi}{x_2} \right)
\frac{1}{x_1 + i x_2} \dx{\VEC{x}} 
...
= \phi(\VEC{0}) = \delta(\phi)
\]
for all $\DS \phi \in \DD(\RR^2)$.  This means that
$L k = \delta$.
\end{egg}
...
$\DS f \in L^1_{loc}(]a,b[)$ satisfies
...
$\DS \int_a^b \psi(x)\dx{x}=1$.  For each
$\phi \in \DD(]a,b[)$, let
\[
...
$\DS C = \int_a^b f(z)\,\psi(z) \dx{z}$.  The readers should
...
$\DS u(\phi) = \int_a^b f(x) \phi(x) \dx{x}$ for
$\phi \in \DD(]a,b[)$, where $\DS f \in L_{loc}^1(]a,b[)$, and
...
in the sense of distributions $\DS \diff^{\VEC{\alpha}} u$ can be
...
$\DS \diff^{\VEC{\alpha}} u$ for $\VEC{\alpha} = (1.0.\ldots,0)$, the
...
$\DS \tilde{\VEC{x}} \in \prod_{j=2}^n ]a_j,b_j[$.
More precisely, $\DS \pdydx{v}{x_1}(x,\tilde{\VEC{x}}) =
...
$\DS \phi(\VEC{x}) = \phi_1(x_1) \, \phi_2(\tilde{\VEC{x}})$ for all
$\VEC{x} = (x_1,\tilde{\VEC{x}}) \in \Omega$, where
$\phi_1 \in \DD(]a_1,b_1[)$ and
$\DS \phi_2 \in \DD\left(\prod_{j=2}^n]a_j,b_j[\right)$, we
...
$\DS \phi_2 \in \DD\left(\prod_{j=2}^n]a_j,b_j[\right)$.
It follows that, for almost all
$\DS \tilde{\VEC{x}} \in \prod_{j=2}^n]a_j,b_j[$, we have
\[
\int_{a_1}^{b_1}
...
\]
If $\DS f \in L^p(X)$, then $Tf:X\rightarrow \CC$ defined by
\[
(Tf)(x) = \int_X K(x,y)f(y) \dx{\mu(y)}
\]
is defined almost everywhere, $\DS Tf \in L^p(X)$ and
$\DS \| Tf \|_p \leq C \|f\|_p$.
...
$p$; namely. $\DS \frac{1}{p} + \frac{1}{q} = 1$.  Since
\[
...
$\DS y \mapsto K(x,y)f(y) \in L^1(\mu)$ for almost all $x\in X$.
...
\]
After taking the $\DS p^{th}$ root on both sides of the previous
inequality, we get
\[
...
It follows that $\DS y \mapsto K(x,y)f(y) \in L^1(\mu)$ for
...
$\DS \| (Tf)(x)\|_\infty \leq C \| f\|_\infty$.
...
$\DS X=\RR^n$ and $K(x,y) = f\left(\VEC{x}-\VEC{y}\right)$.
\end{proof}
...
Suppose that $\DS \phi \in L^1(\RR^n)$ and let
$\DS a = \int_{\RR^n} \phi(\VEC{x})\dx{\VEC{x}}$.  For
$\epsilon >0$, let
\[
\phi_\epsilon(\VEC{x}) = \epsilon^{-n}
\phi\left(\epsilon^{-1}\VEC{x}\right) \ .
\]
If $\DS f \in L^p(\RR^n)$ with $1 \leq p < \infty$, then
$\DS f\ast \phi_\epsilon \rightarrow a f$ in
$\DS L^p(\RR^n)$ as $\epsilon \rightarrow 0$.

If $\DS f \in L^\infty(\RR^n)$ and $f$ is uniformly continuous on
an open set $\DS V\subset \RR^n$, then
$\DS f\ast \phi_\epsilon \rightarrow a f$ uniformly on
...
\stage{i} If $\DS f\in L^p(\RR^n)$ with $1\leq p < \infty$,
...
\end{align*}
If we define $\DS f_{\VEC{z}} \in L^p(\RR^n)$ by
$f_{\VEC{z}}(\VEC{x}) = f(\VEC{x}-\VEC{z})$ for
$\VEC{x}, \VEC{z}\in \RR^n$, then we have
\[
...
$\DS \| f_{\epsilon\VEC{z}} - f \|_p \rightarrow 0$
...
\stage{ii} Suppose that $\DS f \in L^\infty(\RR^n)$ and $f$
is uniformly continuous on $\DS V\subset \RR^n$.
Let $K \subset V$ be a compact set.

Given $\delta >0$, choose a compact set
$\DS W \subset \RR^n$ such that
\[
\int_{\RR^n\setminus W} |\phi(\VEC{x})| \dx{\VEC{x}} <
\frac{\delta}{4\|f\|_\infty} \ .
\]
This is possible because $\DS \phi \in L^1(\RR^n)$.
...
Given $\DS \phi:\RR^n \rightarrow \CC$, we define
$\DS \phi^\vee$ and
$\DS \phi_{\VEC{z}}$ in $\DD(\RR^n)$ by
$\DS \phi^{\vee}(\VEC{x}) = \phi(-\VEC{x})$ for
$\DS \VEC{x} \in \RR^n$ and
$\DS \phi_{\VEC{z}}(\VEC{x}) = \phi(\VEC{x}-\VEC{z})$
for $\DS \VEC{x}, \VEC{r} \in \RR^n$.

This motivates the following definition.  Given
$\DS u \in \DD'(\RR^n)$,
we define $\DS u^\vee$ and $u_{\VEC{z}}$ in
$\DS \DD'(\RR^n)$ by
$\DS u^\vee(\phi) = \ps{u^\vee}{\phi} = \ps{u}{\phi^\vee}$
and
$\DS u_{\VEC{z}}(\phi) = \ps{u_{\VEC{z}}}{\phi}
= \ps{u}{\phi_{-\VEC{z}}}$ for $\DS \phi \in \DD(\RR^n)$.

For two functions $f$ and $g$ in $\DS L^1(\RR^n)$, we have
\[
...
$\DS u\in \DD'(\RR^n)$ with $\DS \phi\in \DD(\RR^n)$ by
\[
...
definition of convolution when $\DS u\in L^1(\RR^n)$.

Suppose that $\DS u \in \EE'(\RR^n)$ and that $K$ is the
compact support of $u$.  Choose $\DS \psi \in \DD(\RR^n)$
such that $\psi(z) =1$ for all $z \in K$.  We have that $\psi u = u$ because
$\ps{\psi u}{\phi} = \ps{u}{\psi \phi} = \ps{u}{\phi}$ for all
$\DS \phi \in \DD(\RR^n)$.  This is obviously independent of
the function $\psi$ chosen.  We can use this property to extend $u$ in a
unique way to a continuous linear functional on
$\DS C^\infty(\RR^n)$,
where the convergence in $\DS C^\infty(\RR^n)$ is defined by uniform
convergence on compact sets.  We define
$\DS u:C^\infty(\RR^n) \to \CC$ by
$u(\phi) = u(\psi \phi)$ for $\DS \phi \in C^\infty(\RR^n)$.

\begin{prop} \label{distr_smooth_convol}
\begin{enumerate}
\item If $\DS u\in \DD'(\RR^n)$ and
$\DS \psi\in \DD(\RR^n)$, then $u\ast \psi$
is in $\DS C^\infty(\RR^n)$.
\item If $\DS u\in \EE'(\RR^n)$ and
$\DS \psi\in \DD(\RR^n)$, then
$\DS u \ast \psi \in \DD(\RR^n)$.
\item If $\DS u\in \EE'(\RR^n)$ and
$\DS \psi\in C^\infty(\RR^n)$, then
$\DS u \ast \psi \in C^\infty(\RR^n)$.
\end{enumerate}
\end{prop}

\begin{prop} \label{distr_comm_der}
If $\DS u\in \DD'(\RR^n)$ and
$\DS \phi\in \DD(\RR^n)$, then
$\DS \diff^{\VEC{\alpha}}(u\ast \phi)
...
\end{equation}
for all $\DS u \in E^\ast$.   We assume that
$\DS E^\ast$ separates points in $E$
\footnote{Namely, for any $y_1$ and $y_2$ in
$E$ with $y_1 \neq y_2$, there exists $\DS u \in E^\ast$
...
\]
for $\DS \VEC{x} \in \RR^n$.  According to the definition of
$u \ast \psi$, we have
\[
...
\]
for $\DS \VEC{z} \in \RR^n$ because $\psi$ and $\phi$ are of class
$\DS C^\infty(\RR^n)$ with one of them having a compact support.
So $\DS \psi \ast \phi \in \DD(\RR^n)$.
We then have that
\[
\big( (u\ast \psi) \ast \phi \big)(\VEC{z})
= \left( u \ast g \right)(\VEC{z})
...
\]
for all $\DS \VEC{x} \in \RR^n$.  If $\DS f(\VEC{w})$ for
$\DS \VEC{w} \in \RR^n$ is the function in $\DS \DD(\RR^n)$
defined by
\[
f(\VEC{w})(\VEC{x}) = \psi(\VEC{w}) \phi(\VEC{z} - \VEC{x} - \VEC{w})
\]
for all $\DS \VEC{x} \in \RR^n$, then we get
\[
\big( (u\ast \psi) \ast \phi \big)(\VEC{z}) = \ps{u}{h}
= \int_{\RR^n} u \circ f \dx{\VEC{w}} \ ,
\]
where we have used (\ref{intVectFunct}) with
$\DS \Omega = \RR^n$ and $\DS E = \DD(\RR^n)$.
If $\DS q(\VEC{w})$ for
$\DS \VEC{w} \in \RR^n$ is the function in $\DS \DD(\RR^n)$
defined by
\[
...
\end{align*}
for $\DS \VEC{z} \in \RR^n$ and all
$\DS \phi \in \DD(\RR^n)$.  Thus,
$\psi \ast (u \ast \phi) = (u\ast \psi) \ast \phi$ for all
$\DS \phi \in \DD(\RR^n)$.  But, by definition of the
...
\end{equation}
where $\mu$ is the Lebesgue measure $m$ divided by $\DS (2\pi)^{n/2}$.
...
two functions $\DS f,g \in L^1(\RR^n)$ as
\[
(f\ast g)\left(\VEC{x}\right) = \int_{\RR^n}
f\left(\VEC{x} - \VEC{y}\right)  g\left(\VEC{y}\right) \dx{\mu(\VEC{y})}
\]
for all $\DS \VEC{x} \in \RR^n$ where the integral converges.  We have
added the factor $\DS (2\pi)^{-n/2}$ to the previous definition of
...
of the factor $\DS (2\pi)^{n/2}$ in the formulae involving
convolution and Fourier transform later in the text.

\begin{egg}
Suppose that $f \in C(\RR)$ has a compact support.

We first show that the function $\DS g:\RR \to \RR$ defined by
$\DS g(x) = \sum_{k\in \ZZ} f(x+ 2 \pi k)$ is a
...
$\DS \sum_{n\in \ZZ} a_n e^{n x i}$ of $g$ is given by
$\DS a_n = \frac{1}{2\pi} \hat{f}(n)$ for all $n$.  It is a simple
...
\]
for all $x$.  If $x = 0$, then we get (\ref{eggPoissonSF}).  The formula 
...
$\DS f(x) = e^{-\|\VEC{x}\|^2}$ for $\DS \VEC{x} \in \RR^n$.
...
\]
If we use the substitution $x = r/2$, then we get
\[
\frac{1}{\sqrt{2\pi}} \int_{-\infty}^\infty  e^{-i x y - x^2} \dx{x}
...
\]
To compute the integral, we note that $\DS e^{-z^2/4}$ with
$z \in \CC$ is an entire function and use Cauchy's theorem for
integrals along a close path to conclude that
$\DS \int_{\Gamma} e^{-z^2/4} \dx{z} = 0$ where $\Gamma$ is
the close path defined by
$\DS \Gamma = \Gamma_1 + \Gamma_2 - \Gamma_3 - \Gamma_4$
...
$\DS f(x) = e^{-a \|\VEC{x}\|^2}$ for
$\DS \VEC{x} \in \RR^n$
and $a$ a positive constant.  We have
\[
\hat{g}(\VEC{y}) = \int_{\RR^n} e^{-i \VEC{x}\cdot\VEC{y}}
e^{-a\|\VEC{x}\|^2} \dx{\mu(\VEC{x})} \ .
\]
If we use the change of variables $\VEC{w} = \sqrt{a}\, \VEC{x}$, then
we get
...
$\DS \int_{\Gamma} = \int_{\Gamma_1} + \int_{\Gamma_2}
...
\begin{enumerate}
\item If $f$ and $g$ are in $\DS L^1(\RR^n)$, then
$\DS (f\ast g)^\wedge = \hat{f}\,\hat{g}$.
\item If $f$ is in $\DS L^1(\RR^n)$, then
$\DS (f_{\VEC{z}})^\wedge(\VEC{y}) =
e^{-i \VEC{z}\cdot\VEC{y}} \hat{f}(\VEC{y})$
\item If $\DS f \in C_c^m(\RR^n)$, $\VEC{\alpha}$ is a multi-index
such that $|\VEC{\alpha}|\leq m$, and $p$ is the polynomial
$\DS p(\VEC{x}) = (-i\VEC{x})^{\VEC{\alpha}}$,
then $\DS (\diff^{\VEC{\alpha}} f)^\wedge = p \hat{f}$ and
$\DS (p f)^\wedge = \diff^{\VEC{\alpha}}\hat{f}$.
\end{enumerate}
\end{prop}
...
$\DS C_0(\RR^n)$ of continuous functions that
{\bfseries vanish at infinity}\index{Functions!Vanish at Infinity}.

The space $\DS C_0(\RR^n)$ is the space of continuous real valued
functions on $\DS \RR^n$ satisfying the following property.  For every
$\epsilon >0$, there exists a compact set $K$ such that
$|f(\VEC{x})|<\epsilon$ for $\DS \VEC{x} \in \RR^n \setminus K$.

\begin{prop} \label{distr_C0_four_transf}
If $\DS f\in L^1(\RR^n)$, then $\hat{f} \in C_0(\RR^n)$ and
$\|\hat{f}\|_\infty \leq \|f\|_1$.
\end{prop}

\begin{prop}[Inverse Fourier Transform] \label{distr_inv_four_transf}
If $\DS f \in L^1(\RR^n)$, $\DS \hat{f} \in L^1(\RR^n)$ and
\[
g(\VEC{y}) = \int_{\RR^n} e^{i \VEC{x}\cdot\VEC{y}} \,
\hat{f}(\VEC{x}) \dx{\mu(\VEC{x})}
\]
for all $\DS \VEC{y} \in \RR^n$, then
$\DS g \in C_0(\RR^n)$.  Moreover, $f = g$ almost everywhere
in $\DS \RR^n$ and $\|f\|_\infty \leq \|\hat{f}\|_1$.
\end{prop}

Given $\DS f\in L^1(\RR^n)$, let
\[
\F^{-1}(f)(\VEC{y}) \equiv \check{f}(\VEC{y}) = 
\int_{\RR^n} e^{i \VEC{x}\cdot\VEC{y}} \, f(\VEC{x}) \dx{\mu(\VEC{x})}
\quad , \quad \VEC{y} \in \RR^n \ .
\]
The previous theorem states that $\DS \F^{-1}(\F(f)) = f$
almost everywhere when $\DS f \in L^1(\RR^n)$
and $\DS \hat{f} \in L^1(\RR^n)$.

It is possible to extend the Fourier transform to $\DS L^2(\RR^n)$.
This will be useful when talking about Sobolev spaces later.

\begin{theorem}[Plancherel] \label{distr_plancherel}
To each $\DS f\in L^2(\RR^n)$ can be associated a function
$\DS \hat{f} \in L^2(\RR^n)$ such that   \index{Plancherel's Theorem}
\begin{enumerate}
\item For $\DS f \in L^1(\RR^n) \cap L^2(\RR^n)$. $\hat{f}$ is the
Fourier transform given in (\ref{distr_fourierT}).
\item $\| f \|_2 = \|\hat{f}\|_2$ for all $\DS f \in L^2(\RR^n)$.
In fact, $\DS \int_{\RR^n} f g \dx{\VEC{x}} =
\int_{\RR^n} \hat{f} \hat{g} \dx{\VEC{x}}$ for all
$\DS f,g \in L^2(\RR^n)$.
\item The linear mapping $\DS \F: L^2(\RR^n) \rightarrow L^2(\RR^n)$
defined by $\F(f) = \hat{f}$ for all $\DS f \in L^2(\RR^n)$
is an Hilbert space isomorphism.
\end{enumerate}
\end{theorem}

In fact, (2) and (3) in Theorem~\ref{distr_plancherel} implies that 
$\DS \F: L^2(\RR^n) \rightarrow L^2(\RR^n)$ is an isometry.

The space $\DS \SS(\RR^n)$ of
{\bfseries rapidly decreasing functions}\index{Functions!Rapidly
Decreasing Functions} on $\DS \RR^n$ (also called 
{\bfseries Schwartz class}\index{Functions!Schwartz Class}) is the space of all
functions $\DS f \in C^\infty(\RR^n)$ such that
\[
...
If $\DS f \in \SS(\RR^n)$, then we have that
$\DS \VEC{x}^{\VEC{\alpha}} \diff^{\VEC{\beta}} f$ itself
vanishes at infinity for all multi-indices $\VEC{\alpha}$ and $\VEC{\beta}$.
Moreover,
$\DS \VEC{x}^{\VEC{\alpha}}\diff^{\VEC{\beta}} f \in L^1(\RR^n)$
...
such that
$\DS \|\VEC{x}\|^{2k}\, \left| f(\VEC{x}) \right| < C$ for
all $\|\VEC{x}\| > r$.  Hence,
\begin{align*}
...
$\DS \int_{\|\VEC{x}\|> r} \|\VEC{x}\|^{-2k} \dx{\VEC{x}} < \infty$
since $2k >n$.  For all multi-indices $\VEC{\alpha}$ and $\VEC{\beta}$, this
reasoning can be applied to
$\DS \VEC{x}^{\VEC{\alpha}} \diff^{\VEC{\beta}} f$ instead of $f$ to
show that
$\DS \VEC{x}^{\VEC{\alpha}} \diff^{\VEC{\beta}} f \in L^1(\RR^n)$.
...
If $\DS f \in L^1(\RR^n)$ has compact support, then $\hat{f}$ is the
restriction to $\DS \RR^n$ of an entire function on
$\DS \CC^n$.  If
$\DS f\in \DD(\RR^n)$, then $\hat{f}$ is a rapidly decreasing
function on any subset of the form
$\DS \left\{ \VEC{z}\in \CC^n : \| \IM \VEC{z} \|_2 \leq c \right\}$ 
for $c>0$.  Recall that
$\DS \IM \VEC{z} =
\begin{pmatrix} \IM z_1 & \IM z_2 & \ldots & \IM z_n \end{pmatrix}^\top$
for $\DS \VEC{z} \in \CC^n$.
\end{prop}
...
\]
converges for all $\DS \VEC{z} \in \CC^n$ because $f$ has a compact
support so the integration is over a bounded set and the
exponential factor is bounded on this set.
Hence, we may define $\hat{f}(\VEC{z})$ for $\DS \VEC{z} \in \CC^n$ by
\[
  \hat{f}(\VEC{z}) =
\int_{\RR^n} e^{-i \VEC{z}\cdot \VEC{x}} f(\VEC{x}) \dx{\mu(\VEC{x})} \ .
\]
$\hat{f}$ is an entire function because
$\DS\VEC{z} \mapsto  e^{i \VEC{z}\cdot \VEC{x}}$ is an entire function
and, for any multi-index $\VEC{\alpha}$, complex
derivatives $\DS \diff_{\VEC{z}}^{\VEC{\alpha}}$ and integration
can be interchanged.

If $\DS f \in \DD(\RR^n)$, choose $r$ large enough such that
...
$\DS \VEC{z}^{\VEC{\alpha}} \diff^{\VEC{\beta}} \hat{f}(\VEC{z})$ is
...
$\DS \VEC{x}^{\VEC{\beta}} f$
instead of $f$ where $\DS \VEC{x}^{\VEC{\beta}} f \in \DD(\RR^n)$.
\end{proof}

The Fourier transform can only be defined on a subset of the space
$\DS \DD'(\RR^n)$ of distributions on $\DS \RR^n$.

\begin{prop} \label{DD_S_IncDense}
\begin{enumerate}
\item $\DS \DD(\RR^n) \subset \SS(\RR^n)$ and the inclusion
$\DS i:\DD(\RR^n) \subset \SS(\RR^n)$ is continuous.
\item $\DS \DD(\RR^n)$ is dense in $\DS \SS(\RR^n)$.
\end{enumerate}
\end{prop}

In the statement of the previous proposition, the topology on
$\DS \DD(\RR^n)$ is the topology defined in
Section~\ref{SectTestFnct} and the topology on
$\DS \SS(\RR^n)$ is the locally convex topology that we
have mentioned above.

If $u$ is a continuous linear functional on $\DS \SS(\RR^n)$, then
$u \circ i$ is a continuous linear functional on
$\DS \DD(\RR^n)$; namely,
$u\circ i \in \DD'(\RR)$.  Since $\DS \DD(\RR^n)$ is dense in
$\DS \SS(\RR^n)$, each continuous linear functional on
$\DS \SS(\RR^n)$ is associated in this manner to a unique
element of $\DS \DD'(\RR^n)$.
The {\bfseries tempered distributions}\index{Distribution!Tempered}
on $\DS \RR^n$ are the distributions on $\DS \RR^n$
associated to continuous linear functionals on
$\DS \SS(\RR^n)$.  For this reason, the space of all the tempered
distributions is denoted $\DS \SS'(\RR^n)$.  Another way to
look at the tempered distributions is to say that
$\DS u \in \DD'(\RR^n)$ is a
tempered distribution if it can be extended to a continuous linear
functional on $\DS \SS(\RR^n)$.

\begin{egg}
Here are a couple of examples of tempered distributions.

If $\nu$ is a positive measure on $\DS \RR^n$ such that
$\DS \int_{\RR^n} (1 + \|\VEC{x}\|^2)^{-k} \dx{\nu} < \infty$
for some positive integer $k$, then $\nu$ defines a tempered
distribution on $\DS \RR^n$.  More precisely,
$\DS u : \SS(\RR^n) \to \CC$ defined by
$\DS u(\phi) = \int_{\RR^n} \phi(\VEC{x}) \dx{\nu(\VEC{x})}$
for $\DS \phi\in \SS(\RR^n)$ is a temperate distribution on
$\DS \RR^n$.

If $\DS g:\RR^n \to \RR$ is a measurable functions such that
$\DS \int_{\RR^n} \left| (1+\|\VEC{x}\|^2)^{-k} g(\VEC{x})
\right|^p \dx{\VEC{x}} < \infty$ for a positive integer $k$ and a real
number $p \geq 1$, then $g$ defines a tempered distribution on
$\DS \RR^n$.
More precisely, $\DS u : \SS(\RR^n) \to \CC$ defined by
$\DS u(\phi) = \int_{\RR^n} \phi(\VEC{x}) g(\VEC{x}) \dx{\VEC{x}}$
for $\DS \phi\in \SS(\RR^n)$ is a temperate distribution on
$\DS \RR^n$.

Every $\DS g \in L^p(\RR^n)$ for $1 \leq p \leq \infty$
defines a tempered distribution on $\DS \RR^n$.
\end{egg}
...
\begin{prop}
$\DS \EE'(\RR^n) \subset \SS'(\RR^n) \subset \DD'(\RR^n)$
\end{prop}
...
linear bijection from $\DS \SS(\RR^n)$ to itself and its inverse is
continuous.
\item If $\phi$ and $\psi$ are in $\DS \SS(\RR^n)$, then
$\DS \phi \ast \psi \in \SS(\RR^n)$ and 
$\DS (\phi\ast \psi)^\wedge = \hat{\phi}\,\hat{\psi}$.
\end{enumerate}
\end{prop}

The {\bfseries convolution}\index{Distribution!Convolution} of a
temperate distribution $\DS u \in \SS'(\RR^n)$ with a
function $\DS \phi \in \SS(\RR^n)$ is defined by
\[
  (u \ast \phi)(\VEC{x}) = \ps{u}{(\phi^\vee)_{\VEC{x}}}
\]
for all $\DS \VEC{x} \in \RR^n$.
The {\bfseries Fourier transform}\index{Distribution!Fourier Transform}
of a tempered distribution $\DS u \in \SS'(\RR^n)$ is the
...
$\ps{u}{\phi} = u(\phi)$ for $\DS u \in \SS'(\RR^n)$ and
$\DS \phi \in \SS(\RR^n)$.

\begin{prop} \label{distr_frr_tempD}
\begin{enumerate}
\item The Fourier transform (\ref{distr_fourierTD}) is a continuous
linear bijection from $\DS \SS'(\RR^n)$ into itself and its
inverse is continuous.
\item If $\DS u \in \SS'(\RR^n)$, $\VEC{\alpha}$ is a multi-index,
and $p$ is the polynomial
$\DS p(\VEC{x}) = (i\VEC{x})^{\VEC{\alpha}}$, then
$\DS (\diff^{\VEC{\alpha}} u)^\wedge = p\, \hat{u}$ and
$\DS (p^\vee u)^\wedge = \diff^{\VEC{\alpha}}\hat{u}$.
\item If $\DS u \in \SS'(\RR^n)$ and
$\DS \phi \in \SS(\RR^n)$, then
$\DS u\ast \phi \in C^\infty(\RR^n)$ and
$\DS u\ast \phi \in \SS'(\RR^n)$.
\item If $\DS u \in \SS'(\RR^n)$ and
$\DS \phi \in \SS(\RR^n)$, then
$\DS (u\ast \phi)^\wedge = \hat{\phi}\,\hat{u}$ and
$\DS \hat{u}\ast \hat{\phi} = (\phi\,u)^\wedge$.
\end{enumerate}
\end{prop}
...
$\DS (\diff^{\VEC{\alpha}} u)(\phi) = (-1)^{\VEC{\alpha}}
u(\diff^{\VEC{\alpha}} \phi)$
for all $\DS \phi \in \SS(\RR^n)$.  Hence,
\[
...
\]
for all $\DS \phi \in \SS(\RR^n)$.  Thus,
$\DS (\diff^{\VEC{\alpha}} u)^\wedge = p \hat{u}$ in the sense of
distributions.
\end{proof}
...
\ps{f}{\phi}
\]
for all $\DS \phi \in \SS(\RR^n)$, where
$\DS f \in \SS'(\RR^n)$ is the
tempered distribution defined by the measure $\mu$; namely,
$\DS f(\phi) = \int_{\RR^n} \phi(\VEC{x}) \dx{\mu(\VEC{x})}
= \frac{1}{(2\pi)^{n/2}} \int_{\RR^n} \phi(x) \dx{\VEC{x}}$ for all
$\DS \phi \in \SS(\RR^n)$, then $\hat{\delta} = f$.
...
not use the factor $\DS (2\pi)^{-n/2}$ in their definition
...
$\DS (\chi_a)^\wedge$ converges to $\hat{\delta}$ in $\SS'(\RR)$.

Since $\DS \chi_a \in L^1(\RR)$, its Fourier transform is
given by
\[
...
\]
for $x = 0$.   Thus
$\DS \lim_{a\to 0} (\chi_a)^\wedge(x) = 1/\sqrt{2\pi}$
for all $x \in \RR$.

Thus $\DS \lim_{a\to 0} (\chi_s)^\wedge(x) = 1/\sqrt{2\pi}$
for all $x \in \RR$.  We have shown that
$\DS (\chi_a)^\wedge$ converges pointwise to the function
...
Let $\DS \Omega \subset \RR^n$ be an open set and consider
...
$\DS C^{|\VEC{\alpha}|}$ for all multi-indices involved in the sum.
...
involved in the sum, $\DS u:\RR^n \rightarrow \CC$ and
$\DS f \in \DD(\RR^n)$.

We assume that the coordinates are such that
$\DS \{ \VEC{x} \in \RR^n : x_n=0 \}$ is not a characteristic surface.
This implies that $a_{(0,0,\ldots,0,m)} \neq 0$.  Without loss of
generality, we may even assume that $a_{(0,0,\ldots,0,m)} = 1$.
We show that there exists a solution $\DS u \in C^\infty(\RR^n)$ for
(\ref{distr_lPDE3}).

We have that $\DS L(\VEC{x},\diff)\hat{u} = (p\, u)^\wedge$ where
...
$\DS g(z) = p(\breve{\VEC{y}}, z)$ for
$\DS \breve{\VEC{y}}\in \RR^{n-1}$.
By Rouch\'e's theorem, the roots
$\DS \lambda_i:\RR^{n-1} \rightarrow \CC$
are continuous functions.

\begin{lemma} \label{distr_lPDE3_ex2}
There exists a measurable function
$\DS \kappa:\RR^{n-1}\rightarrow [-m,m]$ such that
\[
...
\begin{proof}
Given $\DS \breve{\VEC{y}} \in \RR^{n-1}$, there exists a
least one interval
$\DS I_k = [2k-m-1, 2k-m+1]$ with $0\leq k \leq m$ such that
...
$\DS \RR^{n-1} = \bigcup_{1\leq k \leq m} V_k$ and
...
$\DS \kappa(\breve{\VEC{y}}) = 2k-m$ if
$\DS \breve{\VEC{y}} \in V_k \setminus \bigcup_{0\leq j<k} V_j$.
It is a measurable function because
$\DS \kappa^{-1}\big(\{2k-m\}\big)
= V_k \setminus \bigcup_{0\leq j<k} V_j$
is a measurable set.
\end{proof}

\begin{lemma} \label{distr_lPDE3_ex3}
Let $\DS q(z) = z^m + g(z)$, where $g$ is a polynomial of
degree less than $m$ and $g(0) \neq 0$.  Suppose that $\lambda_1$,
$\lambda_2$, \ldots, $\lambda_m$ are the roots of $q$.  Then,
$\DS \left| q(0)\right| \geq \left(\frac{\lambda}{2}\right)^m$,
where $\DS \lambda = \min_{1\leq j\leq m} |\lambda_j|$.
\end{lemma}

\begin{proof}
The factorization of $q$ is
$\DS q(z) = (z-\lambda_1)(z-\lambda_2)\ldots(z-\lambda_m)$.
Hence,
\[
...
$|z/\lambda_j - 1| \leq 2$.  Since $\DS m! = q^{(m)}(z)$,
...
\end{align*}
Thus, $\DS \left(\frac{\lambda}{2}\right)^m \leq |q(0)|$.
\end{rmk}

\begin{theorem} \label{distr_lPDE3_ex4}
There exists a solution of class $\DS C^\infty$ for
(\ref{distr_lPDE3}).
\end{theorem}
...
\]
for $\DS \VEC{x} \in \RR^n$, where $\kappa$ is defined in
Lemma~\ref{distr_lPDE3_ex2} and $p$ is defined in
(\ref{distr_lPDE3_ex1}).  In the previous integral, we
assume that $y_n \in \CC$.

\stage{i} Given $\DS \VEC{y} \in \RR^n$, we first prove that
$\DS |p(\VEC{y})| \geq 2^{-m}$ for
...
on the set $\DS \{\VEC{y} \in \CC^n : \|\IM \VEC{y}\|_2 \leq m \}$
that contains the domain of integration
$\DS \{ \VEC{y} = (\breve{\VEC{y}}, y_n) \in \RR^{n-1}\times \CC :
\IM y_n = \kappa(\breve{\VEC{y}}) \}$ because
$|\IM y_n| \leq m$ since the range of $\kappa$ is $[-m.m]$.

It follows from (i) and (ii) that $\DS
...
exponential in the definition of $u$ is bounded by $\DS e^{m x_n}$.
It follows that
$\DS \VEC{y} \mapsto \diff_{\VEC{x}}^{\VEC{\alpha}}
...
$u$.  Thus $u$ is of class $\DS C^\infty$.
...
Since $\DS y_n \mapsto e^{i\VEC{x}\cdot\VEC{y}} \hat{f}(\VEC{y})$ is a
...
\]
Moreover, since
$\DS \VEC{y} \mapsto 2^{i\VEC{x}\cdot\VEC{y}} \hat{f}(\VEC{y})$ is
rapidly decreasing on the set $S$, we have that there exists a
constant $C>0$ such that
$\DS \left| 2^{i\VEC{x}\cdot\VEC{y}} \hat{f}(\VEC{y}) \right| \leq
C/ \|\VEC{y}\|_2^2$ for all $\VEC{z} \in S$.  Thus, for
$\DS R^2>2m^2$, we have that
\begin{align*}
...
= f(\VEC{x})
\]
for $\DS \VEC{x} \in \RR^n$, where we have used the Formula for the
...
distribution $F$ on $\DS \RR^n$ such that
$L(\VEC{x},\diff)F = \delta$.
\end{defn}

\begin{egg}
Let $\DS R = \{\VEC{x} \in \RR^2 : x_i > 0 \ \text{for}
\ i=1,2 \}$ and $\DS L(\VEC{x},\diff) = \diff^{(1,1)}
...
\end{align*}
for all $\DS \phi \in \DD(\RR^2)$.
\end{egg}

\begin{rmk}
If $\DS F\in \DD'(\RR^n)$ is a fundamental solution for
$L(\VEC{x},\diff)$, then a solution of $L(\VEC{x},\diff)u = f$, where
$\DS f \in \DD(\RR^n)$, is the 
...
\end{equation}
has a fundamental solution on $\DS \RR^n$.
\index{Malgrange-Ehrempreis' Theorem}
\end{theorem}

\begin{proof}
We define a linear functional $F$ on $\DS \DD(\RR^n)$ by
\[
...
$\DS F \in \DD'(\RR^n)$, we have
to show that $F$ is continuous on
$\DS \DD(\RR^n)$.  We will also show
that $L(\VEC{x},\diff)F = \delta$.

\stage{i} Let $\DS K \subset \RR^n$ be a compact set.  We will find a
constant $C_K$ and an integer $N_K$, such that
\[
...
continuous on $\DS \DD(\RR^n)$ and so
$\DS F\in \DD'(\RR^n)$.
...
\]
for $\DS \VEC{y} \in \CC^n$ and
$\DS \VEC{\alpha} \in \NN^n$ that we have obtained in
...
= \phi(\VEC{0}) = \delta(\phi)
\end{align*}
for all $\DS \phi \in \DD(\RR^n)$.  The fourth equality comes from
...
$\DS \{ \VEC{y} \in \CC^n : \|\IM y\|_2 \leq m \}$ according to 
...
$\DS L(\VEC{x},\diff)u = \sum_{|\VEC{\alpha}|\leq m}
a_{\VEC{\alpha}} \diff^{\VEC{\alpha}} u$,
where the $\DS a_{\VEC{\alpha}}\in C^\infty(\Omega)$, is
{\bfseries hypoelliptic}%
\index{Linear Partial Differential Operator!Hypoelliptic} if
$\DS L(\VEC{x},\diff)u \in C^\infty(\Omega)$ implies that
$\DS u \in C^\infty(\Omega)$.
\end{defn}

Recall that we say that $u \in \DD'(\Omega)$ is of class
$\DS C^\infty(\Omega)$ and write
$\DS u \in C^\infty(\Omega)$ if there exists
a function $\DS f \in C^\infty(\Omega)$ such that
$\DS u(\phi) = \int_{\Omega} f(x) \phi(x) \dx{x}$
for all $\phi \in \DD(\Omega)$.
...
If $\DS f \in \DD'(\RR^n)$ is given by
$\DS f\in C^\infty(\RR^n\setminus \{\VEC{0}\})$
and $\DS v \in \EE'(\RR^n)$, then
$\DS f\ast v \in C^\infty(\RR^n\setminus \supp v)$.
\end{lemma}

\begin{proof}
Consider $\VEC{z} \not\in \supp v$ and choose $\epsilon >0$ such that
$\DS B_\epsilon(\VEC{z}) \cap \supp v = \emptyset$.
Choose $\DS \psi \in \DD(\RR^n)$ such that $\psi(\VEC{x}) = 1$ for
$\DS \VEC{x} \in B_{\epsilon/4}(\VEC{0})$ and
$\DS \supp \psi \subset B_{\epsilon/2}(\VEC{0})$.

Then $f\ast v = (\psi f)\ast v + ( (1-\psi)f) \ast v$ where
$\DS ((1-\psi) f) \ast v \in C^\infty(\RR^n)$ according to
item (3) of Proposition~\ref{distr_smooth_convol} and
Proposition~\ref{uvequvu} because
$\DS (1-\psi) f \in C^\infty(\RR^n)$.  Moreover,
\[
\supp \big( (\psi f)\ast v\big) \subset D = \left\{ \VEC{x} + \VEC{y}
: \VEC{x} \in \supp \psi \ , \ \VEC{y} \in \supp v \right\}
\]
and $\DS D \cap B_{\epsilon/2}(\VEC{z}) = \emptyset$.
Hence $((\phi f)\ast v)(\phi) = 0$ for all
$\DS \phi \in \DD\left(B_{\epsilon/2}(\VEC{z})\right)$.
Thus $\DS f\ast v = ((1-\phi)f) \ast v$ on
$\DS \DD\left(B_{\epsilon/2}(\VEC{z})\right)$ and so
$\DS f\ast v \in C^\infty\left(B_{\epsilon/2}(\VEC{z})\right)$.
...
class $\DS C^\infty$ on $\DS \RR^n\setminus \{\VEC{0}\}$.
\item Every fundamental solution of $L(\VEC{x},\diff)$ is of class
$\DS C^\infty$ on $\DS \RR^n\setminus \{\VEC{0}\}$.
...
$\DS L(\VEC{x},\diff)F = \delta \in C^\infty(\RR^n
\setminus \{\VEC{0}\})$ and (3)
implies that $\DS F \in C^\infty(\RR^n \setminus \{\VEC{0}\})$.

\stage{$\mathbf{(2) \Rightarrow (1)}$}
This is trivial because of Theorem~\ref{malgr_Ehr}.

\stage{$\mathbf{(1) \Rightarrow (3)}$}
Let $\DS F \in C^\infty(\RR^n\setminus \{\VEC{0}\}$ be a fundamental
solution of $L(\VEC{x},\diff)$.  Suppose that $u\in \DD'(\Omega)$ is such that
$\DS L(\VEC{x},\diff)u \in C^\infty(\Omega)$.

Given $\VEC{z} \in \Omega$ and $\epsilon >0$ such that
$\DS B_\epsilon(\VEC{z}) \subset \Omega$ , we prove that
$\DS u \in C^\infty(B_{\epsilon/2}(\VEC{z}))$.  Choose
$\DS \phi \in \DD(\RR^n)$ such that
$\DS \supp \phi \subset B_\epsilon(\VEC{z})$ and
$\phi(\VEC{x})=1$ for all $\DS \VEC{x} \in B_{\epsilon/2}(\VEC{z})$.
...
$\DS F\ast (\phi \, L(\VEC{x},\diff)u) \in C^\infty(\RR^n)$ because
$\DS \phi\,L(\VEC{x},\diff)u \in \DD(\RR^n)$.

We have that $v(\phi) = 0$ for all $\DS \phi \in \DD(\RR^n)$ with
$\DS \supp \phi \subset \overline{B_{\epsilon/2}(\VEC{z})}
\cup \left( \RR^n \setminus B_\epsilon(\VEC{z}) \right)$
because $\phi(\VEC{x})=0$ for
$\DS \VEC{x} \not\in B_\epsilon(\VEC{z})$ and $\phi(\VEC{x})=1$ for
$\DS \VEC{x} \in B_{\epsilon/2}(\VEC{z})$.  Hence, from
Lemma~\ref{distr_lemma_hypo}, we have that
$\DS F\ast v \in C^\infty(B_{\epsilon/2}(\VEC{z}))$ 
because $\DS \supp v \subset \overline{B_\epsilon(\VEC{z})} \setminus
B_{\epsilon/2}(\VEC{z})$.
...
$\DS \VEC{x} \in B_{\epsilon/2}(\VEC{z})$, we get that
$\DS u \in C^\infty(B_{\epsilon/2}(\VEC{z}))$.
\end{proof}
