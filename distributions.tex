...
$L(\VEC{x}, \diff)$.  Hence, if $\displaystyle u \in C^m(\RR^n)$ is a
...
$\displaystyle \VEC{\alpha} = (\alpha_1,\alpha_2, \ldots, \alpha_n) \in \NN^n$.
\end{enumerate}
...
Given $\displaystyle \phi \in \DD(\RR^2)$, we have
\begin{align*}
...
\]
After taking the $\displaystyle p^{th}$ root on both sides of the previous
inequality, we get
\[
...
an open set $\displaystyle V\subset \RR^n$, then
...
If $\displaystyle f \in \SS(\RR^n)$, then we have that
...
$\displaystyle \kappa:\RR^{n-1}\rightarrow [-m,m]$ such that
\[
...
\begin{egg}
Let $\displaystyle R = \{\VEC{x} \in \RR^2 : x_i > 0 \ \text{for}
\ i=1,2 \}$ and $\displaystyle L(\VEC{x},\diff) = \diff^{(1,1)}
...
\end{align*}
for all $\displaystyle \phi \in \DD(\RR^2)$.
\end{egg}
